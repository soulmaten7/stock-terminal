...
export async function fetchStockNews(query: string, limit = 8, locale: 'en' | 'ko' | 'ja' | 'zh' = 'en'): Promise<Headline[]> {
  const loc = locale === 'ko' ? 'hl=ko&gl=KR&ceid=KR:ko' : locale === 'ja' ? 'hl=ja&gl=JP&ceid=JP:ja' : locale === 'zh' ? 'hl=zh-CN&gl=CN&ceid=CN:zh-Hans' : 'hl=en-US&gl=US&ceid=US:en';
