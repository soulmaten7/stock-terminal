...
export async function fetchStockNews(query: string, limit = 8, locale: 'en' | 'ko' | 'ja' | 'zh' | 'zh-hk' | 'vi' | 'en-gb' = 'en'): Promise<Headline[]> {
  const loc = locale === 'ko' ? 'hl=ko&gl=KR&ceid=KR:ko' : locale === 'ja' ? 'hl=ja&gl=JP&ceid=JP:ja' : locale === 'zh' ? 'hl=zh-CN&gl=CN&ceid=CN:zh-Hans' : locale === 'zh-hk' ? 'hl=zh-HK&gl=HK&ceid=HK:zh-Hant' : locale === 'vi' ? 'hl=vi&gl=VN&ceid=VN:vi' : locale === 'en-gb' ? 'hl=en-GB&gl=GB&ceid=GB:en' : 'hl=en-US&gl=US&ceid=US:en';
