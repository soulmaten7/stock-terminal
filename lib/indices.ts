// 글로벌 지수 조회 — /api/yahoo/indices와 서버 프리페치(app/[locale]/page.tsx) 공용(STEP 771 §3).
// 내부 HTTP 왕복 없이 양쪽이 이 함수를 직접 호출 — 캐시도 여기 하나(모듈 스코프 공유).
...
export async function getIndices(): Promise<IndicesResult> {
