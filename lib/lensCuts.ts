...
// 시장 컷 로드(DB 1회). 렌즈키 → 컷+메타(N·기준일).
export async function loadCuts(market: string): Promise<CutMap> {
  const sb = createAdminClient();
  const { data } = await sb.from("lens_cuts").select("lens_key,lo,hi,n,as_of").eq("market", market);
  const out: CutMap = {};
