...
export async function computeGbPerf(): Promise<{ ok: true; computed: number; attempted: number; at: string }> {
  ...
  for (let from = 0; ; from += 1000) {
    const { data } = await sb.from("gb_stock_perf").select("symbol, updated_at").range(from, from + 999);
