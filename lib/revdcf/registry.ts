...
export const INPUTS: RegistryEntry[] = [
  ...
  },
  {
    ...
    klass: "B",
    divergence:
      "✅ 847 실측으로 확정: 원전 무차입 현금세율(T6)은 우리 604 중 58%만 5년확보(병목 interest 427)·이상값 16.2%·회사내 변동 SD 8.1%. 한계세율은 상수·전종목·안정 → 채택. (원전은 현금세율이나 조달·안정성 열위) 🔴 877 재정정: 원전은 WACC 세후부채비용에도 현금세율을 쓴다(T7 Inputs!C10=0.165) — NOPAT·WACC 양쪽에 하나의 값(구조는 같고 값만 다름). 🔴 885 순효과 실측(359/464사): A(양쪽 현금세율) vs 기준(양쪽 한계세율) GAP 중앙 10→10 불변·판정버킷 이동(가치훼손 132→125·무성장설명 57→64) — 대가=359사 중 최소 7사(≈2.0%)에서 가치훼손 판정이 원전방식과 갈림. 재검토조건=현금세율 커버리지가 77.4%를 넘고 같은 방향이 재확인되면 재개방",
  },
  {
    id: "costOfCapital",
    ko: "자본비용 (WACC)",
    ...
    klass: "B",
    divergence: "🔴 다모다란 완성 WACC(Cost of Capital 열) 차용 금지 — 세율 불일치. ✅(배선완료) 849: 구성요소 조립(`lib/revdcf/compute.ts`·`assembleWacc`)으로 배선. 94업종 검산 차 중앙 0.08%p. GAP은 WACC 3점(−1/기준/+1%p)으로 산출(단일 금지). ✅ 원전대조판정 확정(881·2026-08-03 장은태) — **현행 유지**(업종 구성요소 조립). 근거: ①T7의 회사별 실측 입력(벤더 베타·실채권 YTM)은 604사×매일 자동화 규모에서 무료 대량 조달 경로가 없다(driver1·4와 같은 유형의 제약) ②881 5단계 분해: 도미노 WACC 차이(5.35%→7.19%)의 절대다수가 **rf·ERP 시점 차**(0단계→1단계에서 WACC+2.1%p·GAP이 이미 years→over_cap 전환) — 우리 방법 선택(베타 재레버리지·합성스프레드·세율)은 그 위에서 ±0.3%p만 움직인다. ③881 §2-3 점검: 합성스프레드가 `std_dev_equity` 쓰는 것 = **원문 문제 없음**(`wacc.xls` FAQ가 업종단위 부채비용을 명시적으로 이 변수로 근사한다고 서술 — 개별기업 syntrating의 이자보상배율표와는 별개 데이터셋). 현금조정 베타 재레버리지 시 현금 미환원 = **원문에 명시적 서술 없음**(재검토 여지로 남김, 확정적 오류 아님)",
    open: "🔶 rf = damodaran(연·ERP와 짝) 채택 유지 — **881에서 '일간 FRED로 바꿀 근거 약화' 확인**(다모다란 본인도 월 단위 갱신·일간 아님). 갱신주기(연1회 vs 월1회) 자체는 별개 미결. 🔴 신규(881): 회사별 실제 YTM 대신 업종 합성스프레드를 쓰는 것 자체는 자동화 제약상 유지하되, 도미노 스팟체크에서 합성스프레드(4.837%)가 실제 YTM(4.546%)보다 높게 나와 **우리 WACC가 보수적(과대)일 수 있음** — 515사 전체에서 이 편향 방향·크기는 미측정",
  },
  {
    id: "inflation",
    ko: "인플레이션 (잔여가치)",
    ...
    klass: "B",
    divergence: "✅(A분류 값선택 확정) 851: i=인플레(실질 0 성장)가 정석. i=0(영구 실질감소·부자연)·T8 0.016(도미노 전용) 기각. 3안 실측 GAP 중앙 16/16/14 = WACC(밴드 6년) 대비 2차적. '어느 값 쓸지'는 A분류 규칙. ✅ 원전대조판정 확정(882·2026-08-03 장은태) — **현행 유지**(`i=expected_inflation`). 근거: ①원전 T8 i=1.6%는 2020 스냅샷이며 다모다란 자신의 규칙(\"영구성장률≤무위험이자율\", `stablegrowthrate.htm` 직인용)을 오히려 위반한다(i 1.6%>rf 0.65%) ②우리 i(2.5%)<rf(3.95%)는 그 규칙을 구조적으로 만족 ③882 재측정: i의 GAP효과는 여전히 WACC 대비 2차적(driver5 전환 후에도 851 결론 유지)·터미널비중 중앙 71.9%(원전 관찰과 근접). 🔴 불리한 사실: 다모다란의 문자 그대로의 권고는 \"i=인플레\"가 아니라 \"i=rf\"다 — 우리는 그 규칙을 만족하는 대안값을 쓸 뿐 그의 권장값 자체는 아니다. `expected_inflation`의 출처도 재확인: 다모다란 데이터셋에서 이 값은 터미널성장률 권고표가 아니라 통화환산 유틸리티 셀(`wacc.xls` \"Expected inflation rate in US $\")을 재목적화한 것. 🔴 883 근거교체(Option A): ~~i=rf는 자동화 부적합~~ — `riskfree_rate` 이미 배선돼 있어 거짓. 883 실측(464사): 터미널발산(WACC≤rf) 0건(계산 가능). **진짜 이유는 앵커 보존** — 도미노에 i=rf(0.65%) 대입 시 GAP이 8→12로 이동해 848 검증기준이 깨짐. 판정(현행 유지) 불변·근거만 교체",
  },
