...
export async function computeUsPerf(): Promise<{ ok: true; computed: number; attempted: number; at: string }> {
  ...
  for (let from = 0; ; from += 1000) {
    const { data } = await sb.from("us_stock_perf").select("symbol, updated_at").range(from, from + 999);
