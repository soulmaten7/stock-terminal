...
## 2026-07-17 (4) — 🌐 ②b-2 관심목록 즉시화 + ①-KR 종목명 영어화 완결 ✅

**HEAD `7bd48e5`.**
- **②b-2 관심목록 즉시화**(`95d4d9f`): `/api/watchlist/quotes`에 `lens_scores` 선계산 톤(강점/주의/보통·state→tone 결정론) 배치 포함 → `WatchlistClient` 로드 즉시 렌더("읽는 중…" 없이)·선계산 밖만 실시간 `/api/lens` 폴백. ②b 완결.
- **①-KR 종목명 영어화**(`147bc39`): `kr_stock_snapshot.name_en`(MCP 컬럼)←야후 `longName`(신규 `scripts/enrich_kr_names.ts`) + `krx/ranking`·`MarketBoard`·`lib/stockName.ts`·`watchlist/quotes`·`WatchlistClient`가 `/en` 영문명 선택(한글 폴백). 종목상세는 기존 `resolveStockName` 자동.
- **🐞 페이지네이션 버그(Claude Code)**: 초기 `select`가 PostgREST 1000행 캡→~1772종목(SK하이닉스 포함) 조용히 누락→`.range()` 루프로 전 2772 커버(`aebda56`). `name_en` **2766/2772**(6=야후 미제공 소형주).
- **렌즈 미리보기 공유카드**(`7bd48e5`): `LensPreview`·`BoardTopLensCard` 종목명도 `locale==='en'?nameEn:name`(한글 폴백). ④ 미리보기가 /en서 한글 뜨던 것 수정.
- **✅ 라이브 실측**: `/en` KR 보드·리스트·미리보기·상세 전부 영문명(SK hynix·Samsung Electronics·Hyundai Motor·Kia·NAVER)·`/ko` 한글 유지.
- **🅿️ name_en 자동화(미착수)**: 별도 크론 불필요 — 매일 `computeKrSnapshot`/`kr-perf`에 "name_en null만 야후 채움" 증분 1스텝→새 종목 자동. 백필=이번 수동/정상운영=증분. 크론 필요성 재검토 요청→대화 후 확정.
- **🧭 운영모델 자문(POTAL 멀티에이전트 조직)**: 결론=프리베타엔 16-Division 과설계. 멀티에이전트=독립 병렬 리서치만 이득(Anthropic)·코딩은 단일 스레드(Cognition)·CC 서브에이전트 2~4개. 처방=단일 스레드 유지+시스템맵+읽기전용 서브에이전트 2~4개+일일 헬스체크. 미확정·대화 계속.
- **▶ 다음**: ①-JP/CN/VN 종목명 · name_en 자동화 방식 확정 · (선택) 운영모델 경량 적용.

## 2026-07-17 (3) — 🔬 렌즈 주인공화: ④ 종목헤더·보드 자동미리보기 + ②b-1 KR 렌즈 선계산 ✅

**HEAD `fe02a41`.**
- **④A 종목 페이지 렌즈 헤더**(`dcb1bf6`): 상단에 점7+강점/주의/보통+"판단은 당신" 노트(LensSummary 재사용·상세 카드 위·ETF 제외). 라이브 삼성전자 강점2·주의2·보통3.
- **④B 보드 자동 미리보기**: 744(`be7407d`·KR) LensPreview example 라벨+BoardTopLensCard(모바일 인라인)+aside `selectedStock ?? sorted[0]`(표시만 폴백) · 745(`9998c7b`) US·JP·CN·VN·GB 미러. 라이브 SK하이닉스·Micron "거래 상위 예시".
- **②b-1 KR 렌즈 선계산**(`fe02a41`+RPC 마이그): computeLensScoresFor(universe,market) 파라미터화 + KR 유니버스(거래대금 상위) + kr-lens-scores 크론(30 10 * * *) + 백분위 RPC 시장필터. **KR 489행**·MCP 검증(삼성전자 강점2·주의2·보통3·백분위 격리·US 무오염).
- **🔑 발견**: KR 이름이 야후 영어("SamsungElec") → ①에 야후 longName 활용 가능.
- **▶ 다음**: ②b-2(관심목록이 lens_scores 배치로 읽어 즉시화) → ①(비미국 종목명 영어화).

## 2026-07-17 (2) — 🔎 베타 준비도 평가 + ⭐ 관심목록 렌즈 개편(②a) + /en 종목명 갭 ✅

**HEAD `44fa289`.**
- **🔎 베타 준비도**(경쟁 3중 조사): "클로즈드 베타엔 충분·네이버/토스/초이스스탁 정면 공개경쟁엔 아직." 차별점=비추천 에토스·방법 투명성·6개국·유사투자자문. 갭=스크리너 UI·재방문훅·비미국 종목명.
- **⭐ ②a 관심목록 렌즈 개편(739~742)**: 739(`9464f0e`) 배치 시세 `/api/watchlist/quotes`(국가별 스냅샷 .in) · 740(`e4571e7`) WatchlistClient 재설계(시세 즉시+행별 지연 렌즈 요약 강점/주의/보통·점7·`/api/lens` tone·동시성4·반응형2줄) · 741(`0fe6971`) 종목 hero+리딩방 /en 숨김 · 742(`44fa289`) 현재가 세로정렬(이름 flex-1·렌즈 w-64)+설명 종목화. 라이브 실측(가격 x=1269 정렬·삼성전자 강점2·주의2·보통3·en 리딩방 숨김).
- **🔑 제약**: `lens_scores`=US 1000종목만 → KR 관심목록 렌즈는 live `/api/lens` 지연(005930→.KS 실시간). ②b=KR 렌즈 선계산 크론(즉시화 후속).
- **🔍 발견/다음**: ① 비미국 종목명 /en 한글(name_en 없음·US SEC만 커버)→영문명 소싱(야후 longName) · ④ 종목페이지 렌즈헤더+보드 상단 자동미리보기(목업 확정·미구현·"거래상위 예시" 중립·스플래시 반대→인라인) · ②b KR 렌즈 선계산.
- **▶ 다음 순서**: ④ → ①(비미국 종목명) → ②b.

## 2026-07-17 — 🎨 베타 UX·가독성 폴리시 + 지수 티커 이중방어 + 광고문의 언어권 차등 ✅

**HEAD `21c6649`.** 클로즈드 베타 준비 겸 다크 가독성·모바일 레이아웃 폴리시 + 간헐 티커 사라짐 이중방어 + 광고문의 en 차등 + /en 푸터 정리. 전부 라이브 실측.
- **코인 탭 숨김**(`4135463`): 데이터 준비 전 노출 제거(라우트 보존).
- **다크 가독성**: muted `#8A8D93→#9CA3AF`(`6b2ce97`·AA턱걸이→AAA·토큰 전역) · 헤더/티커 `#0E1116` 흰색 `/40~55→/65~70`(`ed425f4`·태그라인 3.8→8.2:1·크기 무변).
- **모바일 풀블리드**(`659b0e2`): 박스 `border-y sm:rounded-2xl sm:border`+페이지 `px-0 sm:px-6`=모바일 화면끝·데스크톱 카드. 근거=반응형은 일관성≠동일함(웹서치 3회). + 티커↔보드 빈공간 제거(`155da9e`·모바일 `pt-0`).
- **🐞 티커 사라짐 이중방어**: 원인=`/api/yahoo/indices` `Promise.all`서 심볼 하나 실패→전체 reject→빈 응답(200) + 클라가 빈 응답으로 기존 티커 덮어씀(간헐 ~20%). **클라**(`fa8a201`·데이터 있을 때만+5초 재시도)+**서버 STEP 737**(`d391c0c`·심볼별 try/catch 격리+`_lastGood`). 라이브 16회 0빈(min 20=격리).
- **헤더 로고 반응형**(`0431d06`): 22→`h-6 lg:h-8`(24/32px)·Trillion `leading-none`·태그라인 `mt-0.5`(간격 9→2px).
- **광고문의 언어권 STEP 738**(`8c7c7a8`): en에서 리딩방(room 슬롯·rule2·폼옵션·note·phCompany) 제거·ko 유지·server `getLocale`+client `useLocale`. 라이브 en 2슬롯3규칙/ko 3슬롯4규칙.
- **교훈**: Vercel 배포 큐 지연→빈 커밋 재트리거 · `Promise.all` 하나 실패=전체(부분허용=심볼별 try/catch) · 클라 빈응답 무비판 반영 금지 · `resize_window` 최대화 창 무반영(DOM 검증 대체) · 반응형=일관성≠동일함.
- **🧹 /en 푸터 정리(후속)**: 유사투자자문 disclaimer2 en 숨김(`bba2ec0`·KR 전용·disclaimer1 범용 유지·ko 불변) + 사업자 주소 제거(`21c6649`·무거래 정보서비스=전자상거래법 §10 주소표시 비대상·상호/대표자/사업자번호 유지·법인 전환 시 재추가). → `/en` KR 전용 잔재 0·라이브 실측.
- **▶ 다음**: 클로즈드 베타 초대 · (선택) 모바일 폰트 크기.
...
- **라이브 QA 스윕**(8페이지): 거의 클린, 유일 발견=정적 페이지 6종 `/en` 메타 타이틀 한글.
- **727**(`d15dbed`): about·advertise·feedback·favorites·business·coin 메타타이틀 `generateMetadata` 로케일화(en 영어·ko byte 동일·711 패턴·robots 보존·terms/privacy/admin 제외). 라이브 검증 → **i18n 잔재 0**.
- **다크 D**(`1f661e3`): 미사용 `.shadow-soft`/`.shadow-soft-hover` 죽은 CSS 제거. 폴리시 백로그 소진.
- **729 US IPO 피드**(`9d977f0`): US IPO 탭 뉴스검색→구조화(Nasdaq 공개 API·`/api/ipo/us-feed`+`UsIpoFeed` 2섹션·priced→내부 종목상세·Toolbox 배선·i18n 패리티·JP/CN/VN/GB·KR 불변). **✅ 라이브 실측**=Vercel 200·실데이터 30건(MetaOptics/MOT·Csquare/CSQR)·403 없음·/en·/ko 렌더. **US=KR급 IPO 뎁스.**
- **교훈**: `export const metadata` 정적도 i18n 시 generateMetadata로 · Nasdaq IPO 공개 API(무키·헤더·다음달 0건→이번+지난달 병합·Vercel 403 없음).
- **▶ 다음(선택) = US 배당 캘린더(KR 완전 동급) · ETN 서브탭 · 클로즈드 베타.**
...
- **720**(`2645cf9`) `*_en` 컬럼 마이그(MCP) · **721**(`e34fee3`) brief R2[🐞 `brief_ko` NOT NULL→en-first INSERT 누수 될 뻔·DROP NOT NULL+에러로깅] · **722**(`60d5d8b`) news R3[+`tags_en`·한국어 강제 후처리 ko 게이팅] · **723**(`9329993`) 공시 R1 US · **724**(`5c0c348`) 공시 R1 5개국(723 복제).
- **컬럼 분리로 캐시 충돌 차단·on-demand 과금·KR byte 동일**(라이브 삼성 공시 ko/en 독립 증명). 교훈 `LENS_DEV_PLAYBOOK` #31.
- **▶ 다음(선택) = US 통화기호·빈뉴스 UX·OAuth 로케일 쿠키·다크 폴리시 D·클로즈드 베타.**
...
- **📋 US 감사**(서브에이전트+DB): 배관·보드·피드 7탭·**link_hub 139**(옛 'US 67 미충전'은 낡음)·brokers 17·지수바 완비. US가 더 깊은 곳=렌즈 백분위(US 전용)·공시 심각도 분류·서학개미 한글명. KR 전용(갭 아님)=코스피코스닥·상하한·유사투자자문사·유튜브.
...
- **▶ 다음 US(선택)= P1 통화기호`$` · P2 US IPO 구조화·ETN · (보류) 인라인 증권사 광고=수익화. 그 외=OAuth 로케일 쿠키·다크 폴리시 D·클로즈드 베타.**
...
- **🧭 유사투자자문사 정합**: `48e9802` AdvisorDirectory 패널 '유사투자자문 조회' 제목 + 문서 [이력·폐기] 배너 마무리(SESSION_KICKOFF·PLAYBOOK·NEXT_SESSION_START·INDEX·ROOM_VERIFICATION·TOSS_ANALYSIS·AI_LENS_SPEC) · `e199328` 라벨 '유사투자자문 조회'→**'유사투자자문사'**(금감원 등록업체 목록 성격) · `828e97c` 정렬 상호명 접두어((주)·㈜·(브랜드)) 무시 가나다(`/api/advisors` 전체로드→JS정렬→슬라이스·3뷰 공통) · `f66d77f` 하위탭 순서 증권사→유사투자자문사(증권사=주류 우선).
- **🔍 SEO**(`551d5e1`): 메타 keywords 새 정체성(리딩방검증·신뢰평가허브 제거→종목분석·TR-AI렌즈·검증된투자기법·유사투자자문조회).
- **🎓 온보딩 KR(Option A·자기설명 중심)**(`de58fca`): 헤더 태그라인(lg+)+상단 '소개' 링크(→/about)+LensPreview 문구 또렷("사고팔 신호 아니라 판단할 재료")+/about '이렇게 봅니다' 3스텝. **배너·팝업 없음**(과잉 온보딩 회피).
- **🖥️ /about 폭**(`e5c4a97`): 앱 기본(max-w-7xl)에 맞춤·3기둥 3열 그리드·본문 읽기폭 유지(폭 튐 해소).
- **🌑 다크 테마(라이트→다크·3단계·안 깨지게)**: **1/3**(`f029d91`) 토큰화 — 배경전용 `unjong-strong` 신설·`bg-unjong-primary`→strong(역할 분리·33곳)·`bg-white`→surface(28곳·겉모습 변화 0). **2/3**(`07fc4bf`) 플립 — `app/globals.css` 토큰 값만 다크로(background #0A0A0A·surface #17181C·border #2A2C31·`color-scheme:dark`·스크롤바)·앱 전체 다크·**라이브 검증**. **3/3**(`3c2fc8b`·🔴Opus) 폴리시 — 앰버 배지·게이지 fill·상태색 다크 대비+구글 로그인버튼·StockLogo 실로고원 라이트 유지·레터아바타 이니셜 `text-unjong-strong`. 근거=미드나잇+민트 브랜드·헤더 이미 다크·데이터밀도 금융툴 다크 정석. **개선이지 전환 아님.**
- **▶ 다음 = 다크 폴리시 D 후속(accent 틴트·shadow-soft·라이브 눈으로) · /about 3기둥 이름 직관화(목업 승인·미적용) · 클로즈드 베타 초대(`docs/BETA_INVITE.md` 준비됨) · 2차 i18n 스캐폴드(next-intl)→US 풀뎁스 · Vercel Analytics Enable(1클릭).**
...
- **🔵 브랜드 외부 슬로건 확정**(`docs/BRAND_IDENTITY.md` §0): 슬로건 **"종목을 보는 눈을, 누구에게나."** · 서브 **"모든 시각을 데이터로 — 판단은 당신입니다."** · 각인 = 멍거 원문("The best thing a human being can do is to help another human being know more."). 경쟁 백스페이스(남들 "이기게" ↔ 우리 "제대로 보게, 판단은 당신"). 이전 **"흩어진 금융정보를 한눈에"는 폐기**(편의 프레임·차별 실패).
...
- **🧪 개발 안전망 가동**(STEP 695): `lib/returns.ts` 순수 pct + vitest 유닛테스트 + `.github/workflows/ci.yml`(매 푸시 tsc→test→lint). 이후 로직 변경 회귀를 기계가 자동 검증.
- 📖 **전체 문서 마스터 인덱스 신설**: `docs/INDEX.md`(비-STEP 문서 67개를 카테고리별 카탈로그 + "언제 읽나"). CLAUDE.md 참조 테이블 맨 위에 연결.
...
- **🔴 브랜드 정체성 대개편**(`docs/BRAND_IDENTITY.md` 재작성): 3기둥 = **무기**(Arm·TR-AI 렌즈)·**직시**(See·1차 재료)·**자립**(Compete·판단은 당신). 뿌리=프로메테우스·칸트(Sapere aude)·그레이엄·멍거. 목소리=멍거 톤(건조·인센티브·"덜 멍청하게"). 근간="예언·추천 안 함, 불을 건넨다, 성공=당신이 우릴 덜 필요로 하게 됨". 슬로건: 타이틀 "전문가 시각으로, TR-AI가 무료로 분석해 드립니다"·설명 "가격은 시장이 붙이고, 가치는 당신이 매깁니다 — 판단은 당신 몫". 옛 운종 태그라인=[이력] 보존.
- **682 "AI 렌즈"→"TR-AI 렌즈" 명칭 통일**: 중앙 `AiLensBadge.lensLabel()` 다국어. 미리보기·6보드 힌트·종목 페이지 반영.
- **광고 수익화**(`docs/AD_MONETIZATION_PLAYBOOK.md` 신설·`lib/ads.ts`): 슬롯 인벤토리+어필리에이트+요금표+언어권 합법성 원장(KR). 한국=자본시장법상 퍼블리셔 어필리에이트 없음→직접 광고 제휴. 리딩방 광고=중심축서 내림. 진짜 파이=AI구독+증권사 어필리에이트(성과형).
- **678·679 OG/SEO 문구**: `app/layout.tsx` 메타 + 홈 `app/page.tsx` title override 제거(layout `title.default` 단일 소스). **상하이종합→SSE Composite**(지수 티커 영문 통일).
- **680·685 탭 14→3 재구조**(`ToolboxClient.tsx`): 상단 = 종목·정보·검증. 나머지 12개(뉴스·공시·리포트·기업재무·거시·ETF·공모주·증권사·차트·거래소·토론커뮤니티·유튜브)=**"정보" 하위탭**. 증권사=정보 하위(참조 디렉토리)·검증=상단(KR 게이팅)·유튜브=KR 게이팅. 근거=catch-all 금지+빅테크식 최소·직관.
- **677·681 미리보기 광고 완전 제거**→리스트 10개마다로 일원화. UI 태그라인 3곳(푸터·about·로그인) 새 문구.
- **683·684 증권사 소개글**: 20곳 계열/유형 중립 사실 note(Supabase `brokers.note` 갱신·옛 "20년 연속 1위" 대체·SK증권=독립계)·이름 옆(`ListRow` meta)·PC 너비.
- **685 종목 리스트 인리스트 광고**: 10개마다 증권사 데모 광고(KR 대신증권·`BrokerAdRow`·`lib/ads.boardBrokerAd`). "거래처 안내"지 투자권유 아님. 5개 비KR 보드는 그 언어권 광고 확보 시 동일 패턴.
- **686~690 ETF/상품 구성**(`docs/ETF_LENS_PLAN.md`): `/stock/{ETF}`가 기업재무 렌즈 대신 **"상품 구성"** 뷰(상위보유·섹터·보수율·운용사·추종지수). US=Yahoo topHoldings·KR=네이버 m.stock `etfAnalysis`(키 없음·Vercel 도달 확인). `lib/instrumentType.ts`·`app/api/etf-holdings/route.ts`·`EtfLensClient.tsx`·미리보기 요약(`LensPreview`). **영숫자 KRX 코드(0193T0) 인식 버그 수정**(`krCode /^\d[0-9A-Z]{5}$/`). 라벨="상품 구성"(AI 분석 아님). REIT=단일주식·ETN=구성없음·KRX getJsonData=LOGOUT→네이버 채택. 증권사 거래연결(수익화)=별도·미룸.
- **3중 검수 통과**: 코드리뷰 8파일 클린 + Supabase 브로커 20/20·홍보 0 + 라이브 US/KR ETF·브로커 API·홈 3탭/OG + 링크 카테고리 전부 채워짐.
- ▶ **다음 = ETF 증권사 거래연결(수익화) · ETN 정제 · JP/CN/VN/GB ETF 구성 · AI 구독(Phase 5) · 오늘 개편 모바일 실사용 QA.**

## 2026-07-09 (3rd) — STEP 668~672D · 성능 스냅샷 + 데이터 검수 Round 1 + VN HNX 보류 ✅

**VN HEAD = HOSE 403 복구 커밋.** 6개 보드 즉시화 + 검수로 실제 데이터 문제 다수 수정.
- **성능(668·668B)**: 5개 보드 라이브 야후→크론 스냅샷 DB 서빙·r1y 룩백 400일 복구.
- **669 US 종목명**: SEC company_tickers.json로 placeholder 4,231→55 실명 보강.
- **670 CN ETF 오태깅**: type 필드로 주식탭 정화 + ETF탭 A주 ETF 종목별 통화(CNY).
- **VN HNX(671~672D)**: 야후=HOSE전용→VCI(HNX 커버)→**VCI 클라우드 IP 차단**(Vercel·GH Actions "프로브≠지속")→**Yahoo HOSE 403 복구** + **HNX 보류(배선 완비)**: `scripts/vn_hnx_vci_cron.mjs`+`docs/PARKED_HNX_VCI_ACTIVATION.md`(VPS 필요).
- **🅿️ 보류 기능 프로토콜 표준화**: `LOCALE_SOURCE_PLAYBOOK §11`+CLAUDE.md 세션종료 체크리스트.
- ▶ **다음**: 데이터 검수 Round 2(Chrome)·Round 3 + CN #2(소형주) → 한국어 광고. ⚠️ Vercel 도달성 최종 실측 잔여.

## 2026-07-09 (2) — STEP 668 · ⚡ 5개 보드 가격 스냅샷화 ✅

**HEAD (이번 커밋).** VN·US·CN·JP·GB list 라이브 yf.quote 완전 제거 → DB 서빙(KR 미러).
- 마이그레이션 040: price·amount·r1y·r1d 컬럼 5개 테이블에 추가.
- `{cc}Perf.ts` 5개: volume→amount, r1y(252거래일), r1d 저장 추가. CN A주: eastmoneyBars f57.
- `{cc}-list/route.ts` 5개: yf.quote 제거·DB SELECT만. CN/JP subtab 필터 유지.
- 로컬 크론: VN 385·GB 349·CN 4008·JP 1080 computed 성공. **US Yahoo rate-limit → Vercel 배포 후 prod 크론 트리거 필요.**
- 응답 속도: 300ms (이전 수 초).

## 2026-07-09 — STEP 665~667 · 가독성 리파인 마감 + 지수 티커 6개국 + 대비 AA ✅

**HEAD `51e28c3`.** UI 리파인 묶음 완성 + 언어권 소스 런북 신설.
- **665·665B**: 표 AI렌즈 컬럼 제거·클릭 힌트 컨트롤 줄로 이동·수익률 '전' 통일·브리핑 13px.
- **666**: 지수 티커 TOPIX·상하이종합 추가(22개)·KR→JP→CN→VN→US→GB→ETC 블록 정렬·구분선·라이브 확인.
- **667**: 검증 배지 민트→다크틸(AA)·6개 보드 빨강/파랑 범례. (muted 이미 AA·라이트 톤 유지.)
- **🌍 `docs/LOCALE_SOURCE_PLAYBOOK.md` 신설**: 언어권 데이터소스 발견·검증·기록 런북(의미우선 스키마·검증게이트·실패원장·서학개미/6개국 실측·CLAUDE.md 참조 등록). "값이 아니라 방법·정의를 코드화."
- ▶ **다음**: 광고 대화(진짜 광고 데이터 모델) 또는 서학개미 relevance 파이프라인(플레이북 §5·data.go.kr 키). ⚠️ 배포 후 Vercel 도달성 최종 실측.
...
- **659 CnEventLayer(`f3fee9b`)**: cninfo(증감회 지정 공식 공시)·A주 `.SS`/`.SZ`·isCN. topSearch code→orgId(하드코딩 금지)+hisAnnouncement·온디맨드+캐시.
- **660 CnFilingSummary R1(`73dfc9b`)**: adjunctUrl PDF→unpdf 추출→중국어→한국어 요약→`filing_summaries`[`CN`+id]. 格力 주주총회 요약 정확·텍스트 PDF.
- **661 HK(`4404424`)**: HKEXnews `.HK` 공시층+R1(prefix→stockId→titleSearch)·CnEventLayer/CnFilingSummary 재사용·isCN에 `.HK`·summary SSRF에 hkexnews(acc `HK`+id). **CN 완결.**
- **📊 공식 공시 R1 = US·KR·JP·GB·CN(A주+HK).** VN=뉴스 이벤트층+R3(R1 보류).
- ⚠️ 배포 후 cninfo·HKEXnews Vercel 도달성 실측 대기(东方財富 IP차단 전례).
- 🧠 Phase 5 메모(`docs/AI_BRIEFING_SPEC.md`): TradingAgents 참고 강세/약세 병치(지금 R2 적용 안 함).
- ▶ **다음**: 광고(대화 먼저) 또는 국가 추가(인도·대만).

## 2026-07-08 (2nd) — STEP 657~658 · 🇻🇳 VN 공시층(뉴스·이벤트) + VN 마감 ✅

**HEAD `1b8e1e1`.** VN 종목 페이지에 뉴스·이벤트층 추가, R1은 소스 한계로 보류하고 VN 마감.
- **657 VnEventLayer(`04cae64`)**: `/api/vn-events`+isVN. TCBS `tcanalysis` 전경로 404·CafeF AJAX 빈응답·HNX/SSI 도달불가 → **Google News RSS(vi·VN)** 재무 키워드 필터.
- **657B Vietstock NO-GO(`5459b0b`)**: 공시 AJAX 토큰이 JS 렌더 후 삽입→서버fetch 불가. Google News 유지+정직 라벨("최근 주요 뉴스·이벤트 · Google News").
- **658 VnFilingSummary R1 → 0%(`1b8e1e1`)**: 구글뉴스 `/rss/articles/CBMi...` 링크=JS전용 디코딩(서버fetch 400). 로컬·Vercel 동일 0% → 항목별 숨김(코드 보존).
- **🏁 VN 마감**(사용자 승인): 공식 공시원문 소스 부재. 베트남 시장 규모 대비 노력상한 → VN=이벤트층+R3 뉴스요약 커버, R1 보류. 공시 R1 공식=US·KR·JP·GB 4개국.
...
- **📊 공시 R1 = US(EDGAR)·KR(DART)·JP(EDINET)·GB(RNS) 4개국 완성.**
- **656 VN 정찰**(코드 없음·investigation): TCBS 공개 API(`apipubaws.tcbs.com.vn`) 도달되나 `tcanalysis` 경로 폐기(404)→회사-이벤트 엔드포인트 네트워크 캡처 필요/대안 CafeF·Vietstock 서버렌더 스크랩. VN 빌드는 다음 세션 → 자급형 실행계획 `docs/NEXT_SESSION_VN_PLAN.md` 신설.
- ▶ **다음**: VN 공시(공시층+R1) — **`docs/NEXT_SESSION_VN_PLAN.md` 먼저 읽고 착수** → CN → 광고(대화 먼저).

## 2026-07-07 — STEP 645~648 · 완전성 청산(매매처·JP공시) + 헤더 홈 픽스 ✅

**HEAD (STEP 648 = 이 커밋).** 룰 위반 청산 + UX 픽스.
- **645 매매처 DB 배선(`0023fda`)**: `brokers` 테이블(KR·US·JP·VN·GB 75행, MCP 생성)을 화면 배선 — `/api/brokers?region=KR`+`BrokerRanking`. 언어권 기준(플레이북 §4-3): 한국어=전탭 KR증권사. CN만 미보유.
- **646~647 JP 공시 = EDINET(`0ea7189`·`5d9e90a`)**: 무료 공식 API(키 env)로 청산. `jp_disclosures`+`lib/edinet.ts`+크론(미리계산). **라이브 12,466건·2,148개사·臨時報告書 2,260건**, 도요타·소니 확인. dedup 픽스로 백필 완결. 교훈: env는 Vercel 대시보드+재배포 필요(.env.local≠배포).
- **648 헤더 로고→홈 픽스**: `homeReset`이 미소비+persist 탓 → resetHome이 국가KR+탭market 리셋, ToolboxClient n 구독해 탭·서브·보드(주식) 리셋.
- ▶ **다음**: JP STEP 649(JpEventLayer+R1 UI) · 완전성 GB(RNS)→VN→CN · 광고.

## 2026-07-07 — STEP 640~643 + 검색엔진 등록 · 🔍 한국어 SEO 완결 ✅

**HEAD `6c5e9d7`.** 온페이지 SEO(635~639) 위에 검색엔진 등록 + 해외 한글명 얹어 한국어 SEO 마무리.
- **640** 문서 갱신(`21a87d9`). **641 구글 서치콘솔** + **642 네이버 서치어드바이저** 소유권 인증(`layout.tsx verification` google+naver) + **sitemap.xml 양쪽 제출**(구글 19,983 URL 성공·네이버 등록).
- **643 해외종목 한글명 121종**(`6c5e9d7`): `data/foreign_ko_names.json` + `resolveStockName` 오버라이드(한글 우선·영문 `en` 병기). 라이브 검증 통과(테슬라·애플·메타·도요타·텐센트·대조군 3M). META·BABA·ARM 사이트맵 합류.
- ▶ **다음**: 한국어 광고 설정.
...
**HEAD `3f38f33`.** 국가탭 = US·KR·JP·CN·VN·GB **6개국** 완성. 언어권 거미줄 로드맵대로 한국어권 국가 확장.
- **622 플레이북/로드맵**: §4-4(R3 네이티브 이름·야후 영어명 함정·소스 프로브)·§4-5(3중 검수법) + ROADMAP §2-1(언어권 거미줄).
- **623~627 🇻🇳 베트남 완성**: 링크49·배관(vi·₫)·보드(HOSE 387·야후 `.VN`·vnstock 유니버스)·**지수바(VN-Index/VN30=VnDirect 대체, 야후 미커버)**·매매처13·R3(`vn_names`·vi·3중). 东方財富/HNX 야후 미커버→텐센트/HOSE-only.
- **628~630 🇬🇧 영국 완성**: 링크46·배관(en-GB·펜스)·보드(FTSE 350·349·야후 `.L`·Wikipedia 헤더파싱 유니버스+영문명)·**지수바(FTSE 100/250)**·매매처12·R3(`gb_names`·en-GB·3중). 영어권=이름 클린화만.
- **🔴 완전성 원칙 못박음**(VN-Index·매매처 '후속' 실수 교정): CLAUDE.md+플레이북 §0 = 새 탭 착수 전 플레이북 재독·MVP≠축소·DoD 빠짐없이·소스 막히면 대체.
- ▶ **다음**: 한국어권 마무리(디테일+한국어 SEO+광고→한국어판 MVP). 또는 국가 더(인도·대만).

## 2026-07-06 — STEP 612~620 · CN R3 + JP·CN 네이티브 종목명 + 4개국 R3 3중 검수 ✅

**HEAD `55c94df`.** 4개국 R3 뉴스를 전부 자국어 네이티브로 + 3중 검수·정직 수정.
- **612~616 CN R3 + JP 네이티브(`f1ff19a`)**: 야후가 JP/CN을 영어명으로 줘 ja/zh가 실은 영어검색 → **JPX data_j.xls → `jp_names` 4,014종목(일본어명)** 시드 → ja가 진짜 일본 기사. CN R3 zh + 로컬 0건 시 영어 폴백. 야후 영어명은 폴백.
- **618~619 CN 네이티브(`6a9cecd`)**: `cn_names` **7,095행 = HK 3,227(HKEX 번체·zh-HK) + A주 3,868(텐센트 qt.gtimg 간체·zh-CN)**. **东方財富이 KR·샌드박스 IP 차단(exit52/502)→텐센트 우회**(GBK). `lib/cnName.ts`·`scripts/seed_cn_names.ts`. HKEX xlsx `!ref` A1:R8 오류→범위 재계산.
- **620 3중 검수 수정(`55c94df`)**: 실제 종목 3회 독립생성 → ① KR **NAVER 빈 요약**(영문 상장명→블로그 잠식) = `lib/krName.ts` 별칭(035420→네이버) ② JP/CN **통화 오표기**(SoftBank "원"←엔) = 결정론 후처리(ja 원→엔·zh 원→위안·KR 안 탐) ③ 회사명 CJK = 한글화 프롬프트. 재검증 통과.
- **국가별 AI: US·KR = R1·R2·R3 / JP·CN = R3 네이티브 완성. 4개국 R3 3중 검수 통과.**
- **교훈**: 야후 영어명이 네이티브 검색 막음 → 거래소/포털 원어명(JPX·HKEX·텐센트)이 정답. 东方財富=데이터센터/KR IP 차단→텐센트(GBK) 대체. 통화·회사명=프롬프트 실패→결정론 후처리(610 재확인).
- ▶ **다음**: 베트남 탭 / 전 국가 추가 검수 / SEO.

## 2026-07-06 — STEP 600~611 · 'AI 렌즈' 브랜딩·발견성 + KR AI 확장(R2·R3) + JP R3 뉴스 ✅

**HEAD `b2079b7`.** 브랜딩 통일 + 발견성 + KR·JP AI(데이터 교체).
- **600~601(`121daae`)**: 'TRAI'→'AI 렌즈' 박스 배지("기법별 전망" 제거) · 종목 뒤로=`router.back()`+**시트 URL 동기화**(`lib/useSheetSync.ts` — 홈=보드라 그냥 back하면 모바일 시트 소실 → `?s=심볼` push로 뒤로 시 시트 복원) · 시트 현재가 중복 제거.
- **602~603**: AI 렌즈 발견성 표식(현재가↔1일전 사이 · PC 전용 컬럼/모바일 상단 라벨+행 민트 아이콘 · 신호 전용·탭→시트→'보기').
- **R4 영구 보류**(`AI_BRIEFING_SPEC`): Q&A=어드바이저=정보 허브 포지셔닝 밖+무료 모델 상충. R1~R3=AI 완결 범위.
- **🎉 604~606 KR AI(`cf22aba`)**: R2 브리핑 DART 공시 · R3 뉴스 한글명(`dart_corp_codes.corp_name`)+ko · R3 짜깁기 금지 가드레일. MCP 검수(삼성·SK하이닉스·NAVER) 통과. "US 완성형→데이터 교체" KR 실증.
- **🎉 607~611 JP R3(`b2079b7`)**: JP(.T)=야후 일본명(`fetchYahooName`)+ja. **결정론 후처리** — 요약 한국어 아니면 번역 폴백(야후 영어명 대응)·작년 이전 연도(2023) 언급 문장 정규식 삭제(구글 옛기사 재순환 대응)·pubDate 60일 최근성(전 국가). MCP 검수(도요타·소니·소프트뱅크) 통과. **교훈: LLM 프롬프트 2회 실패→코드 후처리로 확정.**
- **국가별 AI: US R1·R2·R3 / KR R1·R2·R3 / JP R3(공시=EDINET 대기) / CN 미착수.**
- ▶ **다음 = CN R3 / SEO — JP 공시=보류 확정(EDINET/TDnet 무료 실시간 없음·R3 대체). 국가 확장은 승인 후.**
...
- **R1(591)**: 8-K 원문 → 한국어 사실 요약(`eightKSummary`·`/api/events/summary`·`filing_summaries` 캐시). NVDA 5.02 정확.
- **R2(592)**: 서버 `computeSymbolLenses` 재계산+공시 → 핵심 긴장+지켜볼 것(`/api/brief`·`stock_briefings`). 최상단 카드.
- **R3(593)**: Google News → 사실 요약+중립 토픽태그(`stockNews`·`/api/news-brief`·`news_briefs`·조건부).
- **마이그레이션(MCP)**: 030·031·032 (전역 캐시·public read).
- ▶ **다음 = R1-KR(DART)부터 국가탭 데이터 교체 → 전 국가탭(JP·CN) → SEO.**

## 2026-07-06 — STEP 584~589 마감 + 🔴 AI 브리핑 레이어(R1~R3) 전략·설계 확정 ✅

**HEAD `3f4b647`.** 종목 페이지 "AI LENS" 명명·전문가 톤·정직 보이스 마감(584~589) + **AI(LLM) 브리핑 레이어 + 접근/수익 모델 대전환**(대부분 전략·설계 세션).
- **584~589 (코드·문서)**: 585 페이지명 AI LENS+progressive disclosure+이벤트 severity · 586 한국어 보이스 v1(`VOICE_GUIDE` 신설) · 587 전문가 톤 1차(접힘=판정+수치·디스클레이머 통합) · 588 판정 보이스 v2 · 589 시간축 스트립 초보 정리.
- **🔴 AI 브리핑 레이어 (마스터 `docs/AI_BRIEFING_SPEC.md`)**: LLM=텍스트를 사실로 읽는 것만. R1 공시요약·R2 브리핑(긴장+지켜볼것)·R3 뉴스·R4 안 함. 배관 재활용(`ai-analysis`·`eightK`).
- **🔑 접근/수익 대전환 (BUSINESS_STRATEGY 07-06)**: AI 브리핑 무료·공개(**구독 폐기**)=SEO/글로벌 엔진 · 로그인=개인화(즐겨찾기·알림)만 · 수익=광고/디렉토리/제휴(트래픽 뒤).
- **리서치**: 뉴스 감성 팩터 기각(사실 브리핑만) · 추정치 변경 렌즈 보류(라이브 무료·백테스트 유료→참고용).
- ▶ **다음 = AI R1-US 빌드 → R2 → R3 → 전 국가탭 → SEO** (US 완성형→국가 데이터 교체·수익화/문서화 그 뒤).

## 2026-07-05 — STEP 579~583 완료 · 시간축 재구성 + 실시간 이벤트(공시) 사실 레이어 (US 완성형) ✅

**HEAD `c39117b`.** 종목 페이지를 시간축(단기·중기·장기) 중심으로 재편 + 유료급 공시 이벤트 층(EDGAR 8-K).
- **시간축 (579~580)**: `horizon` 필드 + 퍼센타일(`lens_percentiles` 029·방향별·US 1000) → 시간축 스트립(단기 RSI존·중기 모멘텀 퍼센타일·장기 팩터 "N중 M 우호") + 기법별 best-viz(퍼센타일 게이지·RSI존·체크리스트) + 단/중/장 그룹핑. 비US 방향 폴백.
- **이벤트 층 (581~583)**: `lib/eightK.ts`(8-K→렌즈·A/B/general·flagLens) + `/api/events`(EDGAR submissions items 결정론·US) + `EVENT_LAYER_SPEC.md`. "최근 공시" 리스트 + 렌즈 ⚠️/📌. 정직화: 5.02 순화(리스트만)·F-Score 플래그·9.01 제거·A/B 분리. **사실만·예측 없음.**
- **전략 (07-05)**: "3개의 시계"·펀더 신선도=추정치 변경(Zacks)·free/pro=StockTitan 티어·공시 DART+EDGAR 무료. 유료 벤치마크: Stockopedia·StockTitan·Benzinga WIIM·Zacks.
- ▶ **다음**: ② **AI 원문 실독 요약**(정확한 내용 명시·무료N/Pro) → 거래량 맥락(WIIM-lite) → 추정치 렌즈(US) → KR 공시(DART).

## 2026-07-04 — STEP 570~577 완료 · 스크리닝 인프라 + F-Score 실물·표시 헌장 + 🔴 TRAI 정체성 결정 + 6카드 헌장 ✅

**HEAD `be86401`.** 오늘 = 스크리닝 뒷단 완성 + F-Score를 유료 표준급 정직 카드로 실물화 + 규칙을 표시 헌장으로 + TRAI(AI 결론) 폐기하고 제품 정체성 재정립 + 6카드 헌장 확산.
- **스크리닝 토대 (570~573)**: 공용 엔진 `lensCompute` 추출(카드=배치 계산 일치)·`LensRead.value/state` → `lens_scores` 테이블·시드 → `lensPrecompute`(시총 상위 1000·100행 flush) → 매일 20:00 UTC 크론. **스크리너 UI는 안 만듦**(사용자: 종목 페이지=본체·스크리너=픽에 가까워 중립 충돌). 미리계산 인프라=대기(나중 보드 힌트).
- **F-Score 실물 + 표시 헌장 (574)**: 성적표→부실 위험 체크·이름 크게·이게 뭐예요 박스·9칸 트래커·9항목 3그룹(수익성·재무·효율, 전문용어+쉬운풀이)·상단 등급 범례. **`docs/LENS_DISPLAY_CHARTER.md` 신설**(7카드 규칙·F-Score 기준 템플릿). 유료 대조(GuruFocus·Stockopedia)로 구조 검증 + 우리 쉬운말·정직(t≈0.7) 우위.
- **픽스 (575)**: 이익의 질 근거 `>` → `·`.
- **🔴 TRAI 스텁 제거 + 정체성 결정 (576)**: "AI 종합 분석"이 사용자 판단권 침해 → 제거. 리서치(Danelfin·TipRanks: 뉴스+팩터 결합은 표준·위반은 Buy/Sell뿐)로 **④ 재정의**: 뉴스=투명 사실 렌즈(FinBERT 무료+8-K 이벤트·헤드라인)·블랙박스/권유 아님·**결론은 사용자**·맨 마지막 층. **정체성 = "정직한 재료로 사용자가 판단하는 앱".** (BUSINESS_STRATEGY 로그·헌장 §0 원칙5.)
- **6카드 헌장 (577)**: 공용 템플릿 1곳 → 6개 동시(이름 크게·이게 뭐예요 박스·접힘 메뉴) + F-Score 접힘 일관. 7장 통일.
- ▶ **다음**: 6카드 눈검수 → 렌즈별 문구 다듬기 + **기법별 유료 레퍼런스 대조**(헌장 §5) → 조합전략(③) → 뉴스 투명 렌즈(④·맨 마지막). (일/중 카피·수익화 계속 뒤로.)

## 2026-07-04 — STEP 564~568 완료 · 카드 패밀리룩 · "이 기법 방향" 층 · 제품 청사진 ✅

**HEAD `ebbf3d8`.** 카드를 패밀리룩으로 재편하고, 측정에서 멈추던 카드에 **그 기법 방법대로의 방향(outlook)** 을 붙이고, 제품 4층 청사진을 확정·로그.
- **카드 패밀리룩 (564~567)**: **564** 시각 계층(그림자·둥근 모서리·원형 화살표·서랍 틴트) · **565** 정보 순서(설명=이름 밑 서브타이틀·판정→스펙트럼→근거→단기/장기) · **566** 메뉴화(접힘=깔끔한 "기법 메뉴"·판정은 펼침 안으로 — 사용자 "판정 늘어놔도 일반인 눈엔 엇갈림 안 잡힌다"에 설득, 내 '엇갈림 스캔' 논리 약함 인정) · **567** 3구간 스펙트럼(`Spectrum{labels,active,tone}` 공통·활성만 색조·7기법+F-Score 패밀리룩·`SPECTRUM_LABELS` ko/en).
- **"이 기법 방향" 층 (568)**: `LENS_OUTLOOK`(ko/en)·`outlookOf()`·스펙트럼 밑 "이 기법 방향" 줄(F-Score 동일). 그 기법 '방법대로'의 방향 — 시간축 단기/장기·유리/불리/중립·정직 꼬리표. **예측 아님**(역사적 base-rate). 저변동=위험·F-Score=건전성·기술=상태 축 유지(억지 "오른다" X). 사용자가 따로 분석 안 해도 되게. API 검증 + 눈검수 통과.
- **제품 청사진 (568)**: 4층 = ①원자(7 검증 팩터 ✅) → ②방향(outlook ✅) → ③조합전략(가치+모멘텀·방어형 퀄리티·QARP=버핏류·피오트로스키 가치·마법공식 보류 — "~류 근사" 정직 꼬리표) → ④TRAI(사실+뉴스 = 의견·맨 마지막). `BUSINESS_STRATEGY` 결정 로그. 정직: 렌즈=사실·앞은 TRAI(의견)·안 되는 기법 억지 X.
- ▶ **다음**: 세부 문구 다듬기 · **전 종목 하루 1번 미리 계산**(스크리닝 토대) · **검증된 조합 전략**(가치+모멘텀 등 유명 조합·"~류 근사") · 맨 마지막 TRAI. (일본어·중국어 카피=사용자 게이트 · 수익화·유료 TRAI 계속 뒤로.)

## 2026-07-03 — STEP 557~562 완료 · 발생액 탈락 · 3중 교차검증 · UI 편의성 ✅

**HEAD `4237714`.** 새 기법을 규율대로 정리(발생액 탈락→강력 후보 소진), 7렌즈를 3중 교차검증으로 재확인, 페이지를 중립·직관·모바일로.
- **발생액 탈락 (557~558)**: (순이익−영업현금)/총자산 저−고 롱숏 **연 −7.62%(방향 역전)**·t−1.36·FF3 알파 음수 → Sloan 미재현(1996 후 감쇠). 렌즈 미채용. **강력 후보 소진**(퀄리티·자산성장 채용 / 주주환원·발생액 탈락 / 마법공식 보류) — 무료·소표본 데이터 한계.
- **7렌즈 3중 교차검증 (559~561)**: 롱숏을 초·중·후반 3구간 나눠 방향 일관성. **모멘텀 [+,+,+] t3.6 · 퀄리티 [+,+,+] t3.2 = 시기 무관 단단** / 자산성장 [+,+,+] 유의미달 / 밸류 [+,−,+] 시기의존 / 저변동 raw 취약(방어 별도) / 기술 모멘텀 중복. **등급 변경 없음 = 그레이딩이 이미 정직**. 6렌즈 note 반영·플레이북 #27.
- **UI 편의성 Phase 2 (562)**: 한 페이지 유지(탭 X·엇갈림 보존) · 카드 압축(판정+등급+근거수치)/펼침(상세) · F-Score 동일 · **"기법 성향" 종합줄 제거(중립화)**. 모바일 390px 눈검수 통과.
- ▶ **다음**: 일본어·중국어 카피 · 배포 안정화. (새 기법 추가 일단락 — 강력 후보 소진 / 수익화·유료 TRAI 계속 뒤로.)
