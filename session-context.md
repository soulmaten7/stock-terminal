<!-- 2026-06-28 -->
<!-- Last GC: 2026-06-28 (STEP 422~448 완료 — 리딩방·검증 MVP 2.0 평가 디렉토리[클레임·인증·광고]+OG 프리뷰+표형 디렉토리, ROADMAP §3 정책 확정, 테스트 데이터 정리. TODO·다음 후보 갱신) -->
# Trillion(트릴리언) — 프로젝트 맥락

## (이어서 2026-06-28) STEP 422~448 — 리딩방·검증 = MVP 2.0 평가 디렉토리(업체 클레임·인증·광고 + OG 링크 프리뷰 + 표형 디렉토리) + ROADMAP §3 정책 확정 ✅

HEAD `6596ccb`(STEP 448, push 완료). **리딩방·검증을 '업체 클레임·인증·광고' 시스템으로 완성 + ROADMAP §3 게재·광고 정책 확정.** 금감원 유사투자자문 신고가 주체, 인증 업체가 채널 링크 관리(1무료+추가유료), 광고=노출(순위)만. DB = NEW "Trillion"(`ccbwxcszdoyjxvckedfp`).
...
- ▶ **다음**: 관리자/마이페이지 점검 · 크론 실작동 확인 · 결제 PG+본인인증(Phase 2 후반) · 죽은코드 정리 · (최종) Trillion AI 전망.
