  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
        {summaryCards.map((card) => (
          <div key={card.label} className="bg-dark-700 rounded-lg border border-border p-4">
            ...
            <p className="text-text-primary font-bold font-mono-price text-sm">{card.value}</p>
            {card.sub && <p className="text-text-secondary text-xs mt-1">{card.sub}</p>}
          </div>
        ))}
      </div>
      ...
      <div className="bg-dark-700 rounded-lg border border-border p-4">
        <h3 className="text-text-primary font-semibold mb-4">매출/영업이익/순이익 추이 (억원)</h3>
      ...
      <div className="bg-dark-700 rounded-lg border border-border overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {financials.map((f) => (
              <tr key={f.id} className="border-b border-border/50 hover:bg-dark-800/50">
                <td className="p-3 text-text-primary">{formatDate(f.period_date)}</td>
