      ...
      <div className="space-y-px">
        ...
        {asks.map((a, i) => (
          <div key={`a-${i}`} className="flex items-center h-6 relative">
            ...
            <span className="relative z-10 flex-1 text-right text-sm font-mono-price text-[#999999] pr-2">{a.volume > 0 ? fmt(a.volume) : ''}</span>
            <span className="relative z-10 w-20 text-right text-sm font-mono-price text-[#007AFF] font-bold pr-1">{a.price > 0 ? fmt(a.price) : ''}</span>
            <span className="w-20" />
          </div>
        ))}
        ...
        {bids.map((b, i) => (
          <div key={`b-${i}`} className="flex items-center h-6 relative">
            <span className="w-20" />
            <span className="relative z-10 w-20 text-left text-sm font-mono-price text-[#FF3B30] font-bold pl-1">{b.price > 0 ? fmt(b.price) : ''}</span>
            <div className="absolute right-0 top-0 bottom-0 bg-[#FF3B30]/10" style={{ width: `${(b.volume / maxVol) * 100}%` }} />
            <span className="relative z-10 flex-1 text-left text-sm font-mono-price text-[#999999] pl-2">{b.volume > 0 ? fmt(b.volume) : ''}</span>
          </div>
        ))}
      </div>
    </div>
