...
const COPY: Record<Kind, { warn: string; empty: string }> = {
  room: {
    warn: "금감원 신고 여부(사실)와 사용자 평가 순으로 랭킹을 보여줄 뿐, 트릴리언이 추천·보증하지 않아요. 허위·작전·과장이 많으니 가입·결제 전 충분히 확인하세요.",
    empty: "아직 등록된 리딩방이 없어요",
  },
  channel: {
    warn: "주식 관련 유튜브·SNS·디스코드 채널이에요. 트릴리언은 사용자 평가 랭킹을 보여줄 뿐 추천·보증하지 않아요. 투자 판단·결과는 본인 책임이에요.",
