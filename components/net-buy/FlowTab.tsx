    ...
    <div>
      ...
      <div className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden">
        <div className="px-4 py-3 border-b border-[#F0F0F0] flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-[#999]">당일 기준</span>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[#F0F0F0] bg-[#F8F9FA]">
                <th className="px-4 py-3 text-left font-bold text-[#666] text-sm">투자자</th>
                <th className="px-4 py-3 text-right font-bold text-[#666] text-sm">코스피 순매수</th>
                <th className="px-4 py-3 text-right font-bold text-[#666] text-sm">코스닥 순매수</th>
              </tr>
            </thead>
            <tbody>
              {loading && (
                <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-[#999]">로딩 중…</td></tr>
              )}
              {!loading && error && (
                <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-[#FF3B30]">데이터를 불러오지 못했습니다</td></tr>
              )}
              {!loading && !error && rows.length === 0 && (
                <tr><td colSpan={3} className="px-4 py-8 text-center text-sm text-[#999]">데이터 없음</td></tr>
              )}
