...
// STEP 854/856 — 보드 배지 (순수 표시). verdict → 배지 1개. 정렬·필터 없음(851 §4 오해 미해결).
// 🔴 856 §2: 적자(lossMaking)는 verdict 무관 "적용 밖"(중립·위험색 금지) — 종목페이지 배지와 같은 규칙.
export default function RevDcfBadge({ verdict, gapYears, lossMaking }: { verdict: string | null | undefined; gapYears: number | null | undefined; lossMaking?: boolean }) {
  ...
  if (verdict === "over_cap") return <span className="rounded px-1.5 py-0.5 text-[11px] font-medium bg-unjong-mint/15 text-unjong-mint">{t("boardBadge.overCap")}</span>;
