...
export function LoadingState({ title = "로딩 중...", description, action, className = "" }: Partial<StateProps>) {
  return (
    <div className={`text-center text-sm text-unjong-muted italic py-4 ${className}`}>
      <p>⏳ {title}</p>
...
export function EmptyState({ icon, title, description, action, className = "" }: StateProps) {
  return (
    <div className={`text-center py-6 ${className}`}>
      ...
      <p className="text-sm text-unjong-primary mb-1">{title}</p>
      {description && <p className="text-xs text-unjong-muted">{description}</p>}
...
export function ErrorState({ title, description, action, className = "" }: StateProps) {
  return (
    <div className={`text-center py-4 ${className}`}>
      <p className="text-sm text-unjong-danger mb-1">❌ {title}</p>
      {description && <p className="text-xs text-unjong-muted">{description}</p>}
      {action && <div className="mt-3">{action}</div>}
    </div>
  );
