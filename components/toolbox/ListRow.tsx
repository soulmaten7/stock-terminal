...
export default function ListRow({
  ...
  const cls =
    `group flex cursor-pointer items-center gap-3 border-b border-unjong-border px-2 py-2.5 transition-colors last:border-b-0 hover:bg-unjong-background${
      sponsored ? ' bg-unjong-mint/[0.06] ring-1 ring-inset ring-unjong-mint/25' : ''
    }`;
  ...
  const inner = (
    <>
      {sponsored ? (
        ...
        <span className={`w-6 shrink-0 text-center text-sm font-bold ${rank <= 3 ? 'text-unjong-mint' : 'text-unjong-muted'}`}>
          {rank}
        </span>
      ) : null}
      {iconUrl !== undefined &&
      ...
      <div className={`flex flex-col ${hasMeta ? 'w-44 shrink-0 sm:w-52' : 'min-w-0 flex-1'}`}>
        <span className="truncate text-sm font-semibold text-unjong-primary group-hover:text-unjong-mint">{title}</span>
      ...
      <span className="ml-auto flex shrink-0 items-center gap-2.5">
        {stat ? <span className="shrink-0 text-xs font-bold text-unjong-mint">{stat}</span> : null}
        {trailing ? <span className="shrink-0">{trailing}</span> : null}
        <span className="flex shrink-0 items-center gap-0.5 whitespace-nowrap text-[11px] text-unjong-muted group-hover:text-unjong-mint">
