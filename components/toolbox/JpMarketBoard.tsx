...
export default function JpMarketBoard({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  ...
  const [sortKey, setSortKey] = useState<'amount' | 'name' | 'price' | PeriodKey>(() => (loadBoardView('JP')?.sortKey as 'amount' | 'name' | 'price' | PeriodKey) ?? 'amount'); // 정렬 키 — 기본 거래대금순
  const [sortDir, setSortDir] = useState<'desc' | 'asc'>(() => loadBoardView('JP')?.sortDir ?? 'desc'); // 정렬 방향 — 기본 내림차순
  ...
  const [page, setPage] = useState(() => loadBoardView('JP')?.page ?? 0);
  ...
  useEffect(() => {
    saveBoardView('JP', { sub: tab, sortKey, sortDir, page });
    ...
    <section className="min-w-0">
      ...
      <div className="flex gap-4">
                ...
                <Fragment key={r.symbol}>
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-1">
                        <p className="min-w-0 flex-1 truncate text-[16px] leading-tight text-unjong-primary"><span className="font-bold">{r.name}</span><span className="ml-1.5 text-xs text-unjong-muted">{r.symbol.replace(/\.T$/, '')}</span></p>
                        {r.lens ? <LensDots lens={r.lens} size={6} /> : null}
                      </div>
                      <div className="mt-1 flex items-center justify-between gap-2">
                        <span className="text-[14px] tabular-nums text-unjong-muted">{r.price ? formatPrice(r.price, 'JP') : '—'}</span>
        ...
        <aside className="hidden w-96 shrink-0 lg:block lg:sticky lg:top-11 lg:self-start lg:max-h-[calc(100vh-3.5rem)] lg:overflow-y-auto">
          <LensPreview stock={selectedStock ?? sorted[0] ?? null} market="JP" example={!selectedStock} />
        </aside>
      </div>
        ...
        <div className="lg:hidden">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
              ...
              <LensPreview stock={selectedStock} market="JP" compact />
            </div>
          </div>
        </div>
      )}
