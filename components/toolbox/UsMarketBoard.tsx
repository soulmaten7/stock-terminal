...
type SubTab = 'stock' | 'etf' | 'reit' | 'etn';
...
const ENDPOINTS: Record<SubTab, string> = {
  ...
  etn: '/api/yahoo/us-etn-performance',
};
const CACHE_KEYS: Record<SubTab, string> = { stock: 'us-stock-list', etf: 'us-etf', reit: 'us-reit', etn: 'us-etn' };
...
export default function UsMarketBoard({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  ...
  const [sortKey, setSortKey] = useState<'amount' | 'name' | 'price' | PeriodKey>(() => (loadBoardView('US')?.sortKey as 'amount' | 'name' | 'price' | PeriodKey) ?? 'amount'); // 정렬 키 — 기본 거래대금순
  ...
  useEffect(() => {
    saveBoardView('US', { sub: tab, sortKey, sortDir, page });
        ...
        <div className="lg:hidden">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 pb-4">
              ...
              <LensPreview stock={selectedStock} market="US" compact />
            </div>
          </div>
        </div>
      )}
