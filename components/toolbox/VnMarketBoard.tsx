...
function displayName(name: string): string {
  return name.replace(/^CTCP\s+/i, '');
}

async function fetchRows(): Promise<Row[]> {
  try {
    const j = await (await fetch('/api/yahoo/vn-list')).json();
...
export default function VnMarketBoard({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  ...
  const [sortKey, setSortKey] = useState<'amount' | 'name' | 'price' | PeriodKey>(() => (loadBoardView('VN')?.sortKey as 'amount' | 'name' | 'price' | PeriodKey) ?? 'amount');
  const [sortDir, setSortDir] = useState<'desc' | 'asc'>(() => loadBoardView('VN')?.sortDir ?? 'desc');
  ...
  const [page, setPage] = useState(() => loadBoardView('VN')?.page ?? 0);
  ...
  useEffect(() => {
    saveBoardView('VN', { sortKey, sortDir, page });
  ...
  const toggleWatch = (r: Row) => {
    ...
    fetch('/api/watchlist', {
      ...
      body: JSON.stringify({ symbol: r.symbol, name_ko: r.name, market: 'VN', country: 'VN', add }),
                    ...
                    <td className="py-2.5 pl-0.5 pr-2 sm:px-2">
                      <div className="flex min-w-0 items-center gap-2.5">
                        ...
                        <span className="min-w-0 truncate">
                          <span title={r.name} className="font-bold text-unjong-primary">{displayName(r.name)}</span>
                ...
                <Fragment key={r.symbol}>
                  <div onClick={() => { setSheetExpanded(false); selectStock(selectedStock?.symbol === r.symbol ? null : r); }}
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center justify-between gap-2">
                        <span className="text-[14px] tabular-nums text-unjong-muted">{r.price ? formatPrice(r.price, 'VN') : '—'}</span>
