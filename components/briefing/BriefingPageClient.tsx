        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <section className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden">
            <header className="px-4 py-3 border-b border-[#F0F0F0] bg-[#FAFAFA]">
              ...
              <p className="text-xs text-[#999]">Yahoo Finance v7 · S&P / NASDAQ / DOW / VIX</p>
            </header>
            <ul className="divide-y divide-[#F0F0F0]">
              {overnight.length === 0 && (
                <li className="px-4 py-6 text-sm text-[#999] text-center">데이터 없음</li>
              )}
              {overnight.map((item, i) => (
                <li key={i} className="flex items-center justify-between px-4 py-3">
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-sm font-bold tabular-nums ${item.up ? 'text-[#FF3B30]' : 'text-[#0051CC]'}`}>
                      {item.change}
                    </div>
                  </div>
                </li>
          ...
          <section className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden">
            <header className="px-4 py-3 border-b border-[#F0F0F0] bg-[#FAFAFA]">
              ...
              <p className="text-xs text-[#999]">DART · 실적·유상증자·합병·배당</p>
            </header>
            <ul className="divide-y divide-[#F0F0F0]">
              {schedule.length === 0 && (
                <li className="px-4 py-6 text-sm text-[#999] text-center">오늘 주요 공시 없음</li>
              )}
              {schedule.map((item, i) => (
                <li key={i} className="px-4 py-3 text-sm text-[#333] leading-relaxed">
          ...
          <section className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden">
            <header className="px-4 py-3 border-b border-[#F0F0F0] bg-[#FAFAFA]">
              ...
              <p className="text-xs text-[#999]">큐레이션 · 중요도 중 이상 · 7일</p>
            </header>
            <ul className="divide-y divide-[#F0F0F0]">
              {events.length === 0 && (
                <li className="px-4 py-6 text-sm text-[#999] text-center">예정 지표 없음</li>
              )}
              {events.map((e, i) => (
                <li key={`${e.date}-${e.time}-${i}`} className="flex items-center gap-2 px-4 py-3 text-sm">
                  ...
                  <span className={`text-xs font-bold shrink-0 ${IMPORTANCE_COLOR[e.importance]}`}>
