...
export default function FloatingChat() {
  ...
  useEffect(() => {
    if (!mounted) return;
    try { localStorage.setItem(STORAGE_KEY, chatState); } catch {}
  ...
  if (chatState === 'closed') {
    return (
      <button
        ...
        aria-label="채팅 열기"
        className="fixed bottom-6 right-6 z-40 w-14 h-14 rounded-full bg-[#0ABAB5] text-white shadow-lg flex items-center justify-center hover:bg-[#089693] transition-colors"
      >
    ...
    return (
      <div className="fixed bottom-6 right-6 z-40 w-60 h-10 rounded-full bg-white border border-[#E5E7EB] shadow-md flex items-center justify-between px-3">
        ...
        >
          ...
          마켓 채팅
          {unreadCount > 0 && <span className="text-[#FF4D4D] text-xs">({unreadCount})</span>}
        </button>
        <button onClick={() => setChatState('closed')} aria-label="닫기" className="text-[#999] hover:text-[#222]">
          <X className="w-4 h-4" />
        </button>
      </div>
    );
  ...
  return (
    <>
      <div className="fixed bottom-6 right-6 z-40 w-80 h-[440px] rounded-lg bg-white border border-[#E5E7EB] shadow-xl flex flex-col">
        {/* 헤더 */}
        <div className="h-10 border-b border-[#E5E7EB] flex items-center justify-between px-3 shrink-0">
