<!-- 2026-07-06 · 2026-09-05 범위 갱신 -->
# 🌍 국가 탭 표준 틀 (Country Tab Playbook)

> 🔴 **2026-09-05 방향 전환 이후: 이 문서는 옛 "모델 트랙" 국가탭(TR-AI 엔진·link_hub·종목보드·AI 렌즈 R1~R3)에만 적용된다 — 지금은 전면 폐지 상태(`docs/STATE.md` ⓪).** 지금 실제로 하는 **"채널 리포트 국가 확장"은 완전히 다른(훨씬 가벼운) 패턴**이다: `lib/constants/reportCountries.ts`에 `{code, flag, displayOrder}` 한 줄 + `messages/{ko,en}.json`의 `Today.countries.{code}.*` 텍스트만 추가하면 끝 — 컴포넌트 코드 수정 0(2026-09-05 JP 임시추가·복귀 실측으로 확인). **채널 리포트 국가를 추가할 땐 이 아래 §0~§7의 무거운 DoD(link_hub·종목보드·AI 렌즈 등)를 따르지 않는다** — 그건 옛 모델 트랙 것이다.
...
2. 🔴 **(2026-07-18 교체 — 국가탭 완성 룰 · 권위 = `ROADMAP §2-1`)** 새 국가탭은 **그 나라 로케일 기준으로 정보 레이어를 먼저 100% 완성**한다: ① 착수 = **현지 실물 대조 감사**(그 나라 리테일 실물 도구와 항목별 대조 → 있다/예약/안 만든다 체크리스트 DoD) ② TR-AI 엔진은 보편 — 어댑터(입력 가용성·공시 매핑)만 나라별 ③ **기존 완성 언어권(ko·en)에 정확한 번역 레이어**(종목명 브릿지·뉴스·AI 언어)까지가 DoD ④ 그 나라 로케일에선 자국탭 맨 앞(자동). (구 룰 "탭 라벨·설명은 한국어"는 한국어권-먼저 시절 것 — **폐기**. 기존 ko 화면 문자열은 유지하되, 새 탭은 새 룰로.)
