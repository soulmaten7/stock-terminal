...
### 5-5. 🔴 관측하고 싶었는데 남아 있지 않았던 필드 (STEP1058, 2026-08-17 — 목록만, 필드 추가 안 함)

`docs/probe_1017_cron_failure_causes.md`의 "2026-08-16 US 연쇄 실패" 절을 조사하며 실제로 아쉬웠던 관측 공백. 🔴 **아래는 목록이지 신설이 아니다** — 추가 여부·설계는 장은태 판정.

- **`us_market_cap` 갱신 실패 심볼의 원인별 명단** — 오늘은 08-15/08-16 `as_of` 대조로 552~966종목을 사후 재구성했지만, `us_market_cap`이 symbol 단일 PK 누적 캐시라 **다음 날 갱신되면 이 흔적 자체가 사라진다.** "그날 갱신 실패한 심볼이 누구였는지"를 담는 이력 테이블이 없다.
- **`lens-scores` 배치 실패의 청크 단위 위치** — `failedChunks`(개수)는 있지만 **어느 청크(알파벳 구간)가 실패했는지**는 어디에도 안 남는다. 이번엔 `us_valuation` 전/후 대조로 "A~E 구간"임을 우회 확인했지만, `lens-scores` 자체의 heartbeat만으로는 알 수 없었다.
- **`computeAndSaveValuation`의 결측 사유별 분해** — `revdcf` heartbeat에는 `valuationSaved`(전체 저장 건수)만 있고, 그중 몇 건이 `MISSING_MARKET_CAP`/`MISSING_NET_INCOME`/`NEGATIVE_EARNINGS` 등 **어느 사유로 결측인지의 집계**가 없다. 오늘은 SQL로 직접 재구성해야 했다.
- **Vercel 함수 런타임 로그의 과거 조회 경로 부재** — MCP(`get_runtime_logs`)는 403, CLI `vercel logs`는 구조적으로 **"지금부터 5분"만** 지원(과거 조회 불가)임을 이번에 새로 확인했다. 정상종료/강제종료를 우리 쪽 DB만으로 완전히 구분할 방법이 여전히 없다.
