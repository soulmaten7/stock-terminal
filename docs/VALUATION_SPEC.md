...
### STEP 970(2026-08-10) — 새 창(STEP951) 첫 정규 크론의 4축 라이브 변동 실측(08-08 vs 08-09)

🔴 **조사 전용. 계산 정의 무변경 — 이 절은 새 창 적용으로 4축 값이 실제로 얼마나 움직였는지 기록만 한다.** 정본 = `docs/REVDCF_SPEC.md` §10-A(새 창 라이브 검증 전문). 여기서는 4축 값 자체의 절대상대차만 요약.

**절대상대차(부호 무시, 대전제) 중앙값/p90**: PER 9.96%/**75.9%**(n=636, 가장 민감) · PBR 4.93%/28.8%(n=818) · PSR 3.45%/19.1%(n=888) · EV/EBITDA 9.16%/49.1%(n=551). PER의 큰 변동은 원시값 변화(598종목 `net_income` 절대상대차 중앙값 26.3%·p90 184%)로 직접 설명된다 — 창이 밀리며 관측 회계연도의 순이익 자체가 크게 달라지기 때문이지 계산 로직 오류가 아니다.
...
- 🟡 **STEP 968(2026-08-09) — 잔차 규명①: 재척도 근사를 제거한 정면 대조. 958의 "±1~7%" 프레이밍을 재판정·정정.** ①-B stockanalysis.com 재확인 — "회계연도 말 종가를 명시한다"던 958 서술은 **과장**이었다(페이지에 그 방법론을 밝히는 문구 없음, 라벨만 있음) — 다만 Yahoo Finance 독립 조회(query1.finance.yahoo.com, AAPL FY2024)로 실제로는 FY말 직전 거래일 종가($227.79, 2024-09-27)가 맞음을 재확인. `Market Cap` 행에서 역산한 주식수가 우리 FY 희석가중평균보다 일관되게 적어(AAPL −1.9%) 외부는 **기말 실제 발행주식수**를 쓴다는 정황(명시 문구 없어 추론). 5곳 추가 시도(WSJ·Macrotrends·roic.ai·WallStreetZen·Morningstar) 전부 차단 — 966과 동일 패턴.
  - **가설 검증(958의 재척도 방법이 편향됐는가)**: 958의 5종목 잔차 17/18이 음수(체계적 편향 신호). 대상을 결정적 선정(주식수 변동률 상위5/하위5, 분할종목[`BKNG` 25:1·`CRWD` 4:1, WebSearch로 확인] 제외)으로 10종목 추가(`PROP`·`QXO`·`HGTY`·`BRCC`·`CODX`·`GM`·`LUV`·`CROX`·`DVA`·`CARS`) — 15종목 전체에 958식 재척도 잔차를 재계산해 주식수 변동률과 상관: **극단 희석 5종목 포함 시 r=0.73, 제외한 정상범위 10종목만으로는 r=−0.92**(변동폭이 클수록 잔차가 큼, 거의 단조 순서로 재현). 🔑 **가설 참(TRUE)으로 확정.**
  - **재척도 제거 정면 대조**: FY말 시총 = Yahoo FY말 종가(15종목·종목당 1회) × 우리 FY 희석주식수(직접 구성, 재척도 없음) — 15종목 절대상대차 중앙값: **PER 19.83%→0.07%**(사실상 해소) · PBR 17.23%→8.29% · PSR 18.02%→8.34% · EV/EBITDA 15.85%→8.90%(전부 50%대 이상 감소, 부호 편향도 사라짐). **Citigroup PER: 958 −21.43%→963 우선주조정후 −13.04%→968 직접구성 +0.58%** — 963이 못 닫았던 잔차가 거의 전부 재척도 아티팩트였음을 확인.
  - **남는 잔차 분해(2건 개별 규명, 나머지는 미규명)**: ① Citigroup PBR — 우리(`commonEquity`, 963 정책) 기준 +13.64%인데 총자기자본(963 SEC확인 $208,598M) 기준으로 재계산하면 +3.92%로 좁혀짐 — **stockanalysis.com의 PBR은 총자기자본 기준으로 추정**(963의 commonEquity 정책 자체는 유효, 외부 플랫폼과 정의가 다를 뿐). ② `GM` EV/EBITDA(직접구성 후에도 −75.58%) — SEC 원문 직접 대조로 **`debt` 필드가 잘못된 0으로 저장돼 있음을 발견**: 실제로는 `LongTermDebtAndCapitalLeaseObligationsIncludingCurrentMaturities`=$131,758,000,000(FY2024)이 존재하나 우리 `DEBT_LT`·`DEBT_CUR`·`DEBT_TOTAL_SINGLE` 태그 union 어디에도 이 정확한 태그명이 없어 완전 누락 — GM Financial(캡티브 금융자회사) 부채가 커서 영향이 크다. 🔴 **이번 STEP은 조사 전용이라 태그 union을 고치지 않는다** — 발견만 기록.
  - 🔴 **958의 "±1~7%" 프레이밍 재판정 — 부분 무효.** 방향(작은 잔차)은 맞았으나, 958의 5종목 표본이 우연히 주식수 변동이 작은(−0.5%~−8.2%) 종목들이었을 뿐이다 — 같은 재척도 방법을 변동폭 큰 종목(`GM`·`LUV`·`CROX`·`DVA`·`CARS`, −19.9%~−26.9%)에 적용하면 13~27%의 "가짜 오차"가 났다(968 실측). **958이 "SEC 데이터 정확, 잔차는 분자쪽"이라 추정한 것은 맞았으나 그때는 추정이었고 지금은 확인됐다** — 이 차이가 DoD3 재판정의 근거다(아래 `LENS_COMPLETION_STANDARD.md` 갱신 참조).
  - **①-B(신설 직후 두 번째 적용) 완료**: 목표 3곳 중 1곳 성공, 야후 호출 15건(429 없음). 못 한 것 — PBR·PSR·EV/EBITDA 잔여 잔차(중앙값 ~8%) 대부분 미규명(Citigroup PBR·GM EV/EBITDA 2건만 규명), `HGTY`의 PER/PBR/PSR 극단치(시총갭 −8.6%인데 배수갭 최대 +309%) 미조사. 상세 = `docs/probe_968_residual.json`.
- ✅ **STEP 975(2026-08-10) — 968 잔차 8%대의 정체 확정: 대조 방법의 결함(shares 시점 불일치)이었다, 우리 모델 오차 아니다.** 야후 신규 호출 0건(968 재사용)·SEC 신규 호출 0건(`docs/probe_951_cache` 15종목 전량 캐시 보유). ①-A(Damodaran PBV/PS/EV 원전 3건) — **PBV·PS는 애초에 "주당" 정의가 아니라 "총액÷총액" 정의다**(분모 주식수라는 개념 자체가 원전에 없음), EV의 시총은 "primary shares outstanding"(시점 실제 발행) 시사. ①-B(stockanalysis·gurufocus·wisesheets) — 명시적 방법론 문구는 3곳 다 못 찾음(1곳 차단·2곳은 접근됐으나 서술 없음), 966·968과 동일 패턴.
  - **1단계 재현 — 성공.** 968의 갭차감후 수치(PBR 0.08%·PSR 0.16%·EV/EBITDA 6.82%) 정확히 재현. 1% 초과 예외 전수 나열 — PBR 3건(C+10.28·QXO+5.39·HGTY+318.27, Cowork 사전관측과 일치), PSR 3건(C−12.89[🔴 사전관측 목록에 없었음]·GM+11.70·HGTY+115.45), **EV/EBITDA는 11종목 중 9종목이 초과**(AAPL·NVDA만 미만 — 사전관측은 EV/EBITDA를 다루지 않았음, 신규 발견).
  - **2단계 — mcapGapPct의 정체 확정.** 968의 direct construction은 ours·ext 양쪽에 같은 FY말 종가를 쓰므로, mcapGapPct = (ourShares − extImpliedShares)/extImpliedShares다(수학적 항등식). SEC companyfacts `CommonStockSharesOutstanding`(FY말 instant, 우선)로 15종목 중 13종목(BRCC 미확보) 검증 — **11/13이 ±1% 이내로 일치**(PROP·HGTY만 불일치). 🔑 **"외부는 기말 실제 발행주식수를 쓴다"는 968의 추론이 직접 확인됐다.**
  - **3단계 — 재구성 후 재측정, 가설 정확히 확인.** fyMarketCap을 (FY말종가×**기말발행주식수**)로 교체(야후 재호출 없음): **PBR 중앙값 8.29%→0.053%·PSR 8.34%→0.112%**(13~14종목). **PER은 예상대로 커짐 — 0.07%→3.99%(11종목, 약 57배)**: PER은 원래 GAAP 가중평균희석주식수(EPS 구조)와 우리 계산이 구조적으로 일치했던 것이었고, 기말발행주식수로 바꾸면 그 일치가 깨진다 — "안 커지면 잘못 짠 것"이 커졌으므로 방법이 맞았다는 뜻.
  - **4단계 — 남는 것 분해.** GM은 재론 안 함(968 완결). DVA·BRCC·AMT는 GM식 "태그 완전누락(0)" 패턴이 아님(값 자체는 보유) — 잔차 원인은 미규명(EV 원시성분이 968 산출물에 안 남아 정밀분해 불가). 🔴 **HGTY(투자 완료, 968에서 investigated=false였던 것) — 시총 문제가 아니라 Up-C(합자회사) 연결구조의 정의차이 3건 조합**: ① `revenue`=ASC606 매출만(548.6M), 별도 `Revenues`(총계)=1,200M 존재(보험료수익 등 제외 추정) — PSR 잔차 방향·크기 대체로 설명 ② `equity`=모회사귀속만(150.3M), 연결총자본(비지배지분포함)=523.5M 존재 — PBR 잔차 설명 ③ `net_income`=연결총액(78.3M, `ProfitLoss`), 모회사귀속분≈17.0M(항등식 도출, 직접태그 아님) — PER 잔차 설명. **부가 발견(버그, 코드 무변경 확인만)**: `us_fundamentals.shares`=82,327,000은 FY2024가 아니라 **FY2021 값**이다 — `lib/revdcf/drivers.ts:67` `latestYear()`가 목표연도 근접성 없이 "창 안에 값 있는 첫 연도"를 반환해 SHARES_MORE 1순위(Basic)가 2021년에서 멈추고 2순위(CommonStockSharesOutstanding, FY2024 값 345,210,737 보유)로 못 넘어간다. 🔴 **단 345,210,737로 교체하는 게 답이 아니다** — 3단계 재계산 결과 오히려 악화(PBR 309.69%→601.6%). 외부 내재 주식수(90,051,290)는 오히려 버그난 82,327,000에 더 가깝다(Up-C 다중클래스 중 시장이 실제 가격을 매기는 유통주식만 반영하는 것으로 추정, 미확정). **production 영향**: `shares`는 PER·PBR·PSR·EV/EBITDA 계산엔 안 쓰임(전부 `us_market_cap` 라이브 시총 사용) — 무영향. 단 역DCF의 `sharePrice = mcap/dr.market.shares`(`route.ts:255`)에는 쓰인다 — HGTY는 현재 604종목 유니버스 밖이라 오늘 영향 없음.
  - **Citigroup PBR(§4-3)** — 새 항목 아님, 미해결 2번과 동일 건으로 재확인(shares 교체해도 10.28%→10.68%로 거의 안 움직임 — equity 정의차이가 원인, 968의 기존 결론 유지).
  - 🔴 **새로 드러난 것(범위 밖, 이 STEP에서 조사 안 함)**: GM.debt이 라이브 조회에서 다시 0으로 관측됨(969 수정 이후 fiscal_year가 2024→2025로 전진하며 재발한 것으로 추정, 미확인) · `latestYear()` 폴백 메커니즘이 HGTY 외 다른 종목에도 잠재할 가능성(15종목 표본에서는 HGTY 1건만 확인, 전수 스캔 안 함).
  - 상세 = `docs/probe_975_search.md`·`docs/probe_975_residual_decomposition.json`.
