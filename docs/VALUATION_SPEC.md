...
0. 🔴 **YS 고정창으로 최신 회계연도 1~2년이 누락된다(STEP 950, 2026-08-08).** `lib/revdcf/drivers.ts:12`의 `const YS = [2020, 2021, 2022, 2023, 2024]`가 하드코딩이라, 오늘(2026-08-08) 기준 이미 SEC에 존재하는 더 최신 회계연도를 못 본다 — **4축 전부가 영향을 받는다**(분자·분모 모두 이 창에서 나온다). 외부 독립 출처 대조로 발각: **NVDA PER 64.7% 과대**(우리 74.43 vs 실제기준 45.18) · **AAPL PER 19.5% 과대**(우리 48.78 vs 실제기준 40.82) — 둘 다 SEC 원문 재확인 완료(`docs/probe_950_ys_window.json` §0). `us_fundamentals` 861건(1,003 중 스캔 성공분) 기준 **83.9%가 누락연수 1년 이상**, Q1 4축 상대차 중앙값 7~9%·|상대차|>20% 비율 21~51%(축별). 🔴 **현재 화면에 노출되지 않았으므로 사용자 피해는 없으나, 값 자체가 틀렸다.** 역DCF(같은 코어)에도 영향 — 표본 20종목 중 verdict 변동 6/14(43%). 상세 = `docs/probe_950_ys_window.json` · `docs/REVDCF_SPEC.md` §11. **고치지 않았다 — 처방은 장은태 판정.**
