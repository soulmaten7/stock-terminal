...
> **HEAD 커밋**: `628b14d` (STEP 499 — AI보기 진입 버튼 + KR 6자리 해석). **배포 ✓ `onetrillion.app`** (STEP 422~499 push 완료, origin/main 최신). (494~499 = `628b14d` · 479~484 = `44e0aac` · 469~472 = `b741ead`)
> **DB**: NEW 전용 프로젝트 `ccbwxcszdoyjxvckedfp`("Trillion", ap-northeast-2). **`link_hub` KR 138·US 67 — ⚠️ MCP 직접 insert(마이그레이션/git 아님), US 미충전.** POTAL ref `zyurflkhiregundhisky`=절대 금지.
> **렌즈 현황**: `/api/lens`(온디맨드·30분 캐시) + `/stock/[symbol]` 렌즈 3카드 페이지 **라이브**. KR 6자리→`.KS`/`.KQ` 자동해석. 4개 보드 AI보기 진입 버튼 완료.
> **▶ 다음 후보**: ① **재무 렌즈**(F-Score·Z — KR=DART 키, US/JP/CN=야후 재무제표) · ② **유료 AI보기**(LLM 뉴스·공시 맥락 종합 온디맨드) · ③ **US 링크 풀충전**(KR 138 수준으로) · ④ **Phase 2 결제 PG + 빌링 테이블 + 본인인증**
