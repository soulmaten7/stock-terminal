...
> **HEAD 커밋**: `59cb0c1` (STEP 509 — 모멘텀 렌즈 canonical 12-1). **배포 ✓ `onetrillion.app`** (STEP 422~507 push 완료, origin/main 최신). (500~507 = `cc3dc99` · 494~499 = `628b14d` · 479~484 = `44e0aac`)
...
> **렌즈 현황**: `/api/lens`(온디맨드·30분 캐시) + `/stock/[symbol]`(렌즈 3카드 + F-Score 카드). `lib/fscore.ts`(9기준) + `lib/edgar.ts`(EDGAR 어댑터). F-Score = 재무 건전성 해석 확정(수익 예측 아님). EDGAR 다년 백테스트: 대형주 점수↔수익 불분명.
> **▶ 다음 후보**: ① **모멘텀 렌즈**(가격만·야후 다년 백테스트 가능·검증 첫 완주 사이클) · ② **深재무 F-Score(EDGAR) 제품 승격**(야후 5년 한계 대체) · ③ **US 링크 풀충전**(KR 138 수준) · ④ **Phase 2 결제 PG**
