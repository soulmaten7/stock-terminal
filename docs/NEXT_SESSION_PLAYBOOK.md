...
>
> **시점 스냅샷**: 2026-06-18 STEP 271 종료 — **종목 상세 점검 완료**(주식·ETF·ETN·리츠·미국 5종 전부 정상) + 홈 랭킹 표 UI 통일(♡·종목명 w-full·미리보기 클릭 트리거) + 헤더 홈/로고 **완전 리셋** + 미국 상세 차트 yahoo 연결. 탭 = 주식·ETF·ETN·리츠·리딩방 리스트.
> **HEAD 커밋**: `8670ba2` (STEP 271 · 종목 상세 미국 차트 yahoo). 본 문서 커밋이 그 위 docs 커밋.
> 데이터: 주식·ETF·ETN·리츠·미국 수익률 ✅ · 종목 상세 5종 ✅ (미국 호가·체결=카드 미표시=정상, 정보패널=`/api/yahoo/quote-detail`). 펀드 = 제거(무료 수익률 API 없음=유료 영역). 다음(전부 보류·사용자 결정): 리딩방·채널 검증 빌드(`docs/ROOM_VERIFICATION_SPEC.md`, 플랫폼 완성 후) · 모바일 반응형 · 카카오 OAuth · AI 해설 빌드 여부.
> **마지막 코드 변경**: `8670ba2` (STEP 271). 이번 세션(265~271) 큰 줄기: **265~266 홈 완전 리셋**(헤더 홈/로고→주식·국내·전체·1일 초기화, zustand 리셋 카운터+리마운트) + avatarBg 크래시 가드 · **267~270 랭킹 표 UI 통일**(순위 줄바꿈·♡ 전 타입·종목명 w-full·미리보기 hover→클릭) · **271 미국 상세 차트**(yahoo 일봉). 이번 세션 DB 변경 0
