...
> **HEAD 커밋**: `64a5d9a` (STEP 523 — 기술 검증 반영·무료 렌즈 5종 완결). **배포 ✓ `onetrillion.app`** (앱 코드 라이브; 렌즈 검증 STEP 510~523은 렌즈 카드 문구·백테스트 스크립트·문서 위주). (520~521 밸류 = `a56a827` · 508~509 모멘텀 = `59cb0c1` · 500~507 F-Score = `cc3dc99`)
...
> **🏁 렌즈 현황(무료층 5종 전부 판정 — 투자가능 $5+ 유니버스)**: `/api/lens`(온디맨드·30분 캐시) + `/stock/[symbol]`. 엔진 `lib/lenses.ts`(+`lib/momentum`·`lib/lowvol`·`lib/technical`·`lib/fscore`·`lib/edgar`). **✅검증**: 모멘텀(12-1 +2.4%p/년)·저변동(저−고 +7.4%/년)·F-Score(건전성 해석·수익예측 무의미)·밸류(E/P +10.2%p/년·B/M 조건부). **⚪참고용**: 기술(RSI 평균회귀 기각·200일선 약함). 검증 스크립트 `scripts/backtest_*.ts`, 살아있는 문서 `docs/LENS_DEV_PLAYBOOK.md`(#1~17)·`docs/LENS_STRENGTH_MAP.md`. **원칙: 예측 아닌 정직한 방향성. 수익화·유료 AI보기(STEP 511 보류)·UX는 전 기법 검증 후로.**
> **▶ 다음 후보**: ① **KR/글로벌 렌즈 확장**(가격기반 모멘텀·저변동·기술=즉시 / 재무기반 F·밸류=DART 필요) · ② **새 기법**(퀄리티 ROE·마진·마법공식 — 플레이북 틀 반복) · ③ (그 뒤) 유료 AI보기·표시 UX·Phase 2 결제 PG
