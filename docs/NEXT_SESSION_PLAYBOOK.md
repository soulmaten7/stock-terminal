...
> **HEAD 커밋**: `fac8fb1` (STEP 413~421 + 문서). 배포 ✓. (STEP 405~412 = `9984804` · 397~402 = `52ebd5f` · 395~401 = `e21f2cc` · 394 = `e6afa23`.)
> **DB**: NEW 전용 프로젝트 `ccbwxcszdoyjxvckedfp`("Trillion", ap-northeast-2). 신규 테이블 `us_stock_perf`(상위 200 데모, 전 종목은 **prod 크론 매일 22시 UTC** 자동; 라이브 후 첫 실행 시 1주~6개월 채워짐). KR `link_hub` 71 큐레이션. 구 `qxkmwlkchyxfzxbonhtj`("OT-Marketing")=폐기 예정. POTAL ref `zyurflkhiregundhisky`=절대 금지.
