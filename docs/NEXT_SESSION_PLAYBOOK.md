...
>
> **시점 스냅샷**: 2026-06-04 STEP 145 종료 (홈 데이터 품질 P0: 지수 스파크라인·브리핑 안정화) + 마이그레이션 020·021·022 적용 완료
> **HEAD 커밋**: `90cb8a3` (STEP 145 코드) — 본 문서 커밋이 그 위 docs 커밋 (다음 세션 `git log` 로 최신 확인)
> **마지막 코드 변경**: `90cb8a3` (STEP 145 · 브리핑 overnight 안정화 — 누락·0·NaN 가짜 "+0.00%" → "—")
> **빌드 상태**: ✓ exit 0
...
## 11. 다음 STEP 후보 (우선순위 — STEP 146 부터)

### 🥇 P0 — 홈 데이터 품질 (빠른 반복)
- ~~브리핑 overnight 값 안정화~~ ✅ STEP 145 완료
- ~~지수 카드 스파크라인~~ ✅ STEP 144 완료
- **홈 레이아웃 비율 미세조정** — 메인(1fr)/우측레일(320) 균형
- **인기글 예시 시드** — `HotDiscussions`·평가글 0건 → 초기 콘텐츠 ('예시' 명확 표기 필요 — 신뢰 플랫폼이라 가짜글 X)
...
>
> **이 PLAYBOOK 마지막 갱신**: 2026-06-04 STEP 145 종료 (홈 데이터 품질 P0) + 문서 갱신 시점 · 마지막 코드 `90cb8a3`
