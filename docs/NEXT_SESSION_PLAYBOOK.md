...
>
> **시점 스냅샷**: 2026-06-15 STEP 260 종료 (… → **ETN 실데이터 연결**(KRX etp/etn_bydd_trd) + **펀드 시도→제거**(무료 수익률 소스 없음=유료 영역) + 탭 새로고침 유지(`?tab=`). 탭 = 주식·ETF·ETN·리츠·리딩방 리스트)
> **HEAD 커밋**: STEP 260 코드(펀드 탭 제거) — 정확한 해시는 다음 세션 `git log` 로 확인. 본 문서 커밋이 그 위 docs 커밋.
> 데이터: 주식·ETF·ETN·리츠·미국 수익률 ✅ (ETN=KRX etp/etn_bydd_trd 구독 완료). 펀드 = 제거(무료 수익률 API 없음=유료 영역). 다음: US·ETN /market 합류 · 종목→증권사 바로가기 · 리딩방 검증 강화. 사용자 결정: 유튜브 키·카카오 OAuth·AI 해설·평가검증.
