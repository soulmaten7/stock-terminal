<!-- 2026-08-19 -->
> 🔴 **2026-09-05 체계 전환으로 동결 — 현재는 `docs/STATE.md`·`docs/ORDER_*.md`(이력은 git 커밋 메시지). 새 항목 추가 안 함.** 삭제 금지, 참조용 보존.

# Trillion(트릴리언) — 변경 이력

## 2026-08-19 — 🟡 **STEP1090: `CLAUDE.md` 「세션 루틴」 절 갱신 — 새 세션이 빠짐없이 읽게 만든다**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 장은태 요청 — "새 세션이 놓치지 않고 읽으려면 내가 뭐라고 말해야 하는지, 그리고 그 파일들이 나중에 바뀌어도 안 낡는 방법도". 새 파일 신설 없이 `CLAUDE.md`의 기존 「세션 루틴」 절을 완비.

**§1-0 `_archive`의 6개 핸드오프 문서는 왜 죽었나**: `SESSION_BOOT.md`(638줄)·`NEXT_SESSION_START.md`(667줄)·`NEW_SESSION_HANDOFF.md`(198줄) 열람 — **셋 다 같은 병.** 매 세션 상단에 상세 상태(커밋 해시·기능 설명·"다음" 포인터)를 **그대로 복제해 쌓았다**(SESSION_BOOT와 NEXT_SESSION_START는 07-17 시점 내용이 사실상 동일 — 같은 정보를 두 파일에 중복 유지하다 둘 다 낡음). `CLAUDE.md:631`("새 파일 신설 금지")가 이미 이 원인을 규칙화해뒀다. 이번 갱신은 **가리키기만 한다**는 원칙을 §1에 못 박아 같은 함정을 피한다.

**§1 「세션 시작 시」 갱신**: 기존 4단계 → **6단계**로. 신규 편입 = ②`BUILD_SEQUENCE.md §6-C`(판정 대장, 다음 세션의 실제 출발점) · ③같은 문서 §2·§6-B·§6-B-1. 낡은 표현 정정 = ④`SYSTEM_MAP.md`의 "6개국 파이프라인" → US 단독 전환 반영. 각 항목은 **「파일:절」만** 적었다 — 행수·상태·날짜는 옮기지 않음(옮기면 낡는다).

**§1-b 규칙 좌표**: 3번 규칙(`:116`·`:393`)·근거 좌표 의무(`/preferences.md`·`:206`)·낱말의 뜻을 Cowork이 정하지 않는다(`:207`)·기존 파일 업데이트·새 파일 금지(`:631`)·`[3중 점검] ⓪`(`:266`) — **5개 전부 실제로 열어 그 좌표에 그 내용이 있는지 확인**(틀린 좌표 0건, 편집 후 재검증까지 완료 — 아래 검증 참조).

**§2 새 세션에 할 말**: 「세션 시작 시」 절 맨 위에 코드블록으로 한 줄 신설 — `CLAUDE.md와 docs/STATE.md ⓪를 먼저 읽고, CLAUDE.md 「세션 시작 시」 순서대로 진행해.` 파일만 가리키므로 문서가 바뀌어도 이 문장은 안 고쳐도 된다는 주석을 바로 아래에 명시.

**§3 「세션 종료 시」 갱신**: 4단계 → **6단계**. 신규 = 지침 문서 역방향 대조(STEP1089 교훈, `BUILD_SEQUENCE §8` #6 인용) · 마감 체크 대상 명시(`STATE.md`·`CHANGELOG.md`·`STEP_LEDGER.md`는 항상, `SYSTEM_MAP.md`/`LENS_DEV_PLAYBOOK.md`/`INDEX.md`/표시본은 조건부). "다 됐다로 적지 않는다" 문구 명시.

**§4 `:686` hooks 미완료 항목**: `.claude/hooks/stop-reminder.sh` 직접 열람 — **이미 `docs/STATE.md`·`docs/CHANGELOG.md`만 검증하고 있었다**(옛 4문서 언급 0건). ✅ 해소 표기로 취소선+정정(실제로는 이미 처리돼 있던 항목이었음).

**검증 3**: ①상태 복제 검사 — `CLAUDE.md`에 넣은 것 중 행수·날짜·다음할일을 그대로 옮긴 것 **0건**(삭제할 것 자체가 없었음) ②좌표 정확 — 인용한 6개 줄번호(`:116`·`:206`·`:207`·`:266`·`:393`·`:631`) **전부 재조회로 재확인**, 편집이 그 좌표들보다 뒤(695행 이후)에서만 일어나 라인시프트 없음 확인 — 틀린 좌표 0건 ③새 세션 모의 — 새 한 줄 → `CLAUDE.md`「세션 시작 시」→ 1단계 `STATE.md ⓪`에서 판정A·B·확정된칸넷·검산인프라·최근변화6/9·미해결위험 **전부 직접 도달 확인**, 2단계에서 `§6-C` 76행도 독립 도달 — 보강 필요 없음.

**검수 3**: ①새 파일 0(신규는 `STEP1090.md` 하나)·`CLAUDE.md` 절대규칙·금지사항 삭제/약화 **0건**(추가만)·`docs/_archive/` 무접촉(읽기만) ②비대화 — "세션 시작 시" 절 총 39줄, 전부 좌표·짧은 지시문뿐(요약 복제 없음) ③정직 — §1-0(_archive 3개 실제 열람)·§4(`stop-reminder.sh` 실제 열람 후 판단) 둘 다 실측 기반.

🔴 **못 한 것**: `docs/_archive/`의 나머지 3개 문서(`NEXT_SESSION_PLAYBOOK`·`session-context`·`NEW_SESSION_HANDOFF` 중 상세 미확인분)는 열지 않음(§1-0이 요구한 "셋 또는 여섯 중" 셋을 열어 이미 같은 결론에 도달, 나머지는 재확인 생략) · `docs/step_orders/` 신설 요청은 없었으므로 무접촉.

---

## 2026-08-19 — 🟡 **STEP1089: 지침 문서 전수 갱신 — 「빠진 서술」을 역방향으로 잡는다**

> 문서 전용(코드 diff 0·DB 쓰기 0·스키마 변경 0·화면 변경 0·크론 수동 실행 0). 장은태 지적 — "지침용 파일에 있는 모든 세부파일까지 기록과 내용 싹다 업데이트 했어?" → 실측 결과 안 됐다. STEP1086·1088은 씨앗→grep(정방향)이라 「틀린 서술」은 잡아도 「빠진 서술」(문서에 아예 없어 검색 자체가 안 걸리는 것)은 못 잡았다. 이 STEP은 문서에서 출발해 "이 문서가 담당하는 것 중 이번 세션에 바뀐 게 무엇인가"를 역방향으로 묻는다.

**⓪-1 이번 세션 변경 목록**: CHANGELOG 2026-08-19 블록(STEP1073~1088, 310줄) 부분 재확인(볼드 용어 스캔 — 라인 단위 전수 정독은 아님, 정직히 표기) — 주어진 6개 유형 분류(DB/스키마·코드·새자산·새문서절·판정확정·교훈/실패)에서 **추가 발견 0건**.

**⓪-2 대상 지침 문서**: 주어진 13개(`CLAUDE.md`·`SYSTEM_MAP.md`·`LENS_DEV_PLAYBOOK.md`·`INDEX.md`·`DATA_SOURCE_CATALOG.md`·`COMMIT_GATES.md`·`CROSS_REFERENCE.md`·`LENS_ARCHITECTURE.md`·`REVDCF_SPEC.md`·`VALUATION_SPEC.md`·`MODEL_ROSTER.md`·`LENS_STRENGTH_MAP.md`·`LENS_COPY.md`·`LENS_DISPLAY_CHARTER.md`·`ANSWERABILITY_MAP.md`) 전부 열어 확인. `CLAUDE.md`의 참조 파일 경로 테이블·`INDEX.md`를 검토했으나 목록 밖 추가 후보 **0건**.

**§1 역방향 대조 결과**(문서를 열어 자기 담당 범위를 확인한 뒤 채움 — 표를 베끼지 않음):
- **`SYSTEM_MAP.md`**(아키텍처·파이프라인, 직접 담당) → **갱신.** `lens_scores` 19→26컬럼(신규 7개 raw 필드 나열 + 마이그레이션 파일 + 쓰기 지점 `lensPrecompute.ts:569` + `total_assets_prior`만 영구 null인 이유) 반영.
- **`LENS_DEV_PLAYBOOK.md`**(렌즈 문제해결 로그, 직접 담당) → **행 3개 추가**(#116 모멘텀 창작산식 제거 사례·#117 PDF OCR 우회 발견·#118 검산인프라 설계 낙관 오류). 🔴 **`:644`("몰아서 하지 말 것")를 실제로 어겼다는 사실도 함께 기록** — STEP1083·1084·1085·1087 4개 STEP분이 이번에 한 번에 몰려서 기록됨.
- **`INDEX.md`**(전체 문서 마스터 인덱스, 직접 담당) → **갱신.** STEP 명령서 카운트(884→893, 직접 측정) 정정 + `docs/step_orders/*.md`(83개, 별도 명명규칙)가 이 범위 계수에 아예 안 잡히고 있었다는 사실 신규 기록. 비-STEP 문서 97개는 **검증 못 함**(정직히 표기, 재계산은 이 STEP 범위 밖).
- **`DATA_SOURCE_CATALOG.md`**(US 데이터 공급기관 92소스) → **해당없음.** 열어서 확인한 결과 이 카탈로그는 데이터 공급 *기관*(SEC·Yahoo·Nasdaq·Damodaran·SPDR) 전용이고, 학술 원전(논문)은 원래 이 카탈로그의 담당이 아니다 — 기존 6개 학술 논문도 전부 여기 없고 `data/sources/README.md`가 그 역할을 한다(Jegadeesh-Titman도 STEP1084가 이미 거기 등재 완료). 주어진 표의 "🔴 등재" 제안을 **문서를 직접 열어 확인한 뒤 기각**.
- **`CLAUDE.md`** → §2 참조(아래).
- **`COMMIT_GATES.md`** → §3 참조(아래).
- **`CROSS_REFERENCE.md`**(Stock Terminal↔OTMarketing 04-23 시점 기록) → **해당없음(검사함).** 이번 세션 변경과 완전히 무관한 별개 저장소 교차참조.
- **`LENS_ARCHITECTURE.md`**(엔진=검증일치·동작불변 원칙) → **해당없음(검사함).** STEP1087도 같은 `computeSymbolLenses`를 그대로 씀 — 원칙 위반 없음, 문서가 특정 컬럼을 나열하는 구조도 아니라 갱신 대상 없음.
- **`REVDCF_SPEC.md`**(역DCF 모델 자체의 설계 정본) → **해당없음(검사함).** 판정 A·B는 "어느 모델을 언제 만드는가"의 공정 판단이지 역DCF의 기술 설계 내용이 아니다 — 이 문서가 담당하는 것과 다른 층위.
- **`VALUATION_SPEC.md`**(Q1 4축 정의공개표) → **해당없음(검사함).** 이번 세션은 Q1 축 정의를 안 건드림.
- **`MODEL_ROSTER.md`**(모델 전수조사, 판정 없음) → **판정B 반영 스탬프 추가.** `MARKET_MODEL_USAGE_TOP20`·`MODEL_BUILD_ORDER`·`MODEL_UNIVERSE_63`은 STEP1080이 이미 "판정B로 이미 판정됨" 스탬프를 찍었으나 같은 클러스터의 `MODEL_ROSTER.md`만 빠져 있었다(이번 세션 내 형제 문서 간 불일치 — 발견해 반영).
- **`LENS_STRENGTH_MAP.md`**(기법별 적합영역·백테스트 특성) → **해당없음(검사함).** STEP1083의 수정은 창작산식 제거·표기정정이지 백테스트 강도 특성 자체를 바꾸지 않음.
- **`LENS_COPY.md`**(렌즈 겉면 짧은 카피) → **해당없음(검사함).** 전문 읽음(39줄) — STEP1083이 고친 것은 `lensCopy.ts`의 **깊은 `note`** 텍스트(FF/Carhart 정밀화)이고, 이 문서는 **짧은 블러브**만 다룬다고 스스로 명시(§"알아보기" 절 참조) — 다른 층위.
- **`LENS_DISPLAY_CHARTER.md`**(카드 표시 규칙 체크리스트) → **해당없음(검사함).** `adjUsed` 전용 UI 배지는 명시적으로 미신설 상태(화면 변경 0) — 표시 규칙이 바뀔 화면 자체가 아직 없음.
- **`ANSWERABILITY_MAP.md`**(Q0~Q5 답변가능성) → **해당없음(검사함).** 이번 세션은 Q축 콘텐츠를 안 건드림.

**§2 `CLAUDE.md` 판별**(고칠지 말지):
- 판정 A(정의1 폐기·정의3 최상위) → **안 넣음** — `CLAUDE.md`에 "제품 정의"를 서술하는 절 자체가 없음(grep 0건 확인).
- 판정 B(확정된 모델 1개씩 완성) → **안 넣음** — `CLAUDE.md`의 기존 "단일 모델 원칙"(진행은 하나씩)이 이미 이 원칙을 포괄. 판정 B는 그 원칙의 **운영 적용**이지 새 규칙이 아님.
- 판정 대장 §6-C 신설 → **넣음.** 참조 파일 경로 테이블에 `BUILD_SEQUENCE.md` 행 신설(REVDCF_SPEC.md 바로 아래) — "판정이 모이는 자리"가 새로 생겼는데 이 테이블에 없었음을 확인.
- 검산 인프라 7컬럼 → **안 넣음** — `SYSTEM_MAP.md` 영역, 이미 그쪽에 반영.
- STEP1086·1088 grep 방식 결함 → **안 넣음** — `BUILD_SEQUENCE.md` §8(반복된 실수)이 이미 이 역할을 담당(신규 #6), `CLAUDE.md`에 중복 기록 안 함.
🔴 **`CLAUDE.md`의 절대규칙·금지사항은 삭제·약화 0건**(추가만 1건).

**§3 재발 방지**: `BUILD_SEQUENCE.md` §8에 신규 행(#6, grep 정방향 한계 — 위 CHANGELOG 내용 그대로) 추가. `COMMIT_GATES.md` 게이트 신설 여부 판별 = **신설 안 함** — 사유: (a) 이 실패 유형(정방향만으로 빠진 서술을 못 잡음)은 이번이 **첫 발생**이고, 기존 게이트 9개는 전부 **반복된** 실패에서 나왔다 (b) 처방(정방향+역방향 병행)이 이미 `BUILD_SEQUENCE §8` #6에 방법론으로 기록됨 — 매 커밋 게이트로 중복 강제할 필요 없음 (c) 세션마감 mtime 체크는 커밋 단위 게이트들과 주기가 달라(세션마감 vs 매커밋) 9개 게이트 목록에 섞으면 위계가 흐려짐. 게이트 비대화 방지.

**§4 과거 누락**: 역방향 대조 도중 이번 세션 **이전**의 누락은 발견되지 않음 — **0건**(발견됐으면 `BUILD_SEQUENCE §6-C`에 판정 대기 1행으로만 등재하고 고치지 않을 예정이었음, 해당 사례 없어 등재 없음).

**§5 표시본 + STATE.md**: `build_sequence.html` 머리+본문 둘 다 STEP1089 note 추가 + §8 테이블에 STEP1073 재발 기록(행1·4)과 신규 행6 반영(마크다운과 동기화, 이전 세션 드리프트도 함께 해소) + 재발방지 한 줄 문단 추가. `docs/STATE.md` ⓪ 절 갱신(STEP1089 요약 추가) + **🔴 45↔43 표기 불일치 정정**(45가 맞음 — 45+19+12=76 산술 확인, "다음 세션이 먼저 할 것" 목록의 옛 43 오기 수정).

**검증 3**: ①역방향 전수 — 대상 문서 13개 전부 「담당범위×이번변경」을 물음, 안 물은 문서 0개, "해당없음" 8건 전부 사유 붙임 ②빠진 서술 검사 — grep으로는 못 잡을 누락 4건 발견(SYSTEM_MAP·LENS_DEV_PLAYBOOK·INDEX·MODEL_ROSTER) ③역방향의 역방향 — ⓪-1의 6개 변경 유형 각각 최소 한 문서에 기록됐는지 확인, 전부 기록됨(DB/스키마→SYSTEM_MAP, 코드→LENS_DEV_PLAYBOOK, 새자산→기존 STEP1084 기록 유지, 새문서절→CLAUDE.md 참조표, 판정확정→MODEL_ROSTER+기존 STATE, 교훈/실패→BUILD_SEQUENCE§8) — 안 남은 변경 0건.

**검수 3**: ①판정 안 함 — `CLAUDE.md` 절대규칙 약화 0·과거 누락 소급 여부 안 정함(발견 자체가 0건)·게이트 신설 과하게 안 함·`Q1_ENABLED`/`REVDCF_ENABLED` 무접촉 ②삭제·창작 안 함 — 취소선 보존·새 파일 0(신규는 이 명령서 하나)·지어낸 내용 0 ③정직함 — "해당없음" 8건 전부 실제로 문서를 열고 판단, 못 한 것 전부 나열.

🔴 **못 한 것**: `docs/` 비-STEP 문서 97개 카운트 재검증(범위 밖으로 판단, `INDEX.md`에 "검증 못 함"으로 정직 표기만) · ⓪-1 변경목록 CHANGELOG 재확인이 볼드-용어 스캔이었지 310줄 전체 정독은 아님(발견 0건이나 완전성 100% 보장 못 함) · `data_source_catalog.html`·`roadmap_v2.html`·`tombstone.html`은 이번 STEP의 구체적 변경과 무관해 미접촉(검사함, 근거는 §1 각 문서 판정에 이미 명시).

---

## 2026-08-19 — 🟡 **STEP1088: 세션 마감 전체 문서 업데이트 + 인수인계**

> 문서 전용(코드 diff 0·DB 쓰기 0·스키마 변경 0·화면 변경 0·크론 수동 실행 0). 장은태 지시 — "전체 모든 문서 업데이트하면 세션 마감하고 대화할 거야". 판정하지 않는다 — 사실만 반영, 판정 필요는 등재만.

**⓪-1 실행 상태**: `git log`로 STEP1085(`c82963b`)·1086(`50b55e9`)·1087(`12bce75`·`f814060`·`6ce304f`) **전부 커밋 확인**. untracked 명령서 = `docs/step_orders/STEP1088.md` 하나뿐(이 STEP 자신).

**⓪-2 씨앗**: STEP1086의 17건을 그대로 인용 + 신규 11건(#18~#28, STEP1085~1087 산출) = **총 28건**. `git log` 역확인 — STEP1085~1087 구간 커밋은 정확히 5개(1086 1·1085 1·1087 3), 씨앗 밖 추가 발견 **0건**.

**⓪-3 씨앗 #20 정정(축 C ㉢의 실제 사례)**: STEP1085 `#7-부록` §4가 "자산성장 ③값검증 = 예"라 적었으나, STEP1087이 `total_assets_prior` 배선 불가를 확인해 **뒤집혔다**. `#7-부록` §4 자산성장 행에 이미 STEP1087에서 취소선+정정 반영됨(재확인, 중복 정정 안 함). `LENS_DISPOSITION §8-1` 자산성장 행의 "손계산 구조적 불가"는 **유효 상태 유지**(지우지 않음). 판정 대기 신규 등재 = `BUILD_SEQUENCE §6-C` **#79**.

**§1 `STATE.md` 전면 갱신**: 새 "⓪ 직전 세션(2026-08-19) 요약" 섹션을 최상단(기존 헤더 바로 아래)에 신설 — 무엇이 확정됐나(판정A·B·확정된칸넷·검산인프라①)·무엇을 만들었나(최근변화6/9·검산인프라7컬럼)·지금 어디인가(§6-C 대장 좌표)·다음 세션이 먼저 할 것(순서·추천 없이 4개 후보)·건드리면 안 되는 것·미해결 위험(08-16조인·섹터11일격차·41종목·판별불가10개) 6칸 전부 채움. §6-C 대장 행수 정정(74→76)도 이 섹션에 반영.

**§2 정본 ㉠67개 대조**: STEP1086의 분류를 **재조사 없이 그대로 재사용**(`$CLAUDE_JOB_DIR/tmp/g1_final.txt`). 검색어 9그룹(검산인프라·최대7/9·소비처9곳·백필불가·5/9vs6/9·문서분류1226·sectorSource·7컬럼·total_assets_prior)으로 ㉠ 67개 전수 grep. 히트 대부분 이미 반영됐거나(BUILD_SEQUENCE·LENS_DISPOSITION·CHANGELOG·STATE 등 이 세션이 직접 쓴 문서) 무관 문맥(REVDCF_SPEC의 다른 테이블 `select("*")`, VALUATION_SPEC의 다른 백필 등). **역방향**(㉠ 67개에서 "지금 사실과 다른 말을 하나" 직접 훑음) — `LENS_AUDIT_07_ASSETGROWTH.md` §4의 "구조적 불가" 서술을 재확인했으나 `total_assets_prior` 미배선이 확정돼 있어 **여전히 정확**(정정 불필요, 해당없음). 이 경로에서 씨앗 밖 신규 발견 **0건** — 실제 신규 발견은 아래 §3(html 재확인)에서 나왔다(경로가 다름, 혼동 방지로 분리 기록).

**§3 STEP1086 잔여 처리**: ㉣ 판별 불가 10건 — 생사 판정 없이 `BUILD_SEQUENCE §6-C` **#78 한 행**으로 등재(10개 개별 등재 안 함). 씨앗 #18 확인 불가 — STEP1085가 그 뒤 실제 실행됨을 확인해 STEP1086 CHANGELOG 엔트리에 **해소 표기 추가**. html 4개 재확인 — `roadmap_v2.html`·`data_source_catalog.html`·`tombstone.html`은 여전히 무관(검사함·해당없음), **`model_list_30_9.html`은 재확인 결과 갱신 필요 발견** — "검산 인프라 공백(4개 렌즈 연속)" 옛 서술에 STEP1085·1087 해소 결과(6/7컬럼 배선, `total_assets_prior`만 미배선)를 추가 반영.

**§4 표시본**: `build_sequence.html` 머리+본문 둘 다 STEP1088 note 추가(§6-C 대장 76건 갱신 반영) + `model_list_30_9.html`(§3에서 발견한 정정, 위 참조). `roadmap_v2.html`·`data_source_catalog.html`·`tombstone.html`·`CLAUDE.md`는 방향·규칙 불변이라 무접촉(검사함·해당없음).

**§5 세션 마감 인수인계**: `STATE.md` §0에 6칸(§1 참조) 전부 채워 신설 완료.

**검증 3**: ①재현가능 — 검색어 9그룹 전부 기록, STEP1086 분류 재사용(재조사 0건) ②반대증거 셋 — 히트마다 ㉠판정/㉡이미반영/㉢근거뒤집힘 실제로 물음(㉢의 실제 사례 = 씨앗#20, ⓪-3 참조) ③인수인계 검사 — `STATE.md`만 읽고 다음 세션이 이어받을 수 있는지 자문, §0 신설로 6칸 채움 확인.

**검수 3**: ①판정 안 함 — ㉣10건 생사·완성 순서·다음 세션 우선순위 전부 미정(후보만 나열)·`Q1_ENABLED`·`REVDCF_ENABLED` 무접촉 ②삭제·창작 안 함 — 취소선 보존, 신규 파일 = `STEP1088.md` 하나뿐, 지어낸 정정 0 ③정직함 — "다 됐다"류 문장 없음, `STATE.md` §0에 "미해결 위험" 절 명시.

🔴 **못 한 것 / 남은 것**(가장 중요): ㉣ 10개 문서 생사 판정(§6-C #78로 등재만) · `total_assets_prior` 배선 여부 판정(§6-C #79로 등재만) · 최근변화 항목⑨ 라이브 실측 after(다음 정규크론 이후) · 검산 인프라 7컬럼 값 채움 확인(크론 이후) · §6-C 🟢45행 중 무엇부터 할지(판정 = 장은태) · 08-16 조인 원인 코드 여전히 그대로(재발 가능성 남음) · 섹터 11일 격차 상한 미정.
...
**⓪-1 열람**: `LENS_DISPOSITION_2026-08-08.md §7-4 #7`·`§8-1`·`§8-2 ㉢` · `LENS_AUDIT_04_QUALITY.md §4`·`LENS_AUDIT_05_TECHNICAL.md §손계산`·`LENS_AUDIT_06_FSCORE.md §4`·`LENS_AUDIT_07_ASSETGROWTH.md §4` · `LENS_COMPLETION_STANDARD.md` 최근변화 행 · `lib/lensCompute.ts`·`lib/lenses.ts`·`lib/lensPrecompute.ts`·`lib/fscore.ts` · Supabase `lens_scores`(읽기만).

**§1 현재 상태 실측**(Supabase MCP 직접 조회, `ccbwxcszdoyjxvckedfp`): 컬럼 **19개**(원시 필드 0건 — grossProfit·rsi14·pos52w·reason·adjUsed 전부 미저장, 반대증거 ㉠ 기각) · 행 **1,976**(KR 978·US 998) · 테이블 힙 크기 **2,211,840 bytes(2,160 kB)** · 전체(인덱스포함) **3,874,816 bytes** · 행당 평균 = **1,119.3 bytes(산술, 실측 아님)**.

**§2 저장할 필드 확정 — 네 감사 원문에서 역산**: 퀄리티=`gross_profit`(`lib/lensCompute.ts:212-213`가 이미 `LensRead.grossProfit`로 보유 — `costOfRevenue`는 폴백에 흡수돼 **상상 필드로 삭제 1건**) · 기술=`rsi14`·`pos52w`(`lib/lenses.ts:224,226`가 이미 `detail`에 계산, `lensPrecompute.ts` `pick()`이 버림) · F스코어=`fscore_reason_code`(`lib/fscore.ts`의 `reason`이 로케일 고정 문자열이라 3개 상수 비교로 코드화 필요) · 자산성장=`total_assets`·`total_assets_prior`(`lib/lenses.ts:380`이 두 값 다 메모리에 가짐) · 모멘텀=`adj_used`(STEP1083이 이미 `LensRead`에 노출, DB만 안 감). **반대증거 ㉢(시계열류) 확인**: 기술의 원시 종가 252봉·F스코어의 3개년 전체 재무제표는 이 목록에 **없음**(스칼라 컬럼 몇 개로 안 풀리는 규모, 행 하나에 안 들어감 — 별도 테이블 필요·이번 범위 밖). 판별 불가 렌즈 0건.

**§3 대가 실측**: 저장용량 = 행당 +60B(산술, 기존 테이블 실측 평균 대비 자릿수 큰 필드로 상향 추정) × 1,976행 = **+118,560 bytes(+115.8kB, 산술) = 현재 대비 +5.36%(산술) — 2배 미초과.** 쓰기경로 = `lib/lensPrecompute.ts:569`의 `buffer.push({...})` 단일 위치(코드 읽기만, 안 고침). 읽기경로 = 소비처 **9곳 전수 확인**(`explore/lens-top`·`krx/ranking`·`watchlist/quotes`·`cron/health`·`yahoo/{us,cn,gb,jp,vn}-list`) — **전부 명시 컬럼 select, `select("*")` 0곳** → 신규 컬럼 추가해도 기존 9곳 페이로드 무변화. 마이그레이션 초안(`ALTER TABLE ... ADD COLUMN`, 전부 nullable) 문서 안 코드블록으로만 작성, `supabase/migrations/` 파일 0개 생성. 백필 = 기존 1,976행은 원자료가 재취득 없이는 못 채워짐(이 STEP 범위 밖) — **다음 정규크론부터 자연 채움을 권고, 백필 안 함.**

**§4 ③⑦ 해소 여부**: 퀄리티·자산성장 = ③④ 둘 다 예(스칼라 2개로 직접 재계산·대조 가능). 기술·F스코어 = ③ **여전히 아니오**(시계열·다년치 부재로 손계산 자체는 안 풀림), ⑦은 예(저장값=API값 사후 대조는 가능해짐). 🔴 **"9/9가 되는가" = 아니오.**

**🔴 §4 부수 정정(계수 오류 발견)** — `LENS_COMPLETION_STANDARD.md`의 최근변화 행 머리글이 "9항목 중 ✅ 5건"이라 적혀 있었으나, **같은 문서의 바로 아래 표를 실제로 세면 6건**(①🟢·②🟢·④✅·⑤✅·⑥✅·⑧✅)이었다 — STEP1084가 잡은 상단 요약표의 ALL-✅ 오표기와 같은 유형의 계수 오류. 취소선+정정으로 "6건"으로 반영. 🔴 **이 정정이 §5 산출 결론에도 소급 적용됨을 작업 중 자체 발견** — 최초 작성한 "#7-부록"이 옛 "5/9"를 그대로 베껴 "이 인프라로 최대 6/9까지"라 썼던 것을, 위 정정(현재 이미 6/9)을 반영해 **"현재 6/9 → 이 인프라로 최대 7/9까지"**로 재정정했다(⑦ 하나가 오르면 6→7이지 5→6이 아니다 — 산술 자체를 다시 확인).

**§5 산출**: `docs/LENS_DISPOSITION_2026-08-08.md` §7-4 **#7-부록** 신설(§1~§4 전체 + 마이그레이션 코드블록, 새 파일 금지 원칙대로 기존 문서 안에). `docs/BUILD_SEQUENCE.md` §6-C 대장 **#6** 행에 "🟢 ①채택(2026-08-19) — 집행 설계 = §7-4 #7-부록" 반영(옛 문구 취소선 보존) + §6-B-1 최근변화 두 행(모멘텀·기술)에 계수 오류 정정 이력 추가.

**검증 3**: ①역산검사 — 7개 필드 전부 감사 좌표 확인, **상상 필드로 삭제 1건**(퀄리티 `costOfRevenue`, 이미 `grossProfit`에 흡수) ②분모·산술표기 — 용량 수치 전부 "산술, 실측 아님" 명시, 백필은 "못 잰다" 아니라 "이 STEP 범위 밖"으로 정확히 구분 ③역방향 — 이 인프라가 지워주는 것(자산성장의 "구조적 불가", `LENS_DISPOSITION §8-1`) / 안 지워지는 것(기술·F스코어의 "구조적 불가" — 시계열·다년치가 별도로 더 필요, §7-4 C#9의 충돌도 이 둘에선 안 풀림).

**검수 3**: ①안 바꿨는가 — DB 쓰기 0·스키마 변경 0·`supabase/migrations/` 파일 0개 생성·코드 diff 0·크론 수동 실행 0·`REVDCF_ENABLED`·`Q1_ENABLED` 무접촉 ②상상 안 했는가 — 필드 전부 좌표 있음(위 §2)·용량 수치 근거 있음(기존 컬럼 실측 평균 기준 산술)·"판별 불가" 실제로 씀(0건이었지만 §2에 명시) ③새 파일 안 만들었는가 — `#7-부록`은 기존 `LENS_DISPOSITION_2026-08-08.md` 안에, `git status` 신규 = `STEP1085.md` 하나.

🔴 **못 한 것**: F스코어 `reason` 코드화 로직 실제 구현(설계만, §2에서 방법만 제시) · 용량 증가치 실측(컬럼을 만들지 않고는 못 잼, 산술로 대체) · §7-4#7 ②(현행 유지) 대안과의 정량 비교(①이 이미 채택된 판정이라 재비교 불필요했음).

---

## 2026-08-19 — 🟡 **STEP1086: 모든 기록 문서 전수 업데이트 — STEP1073~1085 산출 반영**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 장은태 지시(2026-08-18·재확인 2026-08-19) — 문서 정리(STEP1080에서 끝났어야 함) 후 모델 제작(1083~1085)으로 순서가 넘어가며 밀린 "모든 기록 문서 업데이트"를 3중 규칙으로 수행.

**⓪-1 씨앗 확인**: STEP1086 본문의 씨앗 18건 중 17건은 `git log`로 확인(STEP1073~1084 각 커밋 실존). **씨앗 #18(STEP1085 결과)은 확인 불가** — `git log`에 STEP1085 커밋이 없고 `docs/step_orders/STEP1085.md`가 untracked 상태로 남아 있어(미실행) 그 결과를 씨앗에 추가하지 못했다. 정직하게 17건 기준으로 진행. 추가 씨앗 발견 0건. 🟢 **2026-08-19 STEP1088 — 해소.** STEP1085가 그 뒤 실제로 실행됨(커밋 `c82963b`) — 결과는 STEP1088의 씨앗 #18~#20으로 편입.

**§1 문서 분류**(`docs/` 전수 md 1,226 + html 6 — STEP 본문의 "1,130+5" 추정치보다 많음, 실측치 사용): ㉠살아있는기록 **67** / ㉡시점이력 **1,119**(step_orders 80·STEP_*_COMMAND 893·probe_*.md 62·구식STEP_NNN root문서·DECISION_*·AUDIT_[0-9]+*·PROD_ACCESS_*·2026-08-07 모델조사 클러스터 5건 등) / ㉢죽은문서 **30**(`docs/_archive/` 16 + KR/CN/VN "🅿️ 동결" 자기표기 7건 + `ROADMAP.md`·`DRAFT_MODEL_SELECTION_RECOVERY.md`(자기표기 "대체됨") + "V3 명세·보존문서" 자기표기 5건) / ㉣판별불가 **10**(SITE_MAP_V7·MARKET_RESEARCH_V1·DASHBOARD_SPEC_V1·HOME_REDESIGN_V2_SPEC·SITE_REVIEW_FIXES·SUPABASE_MIGRATION·SUPABASE_MIGRATION_HANDOFF·V3_RELEASE_NOTES·LOGO_PROMPT·MAKE_AUTOMATION — 죽음·생존 표기가 본문에 없어 짐작으로 가르지 않음).

**§2 씨앗×문서 대조**: 12개 검색어 그룹(정의1·1위·7렌즈거취·판정대장·배제율30.5%·lens_scores41·섹터격차·avg(r1,r3)·pos52/Carhart·Jegadeesh·ALL-✅/9-9·최근변화/adjUsed)으로 ㉠ 67개 문서 전수 grep. 히트 대부분이 (a) 이미 정확히 반영돼 있거나(예: `ROADMAP_V2.md:438,470`이 판정B를 이미 취소선+정정으로 반영) (b) 무관한 문맥의 오탐(예: "1위"가 "사용률 76%(사용률 1위)"처럼 다른 지표를 가리킴). **실제 사실정정 2건**을 `docs/STATE.md`에서 발견 — §③ Q1 행과 ⑥"다음에 할 일"이 `sectorSource` 침묵버그를 "신규 발견(08-14, 6일 뒤처짐)"·"필수 2건"으로 여전히 서술하고 있었으나, 이미 STEP1050(2026-08-16)이 null 증상을 해소했고(`docs/ROADMAP_V2.md:483`) STEP1081(§6-C #65)이 격차를 08-18 기준 11일로 재측정한 상태 — 취소선+정정+좌표로 반영(원문 삭제 없음). 등재(판정 필요) 0건 — 이번 대조에서 새로 발견된 미판정 갈림은 없었다.

**§3 역방향**: `docs/STATE.md`(231행) 전문을 처음부터 끝까지 읽어 "이 문서가 지금 사실과 다른 말을 하나"를 직접 훑음 — 위 2건이 이 경로로 발견됨(정방향 grep과 별개 경로). 신규 발견 0건 추가.

**§4 표시본**: `docs/build_sequence.html` 머리(§6 히어로, :208)·본문(§6-B-1 요약, :392) 두 곳에 STEP1086 note 추가(기존 STEP1073~1084 note와 동일 패턴). 나머지 4개 html(`roadmap_v2.html`·`model_list_30_9.html`·`data_source_catalog.html`·`tombstone.html`)은 이번 STEP의 구체적 발견(STATE.md 2건)과 무관한 주제라 미접촉 — 사유 명시, "다 한 척" 안 함.

**⑤ 대장**: `docs/BUILD_SEQUENCE.md` §6-C 74건 불변(🟢43·🟡19·🔴12) — 이번 STEP은 판정하지 않았으므로 대장 갱신 없음.

**⑥ 검증 3**: ①재현가능 — 검색어 12그룹 전부 CHANGELOG·작업 로그에 기록(재현 가능) ②반대증거 셋 — 히트마다 ㉠판정/㉡이미반영/㉢근거뒤집힘을 실제로 물었고, 다수가 ㉡(이미반영)이었음을 §2에 명시 ③역방향 — STATE.md 전문 정독으로 수행, 신규 발견은 정방향과 동일한 2건(중복 아님, 독립 확인).

**⑦ 검수 3**: ①판정 안 함 — 등재 항목 없음(등재할 게 없었음) · ㉢ 죽은문서 판정은 전부 자기표기 근거로만 분류(짐작 없음) · `Q1_ENABLED`·`REVDCF_ENABLED` 무접촉 ②취소선 보존·새 파일 0·새 방식 0(STEP1072 방식 그대로 재사용) ③`docs/` 밖 무접촉·`~/Downloads/` 무접촉.

🔴 **못 한 것**: 씨앗 #18(STEP1085 미실행으로 확인 불가) · ㉣판별불가 10건의 최종 생사 판정(짐작 배제 원칙상 보류) · ㉡ 1,119건 개별 열람(패턴 분류만, 전수 열람 아님 — 명시적으로 §4 규칙에서 "판별 불가는 나열"만 요구했으므로 범위 내) · html 4개(주제 무관이라 미접촉).

---

## 2026-08-19 — 🔵 **STEP1084: 「최근변화」 칸 4/9 → 올릴 수 있는 것 올리기 — ②입력검증 실행 + ①원전 탐색**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). STEP1083이 "작업 안 함"으로 남긴 ②(입력검증)를 실행하고, ①(원전 부재로 부분처리됐던 것)을 추가 탐색.

**⓪-1 먼저 훑기**: 이미 잰 것 발견 — `LENS_AUDIT_02_MOMENTUM_2026-08-07.md:156-157`가 이미 "adjUsed 혼재는 원리적 측정 불가(현 구조에서)"로 결론냄(재조사 안 함, 인용만). `LENS_AUDIT_05_TECHNICAL.md`가 이미 George-Hwang(2004) 확보·산식대조 완료해둠(재조사 안 함, 인용만). ⓪-2 대상필드 확정 — `closes`·`adjCloses`·`adjUsed`(STEP1083이 제거한 `short`/r1단기축은 제외 명시).

**§1 ②입력검증 실측**(조회 2026-08-19 08:53:17 UTC, 분모=US 998): `closes`/`adjCloses` 출처=`lib/lensCompute.ts:111,115,120`(`yf.chart().quotes[].close`/`.adjclose`, 배치·온디맨드 동일 엔진 확인) · `momentum_value` 결측 20/998(2.0%, 정방향=역방향 일치)·p1=−53.05%·p99=363.35%·max=3391.65%(`SNDK`, 2025 WD분사+2026 메모리 슈퍼사이클 랠리로 상위10 전부 메모리·스토리지 반도체 — 경제적으로 말이 됨) · `technical_value` 결측 17/998(1.7%, 정방향=역방향 일치)·p1=−31.98%·p99=62.33%·상위10 하드웨어 랠리와 일치 · `adjUsed` 혼재 비율 = **시도했으나 못 잼**(⓪-1 인용, DB 미저장+라이브 재호출 필요).

**§2 ①원전 탐색**: §2-1 저장소 재확인 결과 STEP1083과 동일(Jegadeesh-Titman·Carhart·FF 파일 부재). §2-2 외부탐색(유료구매 0·robots우회 0·키입력 0) — WebSearch로 University of Houston 교수 코스자료 페이지(`bauer.uh.edu/rsusmel/phd/jegadeesh-titman93.pdf`)에서 **Jegadeesh & Titman(1993) JSTOR 스캔본 전문 확보**(608KB, 유료화면 없음, Read 도구 OCR로 §I~§II 전문 판독 성공) → `data/sources/academic/jegadeesh_titman_1993_momentum.pdf` 저장 + `data/sources/README.md` 대조표 작성. **원전대조 확정**: 원전=10분위 동일가중 포트폴리오·J/K 16조합·1주 skip(선택) / 우리=백분위 개별종목·12-1 단일사양·1개월 skip(고정, Carhart/FF 관행) — 3가지 차이 전부 명시. "현상=J-T, 계산사양=Carhart/FF" 이중귀속이 원문 대조로 확정됨(STEP1083 #10을 뒷받침).

**§3 9항목**: **4/9 → 5/9**(①원전대조 완료·②입력검증 완료 — 단 adjUsed 혼재 미측정 캐비어트 유지, ④⑤⑥⑧ 불변) + 여전히 미완 4항목(③값검증[기술=구조적불가]·⑦화면일관성[전용배지 없음]·⑨라이브실측[after 대기]). 9/9 아님 — 억지로 ✅ 안 붙임. 🔴 **부수 발견**: `LENS_COMPLETION_STANDARD.md`의 "완성 현황표"(요약 테이블)가 모멘텀·기술 행을 **9칸 전부 ✅로 표기하고 있어 실제 감사 결과(STEP1083/1084의 5/9)와 정면 모순**됨을 발견 — 이 표는 STEP1083 이전(감사 이전) 시점의 낡은 자체평가로 판단, 취소선+정정으로 수정(삭제 안 함).

**§4 문서 반영**: `LENS_COMPLETION_STANDARD.md`에 "최근변화" 행 신설(Q0/Q1과 같은 박스 형식, 9항목 표+②입력검증 표) + 완성현황표 상단 요약 정정 · `BUILD_SEQUENCE.md` §6-B-1 최근변화 행 2개(4/9→5/9) · `LENS_AUDIT_02_MOMENTUM`·`LENS_AUDIT_05_TECHNICAL`에 실측치 추가(기존 서술 안 지움) · 표시본 머리+본문.

검증3(①분모정합=모든 비율에 분모[US 998] 명시, momentum_value·technical_value 다른 분모 안 섞음 ②정방향역방향=결측률 둘 다 정확 일치[20=20·17=17] ③재조사여부=⓪-1에서 이미 잰 것 2건 발견해 재측정 안 하고 인용만 함, 재조사 0건)·검수3(①안고침=코드diff0·DB쓰기0·크론수동실행0·REVDCF_ENABLED/Q1_ENABLED무접촉·화면변경0 ②추정안함=출처 전부 코드좌표, adjUsed는 "시도했으나 못쟀다"로 명시, 억지✅0건 ③제약지킴=유료구매0·robots우회0·키입력0, 데이터가 담긴 URL이 대학 교수 개인 코스자료 페이지였을 뿐 결제/로그인 화면 없음)본문기록. 못한 것=adjUsed 혼재 비율(구조적 미측정)·③값검증(기술, 범위 밖)·⑦전용UI배지(범위 밖)·⑨라이브실측after(다음 정규크론 대기). 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 올릴 수 있는 것만 올린 것까지다.** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🔴 **STEP1083: 「최근변화」 칸 완성 착수(모멘텀+기술) — 결함 수정 + 9항목 검증**

> 🔴 STEP1055 이후 처음으로 코드를 고친 STEP(28개 문서전용 이후). 장은태가 STEP1083 명령서 실행 자체를 라이브 화면 수정 승인으로 명시(2026-08-19). `LENS_DISPOSITION §7-4` #2(①)·#3(②)·#5(①) 채택 집행.

**⓪-1 원전 확인**: `LENS_AUDIT_02_MOMENTUM_2026-08-07.md` §제안 전문 재확인 — #1 "avg([r1,r3]) 폐기 — r1 단독 또는 단기축 제거". `data/sources/academic/` 전수 확인 결과 Jegadeesh-Titman·Carhart·FF 원전 파일 저장소에 **없음**(George-Hwang 2004만 있음, 무관). 저장소에 원전 없음을 먼저 기록. **r1 단독도 "추세" 신호로서는 원전 근거가 없어**(1개월 반전 문헌은 다른 개념), `avg(r1,r3)`뿐 아니라 대체안도 창작 위험 — 코드 전수 확인(`grep`)으로 `short`/`long` 필드가 **어떤 라이브 컴포넌트에서도 렌더링되지 않음**(스냅샷 테스트만 소비)을 확인해, "애매하면 제거"(STEP1083 §1 명시)에 따라 **완전 제거**(null)를 선택 — 대체 산식을 짓지 않음.

**§1~§2 수정 4건**(3개 분리 커밋):
- **#7** `avg(r1,r3)` 창작 산식 폐기 → `short: null`(대체 없음, 원전 근거 없음 확인 + 미렌더링 확인). **#8**(±5%→분포컷 전환)은 #7 제거로 대상 코드 자체가 사라져 별도 전환 불필요(구조적 소멸, 사전 예상과 다른 발견 — 명시 보고).
- **#9** `adjUsed` LensRead 필드 신설(`lib/lenses/types.ts`) + momentum이 `d.adjUsed`를 그대로 노출 + `adjUsed=false`(배당조정 결측)일 때 note에 캐비어트 문장 자동 추가(ko/en `LENS_MISC.momentumRawFallback`). DB 쓰기 없음(온디맨드 `/api/lens` 계산 경로에서만, 배치 테이블 무접촉).
- **#10** FF/Carhart 귀속 문구 정정(ko `lensCopy.ts:26`·en `:119`) — "관행이에요"(뭉뚱그림) → "형성기간·30/70 구분만 따르고, 사이즈 2분할(NYSE 중앙값)·NYSE 브레이크포인트·가치가중 포트폴리오는 안 따른다"(정밀 구분).
- **#15** `pos52` 최소표본 가드 — `sma`/`rsi`와 동일 패턴(`win.length>=252` 미달 시 null) 재사용, 새 임계값 창작 없음.
- **#16**(집계 레이어) — `lensTones.ts` 코드 무변경, `messages/ko.json`·`en.json`의 `lensClose`에 "참고용 렌즈도 검증 렌즈와 같은 비중" 공개 문구 한 문장 추가(About 페이지).

**§3 before/after**: `lens_scores` US 기지값 확보(2026-08-19 08:15:54 UTC) — momentum_state: down 294·flat 390·up 294·null 20(총 998) / technical_state: down 307·up 674·null 17(총 998). **코드 dataflow 추적으로 증명**: momentum의 `verdict`·`spectrum`·`headline`·`value`·`state`·`cutoffs`·`cutSource`는 전부 `m121`·`mState`·`cut`에서만 파생되고 이번 수정(short·adjUsed·note)이 닿지 않음 — technical의 `verdict`·`spectrum`·`state`도 `tState`(last vs ma200)에서만 파생, `pos52`는 `detail.pos52w`(표시전용, `lens_scores`에 저장 안 됨 — 스키마 확인)에만 쓰여 판정 영향 0. **판정 변화 = 0건으로 구조적으로 확정**되나, 실제 다음 정규 크론(오늘 밤 US 21:30 UTC±지터) 이후 DB 재확인은 못 함(크론 수동실행 금지 원칙).

**§4 9항목 검증**: **4/9 완전통과**(④컷·분포 — #8 소멸로 기존 분포컷 그대로 유효 · ⑤경계처리 — #15 완료 · ⑥주장정합 — #10·#16 완료 · ⑧테스트 — 스냅샷 갱신, 386/386) + **4항목 부분/미완**(①원전대조 — 형성기간·30/70만 정밀 대조, 사이즈분할 등은 "안 따름"으로 정직 표기했을 뿐 별도 대조표 없음 · ③값검증 — 기술은 손계산 **구조적 불가**(원시 종가 미저장), 모멘텀은 STEP559 백테스트로 검증됨(혼합) · ⑦화면일관성 — `adjUsed`가 LensRead 필드+note로는 노출되나 전용 UI 배지는 미신설(note 텍스트 통합만) · ⑨라이브실측 — before만 확보, after는 다음 크론 대기) + **1항목 공란**(②입력검증 — 이번 STEP에서 별도 작업 없음). **9/9 아님 — 억지로 ✅ 안 붙임**(`BUILD_SEQUENCE:106` "현재 9/9 통과 모델 0개" 유효 유지).

**§5 문서 반영**: `LENS_DISPOSITION_2026-08-08.md` §7-4 #2·#3·#5에 🟢 채택 기록(원 선택지 안 지움) · `BUILD_SEQUENCE.md` §6-B-1 최근변화 행 2개 + §6-C 대장 #1·#2·#4 갱신(취소선+정정) · `model_list_30_9.html` 최근변화 행 + "마" 절 고쳐진것 카운트 갱신 · `build_sequence.html` 머리+본문.

검증3(①원전대조=지어낸 산식·임계값 0개, "애매하면 제거" 원칙대로 avg(r1,r3) 대체 없이 제거 ②before/after=momentum_state·technical_state 정방향 스냅샷 확보 + dataflow 역방향 추적으로 verdict 무변경 증명(after 실측은 다음 크론 대기, 미완 명시) ③역방향 문서=LENS_AUDIT_02·LENS_DISPOSITION §7-4·§8-2 갱신 확인, LENS_AUDIT_05는 이 STEP 범위(pos52 가드만) 밖이라 무접촉 사유 명시)·검수3(①창작안함=지어낸 산식·임계값 0개, #7 애매한 대안(r1단독)도 제거 쪽 택함 ②범위밖안건드림=lensTones.ts 코드 무변경·API소비처 4곳 무변경·REVDCF_ENABLED/Q1_ENABLED 무접촉·금지경로 무접촉·DB쓰기0·크론수동실행0 ③롤백가능=분리커밋 3개(`7191521`·`142c06a`·`e148b56`)·착수전해시 `92e917c` 기록·git status 신규=STEP1083.md 하나)본문기록. 못한 것=②입력검증 미작업·⑨라이브실측 after 미확인(다음 정규크론 대기)·pos52 na 전환 종목수 정확 카운트 불가(DB 미저장)·adjUsed 전용 UI 배지 미신설. 게이트7—미추적파일참조0건. worktree미생성확인. main직접push+CI/Vercel배포확인(라이브 화면 변경 STEP이라 필수). tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386(스냅샷 1건 의도적 갱신). 🔴**판정금지—장은태가 이미 승인한 것을 집행한 것까지다, 이후 판단(예: 나머지 결함·다른 렌즈 착수 여부)은 장은태 몫.** 커밋 3개(CHANGELOG 참고)

## 2026-08-19 — 🟡 **STEP1082: 판정 대장 ㉢ 13건에 선택지를 놓는다(놓을 수 있는 것만) + #65↔#50 연결**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 선택지를 고르지 않는다·놓기만 한다. 낱말의 뜻을 정하지 않는다 — 이 규칙이 이번에 정면으로 걸렸다.

**§1 13건 재분류**(STEP1081의 ㉢ 13건: #5·#16·#40·#43·#50·#53·#55·#59·#61·#64·#71·#75·#76): 각 행의 원 좌표 문서를 전부 다시 열어 ㉮(선택지를 놓을 수 있음)·㉯(낱말 정의라 못 놓음)·㉰(이미 선택지 있음)·㉱(놓아도 안 풀림) 4갈래로 재분류 — **㉮ 0건**. ㉯ 5건(#40 DoD9의 "라이브"가 production 노출을 뜻하는지·#50 섹터 신선도 상한 며칠·#64 "US 단독"이 KR 코드 열람까지 금지하는지·#75 "편하게"의 기준·#76 사실↔판단 경계 — 전부 §1-2 판별 기준 그대로 해당). ㉰ 2건(#43·#53 — STEP1079가 이미 원문대로 선택지를 적어뒀음, 재작성 금지 원칙대로 손대지 않음). ㉱ 6건(#5·#16·#55·#59·#61·#71 — 원 문서를 다시 열어도 대안 자체가 없어 지어내면 규칙 위반이 됨).

**㉯ 5건 처리**(선택지 대신 "실무 관측"만, 사실 서술로 명시): #40 — `DECISION_921_COMPLETION.md:26` 재확인, "라이브 실측" 원문엔 "production" 단어가 없으나 7렌즈 트랙은 실제로 production API 호출로 DoD9을 닫은 전례가 있음(`LENS_COMPLETION_STANDARD.md` 895/897). #64 — `CLAUDE.md` "US 단독" 원문(2026-08-19 재확인) 재확인 결과 "신규 착수 금지" 항목이 기능·수리·데이터소스·조사만 명시하고 "기존 코드를 참고용으로 읽는 행위"는 다루지 않음. #50 — §3에서 별도 처리(아래). #75·#76 — 실무 관측 시도했으나 기존 사용례를 찾지 못함(원문이 정의 자체를 안 내림).

**§3 #65↔#50 연결**: STEP1081이 잰 신선도 격차(11일)를 #50 행에 좌표 병기로 붙였다 — 단 지시대로 "근거가 생겼다고 자동으로 선택지가 생기지 않는다"를 적용해 #50을 ㉮로 승격하지 않고 ㉯ 그대로 유지(며칠로 잡을지는 여전히 기준 설정 물음). `us_sector_wide` 크론화 미비 서술과 중복 등재하지 않고 좌표만 병기.

**§4 대장 반영**: 13행 갈래 표시 갱신(옛 문구 취소선 없이 새 판정 블록 추가 — STEP1081 표기는 보존). §6-C 머리 집계 갱신(🟢43·🟡19·🔴12·총74 불변, 🟡 내부만 ㉡4·㉠부분측정2·㉯5·㉰2·㉱6으로 세분화). `build_sequence.html` 머리+본문 둘 다 갱신.

검증3(①전수분류=13건 전부 4갈래 중 하나, 미분류 0건 ②창작검사=§2에서 새로 놓은 선택지 0건이라 삭제 대상 자체가 없음[㉮가 0건이므로] ③역방향=㉯ 5건 전부 원 문서에 정말 대안이 없는지 재확인, ㉮로 되돌린 것 0건)·검수3(①낱말안정함=㉯ 5건 전부 "실무 관측"만 적고 정의·기준을 제시하지 않음 ②판정추천안함=✅·유력·먼저·권장 0건 ③안고침새파일안함=코드diff0·DB쓰기0·Q1_ENABLED/REVDCF_ENABLED무접촉, git status 신규=STEP1082.md 하나)본문기록. 못한 것=㉱ 6건은 여전히 미해결(원문에 대안 자체가 없어 별도 STEP 필요)·㉯ 5건은 장은태 해석 대기. 열지 않은 파일=없음(⓪-1 목록 8개 좌표 전부 열람). 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 선택지를 놓을 수 있는지 가르고 놓을 수 있는 것만 놓은 것까지다(㉮ 0건이라 실제로 놓은 것은 없다).** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🔵 **STEP1081: 판정 대장 🟡 23행의 「부족한 재료」를 갈라내고, 잴 수 있는 것을 잰다**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). §6-C 대장의 🟡 23행이 스스로 "미측정"이라 적어둔 것들 중 재면 풀리는 것만 잰다. 판정·선택 안 함.

**§1 4갈래 분류**(23행 전수): ㉠지금잴수있음 4(#45·#65·#66·#72, 단 #66·#72는 부분측정) · ㉡코드실행필요 4(#3·#14·#30·#69 — 렌즈 재계산·Altman 재계산 등 이 STEP[읽기전용]으로는 불가) · ㉢재도안풀림 13(#5·#16·#40·#43·#50·#53·#55·#59·#61·#64·#71·#75·#76 — 선택지 미작성·육안검증·정의해석·종속 등) · ㉣이미잼 2(#44·#63).

**§1-1 ㉣ 먼저 훑기**(재조사 방지): #44(STEP955 페이지네이션 처방) — `git log`로 `lib/supabasePaging.ts` 이력 확인 결과 **STEP954가 이미 `fetchAllRows()`를 신설**(orderBy 필수, 비어있으면 throw)해 `lib/sector.ts`·`app/api/cron/revdcf/route.ts` 등 **프로덕션 경로 전부**를 이관 완료했음을 발견(커밋 `fbda42d`) — 단 `scripts/probe_941/943/944` 3개(1회성 조사 스크립트)는 미이관. #63(STEP994 throttle 배포 여부) — `git merge-base --is-ancestor f06e685 origin/main` = 참 확인, `route.ts:264`에 STEP994 구현이 라이브로 남아있음 — **이미 배포됨**(옛 "로컬 커밋까지만" 서술이 낡았음).

**§2 ㉠ 실측**(읽기 전용): #45 — `cron_heartbeats`(최신 2026-08-18 21:37:09 UTC) 직접 조회: `freshCoverage=0.9865`·`coverageOk=true`·`retryAllLen=81`(933/934 당시 511의 6분의 1) — 게이트가 85%로 바뀌고 커버리지도 98.65%로 올라 "97% 못 넘는다"는 옛 산술의 전제 둘 다 무효화. #65 — `us_sector_resolved`·`us_sector_wide` 둘 다 `max(as_of)=2026-08-08`(11일 정체) vs `us_valuation`(2026-08-18, 매일 갱신) — 신선도 격차 실측. #66 — `us_sector_wide` 전체 미분류 1,135/5,820건 확인했으나 952b가 원래 잰 좁은 코호트(89건, `source='yahoo'`)와 다른 지표라 완전히는 못 품(부분측정, 🟡 유지). #72 — 코드 확인(`app/api/yahoo/us-list/route.ts:13`)으로 목록이 `data/us_symbols.json`의 정적 영문명을 쓴다는 원인 절반만 확인, 상세 페이지 소스는 시간 상한으로 미추적(부분측정, 🟡 유지).

**§3 대장 반영**: 4행 🟢 승격(#44·#45·#63·#65, 옛 문구는 취소선 보존 + 정정 추가) · 19행은 🟡 유지하되 갈래 표시(`🟡㉡`/`🟡㉢`/`🟡㉠`) 부착. 머리 집계 갱신 — 🟢 39→**43** · 🟡 23→**19** · 🔴 12(불변) · 총 74건(불변).

**§4 표시본**: `build_sequence.html` 머리+본문 둘 다 갱신.

검증3(①전수분류=23행 전부 4갈래 중 하나, 미분류 0건 ②재조사 안했는가=㉣ 먼저 훑어 #44·#63을 재측정 대신 "이미 있음"으로 찾음, 재조사 후 "알고보니 이미 있었다"로 뒤집힌 사례 없음[처음부터 ㉣ 먼저 확인] ③역방향=측정값(coverageOk=true·as_of 08-08·git ancestor)이 다른 문서 수치와 충돌하는지 확인, 충돌 0건)·검수3(①판정안함=선택지 고르지 않음, 🟢 승격이 우선순위로 안 읽히게 서술 ②추정안함=못 잰 것은 "부분측정" 명시[#66·#72], 산술 없음, 분모·조회시각 전부 병기 ③안고침·새파일안함=코드diff0·DB쓰기0·크론수동실행0·Q1_ENABLED/REVDCF_ENABLED무접촉, git status 신규=STEP1081.md 하나)본문기록. 못한 것=㉡ 4건(#3·#14·#30·#69, 코드 실행 필요라 이 STEP 범위 밖)·㉢ 13건(선택지 작성·육안검증·정의해석 등, 재측정으로 안 풀림)·#66·#72 완전 미해결(부분측정만). 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 🟡를 🟢로 올릴 수 있는 것만 잰 것까지다.** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🟡 **STEP1080: 판정 대장 미확인 25파일 마감 — 생사 판정 후 편입**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). STEP1079가 "전수 개별 항목 추출을 못 했다"고 남긴 §6-C-미확인을 닫는다. "낡아 보인다"만으로 죽었다고 하지 않는다 — 근거 좌표가 없으면 살아 있는 것으로 본다.

**⓪-1 대상 목록 재확정**: STEP1079 목록을 그대로 믿지 않고 grep을 다시 돌려 **44개 파일**(문서 자체 성장분 포함) 확인 → 기존 대장 원좌표 16개 파일을 뺀 **28개 파일**이 실제 대상(STEP1079의 "약 25개" 추정과 다름, 실측으로 정정).

**§1 생사 판정**(28개 파일 전수): ㉠**대체됨** 1건(`DRAFT_MODEL_SELECTION_RECOVERY.md` — 미실행 STEP871 초안, STEP1073 판정B가 목적 자체를 대체) · ㉡**중복** 11건(이미 대장에 있는 판정과 같음, 신규 행 없음) · ㉢**이미판정됨** 8건(`DECISION_884`·`890`은 문서 자체가 승인·적용 완료를 이미 명시, `MARKET_MODEL_USAGE_TOP20`·`MODEL_BUILD_ORDER`·`MODEL_UNIVERSE_63`는 STEP1073 판정B가 "1위" 프레임을 명시적으로 기각, `Q1_AXIS_DECISION`은 STATE.md가 이미 해소로 확인, `LENS_DEV_PLAYBOOK`은 문서 자체가 STEP880 종결을 명시, `INDEX.md`는 설명성 텍스트로 그 자체에 열린 판정 없음) · ㉣**살아있음** 8건(`LENS_AUDIT_04_QUALITY`·`MODEL_ROSTER`·`STATE.md`·`REVDCF_SPEC.md`·`Q1_CARD_DESIGN.md`·`DECISION_921_COMPLETION`·`USER_QUESTIONS_2026-08-08` + `PRIMARY_SOURCE_MAP.md`는 신규 판정 행 0, 남은 항목 3개는 "선택"이 아니라 "미대조·미측정"이라 조사 대기로 이관). 합계 28 = 재확정 대상 수와 정확히 일치. ㉠㉡㉢ 전부 근거 좌표 확인(좌표 없어 ㉣로 되돌린 것 0건). `DRAFT_MODEL_SELECTION_RECOVERY`는 STEP1079가 "STEP1073 판정B로 흡수됐을 가능성 높음"이라 적어둔 추측을 이번에 확인해 ㉠으로 확정(추측을 그대로 옮기지 않음).

**§2 대장 편입**: 살아 있는 8개 파일에서 **21건** 신규 편입(통번호 57~77, `BUILD_SEQUENCE.md` §6-C 「L. STEP1080 신규 편입」) — 🟢 지금가능 8건(퀄리티7번째질문·IFRS외국사·08-10데이터오염·태그배열확장·riskfree/ERP선택지C·N년표현폐기·DoD9분리권고·Q3표시문구) · 🟡 재료일부부족 11건(운영리스자본화·minSample=20취약성·throttle배포·KR공유코드·Q0재생성·나스닥5순위·유니버스자기참조·BUDGET_MS·종목명현지화·편하게기준·요약층사실판단) · 🔴 종속 2건(Beneish임계값→#23종속·Q1카드선택지→#15종속). **대장 총 74건**(53+21) — 🟢39·🟡23·🔴12.

**§3 §6-C-미확인 마감**: 남은 미확인 0건 — 절 머리에 "🔴 마감(2026-08-19 STEP1080)" 표기.

**§4 표시본**: `build_sequence.html` 머리+본문 둘 다 갱신(53→74건, 새 집계 숫자).

검증3(①전수확인=28개 파일 전부 분류, 미분류 0건 ②근거좌표=㉠㉡㉢ 전부 좌표 확인, 되돌린 것 0건 ③역방향=21개 신규 행 전부 원좌표 문서 자체가 갱신 대상, 고아 판정 0건)·검수3(①판정안함=㉠㉢ 전부 근거좌표 기반이지 "낡아보여서"가 아님, ✅·추천·순서 0건, 정본 여부 안 정함 ②창작·삭제안함=지어낸 선택지 0, 원 문서 내용 삭제 0[표기 추가만], 새 형식 0[STEP1079 6칸 그대로] ③새파일안함=`BUILD_SEQUENCE.md` §6-C 갱신, git status 신규=STEP1080.md 하나)본문기록. 못한 것=없음. 열지 않은 파일=없음(⓪-1 대상 28개 전부 열람). 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 STEP1079가 못 한 전수 추출을 마쳐 대장을 완성하는 것까지다.** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🟡 **STEP1079: 흩어진 판정 대기 전수를 한 장으로 + 종속 표시**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). `BUILD_SEQUENCE.md` §6이 원래 "장은태만 채울 수 있는 칸"이라는 그릇이었으나 A·B가 STEP1073으로 풀리며 그 아래 실제 판정들이 `LENS_DISPOSITION §7-4`·WHAT 「나·다·라」·`ROADMAP_V2` F-5·`probe_1017` 등 여러 문서로 흩어져 있었다. 판정 고르지 않음·순서 안 정함·추천 안 함 — 모으고 종속만 표시.

**⓪-3 부수 정정**: `probe_1017_cron_failure_causes.md`에 `### §5`가 두 개(STEP1058 원본 `:252`·STEP1078 추가 `:324`) 있던 것을 발견 — STEP1079 지시대로 나중 것을 §6으로 바꾸려 했으나, 그러면 STEP1078 블록 자체의 기존 §6("검증3")과 새로 충돌한다는 것을 확인해 **캐스케이드 재번호**(§5→§6·§6→§7·§7→§8)로 정정, `model_list_30_9.html`의 참조 좌표도 함께 고쳤다.

**⓪-1 수집**: 지정 7개 파일 전부 열람 + grep 전수(`판정 대기\|미판정\|장은태 몫\|장은태 판정\|판정한다\|정본 미확정`) — 필터 후 **42개 파일·232줄** 매칭.

**§1 판정 대장**(`BUILD_SEQUENCE.md` §6-C 신설, A~K 11개 하위 그룹): **53건**을 통번호로 모았다 — 🟢 지금 가능 31건·🟡 재료 일부 부족 12건·🔴 다른 판정에 종속(또는 보류) 10건. 검증 중 `DECISION_918_AGENDA2.md`(2026-08-06 승인기록, STEP919 적용: "#17 채택·#37 현행유지·#43 현행유지")를 다시 열어 `AUDIT_904_OPEN_ITEMS.md` #17·#37·#43이 이미 판정됐음을 발견 — 대장에서 빼고(번호 결번, 재번호 안 함) 원 문서에 해소 표기를 붙였다. 같은 방식으로 `Q1_YAHOO_REMOVAL.md:9`(밸류 렌즈→Q1 흡수)·`DECISION_907_WC_DEF.md:4`(driver4→910 현행유지)도 이미 판정된 것으로 확인해 `LENS_DISPOSITION_2026-08-08.md`·`AUDIT_904_OPEN_ITEMS.md`에 해소 표기(취소선+좌표)를 붙였다.

**§2 종속 관계도**: 독립 다수, 종속 사슬 5개(F-5①→②·Q1_ENABLED→disagree/배율표시·Q1_ENABLED→NVDA라벨/섹터표통합/EV-EBITDA문구·층구분↔F스코어경로(같은 오류패턴 공유)·9/9충돌↔F스코어경로(같은 뿌리)·REVDCF_ENABLED Production↔Preview(같은 플래그 다른 스코프)) — 순서 아님, 전제만.

**§3 반대 증거**: ㉠이미판정됨 **9건**(밸류렌즈·driver4·F-5⑮·⑯·AUDIT904 #32·#36·#17·#37·#43 — 뒤 3건은 검증 중 신규 발견) · ㉡중복 **5건**(층구분 4곳→1행 병합·CMA처분 3곳→1행·Q1_ENABLED 2곳→1행·REVDCF_ENABLED Production 2곳→1행·PSR×Financials 2곳→1행) · ㉢조사대기(판정 아님) **6항목**(lens_scores 41종목 식별불가·08-16 알파벳A~E청크 원인·AUDIT904 #44·45·48 page92 미확보·#62 인프라부재·F-5⑨ Altman 착수여부 해소가능성·DECISION912 933산술의 게이트85%전환후 유효성).

**§4 표시본 반영**: `build_sequence.html` 머리+본문 둘 다(§6-C 존재·54→53건 정정 반영) · `model_list_30_9.html` 「나·다·라」에 `§6-C #번호` 병기(내용 변경 없음).

검증3(①누락없음 — grep 42파일 중 14파일은 대장에 반영, 3파일(AD_MONETIZATION·PARKED×2)은 스팟체크로 이미판정 확인, 나머지 ~25파일은 전수 미확인으로 명시 등재 ②원문일치 — 지어낸 선택지 0건, 없으면 "선택지 미작성" 명시 ③역방향 — 각 행 원좌표 문서 자체가 갱신 대상이라 고아 판정 0건)·검수3(①판정·순서·추천 0건 ②창작 0건, "미확인"·"선택지 미작성" 실제 사용 ③새 파일 0개, `BUILD_SEQUENCE.md`에 §6-C만 추가)본문기록. 못한 것=grep 매칭 파일 중 약 25개는 개별 항목 전수 추출 못 함(DECISION_884·890·902·908·923·925·929·DRAFT_MODEL_SELECTION_RECOVERY·MARKET_MODEL_USAGE_TOP20·MODEL_BUILD_ORDER·PRIMARY_SOURCE_MAP·Q1_AXIS_DECISION·Q1_CARD_DESIGN·VALUE_LENS_DEFECT_AUDIT 등, `BUILD_SEQUENCE.md` §6-C-미확인에 명시). 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 흩어진 판정을 모으고 종속만 표시한 것까지다.** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🔵 **STEP1078: 08-16 조인 결함 재발 연속 관측 + lens_scores 999→998**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). `probe_1017_cron_failure_causes.md:279`가 미측정으로 남겨둔 "08-17 이후 재발 여부"를 08-17·08-18 이틀치 데이터로 닫는다. 고치지 않는다. 판정하지 않는다.

**⓪-1 원인 코드 재확인**: `app/api/cron/revdcf/route.ts:65,67`(단일-as_of 정확일치 조인)이 `git log`상 마지막 수정 `877ed6c`(STEP1055, probe_1017 분석보다 이전)로 **그대로**임을 재확인. 전제 불변.

**§1 `us_valuation` 커버리지 08-14~08-19**: per/pbr/psr/ev_ebitda 4지표 전부 08-17(2,133/3,364/3,645/1,512)·08-18(2,130/3,361/3,642/1,512) 이틀 다 08-15 수준(2,068/3,283/3,558/1,452) 근처를 유지 — 08-16 사고 수준(1,516/2,381/2,592/1,046)과 뚜렷이 구분됨. 🔵 **재발 없음.** "동시에 무너지는" 지문은 애초에 붕괴가 없어 비교 대상 자체가 없음(적용 안 함).

**§2 `us_market_cap` as_of 분포**: 최신 as_of 5,897종목 · 그 외(stale) 28종목 · 배제율 **0.47%**(08-16 당시 1,807/5,922≈30.5% 대비 대폭 개선). `lib/lensPrecompute.ts:217` `onConflict:"symbol"` 단일-PK 누적 캐시 구조는 코드상 불변.

**§3 `lens_scores` US 999→998**: 오늘 998행(BUILD_SEQUENCE §5 인용치 일치), 전부 `updated_at=2026-08-18 21:34:40 UTC` 동일 배치. 🔴 **사라진 41종목 개별 식별 = 불가(과거 상태 미저장)** — 일별 스냅샷·프루닝 로그 테이블 자체가 DB에 없음(`information_schema` 확인). `cron_heartbeats` note로 **오늘 실행에서 프루닝이 실제로 6행을 지운 것**(`pruneRowsAttempted:6`, 100행 상한 미도달·중단 없음)은 코드+실측 둘 다로 확정되나, 이 6행이 41행 전체 이력을 설명하는지는 **못 찾음**(과거 스냅샷 부재로 경로 재구성 불가).

**§4 반대 증거 셋**: ㉠ 재발 안 한 이유=§2의 낮은 배제율(0.47%)이 근거 · ㉡ 재발했으나 다른 지문인가=해당 없음(재발 자체가 없음) · ㉢ probe_1017 §4 핵심 주장("결측 증가 전부·예외 0건")이 오늘도 성립=**성립**(오늘 stale mcap 28종목=오늘 per-null-and-stale 28종목, 100%·예외 0건 — 규모만 2,826→28로 축소, 메커니즘은 그대로 재현).

**§5 산출**: `probe_1017_cron_failure_causes.md`에 새 절 추가(신규 파일 아님) — 재발여부·배제종목수·원인코드상태·수리선택지(🔴 Cowork 도출·장은태 확정 아님, probe_1017 §4가 직접 함의하는 3개만: ①정확일치→종목별최신값조인 ②단일-PK캐시→이력테이블 ③실패시 마지막성공값 폴백)·각 위험(전부 미측정 명시)·판정(공란) 6행 표. `model_list_30_9.html` 기존 "조인 구조 그대로라 재발 가능·수리 미판정" 문단에 관측 결과 한 줄 추가(삭제 없음).

검증3(정방향·역방향=08-15→08-16 per 552건 완전 일치·기지값 재현=§4/BUILD_SEQUENCE 인용치와 불일치 0건·조회시각=UTC 04:47:13 명시)·검수3(안고침=코드diff0·DB쓰기0·크론수동실행0·REVDCF_ENABLED/Q1_ENABLED무접촉·추정안함="불가"·"못찾음" 명시·새파일안함=probe_1017에 §5만 추가, 신규 STEP1078.md 외 새 파일 0)본문기록. 못한 것/못 찾은 것=41종목 개별 식별(과거 스냅샷 부재)·프루닝이 41행 전체를 설명하는지. 게이트7—미추적파일참조0건. worktree미생성확인. main직접push. tsc저장소정상(로컬debris재확인, 무관건)·vitest386/386. 🔴**판정금지—장은태가 한다, 이 STEP은 재발 여부를 재고 수리 판정 재료를 놓는 것까지다.** 커밋(CHANGELOG 참고)

## 2026-08-19 — 🟡 **STEP1077: WHAT 「나」 선택지를 바뀐 전제 위에 다시 놓는다 + `<title>` 잔여물**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). WHAT 「나」 절이 스스로 *"위 선택지 셋은 SEC 전제라 다시 놓아야 한다"*고 적어둔 것을 이행 — 그 조건(`probe_1054` 원문 재확인 전엔 취소선 금지)이 이번에 풀렸다. 판정 없음, 순서 무관. 인용 기준 = 원격 `main` = `a22addd`(STEP1076까지).

**⓪-1 취소선 조건 확인**: `probe_1054_tag_supply.md:9`("모집단… SEC companyfacts 확보 5,742… 기준연도 보유 3,606")·`:21`("Piotroski F-Score(9신호 전부) 1,166 / 3,606 / 32.3%")·`LENS_AUDIT_06_FSCORE.md` §1("`probe_1054`의 32.3%는 이 두 캐시 테이블을 쓴 숫자가 아닐 가능성이 매우 높다") 원문 직접 대조 — Cowork이 읽은 것과 **일치**. 조건 풀림.

**§1 「나」 절 재배치**: 옛 선택지 셋("32.3%로간다"·"배열을넓혀올린다"·"접는다") `<s>` 취소선 처리(SEC-단독 전제라 성립 안 함) · "전제 정정" 블록 신설(사실 넷 표 + 감사⑥ 결론 문장 그대로 인용, `LENS_AUDIT_06_FSCORE.md` §1·`probe_1054_tag_supply.md:9·:21` 좌표) · 새 선택지 ㉠㉡㉢(라이브경로그대로/SEC3년치확장/둘다병기, **"Cowork이 도출한 문구 — 장은태 확정 아님"** 명시 라벨) + 각 선택지 걸림 한 줄(㉡ 분량·레이트리밋은 "미측정"으로 명시, 추정 안 함).

**§2 C#9 연결**: 「나」 절 끝에 `LENS_DISPOSITION §7-4 C#9`(9/9 vs 구조적불가 충돌)와 같은 뿌리라는 한 줄 추가 — ㉠을 고르면 C#9이 남고 ㉡·㉢을 고르면 풀릴 경로가 생긴다는 연결만, C#9 자체는 판정 대기 그대로.

**§3 `<title>` 잔여물**: `model_list_30_9.html` title(h1과 불일치, "확정된 칸 넷" 누락) → h1과 일치하게 수정(이모지 제외 텍스트 동일) · `build_sequence.html` title(h1 대비 "만드는가" 누락, 단순 절단) → 보정. `roadmap_v2.html`·`data_source_catalog.html`·`tombstone.html` 3개는 title≠h1이나 **의도된 구조 차이**(브랜딩 프리픽스·요약형 title)로 판단해 무접촉(검사함·판단 후 미수정).

검증3(원문대조=사실넷·결론문장 글자단위 일치, Cowork도출 라벨 확인·더나중문서=grep "71.9|32.3" 전수, LENS_COMPLETION_STANDARD의 71.9%는 전혀 다른 지표[DCF 터미널비중]라 무관 확인, 뒤집은 문서 0건·역방향=BUILD_SEQUENCE §6-B-1·LENS_DISPOSITION §8-1·§7-4는 여전히 정확해 무접촉, 사유 명시)·검수3(판정안함=㉠㉡㉢ 중 선택 0·추천 0·C#9 미판정·완성순서 미기재, 창작안함=셋 다 감사⑥ §1 근거, 미측정 표기 준수, 새CSS0·삭제새파일안함=옛선택지 `<s>` 보존, 새파일1개[명령서]) 본문 기록. 못 거친 축=없음, 열지 않은 파일=없음. 산출물=`docs/model_list_30_9.html`·`docs/build_sequence.html` 갱신(정본 md 변경 없음). tsc 저장소 정상(로컬 debris 재확인, 무관 건)·vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 WHAT 파일이 스스로 요구한 것을 이행한 것까지다.**

---

## 2026-08-19 — 🟡 **STEP1076: WHAT 작업 파일에 판정 A·B 확정 + 「확정된 칸 넷」 반영**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). `docs/model_list_30_9.html`(WHAT 작업 파일 — 대응 md 정본 없는 표시본) 전문 파싱 결과 "판정 A"·"판정 B"·"확정된 칸"·"1개씩 완성" 검색 0건 — STEP1073·1074가 단 것은 개별 항목 각주 다섯뿐, 문서 전체의 전제(머리·「완성까지 남은 것」 헤더)는 여전히 08-18 상태였음. 판정 없음 — 정본(`BUILD_SEQUENCE.md`·`ROADMAP_V2.md`) 문구를 그대로 옮기는 데까지. 인용 기준 = 원격 `main` = `58ee128`(STEP1075까지).

**§1 머리 갱신**: `<h1>` "질문 여섯·보류2·탈락3" → "질문 여섯·🟢 확정된 칸 넷·보류2·탈락3"(기존 세 낱말 유지, 넷만 삽입). 첫 `.lead` 아래 판정 A·B 확정 두 줄 신규(정본 문구 그대로, 날짜·STEP 병기).

**§2 「확정된 칸 넷」 섹션 신설**(「완성까지 남은 것」 바로 위, 이 문서 기존 `<table>` 관행 그대로 — build_sequence.html의 이질적 CSS 클래스 대신 이 파일 자체 표 스타일 재사용): ①배수+역DCF(구조확정, Q1트랙 미해결) ②재무건전성/F스코어(구조확정, 9/9충돌+71.9%↔32.3% 재정리 필요) ③④최근변화/모멘텀+기술(구조확정, 고칠 것 3건). 완성 승인 0개 명시.

**§3 기존 절 정합**: 「완성까지 남은 것」 헤더 "가·다 확정·남은 것 셋" 취소선+"가-1=확정·가-2=정합·가-3=방향일치·다=진짜미반영, 한 덩어리 아님" 병기 · 「마. 7렌즈 재사용 판정」에 F스코어·모멘텀+기술이 확정된 칸으로 승격됐다는 한 줄 + "거취 아니라 규칙3 심사" 재확인 · 「나. Piotroski 32.3%」에 "확정된 칸에 올랐으나 완성 승인 아님" 한 줄. 「다」「라」는 판정 대기 그대로 무접촉.

**§4 C~F**: WHAT 파일에 역DCF위치·ROADMAP재작성·7렌즈거취·Q1 4건을 다루는 별도 절 자체가 없음(grep 0건) → **신설 안 함**, "해당 절 없음"으로 보고.

검증3(정본대조=새로 지은 문장 0개, 전부 `BUILD_SEQUENCE.md`·`ROADMAP_V2.md` 인용·손실없음=git diff 23삽입/2삭제이나 삭제 2건은 교체된 헤더 원문 자체[내용 손실 아님]·앞뒤일치="판정A"1건·"판정B"2건·"확정된칸"5건·"1개씩완성"2건 전부 🟢확정으로 일치, 미판정 잔존 0건)·검수3(판정안함="다"·"라"·F-5·§7-4 그대로, 완성순서 미기재, Q1_ENABLED·REVDCF_ENABLED 무접촉·삭제창작안함=전부 기존 `<s>`·`<table>` 관행 재사용, 새 CSS 0·새파일1개[명령서]·Downloads무접촉) 본문 기록. 못 거친 축=없음, 열지 않은 파일=없음(⓪-2 목록 5개 전부 열람). 산출물=`docs/model_list_30_9.html` 갱신(정본 md 변경 없음, 표시본 전용 STEP). tsc 저장소 정상(로컬 debris 재확인, 무관 건)·vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 이미 확정된 것을 WHAT 작업 파일에 옮긴 것까지다.**

---

## 2026-08-19 — 🟡 **STEP1075: 표시본 머리 요약 동기화 — 한 문서 안의 모순 해소**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). STEP1073이 `docs/build_sequence.html`의 §1·§6 표(본문)만 고치고 문서 맨 앞의 "머리 요약"을 두어, 같은 문서가 **앞에서는 "판정 A·B 미판정", 뒤에서는 "🟢 확정"**이라 말하는 상태가 이틀 남아 있었다. 판정 없음 — 이미 확정된 것을 표시본 첫 화면까지 옮기는 데까지. 인용 기준 = 원격 `main` = `597b2cc`(STEP1074까지).

**§1 `docs/build_sequence.html` 머리 요약 4곳**(정본 문구 그대로, 기존 `<s>` 취소선 재사용, 새 문장 창작 0): 「판정 A」 카드=취소선+🟢확정 · 「판정 B」 카드=취소선+🟢확정(「1위」 미채택 근거 3건 병기) · 「C~F」 한 줄=취소선+새 상태(C·D내용=🟡착수가능·E=🟢답 있음·F=🔴§6-B-1 결과대기) · 🆕 「확정된 칸 넷」 카드 신설(`:173` 정본 그대로 이관).

**§2 나머지 표시본 검사**(태그 제거 전문 파싱, 6개 패턴 전수 검색): `roadmap_v2.html` — "1위를 제대로" 2건 모두 STEP1073이 이미 그 자리에 정정 문단을 붙여둔 것으로 확인, 그 외 "판정 대기"류는 전부 A·B가 아닌 다른 판정(F-5·M-4)이라 무접촉 → **검사함·해당 없음**. `model_list_30_9.html` — "미판정"·"판정 대기" 다수는 전부 STEP1073·1074가 이미 정정을 붙인 자리이거나 A·B와 무관한 주제(크론 수정·DCF 표본크기·SWS Past Performance) → **검사함·해당 없음**. 🔴 두 파일 모두 A·B를 다루는 별도 "머리 요약" 섹션 자체가 없어 build_sequence.html과 같은 병은 원리적으로 없었다.

**§3 재발 방지**: `BUILD_SEQUENCE.md` §7(문서 위계) 표 아래 신설 — "표시본 갱신 시 머리 요약과 본문 표를 둘 다 고친다."

검증3(앞뒤일치=hero+본문표 둘 다 🟢확정으로 일치·정본대조=새 문장 창작 0·역방향=두 표시본 "검사함·해당 없음" 명시)·검수3(판정안함=C~F·F-5 그대로·Q1_ENABLED·REVDCF_ENABLED 무접촉·삭제안함=전부 `<s>` 보존·새파일1개[명령서]·새방식창작0[기존 `<s>` 재사용]) 본문 기록. 못 거친 축=없음, 열지 않은 파일=없음(⓪-2 목록 5개 전부 열람). 산출물=`docs/BUILD_SEQUENCE.md` 부-14 신규(기존 보존)+§7 재발방지 한 줄+`docs/build_sequence.html` 머리요약 4곳. tsc 저장소 정상(로컬 debris 재확인, 무관 건)·vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 이미 정본에 반영된 판정을 표시본 첫 화면까지 옮긴 것까지다.**

---

## 2026-08-19 — 🟡 **STEP1074: STEP1073 §4 오류 정정 — 「확정된 모델」 판정을 기존 검수 기록과 맞춘다**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). STEP1073이 §4에서 `git log`(한 계층)만 보고 다섯 항목을 균일하게 "미확정" 처리했으나, 같은 문서 부-7(STEP1069)이 이미 한 줄씩 갈라 대조해뒀던 것을 열지 않은 오류 — `BUILD_SEQUENCE §8` #1·#4 반복 실수의 재발. 새 판정 0(기존 검수 기록에 맞춰 되돌리는 정정). 인용 기준 = 원격 `main` = `d04d079`(STEP1073까지).

**§1 §6-B-1 표 4행 정정**(취소선+정정+좌표): 가-1(배수+역DCF 병합)=🟢**확정으로 승격**(`ROADMAP_V2.md:235` W-2 첫 질문이 이미 병합 형태, STEP1053 2026-08-16 확정 · `model_list_30_9.html` 가-1 자신도 "정본 일치") · 가-2(DCF정방향)=⚠️**"미확정"이 아니라 "정합"**(양쪽 다 미판정이라 어긋남 아님) · 가-3(DDM)=⚠️**표현 차이, 방향 일치**(`ROADMAP_V2.md:307`에 이미 "적정주가류로 판정") · 다(층구분)=✅**STEP1073이 맞음, 유지**(진짜 md 미반영). "다섯이 균일하지 않다" 머리글 추가.

**§2**: 명시적 개수 결론 문장이 원래 없었음을 밝히고 신규로 "확정된 칸은 넷"(①배수+역DCF ②F스코어 ③④모멘텀+기술) 명시 — 완성 승인은 여전히 0개.

**§3**: `BUILD_SEQUENCE §8` #1·#4에 재발 사례 각 한 줄 + 재발방지 신설(git이력·정본본문·기존대조기록 셋을 다 본다).

**§4 F-5 갱신**: ⑮=🟢정본 여섯 확정(표시본 여덟은 스냅샷) · ⑯=🟢정본에 이미 병합 확정 · ⑲=⚠️정본에 이미 방향 일치 · ⑰·⑱은 그대로. 공통 주석 취소선+정정("넷 전부 미반영"→"⑱ 하나만 남음").

**§5**: `LENS_DISPOSITION §7-4 C#10` 범위 축소(삭제 아님) — "남은 판정 대기는 ⑱ 하나".

**§6 표시본 동기화**: `model_list_30_9.html` 5곳·`roadmap_v2.html` F-5 3행+공통주석 반영. `build_sequence.html`은 §6-B-1 자체가 STEP1073에서 이관 안 된 영역이라 해당 사항 없음(기존 안내 유지).

검증3(원문대조=부-7·ROADMAP_V2.md:235·:307과 글자단위 확인·누락없음=grep "미확정" 전수, 다 1건만 의도적 잔존 확인·역방향=F-5⑯·C#10·STATE·표시본3개 확인)·검수3(판정안함=⑰⑱ 그대로·Q1_ENABLED·REVDCF_ENABLED 무접촉·삭제안함=전부 취소선 보존·새파일0개) 본문 기록. 못 거친 축=없음, 열지 않은 파일=없음(⓪-2 목록 7개 전부 열람). 산출물=`docs/BUILD_SEQUENCE.md` 부-13 신규(기존 보존)+`ROADMAP_V2.md`·`LENS_DISPOSITION_2026-08-08.md`·`model_list_30_9.html`·`roadmap_v2.html` 갱신. tsc 저장소 정상(로컬 debris 재확인, 무관 건)·vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 STEP1073의 오류를 기존 검수 기록에 맞춰 되돌린 것까지다.**

---

## 2026-08-19 — 🟡 **STEP1073: 판정 A 확정 반영 + 판정 B 낱말 교체 후 확정 + 「확정된 모델」 목록 정본화**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 장은태가 2026-08-19 AskUserQuestion+대화로 이미 내린 판정을 문서에 옮기는 STEP — 새 판정 0(단 §4는 "확정된 모델이 무엇인가" 수집·대조, 충돌은 판정 대기로만 등재). 인용 기준 = 원격 `main` = `722778f`(STEP1072까지).

**판정 A**(제품 정의 우선순위) — 원문 그대로: *"①정의3(질문)이 최상위, 정의2(정직함)는 신뢰층, 정의1(기법나열)은 폐기"*(`BUILD_SEQUENCE.md:227` 선택지①). **판정 B**(다음 착수 대상) — 🔴 선택지를 고른 게 아니라 **물음 자체를 고쳐 적었다**: *"1위를 제대로가 아니라 확정된 모델을 1개씩 완성을 하자가 맞는말 아니야?"* → 「확정된 모델을 1개씩 완성한다」로 반영, 「1위」 낱말은 3개 저장소 좌표(배수는 동점4개·적정주가1위는 관문②탈락·§8자체가 "1위" 지목을 반복실수로 등재)로 제외.

**§1 판정A 반영 4곳**(취소선+정정+좌표, 지운 문장 0): `BUILD_SEQUENCE.md` §1(정의 셋 표에 층 라벨 열)·§6표 A행·§6-부록 부록-1 결론·`STATE.md` §6 마지막 불릿.

**§2 판정B 반영 5곳**: `BUILD_SEQUENCE.md` §6표 B행("§6-B" 신설 — 옛문구 취소선+새문구 전문)·부록-2 결론·부록-5 판정B 다섯칸(2곳)·`ROADMAP_V2.md` F-4-1②·F-4-3(2곳)·`MODEL_BUILD_ORDER_2026-08-07.md:74`.

**§3 C~F 상태 라벨**(부록-3에 "새 상태" 열 추가, 실착수 0): C=🟡착수가능·D-내용=🟡착수가능·E=🟢답 있음(🔴 "거취" 프레임 자체가 틀렸다 — 실제 목적은 `CLAUDE.md:81` 규칙3 재사용 자격 심사, `STEP1066.md`가 스스로 "규칙3의 종착"이라 명시)·F=🔴§4결과대기.

**§4 「확정된 모델」 목록 정본화**(`BUILD_SEQUENCE.md` §6-B-1 신설): 앞 세 행은 7렌즈 감사 결과(재무건전성=F스코어·최근변화=모멘텀+기술, `LENS_DISPOSITION §8-1~8-4`에서 그대로 옮김, 새 판단 0) — 전부 **구조 확정이지 완성 승인 아님**, `LENS_COMPLETION_STANDARD`의 "9/9✅"와 감사의 "항목3 구조적 불가" 충돌은 STEP1072가 이미 등재(`§7-4 C#9`, 중복 없음). `model_list_30_9.html`에서 "확정" 전수 추출(grep 20건 중 모델/판정 관련 8건→실질 5개 항목: 배수↔역DCF·DCF정방향·DDM·층구분·질문여섯) — `git log --oneline --all -- docs/model_list_30_9.html` = STEP1069·1071 둘뿐(편입 이전 이력 없음, 추측 금지) → 5개 전부 **㉢출처추적불가**로 분류되나 **전부 이미 F-5 ⑮~⑲·`LENS_DISPOSITION §7-4 C#10`에 등재돼 있어 신규 판정 대기 0건**.

**§5 표시본 동기화**(정본 먼저 확인 후): `docs/model_list_30_9.html` 5곳에 "🔴 판정 대기(좌표)" 라벨 부착(내용 삭제 0) · `docs/roadmap_v2.html` F-4-1②·F-4-3 반영 · `docs/build_sequence.html` §1 정의카드+§6 판정표 반영, 단 🔴 **§6-부록 전문(부록-1~5·부-9~12, 약 400줄)은 이 STEP에서 이관 못 함**(정본 참조 안내만 추가, 못 한 것으로 명시).

검증3(문구일치 — "폐기"·"확정된"·"1개씩"·"완성" 원문 낱말 그대로 확인·누락없음 — A·B 관련 미판정 잔존 grep 0건·역방향 — STATE/ROADMAP/MODEL_BUILD_ORDER/LENS_DISPOSITION 4곳 확인)·검수3(창작 0 — 전부 장은태 원문·저장소 좌표 인용·새파일 1개만(이 명령서)·판정 안 함 — §4-2 ㉢ 5건 전부 기존 등재만 인용, C~F 실착수 0, Q1_ENABLED·REVDCF_ENABLED 무접촉) 본문 기록. 산출물=`docs/BUILD_SEQUENCE.md` §1·§6·§6-부록·§7-부록 부-12 신규(기존 보존)+`STATE.md`·`ROADMAP_V2.md`·`MODEL_BUILD_ORDER_2026-08-07.md`·`model_list_30_9.html`·`roadmap_v2.html`·`build_sequence.html` 갱신. 못 한 것=`build_sequence.html`에 §6-부록 전문 이관(정본 참조로 대체). 새로 생긴 판정 대기=없음(전부 기존 F-5·§7-4 C에 흡수). tsc: 저장소 정상(로컬 debris 재확인, 무관 건). vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 이미 내려진 판정을 문서에 옮기고 확정된 모델 목록을 정리한 것까지다.**

---

## 2026-08-19 — 🟡 **STEP1072: 미반영 정정 전수 수집 후 문서에 반영**

> 문서 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 취득 0). 3번 규칙으로 도출: B(새모델vs1위) 없이는 만드는 일을 시작 못 하는데, 문서가 어긋난 채 판정하면 잘못 판정한다 — 그래서 "만들기 전에 정정부터". 인용 기준 = 원격 `main` = `9dfea91`(STEP1071까지).

**§2-1 전수 수집**: 정방향 패턴검색(`정정 재료|정정하지 않|정정 안 함|미반영|반영된 적 없`) — CHANGELOG.md 19·BUILD_SEQUENCE.md 11·LENS_DISPOSITION 10·STEP_LEDGER.md 9·REVDCF_SPEC 3 등. 🔴 **CHANGELOG·STEP_LEDGER·구 STEP_COMMAND류는 이력 문서라 교정 대상 제외**(현재상태=STATE, 이력=CHANGELOG·STEP_LEDGER 원칙 — 소급 수정하면 기록이 왜곡됨). 역방향(STEP1055~1071 각 산출물의 "못 한 것" 절 재열람) — 씨앗 10건 대비 새로 걸린 것 **0건**(씨앗 자체가 이미 전수에 가까웠음).

**§2-2 사실/판정 이분**(10건): 사실 정정 2건(SWS 4개→6/6·MODEL_ROSTER 비공개→공개) · 판정 등재 4건(9/9완성승인 vs 구조적불가 충돌·F-5확정딱지 미반영·Downloads5개존치·probe_B_flows처분) · 기등재 1건(32.3%, 이미 `LENS_DISPOSITION §7-4` A-8) · 사실·표시만 1건(관문7개 낡음) · 사실·갱신 1건(§5 스냅샷) · 해당없음 1건(probe_1041, 이력 문서) · **미확인 1건**(판정A "폐기"가 원문에 없는 낱말 — grep으로 근거 문서를 못 찾아 반영·등재 둘 다 보류).

**§2-3 사실 정정 반영**(취소선+정정+좌표): `MODEL_ROSTER.md:84` · `LENS_DISPOSITION_2026-08-08.md` §2·§4·§7-5 3개소("4개가 수익성"→"6/6 전부 관련, 우리 질문축 기준 3/6" 병기). 표시본(`roadmap_v2.html`·`model_list_30_9.html`) 동기화 불필요 확인(grep 0건).

**§2-4 판정 등재**: `LENS_DISPOSITION_2026-08-08.md` §7-4에 "C. STEP1072 신규 — 4건"(다섯 칸 형식, #9~12) 신설. `ROADMAP_V2.md` F-5 ⑮~⑲ 뒤 공통 주석 1행(model_list_30_9.html "확정" 딱지가 F-5엔 미반영) + `docs/roadmap_v2.html` 표시본 동일 반영. 추천·순위 없음.

**§2-5 스냅샷 갱신**(DB 실제 조회, `docs/BUILD_SEQUENCE.md` §5): `revdcf_results` as_of=2026-08-18 604행(불변) · `us_valuation` 5,820행(불변)·per 2,130·pbr 3,361·psr 3,642·ev_ebitda 1,512(전부 소폭 증가, STEP1058의 08-17 밤 자연복구와 정합) · `lens_scores` US 998행(1,039에서 감소 — 원인 미조사, 범위 밖 명시) · `REVDCF_ENABLED`·`Q1_ENABLED`는 **Vercel SSO가 익명 접근 차단해 직접 확인 불가**(프로덕션 URL 특정 후 재시도해도 302) — 간접 근거(F-5 미승인·전환 기록 없음)로 OFF 추정만, 추측을 실측으로 기재하지 않음. 옛 값은 지우지 않고 나란히 병기.

**§2-6**: `BUILD_SEQUENCE.md` §3 머리에 "폐기된 관문 7개 기반" 명시 + F-2 좌표, 표 내용 무접촉.

검증3(축A 씨앗10건 전부 근거STEP 원문 직접 확인·축B 취소선+정본우선 형식 준수·축C 판정오분류 방지4건/중복0건/근거뒤집힘1건 해소)·검수3(수집10=반영2+등재4+기등재1+해당없음1+표시만1+갱신1+미확인1·씨앗밖신규0건·표시본누락0건) 본문 기록. 못 거친 축=씨앗9의 근거 문서 특정 불가(미확인으로 남김, 반영·등재 둘 다 안 함). 철회·정정=없음. 산출물=`docs/BUILD_SEQUENCE.md` §7-부록 부-11 신규(기존 보존)+`MODEL_ROSTER.md`·`LENS_DISPOSITION_2026-08-08.md`·`ROADMAP_V2.md`·`roadmap_v2.html` 갱신. tsc: 저장소 정상(로컬 debris 재확인, STEP1070·1071과 동일 무관 건). vitest 386/386. 🔴 **판정은 장은태가 한다. 이 STEP은 사실을 맞추고 판정할 것을 한자리에 모으는 것까지다.**

---

## 2026-08-19 — 🟠 **STEP1071: 초안 26개 업데이트·중복 삭제·제자리 고정**

> 문서 변경 + 파일 삭제(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 취득 0). 방향 전환(2026-08-18 장은태): *"지워도 되나"*가 아니라 *"살릴 것을 어떻게 업데이트해 넣나, 그다음 중복을 어떻게 없애나"*. `~/Downloads/_to_delete/2026-08-16_roadmap_drafts/`의 초안 26개(로드맵 계보 9·model_list 계보 14·제목 다른 2·현재본 사본 1)를 전량 텍스트 대조. 인용 기준 = 원격 `main` = `d414037`(STEP1070까지).

**현재본 집합(10개, md 정본 포함)**: `docs/model_list_30_9.html`·`roadmap_v2.html`·`build_sequence.html`·`data_source_catalog.html`·`tombstone.html`·**`ROADMAP_V2.md`**·**`BUILD_SEQUENCE.md`**·**`LENS_DISPOSITION_2026-08-08.md`**·`~/Downloads/model_list_30_9.html`·`~/Downloads/roadmap_v2_2.html`.

**인접 diff(전제① 반증)**: A계보 8쌍 중 6쌍·B계보 13쌍 전부에서 사라진 문단 1건 이상 → **26개 전부 대상으로 진행**(축소 안 함).

**고유 문장 추출**: 거르는 기준(길이25자+·숫자/좌표/판정어 1개 이상·현재본 10개 부재, `dup_12` md5일치로 제외) 고정 후 실행 — **원시 1,680건 → distinct 169건**(`docs/probe_1071_to_delete_diff.json`).

**제목 다른 둘 해소**: `model_list_30_4.html`(공통모델22-채택탈락판정)·`_30_5.html`(쓰는모델19·탈락11) 전문 확인 — 별개 문서 아니라 B계보 내 22-모델 개별판정 표 분기. 🔑 **"탈락11→3"의 8개 행방 확정**: 22개 이전에 걸러진 8개(Magic Formula·Residual Income/EVA·EPV·Net-Net/청산가치·Beneish·CFROI·추정치개정/어닝서프라이즈·Rule of40)가 현재 `model_list_30_9.html`의 "그 앞에서 이미 걸러진 여덟" 절과 좌표 일치 — **추적됨, 잃은 것 없음.**

**반영 2건**(원문 좌표 그대로, `model_list_30_9.html`): ① DCF정방향 보류 사유에 초안 표본크기 병기(현재 "실무1/3" vs 초안 "조사한 8곳 전부" — 미판정 양쪽병기) ② DDM병합 단락 뒤 신규 발견 추가 — "시나리오 밸류에이션(Bear/Base/Bull) → DCF에 흡수" 판정이 현재본 어디에도 없었음(반영 전 확인).

**미반영 167건**(4개 사유 버킷, 각 사유·대표예 `BUILD_SEQUENCE.md` §7-부록 부-10에 표로 기록): ①이미 본문에 실질 존재(~90) ②이후 STEP이 정정한 낡은 사실(~5, dividends 60행 KR·Assets태그미수집 등) ③절차·메타 서술(~40) ④F-5에 이미 판정대기 등재(~32). 보류 1건(배수↔역DCF 커버리지차 수치 — 현재본이 더 정밀한 값 보유). **삼자대조: 169 = 2 + 167, 사유 없는 미반영 0건.**

**`_probe_B_flows.ts`(STEP1070 이월) 처리**: 전문 확인(STEP811 KRX 수급/공매도 API 프로브, mtime 7/28) — 결과가 CHANGELOG·STEP_LEDGER 어디에도 없고, 코드 안 특정 엔드포인트 후보가 다른 파일에 중복 없음 → **판단 못 함, 지우지 않음**(KR 동결 정책상 지금 진행 대상은 아니라는 사실만 병기).

검증3(원문26개대조·STEP1056/1069방법재사용·사라진문단15/21쌍+고유169+제목다른둘=분기확정)·검수3(원시/잔여 둘다기록·삼자대조·삭제는 게이트8 통과 후로 순서 준수) 본문 기록. 못 거친 축=`_probe_B_flows.ts`의 실제 API 응답(외부 취득 금지로 범위 밖). 철회·정정=없음(반영은 누락 보충이지 정정 아님). 산출물 = `docs/BUILD_SEQUENCE.md` §7-부록 부-10 신규(기존 §0~부-9 보존) + `docs/model_list_30_9.html` 2건 갱신 + `docs/probe_1071_to_delete_diff.json`(원자료).

**§2-8·§2-9 후속(게이트8 통과 확인 후, `cf33314` CI success·Vercel Production success)**: 조건 넷 재확인 후 `~/Downloads/_to_delete/2026-08-16_roadmap_drafts/`(26개) 삭제(`rm -rf`, exit 0) — 삭제 전후 `ls -la` 전량 대조(`docs/BUILD_SEQUENCE.md` §7-부록 부-10-삭제에 원문), **`~/Downloads/` 현재본 5개·개인 파일·`docs/` 전부 무접촉 확인.** `docs/STATE.md` 「어디를 보면 되는가」에 이어 문서 고칠 때 원칙(기존 파일 덮어쓰기)·이전 버전 `git show` 명령 두 줄·표시본 위치·**현재본 5개 존치 여부 판정 대기**를 추가(새 문서 미신설).

tsc·vitest는 이 커밋 기준 확인. 🔴 **판정은 장은태가 한다. 이 STEP은 살릴 것을 넣고 중복을 지우는 것까지다.**
...
**⓪-1b 기존 심사 먼저**: `probe_1043_model_final.md`(GP/A·DuPont은 "재무건전성 칸의 후보인가"라는 **다른 질문**에 이미 탈락 판정돼 있었음 — 재심사 안 함) · `probe_1041_model_selection.md`(DuPont #10위, 당시 "Assets 결측" 블록 사유는 STEP1054/1055가 이미 해소한 낡은 사유) · `MODEL_UNIVERSE_63_2026-08-07.md`(Category D·E 전수 — GP/A#40·DuPont#31·RMW#44 셋만 이름 있음, RMW는 표 자신이 "종목 단위 산출물 아님"이라 명시) · `MODEL_ROSTER.md`(**전문 168줄** 5계층 전수, 이름검색 아님) · `MARKET_MODEL_USAGE_TOP20_2026-08-07.md`(`MODEL_ROSTER`가 인용하는 08-07 문서 4건 중 이 절 목록에서 빠져 있던 것을 추가 확인 — `BUILD_SEQUENCE §8 #4`의 "08-07 조사 누락" 재발 방지). **결론: ROE·ROA·ROIC·ROCE는 이 저장소 어디에도 독립 후보로 이름 붙은 적이 없다** — 실무(§9)가 실제로 쓰는 지표가 우리 후보 목록엔 없었다.

**②원전 공식 확보**: 씨앗 8개(GP/A·ROE·ROA·ROIC·ROCE·DuPont·순이익률/영업이익률·FCF마진)+RMW 총 9후보 중 **GP/A만 명명된 단일 원전**(Novy-Marx 2013, ④감사 재사용·재대조 안 함), **RMW도 원전 있음**(Fama-French 2015)이나 포트폴리오 롱숏 산출물이라 종목별 답과 출력 형태가 다름. **나머지 7개는 단일 저자 없는 회계 관행**(PER·PBR·성장률·배당수익률과 같은 급) — CLAUDE.md 규칙 5-1("원전 없는 항목은 정의 공개표로 대체") 대상으로 분류, 정의는 지금 고정하지 않음(④단계 몫).

**③카탈로그 재료 대입**(`DATA_SOURCE_CATALOG.md` 먼저 열람, H-2 준수): 🔴 **발견 1** — 카탈로그 20슬롯은 역DCF 전용이라 **총자산이 슬롯 자체로 없음**(GP/A·자산성장이 쓰는 `total_assets`는 카탈로그 밖 별도 컬럼). 🔴 **발견 2** — Damodaran `margin.xls`(업종별 마진 3종)·`roe.xls`(업종별 ROE 듀폰분해+ROC/ROIC)가 이미 카탈로그에 등재돼 있었음(재발견 아님, 992/1002 세션) — 단 **업종 집계**라 종목별 원천이 아니라 벤치마크 용도. 종목별 재료는 `us_fundamentals`(재취득 없이 기존 테이블): ROE·ROA·DuPont·순이익률/영업이익률은 필요 필드 전부 보유, **ROCE는 유동부채 컬럼이 없어 근사만 가능**, **ROIC는 세율 컬럼이 없어 원료 단계**, **FCF마진은 capex 컬럼이 없어 계산 자체가 불가**.

**DB 커버리지 실측**(GP/A 기존 85.8%[857/999]와 동일 분모 `lens_scores` US 999 기준, `us_fundamentals` 조인): **ROE 78.6%(785)** · **ROA 65.2%(651)** · **DuPont 65.0%(649)** · **ROCE(근사) 63.0%(629)** · **ROIC(원료) 57.6%(575)**. 🔴 GP/A는 Yahoo 파생·나머지는 SEC XBRL 파생이라 인구(999)는 같으나 원천 파이프라인이 다름 — 같은 파이프라인 재계산 대조는 안 함(⑤단계 몫).

**관문 3개 심사 + STEP1067 미해결 둘 해소**: ROE·ROA는 3관문 전부 🟢/🟡(실무 채택 근거 확실·정의 단순). ROIC·ROCE는 정의 미확정·재료 미비로 ③(정직 공개)이 🟡~🔴. FCF마진은 재료 자체가 없어 ③🔴. 🔴 **(a) SWS 6체크 재분류**: §9의 "6/6 전부 수익성 관련"을 **우리 질문 분류축**(성장 vs 재무건전성)으로 다시 나누면 CHECK#1-3(EPS성장 3종)은 **성장 질문**에 속하고, 이 절의 "수익성" 질문엔 **CHECK#4·5·6 셋만 대응** — §9 정정은 아님(다른 분류축), 재료로 병기. 🔴 **(b) Stock Analysis 섹터 확장**: AAPL(기존) 외 **JPM(금융)·NEE(유틸리티) 라이브 WebFetch 2건 추가** — 레벨 전용 테이블 패턴은 n=3 전부 유지, 단 **JPM엔 ROIC·ROCE 항목 자체가 없고 NEE엔 ROIC만 없음**(업종별 필드 구성 차이 신규 발견, 우리 커버리지 실측과 방향 일치).

검증 3(원문 3문서 재독·Supabase 실측 1회·WebFetch 3건)·검수 3(반박시도="이름 없다" 결론 반박재료 못 찾음/수치=전부 이 세션 실제 실행/이전발언 대조="6/6"과 "3/6"은 분류축이 다른 것이지 모순 아님, §10-5-a에 명시) 전부 본문에 기록. 산출물 = `docs/LENS_DISPOSITION_2026-08-08.md` §10 신규 절(기존 §0~§9 보존, 새 파일 미신설). 못 한 것=순이익률/영업이익률 DB커버리지 실측(범위 밖 보류)·ROIC 투하자본 정의 실무 대조(④단계 몫). 철회·정정=없음. 미측정=GP/A vs SEC 파생 커버리지 차이의 원인 분리·ROCE/ROIC 근사치가 정의 확정 후에도 유지되는지·FCF마진 재료 조달 방식. tsc 0·vitest 386/386(변경 없음). 🔴 **판정은 장은태가 한다 — 이 STEP은 후보를 놓고 원전·공식·재료를 붙이는 것까지다. `BUILD_SEQUENCE §2` ④⑤로 넘어가지 않았고 질문·모델·렌즈배정을 하지 않았다.**

---

## 2026-08-18 — 🔵 **STEP1067: 「수익성 수준」 축을 실무가 별도로 두는가 — 몇 곳 중 몇 곳 세기**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 표시본이 *"SWS 5축에 수익성 「수준」을 보는 칸이 있다. 우리는 재무건전성 질문에서 「좋아지고 있나」라는 변화만 묻고 수준을 안 묻는다 — 안 묻기로 한 것이 맞는지는 판정된 적 없다"*로 등재만 하고 미판정으로 남긴 것을 처음으로 셌다. 인용 기준 = 원격 `main` = `3b35615`(STEP1066까지).
>
> **⓪-1b 저장소 전수 먼저(웹보다 먼저)**: `ls docs/` 1,286개 중 관련 7개 파일을 grep + 관련 절 전문으로 확인 — `LENS_DISPOSITION §2`가 이미 *"SWS Past Performance 6체크 중 4개가 수익성(ROE>20%·ROCE 3년전比 증가·ROA 업종평균 이상)"*이라 인용했으나 3개만 이름이 있고 몇 대 몇(수준 vs 변화)인지는 센 적이 없다 — 이 절이 처음.
>
> **🔴 전제① 검증(우리 것부터, 축 C 먼저)**: `lib/fscore.ts`의 Piotroski 9신호를 원전의 Δ 접두어 유무로 분류한 결과 **수준 3개**(F_ROA·F_CFO·F_ACCRUAL, 전부 Δ 없음) · **변화 5개**(F_ΔROA·F_ΔLEVER·F_ΔLIQUID·F_ΔMARGIN·F_ΔTURN) · **사건형 1개**(F_EQ_OFFER). 나머지 여섯 질문(배수·성장·배당·부도위험·최근변화)도 전수 확인 — 배수는 "가격" 수준이지 "수익성" 수준이 아니고, 성장·최근변화는 정의상 전부 변화, 배당·부도위험은 재료 자체가 없다. **결론: "수준을 안 묻는다"는 부정확하다.** 정확한 서술은 *"재무건전성 축에서 이진 문턱(양수/음수) 형태로 수익성 수준을 3/9만큼 묻고 있으나, GP/A·ROE처럼 연속 스펙트럼에서 순위를 매기는 형태의 수준 축은 없다"* — 명령서가 미리 갈라둔 두 가지 서술 중 후자가 맞았다. 정정은 하지 않고 재료로만 남긴다.
>
> **🔴 축 B 실무 세기(SWS·Stock Analysis·AAII 고정 표본, `LENS_DISPOSITION §2`와 동일)**: SWS는 GitHub 공개 저장소 원문을 직접 취득해(`gh api`, `MODEL.markdown`) "Past Performance" 6체크 전문을 확인 — **수준 2개**(ROE>20%·ROA>업종평균) · **변화 4개**(EPS 성장 셋 + ROCE 3년전比) — **6/6 전부 수익성/수익률 관련**이다(`LENS_DISPOSITION`의 "4개" 주장과 다름, `MODEL_ROSTER`의 "세부 비공개" 주장도 이 GitHub 저장소로 반증됨 — 둘 다 정정 재료로 기록, 정정은 하지 않음). Stock Analysis는 AAPL `financials/ratios` 페이지를 라이브 확인 — ROE·ROA·ROIC·ROCE가 연도별 수준 테이블로 표시되고 **계산된 변화/성장 수치는 없음**(대조군 Market Cap Growth엔 명시적 %변화가 있어 이 섹션만 의도적으로 수준 전용으로 보임). AAII는 `LENS_DISPOSITION §2`의 기존 인용을 재사용(재조사 안 함) — 지표 목록에 수익성 수준 지표 0건. **총괄: 3곳 중 2곳(SWS·Stock Analysis)이 수익성 수준 축을 갖고 있다** — SWS는 혼합축(수준·변화가 한 축 안에 섞임), Stock Analysis는 수준 전용으로 명확히 분리, AAII는 0/3.
>
> **축 A 원전 층**: GP/A=수준(④감사의 Novy-Marx 원문 인용 재사용, 중복 대조 안 함). 실무 지표는 GP/A가 아니라 ROE·ROA — 명령서의 예상과 일치, Novy-Marx 원문 자체의 "GP/A≠ROA" 캐비어트가 여기서 정확히 걸린다.
>
> **판정 재료(다섯 칸)**: 선택지 셋(질문 일곱 번째 신설 / 재무건전성 질문에 수준 축 보태기 / 지금 그대로) — 신설 선택지의 결과에는 *"표시본 자신이 '여섯은 되돌린 것'이라 적어, 늘리는 방향이 하루 전에 한 번 되돌려졌다"*는 사실을 직접 반영했다. 추천·순위·선택 없음.
>
> **검증 3**(원문 GitHub·실측 Stock Analysis 라이브·우리 코드 `fscore.ts`) + **검수 3**(SWS Past Performance가 재무건전성 대응인지 성장 대응인지 반론 가능·`accrual` 수준 재분류는 이 STEP의 해석이지 원문 직접 진술 아님·Stock Analysis 관측은 AAPL 1종목 1회 한정) 명시 — AAII만 재사용이지 독립 재확인이 아님을 밝혔다. 웹 조사 실측 비용 = `gh api` 1회 + WebFetch 1회, 실패 0건, 5분 이내(추정 아님).
>
> 산출물 = `docs/LENS_DISPOSITION_2026-08-08.md`에 §9(신규 절, §8 바로 다음) 추가 — 새 파일 미신설, 기존 §0~§8 전부 보존. 표시본 갱신 필요 = 예(고치지는 않음). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0. 못 한 것 = AAII 설문 선택지 원문 재확인(기존 미해결 한계 승계)·Stock Analysis 패턴이 다른 종목/업종에도 동일한지(1종목만). 철회·정정 = 없음(이 STEP 자신은 정정하지 않았다 — `LENS_DISPOSITION §2`의 "4개" 주장과 `MODEL_ROSTER`의 "비공개" 주장이 이 STEP의 원문 대조 결과와 다름을 발견했으나 둘 다 정정 재료로만 남기고 수정하지 않았다). 미측정 = 그 정정 재료를 누가 언제 반영할지(판정 대상). 🔴 **판정은 장은태가 한다. 이 STEP은 세는 것까지다.**

## 2026-08-18 — 🔵 **STEP1066: 7렌즈 재사용 판정표(`CLAUDE.md` 규칙 3의 종착)**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0·새 조사 0). 감사 일곱(①~⑦)이 무엇을 위한 것이었는지의 종착점 — `CLAUDE.md` 규칙 3("① 확정된 질문에 실제로 답하는지 확인하고 ② 결함을 실측으로 뜯은 뒤에만 재사용한다")에서 ②는 감사 일곱으로 끝났고 ①은 STEP1064 §7-5가 냈으나 "그래서 쓸 수 있나"를 아직 안 냈다. 인용 기준 = 원격 `main` = `88dd29b`(STEP1065 + 일일 US 유니버스 갱신까지) — 이 표는 그날(2026-08-17~18 감사 스냅샷)의 상태임을 명시.
>
> **🔴 성립 조건 확인(최우선 게이트)**: `LENS_DISPOSITION_2026-08-08.md` §4의 "네 안"(🅰️Q5축신설·🅱️별도섹션·🅲폐기·🅳최소정리) 전문을 재확인한 결과, 넷 다 "이미 재사용하는 것으로 놓고 어디에 배치하나"를 물을 뿐 "재사용해도 되는가"를 묻지 않는다 — **`CLAUDE.md` 규칙 3과 §4는 다른 물음**이다. `BUILD_SEQUENCE.md` §6-E("7렌즈 거취, A·B 종속")는 §4의 물음이지 이 물음이 아니므로, 이 STEP은 아직 열려 있는 판정 A·B와 무관하게 지금 진행할 수 있다고 확정하고 계속했다.
>
> **§8-1 렌즈별 규칙3 ①②완결 상태**: ②(결함 실측)는 7렌즈 전부 완료. ①(질문 배정)은 넷(밸류=Q1·모멘텀=최근변화·기술=최근변화·F스코어=재무건전성) 확인, 퀄리티는 "성장" 질문 후보로 검토됐다가 SWS 원문 대조로 기각된 이력(STEP1056), 저변동·자산성장은 미분류.
>
> **§8-2 결함 16건을 H-7 판별 셋(공개·검산·기만)으로 재분류**하고 셋 중 하나로 나눴다 — 사용 차단 / 고치면 됨 / 표기로 해소 가능. 🔴 **16건 중 "사용 차단"급으로 분류된 것은 0건**이었다(표기로 해소 가능 8건·고치면 됨 6건·해당 없음[인프라 사고, 자연 해소] 3건) — 이는 규칙 3 자신이 이미 전제한 것("결함이 있다고 재사용을 막지 않는다, 밸류 6건·모멘텀 5건은 그 검증에서 나왔다")과 정합한다. 역방향(결함→렌즈)도 확인 — 퀄리티·F스코어는 신규 결함 0건이라 이 표에서도 0건으로 정합.
>
> **🔴 §8-3 렌즈별 재사용 판정표(7행, 이 STEP의 산출물 본체)**: 각 렌즈에 ①질문 배정 ②결함(차단/고치면 됨) ③관문②③(STEP1064 §7-2를 인용, 재채점하지 않음) ④**"쓰려면 무엇이 필요한가"**(가부 판정이 아니라 필요 조건) ⑤좌표를 채웠다. 퀄리티·F스코어·자산성장은 "없음(당장)"으로 나왔으나, 이것이 "완벽하다"는 뜻은 아니고 — 검산 인프라 공백(④퀄리티 grossProfit·⑤기술 rsi14/pos52w·⑥F스코어 reason·⑦자산성장 2년치 총자산, 4연속 확인된 패턴)은 개별 렌즈 칸이 아니라 이미 `LENS_DISPOSITION §7-4` 판정대기#7(검산 인프라 영구화 여부)에 등재돼 있음을 명시하고 중복 기재하지 않았다.
>
> **§8-4 미분류 둘(저변동·자산성장)과 퀄리티는 "못 쓴다"로 적지 않았다** — "어느 질문에도 안 붙음, 새 질문 후보 여부는 별건"(W-2-1)으로 적고 새 질문을 만들지 않았다. 판정 A·B는 무접촉(장은태가 2026-08-18 B를 "아직 정하지마"로 보류 지시한 것을 그대로 반영).
>
> 산출물 = `docs/LENS_DISPOSITION_2026-08-08.md`에 §8(신규 절, §7 종합 바로 다음) 추가 — 새 파일 미신설, 기존 §0~§7 전부 보존. 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0·새 조사 0. 못 한 것 = 결함 16건 개별의 실사용 영향(측정 수단 없음, §7-1과 동일 한계)·§8-3 "쓰려면 무엇이 필요한가" 항목들의 실제 작업 공수 추정(별건). 철회·정정 = 없음(§7-1·§7-2·§7-5 인용 전부 재확인 결과 정확, 새로 드러난 모순 없음). 미측정 = 퀄리티/F스코어/자산성장의 "없음(당장)" 판정이 검산 인프라 판정(§7-4#7)의 향방에 따라 바뀔 수 있는지(그 판정 자체가 장은태 몫). 🔴 **판정은 장은태가 한다. 이 STEP은 「쓸 수 있는가」에 답할 재료를 놓는 것까지다.**

## 2026-08-18 — 🔵 **STEP1065: 판정 A·B 생존 확인 + `BUILD_SEQUENCE` 정합**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0). 자기진단에서 출발한 STEP — 최근 10개 STEP 중 7개(1057·1059~1064)가 판정 E(7렌즈 거취) 아래의 일이었는데, `docs/BUILD_SEQUENCE.md` §6은 E가 **A·B(제품 정의 우선순위·새 모델 vs 1위를 제대로)에 종속**된다고 명시한다 — E의 답을 모른 채 E의 대상을 일곱 번 감사했다. 인용 기준 = 원격 `main` = `3eb786b`(STEP1064까지).
>
> **2-1 정본↔표시본**: 저장소 안에 `docs/build_sequence.html`(검증·정정이 반영된 최신본, `BUILD_SEQUENCE.md`와 같은 커밋)이 이미 있다. 저장소 밖 `~/Downloads/build_sequence.html`은 텍스트 diff로 **그 직전의 미정정 버전**임을 확인했다(SPEC "2,117줄"·날짜 접미사 없는 파일명·"소표본 오판 6회" 등 이미 정정된 오류가 그대로 남아 있음). 자기지시적 정본 주장(STEP1056이 로드맵 표시본에서 찾은 유형)은 둘 다 없음. **표시본 갱신 필요 — 저장소 카피를 Downloads에 덮어쓰면 해소된다**(이 STEP은 저장소 밖 파일을 고치지 않음).
>
> **🔴 판정 A(제품 정의 우선순위) — 살아 있음**: 역방향 확인(*"해소됐다면 어디에 반영됐어야 하는가"*) 결과 — `ROADMAP_V2.md`의 WHY 조건3(*"보는 방법을 준다"*)이 07-03(`LENS_ROADMAP.md`, 기법 나열)을 흡수했다는 명시 좌표는 없음. W-2-3의 Layer C(방법론 공개)가 08-07(`MODEL_BUILD_ORDER_2026-08-07.md`, "정직함")의 핵심을 거의 그대로 재현하지만, 이는 **"층으로 배치됨"이지 "우선순위가 판정됨"이 아니다** — 이 구분을 흐리지 않는 것이 이 STEP의 핵심 규율이었다. `STATE.md:219`(2026-08-16, 최신 현황 정본)가 **지금도 판정 A를 미판정 칸으로 나열**하고 있다는 사실 하나가 결정적이다 — 해소됐다면 반영됐어야 하는데 안 됐다.
>
> **🔴 판정 B(새 모델 vs 1위를 제대로) — 살아 있음**: `MODEL_BUILD_ORDER_2026-08-07.md` §3-2가 *"이 판단은 장은태 몫이다"*라 적어뒀고, `ROADMAP_V2.md` F-4-3이 *"사실로 확인됐다"*고 한 것은 **"확인"(실측)이지 "판정"(승인)이 아니다** — F-4-3 헤더 자신이 *"최종 승인 = 장은태"*라 명시한다. `STATE.md`도 B를 "2026-08-07 이후 미판정, 가장 먼저"로 유지.
>
> **🔴 종속표(가장 중요한 실용적 결과)**: C~F(`BUILD_SEQUENCE.md` §6) + WHAT의 "남은 것 셋"(F-스코어 32.3%·저변동/자산성장 처분·신규 섹터 공백) + STEP1064의 판정 대기 8건을 전수로 "A·B 어느 답에서도 결과가 같은가"로 갈랐다. **독립(A·B 없이 지금 가도 되는 것) = 6건** — `ROADMAP.md` 재작성 **여부**(US 단독 전환은 이미 확정된 별개 사실)·F-스코어 32.3% 서술 재확인·Q0 신규 섹터 공백(이미 마감된 별도 트랙)·집계 레이어 등급 반영 원칙(어느 제품 정의에서도 유효한 정직성 원칙)·`lens_scores` 스테일 행 정리 정책·검산 인프라 영구화(둘 다 렌즈 구성과 무관한 파이프라인 문제). **종속 = 8건**(대부분 E "7렌즈 거취"를 경유해 A·B에 걸림 — 역DCF 위치·`ROADMAP.md` 재작성 내용·7렌즈 거취 자체·Q1 착수 4건·저변동/자산성장 처분·밸류/모멘텀 결함 처리·배당조정 전환·`pos52` 가드 추가). "독립이 하나도 없다"는 아니다 — 지금 진행 가능한 항목이 실제로 있다.
>
> **관문 7개 폐기의 파급**: `git log` 커밋 시각으로 선후를 확정 — 전부 **2026-08-15 하루 안**에서 13:03(`BUILD_SEQUENCE.md` 신설, 관문7개 기반) → 18:25(F-2, 관문7개 폐기) → 22:41(F-4-3, 관문3+재료% 기준 새 순위 — 역DCF 12위→3위) → 익일 00:37(W-2-4, Altman Z-Score 채택)의 순서다. `BUILD_SEQUENCE.md`는 신설 이후 **커밋이 한 번도 없어** 이 파급을 전혀 반영하지 못했다 — §3-A/B/C·§5는 낡음, §1·2·4·6·7·8은 관문과 무관해 유효. 충돌 실물 둘(Altman·역DCF)을 원문 인용으로 나란히 놓았을 뿐, 어느 쪽이 맞는지는 판정하지 않는다(그 자체도 A·B 미판정 위에 서 있어서).
>
> 산출물 = `docs/BUILD_SEQUENCE.md` §6-부록(신규, 취소선 없이 추가·기존 §0~§8 보존) + `docs/ROADMAP_V2.md`의 "만드는 절차 ⬜" 층에 정본 포인터 한 줄(절차 내용은 채우지 않음 — 그건 판정). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0. 못 한 것 = §5(2026-08-15 스냅샷)의 2026-08-18 기준 재확인(새 조사 금지)·`ROADMAP.md` 재작성 "내용"의 판정 재료(A 선행 필요). 미측정 = `ROADMAP.md` 재작성 구체 내용·판정 A·B가 장은태에게 어떻게 제시돼야 하는지(제품 영역, 이 STEP 범위 밖). 🔴 **판정 A·B는 장은태가 한다. 이 STEP은 그 둘이 살아 있는지 확정하는 것까지다.**

## 2026-08-18 — 🔵 **STEP1064: 7렌즈 전수감사 종합 + 판정 재료 한 장**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0). 새 조사가 아니다 — ①~⑦ 7개 감사 문서 전문을 다시 읽고, 이미 확보된 재료를 판정 가능한 형태로 정리하는 STEP. 인용 기준 = 원격 `main` = `a0979a4`(STEP1063까지).
>
> **결함 개수 정정 — 18 → 16, 근거 둘**: ① `LENS_AUDIT_03_LOWVOL.md`가 스스로 *"`volState()` 데드코드 … 혼동 유발, **결함은 아님**"*이라고 판정했는데, 이후 `LENS_AUDIT_04_QUALITY.md`의 "15건" 집계(①6+②5+③4+⑤3)가 이 항목을 그대로 결함으로 포함시켰다 — ③의 진짜 결함은 3건이지 4건이 아니다. ② ⑤기술·⑦자산성장이 각각 *"③저변동과 같은 뿌리"*·*"⑤ 결함①과 같은 뿌리"*라고 스스로 밝혔는데, 이후 문서들의 "18건" 산식은 단순 렌즈별 합산이라 이 묶음을 반영하지 못했다. 최종 16건 표(결함·좌표·출처·걸리는 렌즈·현 상태·화면 영향)를 작성하고, 공통 패턴 셋(㉠집계 레이어가 등급을 안 봄 ㉡가격 계열 배당 조정이 렌즈마다 다르게 처리됨 ㉢검산 인프라 공백이 4연속 확인됨)을 개별 카운트와 분리해 기록했다.
>
> **화면 영향 A/B/C 판정**: A(라이브에 실제로 나감) 13건 전부 좌표 확인(예: 밸류 6건 전부 카드에 렌더, `lensTones.ts`의 등급 무시 집계는 관심목록·보드 대표배지·KRX랭킹·해외리스트 4개 프로덕션 API 전부 해당). B(당시엔 있었으나 지금은 복구·과거 인스턴스, 구조적 위험만 잠재) 3건 — 모멘텀의 2026-08-07 US 컷 정지, 저변동의 `lens_scores` 스테일 극단치(QH), 08-16 US 취득 실패(STEP1058이 복구 확인). "보일 것이다"로 A에 넣은 항목은 0건 — 좌표 없으면 C로 뺐다.
>
> **관문 셋(F-1 ①②③) 재채점**: 관문①(원하는 정보인가)은 자산성장만 미배정, 나머지 6개는 통과. 관문②(정확한가·검산 가능한가)는 저변동·퀄리티만 🟢고 나머지 다섯은 🟡/🔴 — **㉢(검산 인프라 공백)이 관문②를 갉아먹는 공통 원인**임을 확인. 관문③(정직한가)은 자산성장만 🟢.
>
> **C-1 9항목 격자 — 구표와의 충돌 확인**: 정본(`docs/LENS_COMPLETION_STANDARD.md:28~38`)에 이미 "완성 현황표(렌즈7×항목9)"가 존재하며 **7렌즈 전부 9항목 ✅**로 기록돼 있다(STEP812~818 근거). 이 STEP은 그 표를 고치지 않고(고치는 것은 판정이므로) 이번 감사 시리즈(STEP1057~1063)가 같은 9항목을 다시 봤을 때 확인한 것을 나란히 놓았다. 🔴 **항목3(값 검증: 손계산+외부 3종목 대조) 세로줄에서 격차가 확인됐다** — 저변동만 🟢, 퀄리티는 🟡, **기술·F스코어·자산성장 셋은 "구조적으로 불가능"**(3/7 렌즈)이다. 구표가 당시 무엇을 근거로 항목3을 통과시켰는지는 재조사하지 않았다(새 조사 금지) — `LENS_DEV_PLAYBOOK.md` #48(STEP818)의 *"손계산: computeFScore=API 정확 일치"* 기록만 병기해 "그 시점엔 실제로 손계산이 가능했다"는 사실만 남기고, 그 이유(당시와 지금이 왜 다른지)는 미조사로 남겼다.
>
> **판정 대기 재수집 — "11건"(명령서 추정) → "이 감사 시리즈 고유 8건 + 기존 F-5 참조 2건"**: `ROADMAP_V2.md` F-5(STEP1046, 정본 `probe_1046_material_and_decisions.md`)가 이미 Q1/역DCF 트랙까지 포괄하는 18건 open의 판정 대기 목록을 갖고 있음을 확인 — 그중 ⑧(Piotroski 적용 범위)·⑪(저변동·자산성장 처분) 2건이 이 감사 시리즈와 겹쳐 참조만 하고 중복 등재하지 않았다. 이 감사 시리즈 고유 항목 8건(밸류 6개 결함 처리·모멘텀 4개 결함 처리·집계 레이어 등급 반영 여부·배당조정 전환 여부·`pos52` 가드 추가 여부·`lens_scores` 스테일 행 정리 정책·검산 인프라 영구화 여부·F-score 32.3% 서술 반영 방식)을 각각 다섯 칸(무엇을 정해야 하나/선택지/각 선택의 결과/확보된 재료/재료가 더 필요한가)으로 작성 — 추천·순위는 없음, 재료 부족은 명시(④·⑥번 항목).
>
> **렌즈↔질문 귀속 종합**: 밸류=Q1, 모멘텀·기술=최근변화(이미 배정 확인), F스코어=재무건전성(이미 배정 확인) — 비는 칸(미분류) = **저변동·자산성장 둘**. 억지로 맞추지 않았다(W-2-1) — 퀄리티는 "Q3 후보였다가 STEP1056 원문 대조로 기각"된 이력을 그대로 옮겼다.
>
> 산출물 = `docs/LENS_DISPOSITION_2026-08-08.md`에 §7(신규 절) 추가 — 1순위 기존 문서 확장(그 문서가 정확히 "렌즈를 어떻게 처분할 것인가" 문서이고, ③ 감사가 이미 그 문서의 §3 오류를 정정한 전례가 있어서), 새 파일 미신설, 기존 §0~§6은 그대로 보존. 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0. 못 한 것 = C-1 항목2·7·8·9 전 렌즈(7개 감사가 체계적으로 다루지 않아 미확인)·구표 항목3 통과 당시 근거(새 조사 금지)·결함#9·#15의 정확한 영향 크기. 철회·정정 = 결함 개수 18→16, 판정 대기 "11건"→"고유 8건+F-5 참조 2건". 미측정 = 결함 16건의 실사용 영향(측정 수단 없음)·Cooper-Gulen-Schill(2008) 원문(재시도 안 함)·구표 항목3 근거 원문. 🔴 **판정은 장은태가 한다. 이 STEP은 판정을 가능하게 만드는 것까지다.**
