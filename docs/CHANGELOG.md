...
## 2026-07-10 — STEP 673~692 + 🔴 브랜드 대개편 — 정체성 3기둥(무기·직시·자립)+멍거 목소리 + 보드 정렬 개선 + 리딩방 채널 연결 + 광고 수익화 기반 + 브랜드 문구 확정 + 탭 14→3 재구조 + 미리보기 광고 제거·태그라인 교체 + "TR-AI 렌즈" 명칭 통일 + 증권사 소개글·너비 + 위치 이름옆 + 증권사 정보하위·인리스트 광고(KR) + ETF "상품 구성" 상세(US·KR) + 미리보기 ETF 구성 요약 + ETF 영숫자코드·라벨정정 + ETN 상품정보 + ETF 너비·뒤로가기·검증탭 행클릭
