...
검증3(원문26개대조·STEP1056/1069방법재사용·사라진문단15/21쌍+고유169+제목다른둘=분기확정)·검수3(원시/잔여 둘다기록·삼자대조·삭제는 게이트8 통과 후로 순서 준수) 본문 기록. 못 거친 축=`_probe_B_flows.ts`의 실제 API 응답(외부 취득 금지로 범위 밖). 철회·정정=없음(반영은 누락 보충이지 정정 아님). 산출물 = `docs/BUILD_SEQUENCE.md` §7-부록 부-10 신규(기존 §0~부-9 보존) + `docs/model_list_30_9.html` 2건 갱신 + `docs/probe_1071_to_delete_diff.json`(원자료). 삭제(§2-8)·STATE.md 제자리고정(§2-9)은 게이트8(CI+Vercel) 통과 확인 후 별도 커밋으로 진행(순서 준수). tsc·vitest는 이 커밋 기준 확인. 🔴 **판정은 장은태가 한다. 이 STEP은 살릴 것을 넣고 중복을 지우는 것까지다.**
