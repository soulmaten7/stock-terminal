...
## 2026-08-18 — 🔵 **STEP1063: 렌즈 전수감사 ⑦ 자산성장(CMA) — 7렌즈 마지막**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·재무 재취득 0 — 외부 신규 취득은 원문 논문 확보 목적만). **①밸류·②모멘텀·③저변동·④퀄리티·⑤기술·⑥F스코어에 이어 이것으로 7렌즈 전수감사가 완결된다**(7렌즈 종합·정리는 이 STEP의 몫이 아니다).
>
> **⓪-4① 최우선 — 집계 레이어 확정 결함, ⑤ 결함①과 같은 뿌리로 묶어 기록**: `lib/lenses.ts:367~368`의 렌즈 상단 주석은 *"라벨=확장 강도(공격적/보통/보수적 · **verdict 아님**)"*라고 명시하는데, 같은 파일 396행의 실제 `verdict.tone`은 `conservative→"pos"`·`aggressive→"warn"`이다 — **주석이 "verdict 아님"이라 단언한 필드가 실제로는 pos/warn 톤을 가진 verdict 객체.** `lib/lensTones.ts`의 `STATE_SPEC.assetgrowth = ["conservative","aggressive","mid"]`가 렌즈 등급(`partial`=표본 약함·t≈1.6 유의미달)을 전혀 보지 않고 검증된 렌즈와 동일 가중치로 집계한다 — ⑤ 감사가 찾은 4개 소비처(관심목록·보드 대표배지 `firstPosLens`·KRX 랭킹·해외 리스트) 전부 해당. `lensCopy.ts`의 note는 매우 상세하지만 "이 판정이 집계 화면에서 검증 렌즈와 동일 가중으로 셈된다"는 사실은 어디에도 없다. **개별 결함 2건이 아니라, 「`lensTones.ts`의 집계 레이어가 렌즈 등급을 반영하지 않는다」는 하나의 구조적 결함이 이제 두 렌즈(기술·자산성장)에서 확인된 것으로 기록한다.**
>
> **⓪-4② 연도 정렬 가드**: `nonConsecutive()`(STEP803)가 코드에 실재함을 확인(연도 차 ≠1이면 `assetGrowthPct`가 무조건 null). 🔴 그러나 **역방향 검사(연도 차 ≠1인데 값이 계산된 종목이 있는가)는 구조적으로 수행 불가** — 야후가 반환한 연도 목록이 DB 어디에도 저장되지 않는다(`lens_scores`엔 `assetgrowth_value`·`assetgrowth_state`뿐). 98.7%(986/999) 커버리지가 "가드를 통과해서" 나온 것인지 "가드가 걸릴 일이 애초에 드물어서"인지는 이 STEP만으로 최종 확정하지 못한다 — STEP1060의 175종목 야후 실조회에서 연도 목록 내부 결측이 0건이었다는 간접 방증만 있음(자산성장 전용 검증은 아님, 미측정으로 남김).
>
> **⓪-4③ 원전 두 층 — 이미 화면에 공개돼 있음(STEP553·816·826이 이미 완료, 중복 대조하지 않음)**: 현상의 원전 = Cooper, Gulen & Schill(2008, JF), 계산 사양의 출처 = Fama-French(2015) 5팩터 CMA — `lensCopy.ts:76`의 note가 *"Cooper-Gulen-Schill 2008 / 파마-프렌치 5팩터 CMA · **같은 계열**"*로 관계까지 이미 명시한다(④퀄리티·⑤기술이 지적한 원전 인용 누락과 정반대 상황). "포트폴리오 기법(소형·초소형·동일가중) vs 우리의 종목별 백분위(상위 1,000·시총가중)" 차이도 STEP826이 이미 note·scope에 상세히 공개해뒀다. 🔴 이 STEP은 Cooper-Gulen-Schill(2008) 원문을 ④⑤⑥과 같은 방식(무료 공개본)으로 물리적 재확보하려 시도했으나 — 저자(Cooper, 유타대) 홈페이지·SSRN(두 워킹페이퍼 버전, 403 차단)·구글 다각도 검색을 거쳤음에도 무료 전문 PDF를 찾지 못해 **「원문 미확보」로 종결**한다(유료 구매·robots 우회 금지 규정 준수 — STEP816 당시의 원문 대조는 2026-08-01 「원전 인벤토리」 관행 신설 이전이라 파일이 보존돼 있지 않았다).
>
> **⓪-4④ 손계산 — 네 번째 「검산 인프라 공백」 사례 확정**(④퀄리티의 grossProfit/costOfRevenue → ⑤기술의 rsi14/pos52w → ⑥F스코어의 reason 및 3년치 SEC 데이터 부재에 이어): `us_fundamentals_snapshot`은 `total_assets` 컬럼 자체가 없고(15개 컬럼 재확인), `us_fundamentals`는 종목당 정확히 1개년뿐(⑥의 확인 재사용) — **2개년치 총자산이 우리 SEC 캐시 어디에도 없어 손계산이 구조적으로 불가능**하다.
>
> **DB 전수**(`lens_scores` US 999행, `updated_at` 균일 `2026-08-17 22:03:40 UTC`, `lens_cuts.as_of`=`2026-08-17`): `assetgrowth_value` 비결측 986(98.7%)·na 13(1.3%). 분포 min −55.56%~max **120,677.99%**(BMNR — 자산 대규모 재편 기업, IONQ 1,192%·ASTS 425% 등도 2026년 실제 고성장 기업들 — 자릿수 이상으로 보고 오염 단정하지 않음). state mid 395·aggressive 296·conservative 295·na 13. `lens_cuts`(lo=2.52·hi=12.725·n=986)가 분포 p10/p50/p90과 정합(분포 유도 확인). `lens_state_changes`(07-20~08-07) 재조회 **52건/38종목 — `LENS_DISPOSITION` §1 정확히 재현**(7렌즈 중 노출 최소라는 배경 서술과 정합). 중간 판정 근거(reason)가 DB에 저장되지 않는 것도 위 네 번째 패턴과 동일한 사실.
>
> **🟢 잘 된 것**: 원전 두 층·컷 방향 근거(`dir:"low"`)·na 사유 3갈래 분기(`naMissing`/`naOneYear`/`naGap`, STEP826)·규모 의존성 캐비어트·Hou-Xue-Zhang(2020) 재현연구 인용까지 전부 이미 화면에 공개돼 있다 — ④⑤가 다른 렌즈에서 지적했던 결함 유형들을 이 렌즈는 애초에 갖고 있지 않았다.
>
> **앞선 결함 18건**: ⑥의 결과를 인용하고 2026-08-18 이후 관련 파일(8개) `git log` 커밋 0건만 재확인 — **18/18 그대로.** **질문 귀속**: `lensCopy.ts:72`의 "몸집을 무리하게 불리지 않았나?"는 W-2 정본 "성장" 질문과 인접하지만 다르다(성장="커지는가" 사실 질문, 자산성장="너무 무리하게 커지는가" 과잉투자 경계 질문) — 성장은 이미 매출 5년 CAGR로 채워져 있고, `ROADMAP_V2.md` W-2-4가 자산성장을 "구조 잔여 3건 중 하나·처분 미정"으로 이미 분류해뒀다. **새 질문 후보 또는 미배정 상태로 재료만 남기고 결론 내지 않는다**(⑤·⑥과 달리 이미 배정된 칸이 없는 상황).
>
> **7렌즈 전수 완결 확인**: `lib/lenses.ts`의 `Lens` 타입 `export const` 6개(momentum·technical·valuation·lowVol·quality·assetGrowth) + `fscore.ts`의 별도 모듈 1개 = 7개 전부가 감사 문서(①~⑦)와 매칭됨을 확인.
>
> 산출물 = `docs/LENS_AUDIT_07_ASSETGROWTH.md`(신규, 7렌즈 감사 시리즈 완결). 철회·정정 = 없음(직전 감사 ①~⑥ 및 `LENS_DEV_PLAYBOOK` #25·#46·#56 인용 전부 코드 재확인 결과 정확). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·재무 재취득 0. 못 한 것 = 손계산(원자료 범위 밖)·역방향 검사(구조적 불가)·CGS(2008) 원문 재확보(원문 미확보). 미측정 = 98.7%가 가드 통과인지 우회인지의 정확한 구분·극단치 개별 원인·`firstPosLens` 라이브 관측 비율. 🔴 **판정은 장은태가 한다. 7렌즈 전수감사 시리즈(①~⑦)는 여기서 완결되며, 7렌즈 종합은 다음 STEP의 몫이다.**

## 2026-08-18 — 🔵 **STEP1062: 렌즈 전수감사 ⑥ F-스코어(Piotroski)**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·재무 재취득 0 — 외부 신규 취득은 원문 논문 확보 목적만 예외). 새 기준("주장과 근거의 간극이 가장 큰 것부터")대로 F-스코어를 자산성장보다 먼저 감사 — 「검증」 등급인데 원전 표본은 고B/M 가치주 5분위이고 우리는 전 종목에 적용한다는 간극, 그리고 W-2-4(재무건전성) 판정 대기 하나("나" — Piotroski 32.3% 수용 여부)의 재료를 이 STEP이 직접 만든다는 이유.
>
> **⓪-4① 최우선 — 71.9%(라이브) vs 32.3%(`probe_1054`) 갈림 해소**: 1단계로 최악의 시나리오(부분 합산)부터 코드로 배제 — `lib/fscore.ts:103~150`이 3중 게이트(`rows.length<3`→`needThree`, `!ok(T)‖!ok(P)‖PP.totalAssets≤0`→`dataMissing`, 연도 불연속→`gap`)를 전부 통과해야만 9개 신호를 전부 계산하는 구조라 **"몇 개만 계산해서 합산"하는 경로 자체가 코드에 없다 — 확정 배제**. 2단계에서 결정적 사실 발견: 우리가 캐시해 둔 SEC 원자료 두 테이블(`us_fundamentals`=5,820행=5,820종목, 즉 **종목당 정확히 1개년**뿐 / `us_fundamentals_snapshot`=5,755행·1,247종목, 종목별 보유 연도 분포 0개년 216·1개년 431·2개년 600·**3개년 이상 0종목**) 둘 다 Piotroski의 "3개 회계연도" 요건을 **구조적으로 단 한 종목도 충족하지 못한다.** `probe_1054`의 32.3%는 이 두 캐시 테이블 경로에서 나온 숫자가 아닐 가능성이 매우 높다(정확히 무엇을 원자료로 썼는지는 이 STEP 범위 밖). 반면 라이브 F-스코어는 야후 `fundamentalsTimeSeries`를 쓰는데(STEP1059 §2 확정), STEP1060이 175종목 실측으로 이미 확인했듯 **야후가 반환하는 연도 목록은 175/175 전부 3~5개**(중앙값 5) — 야후 한 번의 호출이 우리가 캐시해 둔 SEC 데이터보다 구조적으로 깊은 이력을 준다. **갈림의 원인은 부분 합산(㉢)이 아니라 데이터 출처(㉡)이며, 유니버스 크기(㉠)의 순수 기여도는 ㉡ 효과가 압도적이라 분리 측정이 안 된다.**
>
> 🔑 **"나" 판정 재료로 명시**: *"32.3%는 SEC 원자료를 3년 요건으로 걸렀을 때의 숫자이고, 라이브 71.9%는 야후 한 번의 호출로 얻는 이력 깊이 덕에 나온 별개 파이프라인의 숫자다 — 현재 라이브 F-스코어 렌즈는 SEC 파이프라인을 애초에 쓰지 않으므로, 32.3%가 지금 라이브 커버리지의 근거로 인용되면 안 된다."* 32.3% 자체를 틀렸다고 취소하지는 않는다(그 숫자가 정확히 무엇을 쟀는지 원문 재확인이 이 STEP 범위 밖이라서) — 판정은 장은태 몫으로 남긴다.
>
> **⓪-4② 원전 대조**: `LENS_DEV_PLAYBOOK.md` #48(STEP818)이 이미 Piotroski(2000) 원문 Table1+§2.3 전문 대조를 완료해뒀다 — 중복 대조하지 않고, 이 STEP은 원문(JAR 38 Supplement, Ivey Business School 무료 사본 `data/sources/academic/piotroski_2000_value_investing_fscore.pdf` 신규 확보)의 §2.3.1~2.3.3을 **직접 재확보해 독립 재확인**했다. 818의 결론이 전부 재현됐다: ROA·CFO는 *"scaled by beginning-of-year total assets"*(우리=기초, 일치) · ΔLEVER는 *"to average total assets"*(원전=평균, 우리=기말 — 818이 이미 지목한 편차, probe_1054가 지목한 병목 신호) · ΔLIQUID는 *"at fiscal year-end"*(일치) · **ΔTURN은 본문(§2.3.3) 자체가 "beginning-of-the-year"를 명시**(818이 지목한 "Table1과의 원전 자기모순"은 표 vs 본문 사이에 있고, 우리는 본문=기초를 따라 원전과 일치) · EQ_OFFER는 원전이 현금흐름표 신주발행 항목을 직접 쓰는데 우리는 발행주식수 변화로 근사(818이 이미 지목, 재검토 안 함).
>
> **⓪-4③ 계산 불가 사유 분기**: 코드 레벨에서 `needThree`·`dataMissing`·`gap` 셋이 서로 다른 `return`문으로 확정 존재 — STEP889·①밸류 감사가 지적했던 "뭉뚱그림" 함정을 F-스코어는 처음부터 피해 있었다. 🔴 그러나 `lens_scores`엔 `fscore_value`·`fscore_state`만 저장되고 `reason` 컬럼 자체가 없어, **결측 281건이 세 사유로 각각 몇 건씩 갈리는지 모집단 분포는 측정 불가**(④퀄리티의 grossProfit·⑤기술의 rsi14/pos52w와 같은 "파생 표시값만 저장, 중간 근거는 저장 안 됨" 패턴이 F-스코어에서 세 번째로 확인).
>
> **⓪-4④ 집계 레이어 — 해당 없음**: `lib/lensTones.ts`의 `STATE_SPEC.fscore = ["strong","weak","mid"]`를 확인했으나, F-스코어는 `LENS_STRENGTH_MAP`이 명시한 「검증」 등급이라 ⑤ 감사의 결함①(참고용 등급이 검증 등급과 동일 가중으로 집계되는 문제)과 다른 상황이다. **이중 계상하지 않고 "해당 없음"으로 기록.**
>
> **DB 전수**(`lens_scores` US 999행, `updated_at` 균일 `2026-08-17 22:03:40 UTC`): `fscore_value` 비결측 718(71.9%)·na 281(28.1%). 점수 분포 1~9점(0점 0건), 합계 718 정합. state mid 409·strong 285·na 281·weak 24. `lens_cuts`(fscore)는 **빈 배열** — 분포 유도 컷이 아니라 고정 임계값(≥7/≤3)을 쓴다는 818의 기록과 정합(결함 아님, `lensCopy.ts` scope에 이미 "자체 기준"으로 공개). `lens_state_changes`(07-20~08-07) 재조회 **112건/111종목 — `LENS_DISPOSITION` §1 정확히 재현**(독립 검증 완료).
>
> **역방향 검사(SEC엔 3년 있는데 라이브는 결측)**: 시도했으나 위에서 이미 확인했듯 3년 연속 보유 종목 자체가 SEC 캐시에 0개라 비교할 모집단이 없다 — 검사 자체가 성립하지 않는다는 사실 자체가 실측 결과.
>
> **손계산 검산 3종목 — 🔴 이 STEP 자신의 전제가 틀렸음을 발견·정정**: STEP1059의 실패(낡은 수치로 손계산)를 피하려 회계연도를 먼저 고정했으나, AAPL·MSFT·KO를 직접 확인한 결과 `us_fundamentals`는 종목당 **1개년만** 저장한다(revdcf가 최신 회계연도 하나만 쓰기 때문 — STEP1060이 이미 확인했던 사실을 이 STEP이 §2-7 작성 시 충분히 반영하지 못했다). `us_fundamentals_snapshot`도 최대 2개년뿐이고 유동자산·유동부채·영업현금흐름 컬럼 자체가 테이블에 없어 다수 신호를 낼 수 없다 — **손계산은 구조적으로 불가능**하다고 정정한다(⑤가 원시 종가 없어 못 한 것과 다른 이유로, "사후 검증 인프라 없음" 패턴이 세 번째 영역에서 재확인).
>
> **앞선 결함 18건**(①6+②5+③4+⑤3): ⑤의 결과를 인용하고, 2026-08-18 이후 관련 파일(`fscore.ts` 포함 7개) `git log` 커밋 0건만 재확인 — **18/18 그대로.** **질문 귀속**: `lensCopy.ts:98`의 "재무가 튼튼한가?"가 `ROADMAP_V2.md` W-2-4의 "재무건전성" 칸 단독 모델(Piotroski F-Score) 확정과 무모순임을 재확인(신규 도출 아님).
>
> 산출물 = `docs/LENS_AUDIT_06_FSCORE.md`(신규) · `data/sources/academic/piotroski_2000_value_investing_fscore.pdf` + `data/sources/README.md` 등재. 철회·정정 = 이 STEP 자신의 §2-7 전제("`us_fundamentals`에 3개 연도로 이미 있다")가 틀림 → 손계산 구조적 불가로 정정. 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·재무 재취득 0. 못 한 것 = 손계산(원자료 범위 밖)·`probe_1054` 32.3%의 원경로 재확인·계산불가 281건 사유별 모집단 분포·유니버스/데이터출처 기여도 분리. 미측정 = 위와 동일 + `ROADMAP_V2.md` W-2-4의 32.3% 서술(판정 재료로만 남기고 원경로 재확인 전에는 취소선 처리하지 않음 — STEP1060의 성급한 정정 교훈 반영). 🔴 **판정은 장은태가 한다. 이 STEP은 결함을 놓는 것까지다.**

## 2026-08-18 — 🔵 **STEP1061: 렌즈 전수감사 ⑤ 기술(RSI·MA·52주)**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0 — STEP1060의 야후 조회 예외는 승계되지 않음). `strong` 등급 넷(①밸류·②모멘텀·③저변동·④퀄리티)이 소진된 뒤 새 기준("주장과 근거의 간극이 가장 큰 것부터")으로 기술을 먼저 감사 — 우리 백테스트가 RSI를 기각했는데(`LENS_STRENGTH_MAP:12`) 노출은 `lens_state_changes` US 2위(587건·26%)였다.
>
> **⓪-4① 주장 정합 3개 코드 대조**: ㉠ "RSI·52주 위치는 판정에 안 써요" — **지킴**(`tState`는 `last`/`ma200`만 쓰고, verdict·spectrum·outlook·value·state 전부 그 소스 하나에서 파생 — RSI·52주 위치는 `detail`(표시)에만 존재). ㉢ "30/70은 와일더의 경험칙" — **지킴**(`messages/ko.json:769` `narrativeCutoffTechnical`이 실제 렌더되고 그 문장 자체에 "판정은 200일선 기준·RSI는 참고 수치"라는 캐비어트까지 포함, en도 패리티). ㉡ "매매 신호가 아니라 '지금 상태' 표시" ↔ `verdict.tone`(pos/warn) — **개별 카드 텍스트는 지킴**(`lensCopy.ts`의 up/down 문구가 "상승 추세"/"지금 가격이 장기 평균선 위에 있어요"처럼 완전히 서술적·중립적이라 밸류 렌즈가 STEP805~806에서 받았던 "절대 임계값+절대 방향 문구" 문제 자체가 없음) — 🔴 **그러나 `lib/lensTones.ts`의 `STATE_SPEC`이 기술을 검증된 6렌즈와 동일한 가중치로 취급한다**는 새 결함을 발견: `toneForKey`·`tonesFromStates`·`tonesFor`·`firstPosLens`는 렌즈 등급(`strong`/`ref`)을 전혀 보지 않아, 관심목록 "강점 N" 집계와 **보드 상위 종목 자동 미리보기의 "대표 렌즈" 배지**(`app/api/explore/lens-top/route.ts`의 `firstPosLens`, STEP744/745)에서 `reference` 등급(검증 안 됨)인 기술의 "up" 신호가 검증된 팩터와 구분 없이 "강점"으로 셈된다. 카드 층위(C-1 #7)는 무모순이나 **집계 층위에서는 문구가 약속한 "참고용" 지위가 사라진다.**
>
> **⓪-4② 배당 조정 — 확정 결함(공통 결함)**: `lib/lenses.ts:196` `const closes = d.closes`(raw) — RSI·SMA200·52주 고저 셋 다 배당 미조정 종가를 쓴다(momentum만 `d.adjCloses ?? d.closes`). `lensCopy.ts` technical 블록에 배당 조정 언급 0건 — **③저변동 신규①과 뿌리가 같은 결함이 세 번째 렌즈(기술)에서 반복**된다. 개별 결함으로 세지 않고 공통 결함 하나로 기록.
>
> **⓪-4③ 최소 표본 가드 비대칭 — 확정 결함**: `sma(closes,n)`·`rsi(closes,period)`는 둘 다 `closes.length < 최소길이 → null` 가드가 있어(결함 아님) 결측을 정직하게 na로 표시한다. **그러나 `pos52`(`lib/lenses.ts:201~204`)는 `win.length>0`만 요구할 뿐 최소 표본 가드가 사실상 없다** — 상장 며칠 안 된 종목도 종가 2일치만 있으면 "52주 위치"라는 이름의 숫자가 그대로 나간다. 정확한 영향 비율은 `rsi14`·`pos52w`가 DB에 저장되지 않고(`technical_value`·`technical_state`만 존재) 외부 재취득도 이 STEP엔 금지돼 **직접 측정 불가** — `sma200`이 na인 종목 수(200일 미만 데이터의 하한 프록시) **16/999(1.6%)**만 하한으로 기록.
>
> **원전 두 층(H-7)**: RSI=와일더(1978, 계산+현상 둘 다 이미 공개) · 200일선=Moskowitz-Grinblatt(2012)·Faber(2007) 사촌 + Brock 외(1992)→Sullivan-Timmermann-White(1999) 반박(이미 공개) · **52주 위치=코드·카피 어디에도 원전 없음** — George & Hwang(2004, JF) 원문 무료본을 저자 자신의 교수 홈페이지(`bauer.uh.edu/tgeorge`)에서 확보(`data/sources/academic/george_hwang_2004_52week_high_momentum.pdf` 신규 저장 + `data/sources/README.md` 등재)했으나, **원전 정의(`P_{t-1}/high_{t-1}`, 52주 저가 불사용)가 우리 코드(`(last-lo)/(hi-lo)*100`, 저가 사용)와 산식 자체가 다르다** — 인용할 수 없는 논문이라 **`lensCopy.ts`에 인용이 없는 지금 상태가 오귀속보다 오히려 정확**하다는 결론. 우리 산식은 형태상 스토캐스틱 오실레이터 %K(조지 레인, 1950년대 실무 관행·구간만 14일→252일로 확대)와 같으나 그 계보도 학술 검증은 없다.
>
> **DB 전수 실측**(조회 2026-08-18 06:18~06:24 UTC, `lens_scores` US 999행 `updated_at` 균일 `2026-08-17 22:03:40 UTC`): `technical_value` 분포 min −43.53~max 107.36 · state up 695(69.6%)/down 288(28.8%)/na 16(1.6%)/**flat 0건**(부동소수 특성, 코드 분기는 존재하나 실사용 없음). `lens_state_changes` 07-20~08-07 재조회 **587건/288종목 — `LENS_DISPOSITION` §1 정확히 재현**(독립 검증 완료). 전체 창(07-20~08-17) 815건/334종목 — 회전율 28.8~33.4%. `LENS_STRENGTH_MAP`의 "회전율 66%"는 **월별 백테스트 포트폴리오 재구성 비율**로 정의가 달라 **직접 비교는 성립하지 않는다**(방향만 일치, 억지 대사는 하지 않음).
>
> **손계산 검산 — 못 함(구조적 제약)**: `us_stock_perf`가 `r1w~r1y` 집계 수익률만 갖고 있어 RSI(14)·SMA(200)·52주 고저를 역산할 방법이 없고, 이 STEP은 외부 신규 취득을 명시적으로 금지한다(STEP1060의 야후 조회 예외는 승계 안 됨) — ④퀄리티·STEP1060과 동일한 "사후 검증 인프라 없음" 패턴이 세 번째 영역(가격 계열)에서 확인됐다. STEP1059의 실패(낡은 수치로 손계산)를 반복하지 않기 위해 시도 자체를 하지 않는 것이 맞는 판단이라 기록.
>
> **앞선 결함 15건**: `LENS_AUDIT_04_QUALITY.md` §8의 "15/15 코드 무변경" 결과를 인용하고, 그 이후(2026-08-18 이후) 관련 파일(`lenses.ts`·`lensCopy.ts`·`technical.ts`·`lensTones.ts`·`lensCompute.ts`·`lowvol.ts`) `git log` 커밋 0건만 재확인 — **그대로 유지.**
>
> **질문 귀속(2-9)**: 억지 매칭 없이 `ROADMAP_V2.md:472`의 기존 기록을 그대로 인용 — *"최근변화(모멘텀+기술) = 엔진 보편·소스는 KR/US 이미 국가별 분리"*로 **이미 배정돼 있음**을 재확인(이 STEP이 새로 도출한 게 아니라 기존 로드맵 결정과 코드 질문 문구가 무모순임을 확인).
>
> 산출물 = `docs/LENS_AUDIT_05_TECHNICAL.md`(신규, 앞선 넷과 동일 형식) · `data/sources/academic/george_hwang_2004_52week_high_momentum.pdf` + `data/sources/README.md` 등재. 철회·정정 = 없음(직전 인용 전부 원문 재확인 결과 정확). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0·외부 신규 취득 0. 못 한 것 = 3종목 손계산(구조적 불가)·`pos52` 최소표본 미달 정확 비율(DB 미저장+외부취득 금지)·회전율 66% vs 28.8~33.4% 엄밀 대사(정의가 달라 방향만 비교). 미측정 = `pos52` 결함의 실제 영향 비율·`firstPosLens` 소비처 라이브 관측(코드 구조 확인까지)·`flat` 0건이 부동소수 단독 원인인지 전수 미확인. 🔴 **판정은 장은태가 한다. 이 STEP은 결함을 놓는 것까지다.**

## 2026-08-18 — 🔵 **STEP1060: 7렌즈 재무 입력 출처·연도 정합성 검증 (야후 ↔ SEC 대조)**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0) — **단 이 STEP에만 유일 예외로 야후 실조회 허용**(§2-0, 상한 200종목·순차호출·레이트리밋·DB 쓰기 0·결과는 `docs/probe_1060_*.json`로만). 감사⑤⑥⑦(자산성장·F스코어·기술)보다 이것을 먼저 놓은 이유 — STEP1059 §2가 "퀄리티(그리고 F스코어·자산성장·밸류폴백까지) 전부 `fundamentalsTimeSeries` 한 함수에서 나온다"고 확정했으므로, 입력 자체가 의심되면 그 위에 쌓는 감사들이 전부 흔들린다.
>
> **⓪-1b 중복조사 확인**: `ls docs/` 전수로 「재무 출처·야후·edgar·fundamentals」 문서를 먼저 훑음 — `probe_1006~1009`·`probe_941`·`probe_987`은 전부 "로컬↔프로덕션 marketCap 취득 차이"(별개 주제, 티커 길이 기반 환경차이)라 이 STEP(회계연도 정합)과 무관 확인. `Q1_YAHOO_REMOVAL.md`(STEP981/982)는 같은 `fundamentalsTimeSeries` 공유 사실은 이미 확정했으나 TTM/FY 기간타입 차이만 다뤘을 뿐 회계연도 지연은 다룬 적 없어 중복 아님.
>
> **⓪-4① 판명 — 확정**: 명령서 작성 중 Cowork이 직접 조회한 SEC 값(MSFT FY2026·KO FY2025)이 STEP1059 손계산(MSFT FY2025·KO FY2024)보다 최신이라는 전제를 두고, MSFT·KO·AAPL 강제 3종목을 실제로 야후에서 받아 재현. 결과 **셋 다 야후 최신행 = SEC(`us_fundamentals`, `fetched_at` 2026-08-17 23:38) 최신 `fiscal_year`, 매출·총자산 값까지 완전 일치**(AAPL 416,161M/359,241M · MSFT 331,839M/758,376M · KO 47,941M/104,816M, 셋 다 SEC=야후). 결론: **STEP1059 §6의 MSFT 1.59%p·KO 0.39%p 편차는 야후 실배선의 결함이 아니라, 그 STEP의 손계산이 오래된 수동 수치를 가져다 쓴 것이 원인이다.**
>
> **⓪-4② 판명 — 기각("7렌즈가 뒤처진 회계연도를 쓴다"는 가설은 이번 실측에서 확인되지 않았다)**: 층A(SEC 쪽 전수, 재취득 0) — `us_fundamentals` 5,820행 중 `fiscal_year` 비결측 3,762행(2025가 93.4%로 압도, `total_assets`는 STEP1055 신규 필드라 최신 크론분 1,025종목만 보유). 층B(야후 쪽 상한 200 내 층화표본) — 시가총액 5티어(mega/large/mid/small/micro)×섹터로 뽑은 175종목을 실조회(성공 175·실패 0·소요 213.8초, 2026-08-18T04:44:13Z~04:47:46Z UTC). fiscal_year "라벨 불일치" 33건을 **금액값 대조**로 재분류하니 — **23건(69.7%, 전부 1월 등 비12월 결산사)은 값이 완전 일치하는 라벨 컨벤션 아티팩트**(SEC의 회계연도 넘버링과 원자료 캘린더연도가 회사 자체 관행에 따라 어긋날 뿐, `financials[length-1]`처럼 배열 위치로 최신을 고르는 코드는 이 라벨 문제와 무관), **5건은 오히려 SEC(`us_fundamentals`)측이 옛 연도에 멈춘 반대 방향**(AGNC FY2019·AMH FY2020·LCNB FY2013·FLG FY2022·FRD — 전부 REIT/금융/소형은행주, 야후가 SEC보다 최신인 FY2025 데이터를 갖고 있었음), 5건은 야후 매출 결측으로 비교 불가. **야후가 SEC보다 뒤처진 확정 사례 = 0/175.** 부가: revenue 146/175(83.4%)·total_assets 163/175(93.1%) ±2% 이내 일치, 야후 내부 `grossProfit`(direct) vs `revenue−costOfRevenue`(computed) 138/138(100%) 완전 일치, 175종목 전부 야후 연도 목록 내부 결측(비연속) 0건·F-Score 3개년 요건(길이≥3) 175/175 충족 — STEP803 `nonConsecutive()` 가드가 실제 발동하는 사례는 이 표본에서 재현되지 않았다(불필요란 뜻이 아니라 트리거 조건 자체가 이번 표본엔 없었다는 뜻).
>
> **⓪-4③ 판명 — 부분 정정**: `LENS_AUDIT_04_QUALITY.md` §4의 "전수 대조는 구조적으로 불가능"을 좁혔다 — `us_fundamentals`에 SEC의 `revenue`·`total_assets`가 이미 있으므로(STEP1055 신규 필드) **분모(총자산)와 매출은 재취득 0으로 대조 가능**했고, 실제로 대조했다(§4-1 신설). 진짜 불가능한 건 `grossProfit`·`costOfRevenue` 두 필드뿐(야후 원시값 미저장). `LENS_AUDIT_04_QUALITY.md` §4·§6·요약표·마무리절을 취소선+정정+좌표로 갱신(문서를 다시 쓰지 않고 정정 문단만 추가).
>
> **4개 렌즈 코드 경로 재확인(§2-5)**: 퀄리티(`lenses.ts:328`)·밸류폴백(`lenses.ts:251`)·F스코어(`fscore.ts:70·83·111~113`)·자산성장(`lenses.ts:372~373`) 전부 `financials[length-1/2/3]` **배열 위치**로 최신 연도를 고른다 — "연도 번호"를 직접 비교하지 않으므로 위에서 확인한 라벨 컨벤션 문제 자체와 구조적으로 무관하다. 자산성장의 STEP803 `nonConsecutive()` 가드는 이번 175종목 표본에서 트리거 0건(내부 연도 결측이 관측되지 않음).
>
> 산출물 = `docs/probe_1060_yahoo_sec_fiscal_year.json`(원자료, 175건 전체)·`scripts/probe_1060_yahoo_sec_fiscal_year.ts`(재현 스크립트)·`LENS_AUDIT_04_QUALITY.md` §4·§4-1(신규)·§6·요약표·마무리절 정정·`LENS_DEV_PLAYBOOK.md` 문제해결 로그 신규 행(#115) — 새 파일을 신설하지 않고 두 기존 문서를 확장했다(§2-7 "새 파일 최소화" 지시 준수). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0. 못 한 것 = AGNC·AMH·LCNB·FLG가 왜 `us_fundamentals`(SEC/revdcf 파이프라인)에서 옛 연도에 멈췄는지 근본 원인(이 STEP 범위 밖 — 7렌즈 야후 입력 감사이지 revdcf 파이프라인 진단이 아님)·47종목 SIC코드(④감사 잔여, 별건). 철회·정정 = `LENS_AUDIT_04_QUALITY.md` §4·§6(이 STEP 자체의 성과). 미측정 = AGNC류 4~5종목의 SEC측 정지 원인·`decomposition.source` 전종목 분포(④감사 이월). 🔴 **판정은 장은태가 한다. 이 STEP은 입력이 맞는지 확인하는 것까지다.**

## 2026-08-18 — 🔵 **STEP1059: 렌즈 전수감사 ④ 퀄리티(GP/A)**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). `strong` 등급 넷(모멘텀·저변동·퀄리티) 중 마지막.
>
> **⓪-4 반증조건 셋 다 원문으로 확정**: 전제①(원전 있음) = **참** — Novy-Marx(2013) JFE 원문 직접 취득(`data/sources/academic/novy_marx_2013_gross_profitability_premium.pdf`), 정의(*"gross profits (REVT−COGS) scaled by assets (AT)"*)가 `lib/lenses.ts:325` 주석과 정확히 일치. 🔑 **STEP815가 STEP801의 "분모=기초" 변경을 되돌린 것(=같은 회계연도 기말 총자산)이 원문과 정확히 일치함을 원문으로 재확인.** 전제③(금융사 처리) — 원문이 SIC=6(금융업) 명시 제외를 3곳(Table 1·2·Fig.1)에서 반복 확인. 우리는 능동적 SIC 필터가 아니라 매출총이익 미보고로 인한 수동적 결측이며, 이는 **이미 `lensCopy.ts`의 note에 정직하게 공개돼 있던 내용**(결함 아님, 이중 계상 안 함) — DB로 정량 확인(quality na 142건 중 115건=81.0%가 Financials 섹터, 역으로 Financials 162종목 중 47건은 GP/A 점수를 받음 — MA·V·MCO·MSCI 등 결제망·신용평가·데이터업체로 전통 은행·보험 아님).
>
> **재무 출처 확정 — ①밸류 결함 2 재현**: `lib/lensCompute.ts`의 `fundamentalsTimeSeries`(야후) 하나가 퀄리티·F스코어·자산성장·밸류폴백 전부의 원천이고, `lib/edgar.ts`(SEC)는 7렌즈 경로에서 **0건 참조**(전수 grep 확인) — SEC 원자료 경로는 revdcf/Q1 파이프라인 전용. `lenses.ts:71`이 야후 `grossProfit`(벤더 계산값)을 `totalRevenue−costOfRevenue`(재구성)보다 우선 — 원전은 계산이 먼저인데 코드는 반대.
>
> **⓪-4② 분자 출처 전수 대조 = 못 함(구조적 제약)**: `us_fundamentals`(SEC 전용)엔 야후 원시 필드(grossProfit·totalRevenue·costOfRevenue)가 저장되지 않고, 이 STEP은 재무 데이터 재취득을 명시적으로 금지 — **7렌즈 퀄리티(밸류·F스코어·자산성장 포함)는 사후 감사 인프라 자체가 없다**는 것 자체가 실측 결과. 대신 손계산 3종목(공식 IR·3자 데이터 사이트)으로 검증: AAPL 54.34(DB) vs 54.337%(Apple 공식 FY2025 재무제표) **사실상 완전 일치** · MSFT 29.73 vs 31.32%(MS 공식 IR) **1.59%p 차이** · KO 28.19 vs 28.58%(stockanalysis.com) 0.39%p 차이 — MSFT 편차는 공식 1차 출처 대비라 원인 미확정으로 판정 재료 남김.
>
> **DB 전수 실측**(조회 `now()=2026-08-18 03:02:06 UTC`, `lens_scores` US 999행 `updated_at` 균일 `2026-08-17 22:03:40 UTC`): 분포 min −23.02~max 114.98 · state mid343/high257/low257(대칭)/na142. `lens_cuts.quality`(lo=14.038·hi=30.52·n=857·as_of 08-17)가 분포 p30/p70과 정확히 일치(STEP805 약속이 코드에 실재 확인). `lens_state_changes` 07-20~08-07 재조회 결과 **179건/95종목 — `LENS_DISPOSITION` §1과 정확히 재현**. ③ 감사의 스테일 극단치(신규②, QH)는 오늘 재현 안 됨 — QH 행 자체가 사라짐(08-17 정상 프루닝 추정).
>
> **앞선 결함 15건(①밸류6+②모멘텀5+③저변동4) 생존 확인**: 코드가 실제로 바뀐 건 **0건**. 신규②(QH 스테일)만 데이터 자연갱신으로 해소, 신규③(08-16 US 취득실패)은 STEP1058/본 STEP §1-2에서 이미 "복구" 확정된 것을 재조사 없이 인용.
>
> **질문 귀속(2-9)**: `lensCopy.ts`의 자체 질문("돈을 잘 버는 회사인가?")은 W-2 여섯 질문 어디와도 정확히 안 맞음(재무건전성=방향/변화, GP/A=수준 — 다른 것. 성장 편입은 STEP1056에서 이미 불일치 확정). **분류 = 새 질문 후보** — 신설 여부는 결론 안 내고 재료만 놓음(원전 있음·정의 공개·컷 정합·결함 15건이 화면에 영향 없음을 확인 완료).
>
> 산출물 = `docs/LENS_AUDIT_04_QUALITY.md`(신규). 철회·정정 = 없음(직전 인용 전부 원문 재확인 결과 정확했음). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. 브랜치 규약 그대로 — `main` 직접 push. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0. 못 한 것 = 벤더 grossProfit vs 계산값 전수대조(캐시 없음+재취득 금지)·47종목 SIC코드 재확인·MSFT/KO 편차 원인. 미측정 = decomposition.source 전종목 분포·Ball et al.(2015/2016) 원문 확보. 🔴 **판정은 장은태가 한다. 이 STEP은 결함을 놓는 것까지다.**

## 2026-08-18 — 🔵 **STEP1058: 2026-08-16 밤 US 파이프라인 연쇄 실패 원인 규명**

> 읽기 전용(코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0). 렌즈 감사④(퀄리티)보다 먼저 — 감사④~⑦이 전부 이 DB를 재료로 쓰기 때문. 재실행·수정 여부는 판정하지 않음.
>
> **⓪-4 전제② 기각(코드 배포 아님, 5분 확인)**: `lib/valuation.ts` 2026-08-14~18 변경 0건, `app/api/cron/revdcf/route.ts` 마지막 변경 08-14 10:03 KST(실패 이틀 전). 계산 코드는 안 바뀌었다.
>
> **⓪-4 전제① 절반만 참**: `us-perf`(나스닥 20초 timeout, 08-13 probe_1017과 byte-일치하는 에러 문자열 재확인 — 3일 이상 반복되는 상시결함)는 배수 결측과 **완전히 무관**(`us_market_cap_nasdaq` write-only, 소비처 0개, 전수 grep 확인). `lens-scores`(US, 야후 시총 배치)는 배수 결측과 **실제 인과관계 확인**(아래). `revdcf`의 "예산소진(processed 1561)"도 배수 결측과 **무관** — `computeAndSaveValuation`은 처리 건수와 무관하게 `us_fundamentals` 5,820종목 전량을 매번 재계산.
>
> **규모 불일치(4,259 vs 552) 완전 해명 — 이 STEP의 핵심 성과**: `computeAndSaveValuation`(`app/api/cron/revdcf/route.ts:65~67`)이 `us_market_cap`에서 **가장 최신 단일 `as_of` 하나와 정확일치**로만 조인한다는 것을 코드로 확정. `us_market_cap`은 symbol 단일 PK 누적 캐시라 갱신 실패 심볼은 옛 `as_of`인 채 남는데, 그 심볼들은 시총이 있어도 이 조인에서 **통째로 배제**된다(오래된 값 사용이 아니라 완전 배제). 08-15↔08-16 전수 대조(정방향+역방향) 결과 PER 552·PBR 902·PSR 966·EV/EBITDA 406건 결측 증가 **전부(2,826건 100%, 예외 0)**가 `us_market_cap.as_of<2026-08-16`(전일 08-16 밤 야후 시총 배치 갱신 실패) 부분집합과 정확히 일치. `us_market_cap`은 `lens-scores`(US) 하나만 쓴다(`lensPrecompute.ts:217`, 전수 grep 확인) — **`lens-scores`의 야후 배치 실패(freshCoverage 68.8%) → `us_market_cap` 부분 갱신 실패 → 20분 뒤 실행되는 별개 크론 `revdcf`의 밸류에이션 계산이 그 갭을 그대로 받는 진짜 연쇄**임을 확정.
>
> **부수 발견**: PER 결측 552종목 시총 분포 = 최소 $389만~최대 **$2.83조(AMZN)** — 소형주 편향 아님. 대신 첫 글자 A135·B108·C202·D70·E37=552(100%, F이후 0건) — `STOCK_SYMS`(5,975개, 알파벳순) 100개씩 60청크 중 실패한 18개 청크가 **알파벳 앞쪽(A~E)에 몰림**(원인은 미측정, 반복재현 금지). 섹터 없음(1,135건) 코호트와는 **무관**(교집합 17/552=3.1%뿐).
>
> **2-2(08-17 US 크론 실행여부)**: DB 조회시각 `now()=2026-08-17 15:43:45 UTC` 기준 US 크론 스케줄(21:30/22:00/22:45 UTC)이 **아직 안 왔다** — "확정불가·시각미도래"로 명시(probe_1017 A의 "로그접근막힘"과는 다른 종류의 미확정).
>
> **2-7(결측 화면 표기)**: `us_valuation`은 `app/api/q1` 하나만 소비(전수 grep, 다른 화면·이메일·브리핑 0건). `Q1_ENABLED`는 프로덕션 Vercel 환경변수 목록(44개 전수 확인)에 **키 자체가 없음 = OFF** — 지금은 실사용자에게 노출 안 됨. UI는 `NO_VALUE`/`NO_SECTOR`/`SAMPLE_TOO_SMALL` 세 갈래로 결측사유를 구분해 표시하도록 설계돼 있으나, `computeValuation`의 구체 사유(`MISSING_MARKET_CAP`)가 `computeSectorRelativeBatch` 단계에서 `NO_VALUE`로 뭉뚱그려짐 — 그 문구(`messages/ko.json:950` "적자·자본잠식 등")가 흑자 초대형주(AMZN·AVGO·BRK-A/B·AMD)에 붙으면 오귀속 소지가 있음을 가정형으로 남김(Q1이 꺼져 있어 실제 노출은 없었음).
>
> Vercel 런타임 로그: MCP(기존 403, 반복 안 함) + CLI 새 경로(`vercel inspect --logs`=빌드로그뿐, `vercel logs`=공식 도움말상 "지금부터 5분"만 지원) 둘 다 구조적으로 과거 로그 불가임을 이번에 새로 확인.
>
> 산출물 — 새 파일 대신 기존 문서에 절 추가(장은태 규칙): `docs/probe_1017_cron_failure_causes.md`에 "2026-08-16 US 연쇄 실패" 절 추가(기존 08-13 절 불변), `docs/CRON_OBSERVABILITY.md`에 "관측 공백 목록"(5-5) 추가(필드 실제 추가는 안 함). 게이트7(미추적 파일 참조 0건). 게이트9 해당없음. tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0·크론 수동 실행 0. 못 한 것 = 알파벳 앞쪽 청크가 왜 몰려 실패했는지(반복재현 금지)·Vercel 런타임 로그 원문(구조적으로 불가 확인만 됨)·08-17 크론 실제 성공 여부(시각 미도래). 철회·정정 = ⓪-1a WHY조건1 인용 "지금 화면에 나가는 배수가 27% 결측"을 "DB 상태는 맞으나 화면에 나가는 중은 부정확(Q1 꺼져있음)"으로 정정. 미측정 = `us_market_cap` 갱신 실패 상류원인(야후 vs 우리 쪽)·재발 여부(연속관측 필요)·`computeSectorRelativeBatch`가 구체사유를 보존 안 하는 설계 이력. 🔴 **판정은 장은태가 한다. 이 STEP은 원인을 놓는 것까지다.**
...
>
> **2-1 양방향 전수 대조**: 정본 W-1~W-3 전체 vs 표시본 WHAT 절 전체. 이미 알려진 다섯(㉠성장질문 표시본 재복원 확인·㉡질문개수·㉢배수+역DCF 병합/분리·㉣DCF정방향 신설·㉤층구분) 외에 **㉥ DDM 소속 갈림**을 새로 발견 — 정본(`:307`)은 "DDM은 적정주가류로 판정(다른 칸)"이라 배당 칸을 배당수익률+연속증가연수 둘로만 확정했는데, 표시본 질문6 카드는 DDM을 배당의 세 번째 축으로 명시 포함한다. 반증조건(⓪-4) 판명: **틀렸다 쪽**(추가 갈림 존재) — 같은 형식(정본/표시본/근거/성격)으로 §2-4에 등재.
>
> **2-2 확정 3건 정본 반영**(취소선 보존+정정사유+좌표): W-2-4 부도위험(`:308`, "지금 만들 수 없다" → 실측 Assets 73.8%·Liabilities 65.9%/재구성 73.6%·Z+Z″ 65.1%로 만들 수 있다) · 구조잔여3건 중 자산성장(`:312`, "처분 미정" → 실측 99.7%로 재료가 병목이 아님이 확인됨, 처분 판정 자체는 여전히 미정) · W-2-4 재무건전성(`:305`, 조건이 표본 하나에서 표본+산출가능범위 32.3% 둘로).
>
> **2-3 표시본 전용 확정 결과 정본 이관**: 새 서브섹션 W-2-5 신설 — 탈락 셋(PEG·피터린치/애널리스트목표가·컨센서스/퀀트종합랭킹, 각 사유) · 그 앞에서 걸러진 것들(Magic Formula 외 10종, 탈락 셋과 "원리적 불가 vs 수요미확인"으로 성격이 다름을 명시 — 🔴 STEP1056 명령서 자체가 "여덟"이라 적었으나 표시본 실제 목록은 11개라 숫자를 단정하지 않고 목록 그대로 옮김) · 판정 해소 둘(DCF 기본값·모멘텀 일관성, 무엇이 어떻게 틀렸는지까지) · 극단치 원칙(분모≤0·원전표본밖 둘만 자름, winsorize는 관문②위반).
>
> **2-4 판정 대기 등재**: F-5에 ⑮(질문개수)~⑲(DDM소속) 5개 신설. 🔴 각 항목에 어느 쪽이 왜 그런지 근거만 붙이고 판정은 안 함.
>
> **2-5 층구분 근거상태**: ⑱ 항목에 "층 구분은 2026-08-16 세션 도출이며 원전 대조 미실시, 근거 있는 것은 베타의 「부품」 판정(`probe_1041`) 하나뿐"을 그대로 명시.
>
> **2-6 표시본 재생성**: `Downloads/roadmap_v2_2.html`을 기존 파일째 덮어써 정본에 맞춤(새 파일 생성 금지) — 질문개수 "여덟"로 내부 일관성 수정(기존 "일곱/여덟" 혼재 발견·수정), F-5 ⑮~⑲ 좌표 추가, 🔴 **가장 큰 발견** — 표시본 자체가 "이 절이 정본이다(2026-08-16 장은태 판정)"라고 적어 두고 있었다(별도 「층별 정리」 문서를 은퇴시키며 남긴 문구). STEP1056의 전제(정본=`ROADMAP_V2.md`, 표시본=이 HTML)와 정면 충돌하는 자기지시적 주장이라 정정 — "정본을 둘로 두지 않는다"는 원칙은 유지하되 그 정본이 `ROADMAP_V2.md` 하나임을 명시.
>
> **2-7 브랜치 규약 신설**: `docs/COMMIT_GATES.md` 게이트 8 바로 아래 "브랜치 규약" 신설 — STEP은 `main`에 직접 push, 브랜치·PR 생성 금지(장은태 위임 2026-08-17 "너가 베스트라고 생각하는 방식으로 하자"). 근거 넷(DB 브랜치 미격리·협업충돌 없음·STEP990 교훈은 브랜치가 아니라 게이트7·8·기존 방식이 이미 직접 push) 문서화. **이 STEP 자신부터 적용** — STEP1055(직전 STEP)가 만들었던 `worktree-step1055` 브랜치+draft PR #1을 이 STEP 실행 중 `git push origin worktree-step1055:main`으로 직접 병합(PR 자동 MERGED 전환 확인)하고 원격 브랜치를 삭제, 이후 STEP1056 자신도 브랜치·PR 없이 `main`에 직접 push.
>
> 게이트7(미추적 파일 참조 0건, 문서 전용이라 해당 파일 없음). 게이트9 해당없음(신규 테이블·크론 없음, "정본 먼저·표시본 나중" 순서는 §2-6에서 지킴). tsc 0·vitest 386/386·코드 diff 0·DB 쓰기 0·화면 변경 0. 못 한 것 = 없음. 철회·정정 = 표시본의 "이 절이 정본이다" 자기주장 정정(위 2-6). 미측정으로 남은 것 = ⑮~⑲ 다섯 항목의 판정(전부 장은태 대상), STEP1055가 미측정으로 남긴 셋(Piotroski 병목배열·Altman SIC·총부채 경로)은 이 STEP도 건드리지 않음. 🔴 **판정은 장은태가 한다. 이 STEP은 갈림을 놓는 것까지다.**
...
> `probe_1053`이 질문을 확정했으나 재료가 실제로 있는지는 안 쟀다. 이 STEP이 총자산·총부채·이익잉여금·영업현금흐름·배당(다섯 재료)의 실제 공급을 US 모집단 전수로 잰다. 🔵 프로덕션 diff 0·DB 쓰기 0 — 태그를 배열에 추가하지 않는다.
>
> **2-1 모집단·캐시**: `us_fundamentals` 모집단 5,820. 로컬 캐시(`docs/probe_951_cache/`) 1,167건에서 시작해 부족분 4,653건을 SEC에서 전수 수집(150ms 간격 순차) — 실측 타이밍(요청당 449ms, 지시문이 가정한 0.1초의 4.5배 — companyfacts 평균 1.35MB라 다운로드 시간이 간격보다 훨씬 큼) 기준 예상 35분, 실제로는 백그라운드 실행 중 하네스가 90.3% 지점에서 프로세스를 종료해 재실행으로 완료. **최종 확보 5,742/5,820(98.66%)**, 나머지 78건은 SEC `companyfacts` 자체가 `404`(폐쇄형 펀드류 등 XBRL 미제출 발행인으로 추정, 재시도로 해결 안 됨).
>
> **2-2 태그 전체 나열**: 5,742건 전수의 `us-gaap` 키를 이름 검색 없이 전부 나열해 빈도표 작성. `drivers.ts` 참조 태그와 양방향 대조 — **죽은 태그 4개**(`CostOfSales`·`CostOfOperatingRevenues`·`CostOfRevenues`·`DepreciationAmortizationAndDepletion`, 배열엔 있으나 실제 0건) · **고빈도 누락 후보 60개**(30%+ 출현, 배열 밖) 확인, 이 STEP이 다루는 5개념 중 4개가 스스로 그 상위 목록에 나타나 교차 확인.
>
> **2-3 후보 태그 확정**: 로컬 캐시에 태그당 내장된 us-gaap 공식 `description`(SEC 라벨링크베이스 원문, 추가 SEC 호출 0)으로 11개 후보 전부 정의 대조. 🔴 **⓪-4 반증조건 정확히 적중** — bare `RetainedEarnings`는 5,742건 전수에서 **0건**, 지시문이 지목한 `RetainedEarningsAccumulatedDeficit`가 실제 표준 태그(84.9% 출현)로 확인. `Liabilities`(직접, 65.9%)보다 `LiabilitiesAndStockholdersEquity`(재구성 경로, 73.6%)가 7.7%p 더 넓다는 것도 지시문 예측과 일치.
>
> **2-4 커버리지 ㉠㉡㉢**: 총자산 73.8%/73.8%/72.0% · 총부채(직접) 65.9%/64.9%/64.0% · 이익잉여금 72.2%/71.8%/70.2% · CFO 72.3%/72.3%/70.3% · 배당(지급기준) 34.2%/27.9%/32.6% · 배당(선언기준·주당) 24.8%/18.7%/23.1%. 배당 세 축(지급·선언·지급·주당)이 서로 다른 종목을 담아 "섞지 않는다"는 지시가 실측으로 정당화됨.
>
> **2-5 모델 완성 가능성**(분모=기준연도 보유 3,606): **Piotroski 9신호 전부 성립 1,166건(32.3%)** — 병목은 ΔLever(64.2%)·ΔMargin(51.6%), 나머지 7신호는 85%+. **Altman Z(제조업 근사)+Z″(비제조업 근사) 합계 2,348건(65.1%)**, 섹터 라벨 결측 159건(4.4%)은 분류 자체 불가 — 🔴 섹터 분류는 실제 SIC코드가 아니라 GICS 유사 라벨 근사(로컬에 전 종목 SIC 없음, 한계 명시). 배당수익률 성립 1,829건(50.7%). 자산성장(연속 2년 `Assets`) 3,596/3,606(**99.7%**) — F-5 ⑪"재료 문제였다"가 실측으로 확인(태그 자체 공급은 낮아도 한번 보고하면 계속 보고). B/M(장부가, 기존 파이프라인 값 재사용) 3,726/5,820(64.0%) — 로드맵 HTML이 예전에 "추정이지 실측 아니다"라 적어둔 수치가 이번에 실측과 나란히 확인됨.
>
> 문서 = `docs/probe_1054_tag_supply.md` + 원자료 `docs/probe_1054_analysis.json`(top200 태그·전체 결과). 스크립트 = `scripts/probe_1054_population.ts`·`probe_1054_fetch_shortfall.ts`·`probe_1054_analysis.ts`(전부 `lib/revdcf/drivers.ts` import만, 배열 무변경). 게이트7 — 기존 965/967/969 `/tmp` import 3건 무관(범위 밖). 게이트9 해당없음. tsc 0(무관 사전 오류 2건 확인, STEP1018 기존)·vitest 386/386 불변. 못 한 것 = SEC 404 78건 개별 원인·Altman 결측 1,099건 항목별 분해·배당 "무배당 vs 데이터없음" 미분리. 철회·정정 = 지시문 자체의 SEC 비용모델(0.1초/건)이 실측(0.45초/건)과 어긋난 것을 정정 기록. 미측정 = RetainedEarnings taxonomy 공식 폐기 여부(release notes 직접 대조 안 함)·Z/Z″ 섹터군 전체 모집단(성립 아닌 분모 자체). 🔴 **판정 금지 — 5개 태그 배열 추가·죽은 태그 제거·섹터 근사 교체는 전부 장은태 판정 대상, 이 STEP은 숫자를 놓는 것까지다.**

## 2026-08-16 — 🔵 **STEP1053: 질문 층 확정 — `Q0`~`Q5` 번호 폐기 · 재무건전성 질문 복원**

> 장은태 지시(2026-08-16): *"질문에 대해서도 기존 질문지를 사용하는 게 아닌 재검토로 질문 내용이나 개수를 바꿔도 된다고 분명히 말했다. 그리고 Q~ 에 관한 것은 재정의를 내릴 때 바뀔 수도 있으니 빼라고 했다."* → *"이것들을 하기 전에 질문할 내용을 먼저 확정해."* 🔵 코드·DB 쓰기 0·화면 변경 0.
>
> **1-1 기준(질문 보기 전에 먼저 씀)**: 지시된 5개(관문①②③·배타성·완전성) + 자체 추가 2개(전달성·명확성·독립출처, 총 9개).
>
> **1-2 상향(정보→질문)**: `probe_1040`·`1043`·`1045`가 확정한 정보 칸(0층 2·1층 5·시간축 2)을 재조사 없이 가져와 독립적으로 문장화. 업종·이익상태 둘 다 "전제인가 질문인가"를 이 STEP에서 다시 판정해 **둘 다 전제로 재확인**(업종 = `USER_QUESTIONS`§0 논리를 독립 재확인, 이익상태 = 신규 검토해 배수 계산 성립 조건이라는 게이트 역할 재확인). 배수+시장내재기대는 "같은 질문, 두 방법"으로 **병합**(`USER_QUESTIONS`§5와 독립 수렴), 시장내재기대(미래 가정)와 성장(과거 실적)은 개념적으로 배타적이라 **안 합침**.
>
> **1-3 하향(실무 화면 문구)**: 배경 Agent가 5개 플랫폼(Stock Analysis·WallStreetZen·MarketBeat·Simply Wall St·Morningstar) AAPL 종목상세 raw HTML을 직접 수집(AI 요약 경유 안 함) — Simply Wall St는 Cloudflare 403으로 차단(검색엔진 요약만 확보, 화면 인용으로 안 씀). 🔑 **핵심 발견**: 5곳 모두 섹션 제목이 명사구지 의문문이 아니었다 — **"~걸까?" 구어체 질문형은 업계 관행이 아니라 우리 선택**이라는 사실이 드러남("업계도 이렇게 쓴다"는 근거로 정당화하면 안 됨, 전달성 기준 자체는 유효). 재무건전성 = WallStreetZen·Morningstar가 valuation과 분리된 전용 섹션으로 명시(MarketBeat만 예외) → 독립 칸 판단을 간접 지지.
>
> **1-4 도출**: 6개 질문 — 배수+시장내재기대(병합) · **재무건전성(신규)** · 성장 · 배당(🔴 US 재료 0) · 부도위험(🔴 재료 미구축) · 최근변화.
>
> **1-5 대조(도출 이후에만)**: 5/6은 기존 `USER_QUESTIONS_2026-08-08` 문구가 문구만 다듬어진 채 유지(근거만 "물려받음"→"도출"로 전환). **1/6(재무건전성)은 실제 신규** — `MODEL_DEMAND_SURVEY_2026-08-07` #7 "재무건전성 스코어"가 당시 최상위 수요(●●●, SWS Health·Alpha Spread 근거)였는데 **다음날** `USER_QUESTIONS_2026-08-08` 6문항 확정 시 SWS Health 신호 전부를 부도위험(`Q4`) 하나에만 배정하면서 누락됐다 — 성장/`Q3`이 겪은 것과 같은 유형(정보는 이미 있었는데 질문 목록에 안 옮겨짐).
>
> **1-6 영향 지목만(재판정 안 함)**: Novy-Marx GP/A 재심 필요성 · Layer C "주 사용자" 문장 영향 · F-4-3 근거 문구 갱신 필요 — 전부 장은태 판정 대상으로 지목만.
>
> `ROADMAP_V2.md` W-2절에 **질문 층 정본 신설**(번호 없이 문장으로 부름) + 교차 참조 3곳 갱신(역사 인용 3곳은 의도적 보존) + `roadmap_v2.html` 동기화(html.parser·태그균형 재검증) · `STATE.md` §③ 헤더에 역사 보존 배너 추가(하위 수십 개 Q번호 개별 기록은 재작성 안 함, 범위 밖). 문서 = `docs/probe_1053_question_set.md`. 게이트7 — 기존 965/967/969 `/tmp` import 3건 무관(범위 밖). 게이트9 해당없음. vitest 386/386(불변, 코드 파일 무접촉 확인). 못 한 것 = Simply Wall St 화면 원문(403 차단). 철회·정정 = 없음(신규 도출). 미측정 = 시장내재기대·성장 직접 상관계수 · "망할 위험" 질문 ② 재검증 · 재무건전성 질문 사용자 테스트 · MarketBeat FAQ 개별 질문 문장 · 5개 사이트 robots.txt 개별 조회. 🔴 판정금지 — GP/A 재심·Layer C 문장 갱신·망할위험 ② 재검증은 전부 장은태 판정 대상, 이 STEP은 지목까지다.
...
>
> **원인**: `app/api/q1/[symbol]/route.ts:42`가 `us_sector_wide`를 `us_valuation`의 오늘 날짜(`asOf`)로 **정확일치** 조회했다. 그런데 `us_sector_wide`는 설계상 `as_of`가 고정된 1회 적재 테이블(STEP973·974, 장은태 승인)이라 정확일치는 영구히 실패했다 — "신선도" 문제가 아니라 **다른 테이블의 as_of를 그대로 가져다 쓴 조인 키 버그**였다. 같은 저장소 안에 정답 패턴(`app/api/cron/revdcf/route.ts:111` — 최신 1건 조회, `lib/sector.ts`의 `latestAsOf()`로 이미 공용 함수화돼 있었다)이 있었다.
>
> **1-1 전수 스캔**: `us_sector_wide`를 읽는 모든 지점(app/lib/scripts)을 규약별로 분류 — 정확일치를 쓰는 12곳 중 **11곳은 자기참조 또는 1회성 백필 스크립트의 의도된 과거 시점 고정**(버그 아님, 재현성 보호를 위해 미수정)이었고 **`route.ts:42` 1곳만 진짜 결함**이었다.
>
> **1-2 수정**: `route.ts`가 `lib/sector.ts`의 기존 export `latestAsOf()`를 재사용하도록 변경(새 패턴 발명 없음, STEP1004가 원래 의도한 재사용) — **적재 로직(크론)은 한 글자도 안 건드림**. 회귀 방지 유닛테스트 2건 추가(as_of 어긋남·완전 결측 케이스).
>
> **1-3 두 방법 검증**: ① 코드 대조로 크론과 동일 규약 확인 ② 로컬에서 `Q1_ENABLED=true`를 프로세스 하나에만 오버라이드해 실제 `GET()`을 real DB에 대고 호출(`scripts/verify_step1050_sector_source.ts`) — `probe_1042` 표본 4종목(AAPL·AAOI·AAME·CTO) **전부 `sectorSource`에 값이 실림**(AAPL="spdr", 나머지="damodaran"). `us_sector_wide.source` NULL 비율 실측 = 1,135/5,820(19.5%, 조인과 무관한 별도 데이터 갭 — 처방 안 함, 비율만 기록). tsc clean · vitest 386/386. `npm run build`는 라이브 dev 서버(포트 3333) 보호를 위해 의도적으로 생략(대체: tsc+vitest+real-DB API 호출). 화면 diff = 컴포넌트 파일 0건(라우트+테스트 2개뿐), `Q1_ENABLED` OFF 유지로 구조적으로 보장.
>
> 🔑 **1-4 기록 신뢰 복구**: `docs/PARKED_*` 5개를 `ls`로 전수 확인(이름 패턴 아님 — STEP1048이 "`_ACTIVATION` 4종"이라 적어 `PARKED_FIELD_SURFACES.md`를 놓쳤던 것과 같은 실패를 반복하지 않기 위해). `PARKED_FIELD_SURFACES.md`의 `ToolboxClient` 진입점 0개 서술(2026-07-28)이 **정확했는데 STEP1048~1049에서 안 읽혔다**는 사실을 그 문서에 명시 추가(옳았던 기록도 남겨야 다음 신뢰가 쌓인다). `PARKED_HNX_VCI_ACTIVATION.md`의 "HOSE 정상" 서술이 이후 §7(VN 전체 차단)로 덮어써졌음을 정정. `STATE.md` #27의 원인 지목(신선도)을 정정(실제 원인은 읽는 쪽 규약, #27 자체는 별개 질문으로 판정 대기 유지). `_TEMPLATE.md` ⓪-1b에 "문서 목록을 이름 패턴으로 만들지 않는다" 한 줄 추가.
>
> `ROADMAP_V2.md` F-4-4 "켜기 전 필수" 2건→1건(조건부 4칸만 남음) + F-5 ③ 해소 표기, `roadmap_v2.html` 동기화(html.parser·태그균형 재검증). 문서 = `docs/probe_1050_sector_source_fix.md`. 게이트7·9 통과(절대경로·미추적 참조 0, 신규 테이블·크론 없음). 못 한 것 = `npm run build`(의도적 생략, 사유 명시) · F-5 헤더 "14건" 카운터 갱신(범위 밖). 철회·정정 = 위 §1-4 3건. 미측정 = `us_sector_wide.source` NULL 1,135행의 원인. 🔴 **판정 금지 — 조건부 4칸(405건)·F-5 카운터 정리는 전부 다음 STEP·장은태 판정.**

## 2026-08-16 — 🔵 **STEP1049: DB 테이블 69개 인벤토리 + 범위를 「US 종목 분석」으로 명시**

> 장은태 판정(2026-08-15): *"이 모델을 만드는 과정을 US 종목에 대해 분석할 때로 정의를 내려야겠다. 다른 국가 탭을 만들 때는 우리의 이 로드맵을 분석해서 그에 맞춰 데이터를 넣어 사용하면 된다."* 조사·기록 전용 — **코드 diff 0·DB 쓰기 0·화면 변경 0.** 삭제·이관은 STEP1047·1048에서 이미 끝났고, 이번은 남은 69개 테이블 전수를 세고 US 범위를 문서에 못박는다.
>
> **1-1 전수 인벤토리**: Supabase MCP로 69개 테이블 행수·최신갱신(`as_of`/`updated_at`/`created_at` 최적 컬럼)을 한 번에 조회, 코드 전체에서 `.from()`/`.rpc()` 호출을 정방향+역방향 두 방법으로 전수 수집(귀속을 이름이 아니라 소스·코드로 판정). 결과 — US 27·KR 3·JP/CN/VN/GB(파킹) 9·공용 21·불명 1(`products`). 미사용(코드 참조 0) 8건 · 뷰 0개(전부 STEP1035·1048로 DROP됨) · 함수/RPC 9개(그중 `lens_distribution`·`lens_percentiles`만 코드가 직접 호출).
>
> 🔑 **핵심 발견 — `docs/PARKED_FIELD_SURFACES.md`(2026-07-28)가 이미 있었다.** STEP1048이 "발견"이라 표현한 `components/toolbox/*`(26개 파일) 도달 불가는 사실 **2026-07-21 장은태의 의도적 제품 결정**("필드=오늘·탐색·종목상세·관심·마이 5면뿐")으로 이미 문서화돼 있었다 — STEP1048이 ⓪-5(자체 인벤토리 먼저)를 이 문서까지는 확인하지 않고 진행한 결과다. 이 문서를 근거로 `link_hub`·`link_hub_clicks`·`link_hub_favorites`·`youtube_channels`·`brokers`(구 보드·정보탭·유튜브·증권사 광고 클러스터) 화면 도달 불가를 판정했고, §7(JP/CN/VN/GB 시장 차단, `lib/activeMarkets.ts`)로 9개 국가별 시세 테이블의 도달 불가도 확정했다.
>
> **1-2 카탈로그 4층**: `DATA_SOURCE_CATALOG.md`에 "이 카탈로그는 US 전용"을 처음 명시(이전엔 암묵)하고, H-2-1의 3층(기관→카테고리25→슬롯20) 아래에 4층 「우리 DB 테이블」(슬롯→테이블 역인덱스)을 추가 — 20슬롯 중 24개 테이블이 연결되고 45개는 카탈로그 밖(산출물·KR/타국·AI캐시·인프라·파킹/미사용)임을 명시.
>
> **1-3 범위 명시**: `ROADMAP_V2.md` 머리에 "적용 범위 = US 상장 종목 분석" 명시 + 국가탭 절차 연결(`CLAUDE.md` 배너 룰②·`COUNTRY_TAB_PLAYBOOK` §0-2 — 엔진 보편·어댑터만 나라별) + `probe_1039` ⓒ 7건 중 `COUNTRY_TAB_PLAYBOOK.md` **존치 확정**(US 단독이면 불필요가 아니라 이 결정의 실행 장치이기 때문).
>
> **1-4 모델별 보편/US특정**: `probe_1043` 확정 모델 전부에 세 축(원전 표본조건·국가파라미터·데이터소스) 적용. 배수·성장=엔진 보편·소스만 US특정 · 시장내재기대(역DCF)=엔진 보편·국가파라미터(rf·ERP·세율)+소스 US특정(Damodaran 157개 시장 데이터 보유) · 재무건전성(Piotroski)=🔴 원전 표본조건 있음(고 B/M) · 부도위험(Altman Z)=🔴 원전 표본조건 있음(US 제조업 1968)이라 **불명**(재추정 필요) — "전부 보편"으로 나오지 않아 ⓪-4 반증 조건이 실제로 걸렸다.
>
> 문서 = `docs/probe_1049_table_inventory.md`(69행 전수표+각주). `DATA_SOURCE_CATALOG.md`·`ROADMAP_V2.md`·`roadmap_v2.html`(html.parser·태그균형 재검증)·`SYSTEM_MAP.md`(스테일 참조 정정: `ai_view_cache` 죽은테이블 명시, STEP1035로 이미 사라진 리딩방 테이블군 안내)·`STATE.md` 반영. 게이트7·9 해당없음(코드·DB 변경 0). 못 한 것 = `sector_cuts`·`us_sector_wide_snapshot`의 정확한 쓰는 코드 file:line 특정. 철회·정정 = STEP1048 "고아 코드 발견"→"이미 문서화된 파킹" 정정. 미측정 = `us_market_cap_nasdaq`가 크론이 매일 upsert하는데 왜 0행인지(원인 미규명) · `products`(10행) 적재 경로. 🔴 **판정 금지 — 처분(DROP·유지·재활성화)은 전부 다음 STEP·장은태 판정.**
...
>
> **A-1**: FK 조회로 11개 테이블(주문서 "12개"는 오류) 확정, 마이그레이션 계보(`001_initial_schema.sql`·`012_quant_factors.sql`·`013_stock_snapshot_view.sql`) 확인, `kr_stock_snapshot` 계열과 FK·뷰 교차 0건으로 경계 확인.
>
> **A-2 두 방법**: 방법①(정방향 문자열 검색, 따옴표 3종)·방법②(코드의 `.from()`/`.rpc()` 62개 전수 수집→대조) 둘 다 **`dividends` 1건**만 HIT — 나머지 10개는 코드 참조 0. `dividends`는 `app/api/dividend/feed/route.ts`가 `.from("dividends").select(..., stocks(symbol, name_ko))`로 `stocks`와 조인해 서빙하고, 그 라우트는 `DividendFeed.tsx`→`OfferingsFeed.tsx`→`ToolboxClient.tsx`(KR 모아보기 '배당' 탭)로 실제 렌더 체인이 연결돼 있다. RLS는 두 테이블 다 `Anyone can read`(공개 읽기). DB 실측 top10에 JB금융지주·HD현대·현대엘리베이터·우리금융지주·기업은행 등 **실제 KR 종목명이 반환**됨을 확인 — 회원탈퇴·관리자·크론 경로에는 참조 없음(크론 자체가 없어 60행은 2026-06-25 이후 정체).
>
> **A-3**: 뷰 `stock_snapshot_v`(`stocks`+`quant_factors`+`dividends`)는 3중 확인(2026-07-12 하드닝 마이그레이션 주석 "앱 미사용(레거시)" · `probe_1036` 기존 미사용 플래그 · 오늘 grep 0건)으로 **미사용 확정**. FK·트리거·함수 신규 발견 없음. RLS는 11개 전부 공개 읽기 정책 보유.
>
> **A-4 판정 — NO-GO**: 주문서 ⓪-4 표의 "참조가 하나라도 있으면 Phase B 중단·임의로 고쳐 진행하지 않는다"를 그대로 적용해 **Phase B(백업·DROP·문서정정) 전부 미실행**. `dividends`+`stocks` 제거 시 KR '배당' 탭이 20개 종목 표시→0개로 조용히 바뀌는 화면 영향도 동시 확인돼, CLAUDE.md의 "프로덕션 화면 변경은 장은태 명시 승인 없이 진행 금지"에도 걸린다.
>
> **선제거 계획(제안만, 미결정)**: 옵션A(9개 0행 테이블+뷰만 먼저 제거, `dividends`+`stocks`는 별도 트랙) / 옵션B(KR 배당 탭을 `kr_stock_snapshot`으로 먼저 마이그레이션 — 단 해당 테이블에 배당 컬럼 없음, 새 파이프라인 필요) / 옵션C(배당 탭 자체를 제품 결정으로 폐지). **셋 다 이번 STEP에서 결정하지 않는다.**
>
> 문서 = `docs/probe_1047_kr_pilot_removal.md`. 게이트7·게이트9 해당 없음(코드·DB 변경 0). 못 한 것 = 브라우저 육안 렌더 확인·Phase B 전체. 철회·정정 = 주문서 "12개"→11개, "8개 0행"→9개 0행(둘 다 결론 불변). 🔴 **판정 금지 — Phase B 착수 여부·선제거 계획 A/B/C 선택은 전부 장은태 판정 대상.**

## 2026-08-16 — 🔵 **STEP1046: 재료 실상 정정 · 파이프라인 지도 · 판정 대기 한 장**

> 코드 diff 0 · DB 쓰기 0 · 화면 변경 0 · 플래그 무접촉. **이 STEP은 판정을 하나도 안 했다** — 정정·지도·수집까지. 모든 수치는 오늘(2026-08-16) DB 재측정, 문서 인용 없음.

**① 재료 통일 재측정**(유니버스=`us_valuation` 최신as_of distinct=5,820): P/E 35.5%·P/B 56.4%·**PSR 61.1%**·EV/EBITDA 24.9%·역DCF 10.4%·Piotroski 12.7%·모멘텀17.5%/기술17.6%·**성장 7.7%**(446/5,820, 오늘 단일스냅샷)·**배당 0.46%**(27/5,820)·부도위험·DuPont·Graham 0%.

🔴 **정정 2건**: ① `probe_1045`의 성장 "이미 완성"·"8.9%(520 누적)" → **과장이었음**, 방법을 통일하면 **7.7%**(오늘 단일스냅샷). ② 배당 "60행(1,977종목 중)" → **행수를 종목수로 오기** — 실제 `dividends`는 **27개 종목**(60행=종목당 평균2.2개년), 0.46%로 기존 표기(3%)보다 6.5배 나쁨. **PSR 재분류**: EV/Sales와 묶여 "부분구현"으로 잘못 분류돼 있었음 — 실측하니 Q1Section이 4축(PER·PBR·PSR·EV/EBITDA)을 동일하게 서빙 중이라 PSR도 "이미완성" 그룹.

**② 순위 재산출**: 1위(동점4개, PSR 승격)·2위(EV/Sales 단독, 진짜 미완성)·3위 역DCF·4위 재무건전성·5위 최근변화·**6위 성장(순서 상승)**·**7위 배당(순서 하락, 성장보다 15배 낮은 재료로 뒤집힘)**·8~10위 부도위험/DuPont/Graham(0%).

**③ 🔑 재료 파이프라인 지도(전수)**: `app/api/cron/revdcf/route.ts` **하나가 4개 칸**(배수=`us_valuation`·업종대비=`us_sector_relative`·시장내재기대=`revdcf_results`·성장=`sales_growth`)을 동시에 좌우함을 코드로 확인 — `us_sector_wide`(STEP1044)에 이은 **두 번째 "한 파이프라인·여러 칸" 사례**. **플래그 확인**: `REVDCF_ENABLED`·`Q1_ENABLED` 둘 다 코드 grep 전수 — 서빙 라우트·화면 렌더에만 걸리고 생산 크론(`route.ts`)엔 두 플래그 문자열이 아예 없음 → **의도된 분리(부작용 아님)**, 플래그 OFF여도 재료는 계속 쌓임. **배당**: 크론 자체가 없음(`app/api/dividend/feed/route.ts`는 읽기전용), `created_at` 전부 **2026-06-25 단일시각**(52일 정체) — "재료갭"이 아니라 **활성 파이프라인 부재**.

**④ 주장 정합(C-1 항목6)**: `REVDCF_SPEC.md:2048`의 부호상충 10.7%는 driver1 실험(야후 컨센서스 대체)의 원자료가 DB에 없어 **재현 불가(미측정)**. 대신 직접 측정 — 과거매출성장(+)인데 역DCF판정=`value_destroying` **151건(33.9%)** + 과거역성장(−)인데 판정≠`value_destroying` **11건(2.5%)** = **162/446(36.3%)**이 표면상 반대방향. 🔴 **처방 없음, 위반 후보로만 등재**(성장=과거매출·역DCF=미래초과수익, 서로 다른 개념이라 논리모순은 아닐 수 있으나 무설명 병치 시 혼란 가능).

**⑤ 🔑 판정 대기 한 장(이 STEP의 목적)**: 두 방법(①문서검색 ②구조역산) 교차 — **14건**, 10건을 넘어 그 자체가 보고 대상. 의존순서 1층(①배수4축동점→②조건부4칸→③sectorSource버그, Q1켜기 전 필수·서로전제) → 2층(④Q1_ENABLED승인) → 독립 10건(⑤REVDCF_ENABLED승인·⑥ⓛdisagree·⑦ⓞ배율표시·⑧Piotroski조건부범위·⑨AltmanZ착수·⑩BeneishM칸신설·⑪저변동성자산성장처분·⑫G7소속·⑬🆕재료부족3건H-4경로·⑭🆕반대방향표시). **하나도 판정 안 함.**

**⑥ ROADMAP_V2.md 반영**: F-4-3 재료열 통일+순위 재산출, **F-5 신설**(판정대기 14건 한장) + 헤더 판정대기표에 F-5 안내 추가. `roadmap_v2.html` 전부 동기화.

산출물 = `docs/probe_1046_material_and_decisions.md`. **게이트**: 게이트7·9 해당없음. **못 한 것**: STEP871 야후컨센서스 재현(원자료 없음, 재조사 범위밖) · verdict 4종 전체 교차표(value_destroying 축만 봄). **철회·정정**: 배당·성장 재료표기 2건(위 ①). **미측정**: 부호상충10.7% 오늘재현 · EV/Sales 재료% · 배당파이프라인 방치 원인 · 성장 보조축(D&A전영업이익) 재료%.

🔴 **판정 금지 — F-5의 14건 전부와 3중규칙 "아직 안 함"·"미측정" 하나도 이 STEP에서 정하지 않았다. 전부 장은태 판정 대상.**

---

## 2026-08-16 — 🔵 **STEP1045: Q3(성장) 복원과 하류 재적용**

> 코드 diff 0 · DB 쓰기 0 · 화면 변경 0. STEP1044가 발견한 "Q3이 재정의 구조에서 통째로 빠졌다"의 뒤처리 — 신설이 아니라 복원. 새 조사는 `LENS_DISPOSITION` 원문 재열람뿐(모델·수요는 새로 안 찾음).

**① 역방향 대조(표 첫 열=Q)**: `STATE.md` Q0~Q5 전부 + `LENS_DISPOSITION`§4 제안 6개(momentum·technical·quality·valuation·fscore·lowvol/assetgrowth) 전수 대조 — **Q3 하나만 빠져 있었다.** quality→Q3 제안은 "빠진 것"이 아니라 "제안됐는데 검토 안 된 것"으로 성격이 달라 별도 처리(④).

**② 칸 질문 확정(후보 보기 전)**: `CLAUDE.md:217`의 철회(이익성장 주축→매출성장 주축, `chan_karceski_lakonishok_2003` 근거)를 먼저 읽고 "매출이 커지는가(주축)+감가상각전 영업이익도 함께(보조)"로 확정. **"5년"의 근거는 원전에 없음** — `revdcf_results.sales_growth`가 이미 5년 창으로 계산돼 있는 재료 재사용·내부 정합성이 근거이지 학술 근거가 아님을 명시.

**③ 층 도출(두 방법 교차)**: DB실측 — 섹터별 매출성장 중앙값 3.2%(Materials)~10.0%(Consumer Discretionary) 분산 확인(업종 의존 있음), 446개 유효값 중 55개(12.3%) 마이너스 성장. 🔑 **최근변화(모멘텀)와의 상관계수 r=−0.10**(445종목, 오늘 DB) — 통계적으로 독립. 논리 — CAGR은 시작·끝값 존재를 전제(값 선행)하나 스케일이 회계연도 단위라 최근변화(매일)와 원리적으로 다른 시간축. **결론: 시간축이되 최근변화와 다른, 두 번째 독립 시간축**(probe_1040의 구조를 시간축 1종→2종으로 확장).

**④ 모델 확정**: 기준①(원전 목적 일치) — `USER_QUESTIONS`§Q가 이미 "원전 없음→규칙5-1·5-2"로 확정해 둔 것을 재확인. `chan_karceski_lakonishok_2003`은 원전이 아니라 반대증거(어느 지표가 지속성 있는지 가르는 용도). **새 모델을 고르는 문제가 아니었음** — C-1 항목1을 정의공개표로 대체, 기존 정의 그대로.

**⑤ 하류 재적용(전수)**: `probe_1041`의 22개(안 붙는 모델) 전수 재검토 — 값매기는모델10·팩터7·추정치3·위험성과5·기타4(Rule of40이 가장 근접하나 SaaS한정으로 불일치)·퀀트분식2, **성장 칸에 새로 붙는 것 0개**(63개 우주 자체의 공백). 🔑 **GP/A·CMA 재판정**(`LENS_DISPOSITION` 원문 §2·§3 직접 재열람, 요약 재인용 안 함): quality→Q3 제안의 유일 근거는 SWS "Past Performance"(ROE·ROCE·ROA — **수준 지표**)인데 Q3 정의(매출·이익 **CAGR, 변화율**)와 원리적으로 다르고, §3 원문 자체가 이미 "GP/A≠ROA" 캐비어트를 달아뒀음. CMA는 §3 원문이 이미 "Q3과 다른 축"이라 명시(총자산 증가율, 대상 자체가 다름). **결론: 둘 다 안 붙음 — 기존 제외 판정은 정정이 아니라 재확인**(근거만 더 정밀해짐).

**⑥ ROADMAP_V2.md 반영**: W-1·W-2-3(시간축 2종 확장) · W-2-4(성장 행 추가+STEP1044 ⓑ문구 "해소"로 정정) · F-4-3(성장 행 추가, "재료완성·화면미착수" 별도범주). `roadmap_v2.html` 동기화. `STATE.md:75` Q3 행 갱신.

산출물 = `docs/probe_1045_growth_restore.md`. **게이트**: 게이트7·9 해당없음. **못 한 것**: `chan_karceski_lakonishok_2003` PDF 재열람 안 함(기존 인용 신뢰, 47p 판독 완료 기록 있음) · Rule of 40 원출처 재검색 안 함(기존 "업종한정" 판정 인용만). **철회·정정**: STEP1044의 ⓑ 등재 문구("Q3 칸 신설 여부")를 "복원 완료"로 정정. `probe_1043`·`1041`의 GP/A·CMA 제외는 재확인됐을 뿐 정정 안 됨. **미측정**: 매출축 AAII수요 44%가 정확히 Q3 정의와 일치하는지 재확인 안 함 · 감가상각전영업이익(보조축)의 업종·이익상태 의존성(오늘은 매출축만 실측) · 관문① 강도의 상대비교.

🔴 **판정 금지는 "아직 안 함"·"미측정"뿐. 나머지(층 도출·모델 확정·22개 재검토·GP/A·CMA 재판정)는 이 STEP에서 실행 완료. 최종 승인(성장 카드 착수 시점)은 장은태.**

---

## 2026-08-16 — 🔵 **STEP1044: 로드맵 전수 대조 — 쌓인 어긋남을 모은다**

> 코드 diff 0 · DB 쓰기 0 · 화면 변경 0. `_TEMPLATE.md` ⓪-1a 신설 규칙("로드맵을 항상 열어보게")의 첫 적용이자 지금까지 안 열고 만든 것의 뒤처리. 새 조사는 §1-3(Piotroski 원전 확인) 하나만, 그마저 파일이 없어 기존 내부기록(`LENS_DEV_PLAYBOOK` #48)을 인용 — 새로 검색 안 함.

**① 층별 주장 목록화(여섯 층 전부, 건너뜀 없음)**: WHY 0건(가치서술뿐)·HOW 4건(전부검증, 어긋남0)·WHAT 8건·무엇을먼저 9건·완성의정의 2건(전부검증, 어긋남0)·수익모델 4건 — 총 27건 중 6건이 ⓐ(사실오류), 2건 ⓑ(판정필요), 1건 ⓒ(미측정), 1건 어긋남아님 확정.

**② 씨앗 4건**: **ⓐ**(F-4-3 "F-Score+GP/A" vs `probe_1043` "Piotroski단독") — probe_1043이 정본, F-4-3이 이전 STEP(1041) 요약을 안 갱신한 것으로 확정, 정정. **ⓑ**(F-4-4 "6일뒤처져" vs route.ts 설계주석) — 🔑 **코드로 직접 확인**: `computeAndSaveSectorRelative()`(같은 route.ts 파일)는 이미 `us_sector_wide`를 "최신 as_of"로 조회하는 정답 패턴(STEP973·974 의도된 설계)을 쓰고, `app/api/q1/[symbol]/route.ts`만 정확일치 조회라 어긋난다 — "신선도 정책 판정"이 아니라 "조인 키를 기존 정답 패턴으로 교체"가 처방, 진단 정정. **ⓒ**("7렌즈" vs `lib/lenses.ts` 6키) — `lib/fscore.ts`(별도모듈)+`lensCopy.ts`(7번째 키 `fscore` 명시 보유) 확인, **어긋남 아님**으로 확정. **ⓓ**(probe_1043 Piotroski "일치" 판정) — §1-3에서 처리.

**③ 🔑 원전 확인(Piotroski 2000)**: `data/sources/`에 PDF 미보유(⓪ 원전 인벤토리 규칙 미이행, URL 인용뿐이었음) 확인, 새로 안 받고 `LENS_DEV_PLAYBOOK.md` #48(STEP818)의 기존 원문대조 기록 인용 — "표본=고B/M 가치주 최상위5분위·소형중형 소외주 집중·대형 약함(우리는 전 종목)", "등급 원전=8~9/0~1(우리 자체밴딩 ≥7/≤3)". `LENS_ROADMAP.md:20`("F-스코어 🟡건전성 해석만, 수익신호아님 t0.70") 직접 열람 — **반증 아니라 뒷받침**(우리 시스템이 이미 F-Score를 건전성용으로 등급매김, probe_1043의 칸 질문과 방향 일치). **결론**: probe_1043의 "일치" 판정을 **"부분 일치(조건부)"로 정정** — probe_1043 자신의 지침 예시(Piotroski=조건부)를 정작 판정 때 스스로 안 지켰던 것. 최종 모델 선택(Piotroski 단독)은 불변.

**④ 역방향(실물→로드맵)**: 🔑 **Q3("커지고 있나")이 `probe_1040`의 0층·1층·시간축 어디에도 없다** — `STATE.md`는 이미 "매출5년CAGR+감가상각전영업이익"으로 확정해 뒀고 `LENS_DISPOSITION`§4 최선안이 quality(GP/A)를 Q3에 편입하자고 제안까지 해뒀는데, STEP1040~1043 재정의 작업 전체가 Q3을 건드리지 않았다 — **방법①(로드맵→실물)만으론 절대 못 잡는 종류**(로드맵에 없는 걸 로드맵에서 찾을 수 없음), `STATE.md`를 먼저 열어야 보임. ⓑ로 등재.

**⑤ 추가 발견(전수 스캔 중)**: `USER_QUESTIONS §558`(존재하지 않는 섹션 — 실제로는 §8의 558번째 줄)·`BRAND_IDENTITY §33`(존재하지 않는 절 — 실제로는 33번째 줄) — **같은 발생 경로 2회 반복**(줄번호를 §로 오표기). M-1 "T3=T4 명시 등급" — 원문은 "중"·"중·장기"로 다른 정성 라벨, "명시"는 과한 표현. "dividends 60행(1,977종목 중)"의 1,977 근거 불명(`MODEL_UNIVERSE_63`§0, 08-07, 4개 문서에 걸쳐 검증 없이 반복 인용됨) — ⓒ 미측정으로 등재.

**⑥ ROADMAP_V2.md 반영(ⓐ 6건만 정정, ⓑ는 등재만)**: F-4-3 재무건전성 행 정정 · F-4-4 sectorSource 진단 정정 문단 추가 · W-2-4 Piotroski 행 "일치"→"부분일치" + Q3 판정필요 신설 · W-2 AAII 인용 §558→§8(line558) · M-4 BRAND_IDENTITY §33→§0(line33) · M-1 "명시 등급"→"정성 라벨을 대략 등급화". `roadmap_v2.html` 전부 동기화.

산출물 = `docs/probe_1044_roadmap_audit.md`. **게이트**: 게이트7·9 해당없음(코드·DB 무변경). **못 한 것**: W-3의 5개 라우트 실존을 코드로 재확인 안 함(자명하다고 판단, 과거 여러 STEP에서 반복확인됨) · `DATA_SOURCE_CATALOG.md` 슬롯 #14·#19·#20 이력을 본문까지 재대조 안 함(요약문구만 대조). **철회·정정**: 위 ⑥의 6건. Piotroski 최종 모델 선택은 불변. **미측정**: "1,977종목"의 정확한 오늘자 대체값(재측정 필요, 오늘 `us_valuation`은 5,820으로 이미 다름) · H-5의 T5 서술이 M-1(2026-08-15 완전제거)보다 옛 상태를 두고 있는 것(사실오류 아니라 서술시점차이로 판단, 정정 안 함, 기록만).

🔴 **판정 금지는 ⓑ 2건(Q3 칸 신설 여부)과 "1,977" 재측정뿐. 나머지(ⓐ 6건 정정·씨앗ⓒ 확정)는 이 STEP에서 실행 완료.**

---

## 2026-08-16 — 🔵 **STEP1043: 각 칸의 최종 모델 확정 (①원전목적 → ②원전재현 → ③전문가실사용)**

> 코드 diff 0 · DB 쓰기 0 · 화면 변경 0 · `Q1_ENABLED`/`REVDCF_ENABLED` 무접촉. 원전 재검색은 저장소 미확보분에만 함(대부분 STEP1040/41/42가 이미 확보한 것 재인용).

**① 칸 질문을 후보 보기 전에 먼저 씀**(`probe_1037`§1-5·`USER_QUESTIONS`§2 기존 정의 그대로 인용, 신규 창작 없음): 배수="비싼가·싼가"·재무건전성="개선되는가·악화되는가"(수준 아닌 **방향**)·시장내재기대="주가가 무엇을 가정하는가"·배당="얼마나·얼마나 오래"·부도위험="망할 위험 있는가"·최근변화="최근 무엇이 달라졌는가".

**② 🔑 기준①(원전 목적 일치) 대조 — 후보 전수(배수12·재무건전성3·시장내재기대1·배당4·부도위험2·최근변화7)**: 배수 칸은 12개 전부 **①일치**("같은 질문의 다른 분모"라 Damodaran 원전 자신이 이렇게 규정 — 칸이 안 갈림). 🔑 **재무건전성 칸에서 기준①이 실제로 작동**: Piotroski F-Score(방향 신호, 원전 그대로 일치) vs Novy-Marx GP/A(`LENS_ROADMAP.md:22`가 이미 "수익률 예측 팩터·독립 프리미엄"으로 분류 — **다른 질문**) vs DuPont(설명·분해 도구 — **다른 질문**) → **F-Score만 생존**. 배당 칸: 배당수익률·연속증가연수=일치(두 하위축), 총주주환원수익률=원전상 부분일치이나 **저장소 기존판정**(`LENS_ROADMAP.md:45` STEP551 백테스트로 이미 탈락, 가치HML 재포장)으로 제외, DDM/Gordon=**불일치**(적정주가류=값을 매기는 모델, 관문②에서 이미 탈락한 것과 동일 성격 — 배당 칸이 아니라 "만들지 않는다" 목록).

**③ 기준②③(재현·사용률) — 살아남은 것에만, 오늘 DB 실측**: 배수 칸(①로 안 갈림) — EV/EBITDA·P/E·P/B·PSR **4개 동점**(②이미 us_valuation에 완성 + ③MARKET_MODEL_USAGE_TOP20 개별사용률 확보, 나머지 8개는 개별사용률 근거 없음) — 🔑 **`STATE.md`의 기존 "C안"(주축PER·보조EV/EBITDA·PBR·적자대안PSR) 구성과 독립적으로 동일 결론 도달(교차검증)**. 부도위험 — Altman Z 확정(③에서 Ohlson 앞섬)이나 **오늘 재확인: `Assets`·`Liabilities` 컬럼 여전히 없음 → 지금 만들 수 없다**로 명시. 최근변화 — 모멘텀12-1+기술(RSI+MA200) 이미 완성·확정, MACD·볼린저밴드·OBV·IBD RS는 ②비용+③개별근거없음으로 후순위(동점 아니라 실제로 갈림).

**④ 칸별 확정**: 배수=동점4개(판정대상) · 재무건전성=Piotroski F-Score · 시장내재기대=역DCF · 배당=배당수익률+연속증가연수(보완) · 부도위험=Altman Z(재료부재로 보류) · 최근변화=모멘텀+기술(보완). "여럿이면 같은질문 다른방법(배당·최근변화=보완) vs 같은질문 각자완결(배수=동점)"을 구분해 명시.

**⑤ 구조 잔여 3건(판정 재료만, 미판정)**: Beneish M-Score — 원전목적("회계조작탐지")이 재무건전성·부도위험 어느 칸과도 다른 **세 번째 질문**, `MODEL_ROSTER.md`§3 임계값 정본 미확정+재현비용 최고등급+사용률 전문가 근거 0(개인담론중심) 재인용. 저변동성·자산성장 — 원전(BAB·CMA)이 "포트폴리오 구성 기법·종목단위 산출물 아님"이라 명시하는데 `lib/lenses.ts` lowvol/assetgrowth 렌즈가 종목별 백분위로 **변형해 라이브 서빙 중**임을 코드로 확인. 0층(업종·이익상태) — 후보0개 재확인, 모델 아니라 데이터·상태값(Q0 2-of-2합의제·NEGATIVE_EARNINGS 사유코드).

산출물 = `docs/probe_1043_model_final.md`. `ROADMAP_V2.md` W-2-4 신설 + `roadmap_v2.html` 동기화.

**게이트**: 게이트7·9 해당없음(코드·DB 무변경). **못 한 것**: DuPont·MACD·볼린저밴드·OBV의 정밀 원전 출판연도 검색(목적 서술만으로 판정 충분해 생략). **철회·정정**: 없음(신규 도출, 배수 칸에서 기존 STATE.md 결정과 독립 재확인). **미측정**: PSR 개별 실사용률(카테고리 수치뿐) · IBD RS 계산식 공개도 원전 재확인 안 함 · DDM 재배치의 화면 영향(표시 설계, 범위 밖).

🔴 **판정 금지는 "아직 안 함"·"미측정"뿐. 칸별 확정·잔여3건 재료는 도출 결과이며 최종 승인은 장은태.**
...
**④ 선행층 분리(1-4)**: `financials` 테이블 오늘도 0행이나 **`MODEL_UNIVERSE_63` §3-1("이 하나가 1~8위 전부 푼다")은 8~11위에만 유효하게 축소** — 1~7위는 별개 파이프라인(`us_fundamentals`, 역DCF가 깔음)이 이미 채웠음(부분 철회). 0층(업종·이익상태) — **0층이 모델보다 선행**(P/E는 이익상태43.4%, 재무건전성은 업종·Financials 70.9%에서 가장 강하게 흔들림), `Q1Section`이 이미 `us_sector_relative` 백분위를 함께 서빙하는 설계인 것이 이 판단이 실무에 이미 반영된 증거.

**⑤ 구순위 대조(1-5, 도출이후에만)**: 유지(1~3위)·상승(P/S계열 5→4, 역DCF 16→5 — **같은 재료·다른 정렬규칙**이 바꾼 사례, ⓪-4 둘째조건과 일치)·하락(이동평균 4→7, 배당 6→8, DuPont 7→10)·통합(Piotroski+GP/A→재무건전성, RSI등→최근변화). **KR근거 재판정**: 이동평균의 "KR·US유일즉시"→"US즉시가능(7렌즈로 이미구현)"·배당의 "KRX공표"→"AAII73%"로 근거 교체(판정은 유지, 순위는 재료갭으로 하락).

**⑥ F-4-1②에 답함(1-6)**: "1위가 이미 있는가 — 예." "제대로"의 정의를 구체화 — 계산 미완이 아니라 **①육안검증②0층(이익상태) 화면표시 미확인③REVDCF_ENABLED 별도 크론여부 미결(KNOWN_ANSWERS)** 3가지.

**⑦ 정정 3건 반영**: 2-1 `ROADMAP_V2` W-2-2(전문가사용률="타깃어긋남"→"판단에 실제로 드는 것의 증거", F-1-1과의 모순 해소) · 2-2 `probe_1040` 1-7①(43.4%>32.5% 크기비교→범주차이로 정정, 결론은 유지) · 2-3 `ROADMAP_V2` W-2-3("측정된 상호의존 0건"→"미측정, 측정방법 자체를 못찾음", LENS_DISPOSITION §2 인용).

**⑧ `ROADMAP_V2.md` "무엇을 먼저 만드는가"(281행) 갱신**: 헤더상태 "관문확정·순위대기"→"관문확정·모델순위도출(STEP1041)·최종승인대기", **F-4-3 신설**(순위표+F-4-1②답+선행층). `roadmap_v2.html` 동기화(F-4-3 신설+badge 갱신+정정2건 동기화).

산출물 = `docs/probe_1041_model_selection.md`. **게이트**: 게이트7 재현검사 — 기존965/967/969의 `/tmp`import 발견(이STEP 무관, 코드diff0이라 손대지않음). 게이트9 해당없음.

**못 한 것**: 22개 안붙는모델 개별 0층 재검토(일괄제외) · Ohlson·Altman 원전 재검색(인용만). **철회·정정**: 위 ⑦ 3건 + `MODEL_UNIVERSE_63` §3-1 부분철회(8~11위에만 유효). **미측정**: 부도위험·DuPont의 0층 적용성(모델 미구축이라 실측불가) · Q1_ENABLED 활성화 시 실측(운영데이터, 범위밖).

🔴 **판정 금지는 "아직 안 함"·"미측정"뿐. 나머지(순위·선행층·구순위대조·F-4-1②답)는 도출 결과이며 최종 승인만 장은태.**

---

## 2026-08-15 — 🔵 **STEP1040: WHAT 재정의 — 층 A·B·C 도출(의존 실측 9쌍) + ROADMAP_V2 W층 갱신**

> 코드 diff 0(문서 전용). DB 읽기는 실측을 위해 함(Supabase MCP `execute_sql`, `ccbwxcszdoyjxvckedfp`). 선택지 나열 금지 — 도출된 하나의 답을 낸다.

**① 층 A(재조사 없음)**: `probe_1037`§1-5+`probe_1038`§1-5를 정보 단위로 옮김 — 배수·재무건전성·최근변화·업종대비위치·배당·시장내재기대(역DCF)·부도위험 7항목, `ROADMAP_V2` F-4의 관문①②③ 통과 여부 인용.

**② 🔑 층 B — 의존 9쌍 실측**(`lens_scores`×`lens_cuts`×`sector_cuts`×`us_valuation`×`revdcf_results` SQL 직접 조회, 2026-08-15): 업종→배수 32.5%(302/928, 기존33.0% 갱신)·업종→재무건전성(quality) 34.0%(297/874, 기존33.8% 갱신)·업종→재무건전성(F-Score, 비교체계 다름 주의) 37.4%·업종(Financials)→재무건전성 **성립불가 70.9%**(원전 Novy-Marx 2013 금융업제외와 실측 일치)·업종→최근변화(momentum) 19.0%(기존20.1%, CLAUDE.md 시점효과 경고 유지)·업종→최근변화(technical) 43.9%·업종→시장내재기대 **실측불가**(섹터컷 11개 중 1개뿐, n=32)·이익상태(적자)→배수(PER) **43.4%**(1,584/3,651, 데이터 있는 표본 중)·이익상태→시장내재기대 약함(skip_reason에 적자 사유 없음, NOPAT 정의상 논리적으로도 약함)·값→변화(모멘텀) 자명(논리, Jegadeesh-Titman 1993 12-1 정의)·배당→이익상태 **실측불가**(`payout_ratio` 60/60 전부 NULL).

**③ 그래프→층**: 순환 없음. **0층(전제)=업종+이익상태**(🔑 이익상태는 신규 — Q0 근거서술 "적자성장주 PER 성립안함"에 섞여 있었을 뿐 별도 카드가 없었음) → **1층(정보, 상호 순서 없음)=배수·재무건전성·시장내재기대·배당·부도위험** → **시간축=최근변화**(1층 파생 아니라 가격시계열 자체 전제).

**④ Q0~Q5 대조(도출 이후에만)**: 정보 매핑은 거의 전부 유지(Q1=배수+역DCF·Q2=배당·Q4=재무건전성+부도위험·Q5=최근변화). 🔑 **재정의의 실질은 두 곳**: (a) 업종대비위치는 독립 8번째 항목이 아니라 업종(0층)을 1층에 적용한 **표시형식**임을 확인 — 실측·Q0기존결정("카드아님·각카드에녹임")·실무4곳조사(Stock Analysis·WallStreetZen·MarketBeat·Robinhood, 전부 업종비교를 독립카드로 안둠) 셋이 일치. (b) 이익상태라는 미분리 전제 발견(위 ②).

**⑤ 층 C**: 방법론공개는 Layer A 항목이 아니라 **모든 항목에 붙는 신뢰층**(Model Card 9섹션). `lensCopy.ts` 대조 — 5개 채움(원전·용도·표본·평가데이터·한계) · 3개 빈칸(모델버전/날짜/문의처·주사용자·Ethical Considerations) · 1개 해당없음(Training Data, ML모델아님).

**⑥ ①-B 실무(재조회 없음, 인용만)**: `probe_1037`·`probe_1038`·`Q1_CARD_DESIGN`§2의 Stock Analysis·WallStreetZen·MarketBeat·Robinhood 4곳 — 전부 업종비교가 배수(PER)에 붙는 배율/문장 병기이지 독립 카드가 아님. `link_hub` 병행은 `Q1_CARD_DESIGN`§2가 이미 수행(16곳 후보·4곳 실조회).

**⑦ ROADMAP_V2.md W층 갱신 + html 동기화**: W-1 ①상태를 "재검토 필요"→"✅ 도출 완료"로, W-2-1에 **재정정 블록 추가**("Q시리즈는 정보의 그룹핑"은 절반만 맞음 — Q0 근거서술이 이미 의존순서를 담고 있었으나 업종·이익상태 두 전제가 하나로 뭉쳐 있었을 뿐), **W-2-3 신설**(도출 결과 요약). `roadmap_v2.html` 동기화 + 헤더 "판정 대기" 목록에서 "Q시리즈 재검토" 행 제거(4건→3건).

산출물 = `docs/probe_1040_what_structure.md`. **근거 없음—판정 필요로 명시한 것 3건**: 업종→시장내재기대(표본부족)·배당→이익상태(재료갭)·부도위험의 업종의존(모델미구축, 원전 재검색 안 함).

**게이트**: 게이트7(미추적파일참조) 재현검사 실행 — 기존 965/967/969의 `/tmp` import 3건 발견되었으나 **이 STEP이 만든 게 아니고 코드 diff 0**이라 손대지 않음(범위 밖). 게이트9(신선도점검등록) 해당없음(새 테이블·크론 없음). HTML은 파서 검증만 함(`html.parser` 무오류) — **브라우저 육안검증은 못 함**(도구 부재), 3중규칙 "못한축"에 명시.

**못 한 것**: 부도위험(Altman Z) 업종 배제 여부 원전 재확인(`MODEL_ROSTER`§3 인용만, 재검색 안 함, ⓪-1b 재조사금지 준수) · link_hub 신규 조회(기존 기록 인용만). **철회·정정**: momentum 20.1%→19.0%·valuation 33.0%→32.5%·quality 33.8%→34.0%는 표본 변화에 따른 **갱신**(방향·결론 불변, 정정 아님). **미측정**: 1층 항목 간 상호 의존(배수↔재무건전성 등, 측정 방법 자체를 못 찾음) · fscore/technical의 "고정임계 vs 섹터tercile" 비교가 실제 판정변경 이력과 다른 크기 차이의 원인.

🔴 **판정 금지 항목은 "근거없음 3건"·"미측정"뿐. 나머지(0층 구성·1층 매핑·업종대비위치 재배치·Layer C 정체)는 도출 결과이며 최종 승인만 장은태.**

---

## 2026-08-15 — 🔵 **STEP1039: 완성 기준 정정 동기화(C-2③·M-1) + DoD9 US 2종목 재판정 + KR 전제 전수 색출**

> 코드 diff 0(문서 전용). 세 지시를 순서대로 처리 — ①ROADMAP_V2.md↔html 동기화 ②LENS_COMPLETION_STANDARD.md 항목9 정정+보류 해제 ③docs/ 전체 KR 전제 전수 색출.

**① C-2③·M-1 동기화**: `ROADMAP_V2.md`는 이미 정정돼 있었다(C-2③ "노출 여부"→"실제 분포 값 min·p30·중앙·p70·max", M-1 규모 등급 T1>T2>T3=T4>~~T5~~ 복원). `roadmap_v2.html`에 두 곳 모두 동기화하고, header "남은 판정 대기 항목" 표에서 이 둘이 애초에 별도 행으로 없었음(요약 오류였지 판정 대기 항목이 아니었으므로)을 확인 — 손댈 행 없음.

**② DoD9 정정 + 보류 해제**: `LENS_COMPLETION_STANDARD.md` 항목9 "KR·US 각 2종목"→**"US 2종목"**으로 정본 정정(`CLAUDE.md:31` 근거). 이 문구 때문에 "구조적 불일치·판정 보류"로 잠겨 있던 항목을 전수로 찾아 해제·재판정 — Q0 항목9(요약표+상세표)·Q1 항목9(요약표) 총 4곳, 전부 🟡보류해제(⬜는 유지 — 구조적 불일치만 소멸, 라이브 실측 자체는 미실시). `STATE.md` 미결 20번도 ✅해소로 갱신(`DECISION_929_DOD_SCOPE.md`는 2026-08-06 시점 사실 기록으로 보존). `roadmap_v2.html` header 판정 대기 목록에서 "완성 기준 항목9 KR·US 표기" 행 제거(5건→4건 — KR·US 표기 항목은 원래 이 목록 1개 행뿐이었고, C-2③·M-1은 ①에서 확인한 대로 애초에 이 목록에 없었다), C-3 ㉠·C-1 항목9 정의 동기화.

**③ KR 전제 전수 색출**: `_TEMPLATE.md` ⓪-5 「전수의 조건」에 따라 두 방법을 교차 — 방법①(검색어 8개: `KR·US`·한국·KR·`kr_`·코스피·원화·국가탭·언어권, `docs/*.md` 1,108개 중 매칭 569개→STEP/probe류 300개는 장르 전체 ⓑ 일괄분류→"살아있는 문서" 82개 개별검토) · 방법②(`STEP_*`/`probe_*` 제외 113개 문서를 2026-08-08 전후 날짜로 나열). 교집합에서만 ⓐ 후보가 나옴(전수 조건 충족). **ⓐ(즉시 고침) 7건**: `STATE.md` 미결20번(②와 동일건)·`LENS_COMPLETION_STANDARD.md`(②와 동일건)·`RELEASE_ROADMAP.md`(INDEX가 "[최신·핵심]"인데 "1차=한국 완결" 전제 — 동결 배너 신설)·`LAUNCH_PLAYBOOK.md`(동일 패턴, "한국탭 완성→공개" 제목)·`ETF_LENS_PLAN.md` §1(KR ETF 항목만 인라인 동결)·`ABOUT_REWRITE_DRAFT.md` §5("6개 시장" 카피 정정표시)·`LOGO_PROMPT.md`(타깃 문구 정정표시). `INDEX.md`도 해당 행마다 갱신. **ⓑ(보존) 약 68건**: 이미 🅿️/[보존]/[아카이브] 표시됨·원문 인용 사실기록(DECISION_*)·자체철회완료·"KR은 동결이지 삭제 아님"이라 여전히 사실인 서술(KR 크론 3개·렌즈 976종목 등)로 세분류 확인, 무수정. **ⓒ(애매·목록만) 7건**: `COUNTRY_TAB_PLAYBOOK.md`·`LOCALE_SOURCE_PLAYBOOK.md`(범용 방법론 vs 착수금지 규칙 긴장, 장은태 판정 대상) 등.

산출물 = `docs/probe_1039_kr_premise_sweep.md`. **못 한 것**: STEP/probe류 300개 개별 내용 대조(장르 일괄분류로 대체) · ⓒ 7건 판정(목록만). **철회·정정**: 없음(①②는 이미 완료된 정정을 검증·동기화). **미측정**: 키워드 8개 밖에서 KR을 암묵 전제로 까는 표현(예: 한국어 화자 가정) — 이 스윕의 원리적 한계.

🔴 **판정 금지(③의 ⓒ 항목만) — ①②는 이미 정정 완료 사항의 동기화·재판정이라 판정이 아니라 집행.**

---

## 2026-08-15 — 🔵 **STEP1038: 채널 역추적 + 유료 시장 역추적(관문① 근거 보강) + roadmap_v2.html F-4 동기화**

> 장은태 지적: "채널만 쟀다면, 그 채널에서 어떤 정보를 주는지는 왜 검색 안 해?" · "유료 플랫폼 시장을 누가 장악하고 있는지도 검색해볼 내용 아냐?" — 1037이 멈춘 자리를 이어감. 🔵코드·DB·크론 미접촉·판정금지.

**① roadmap_v2.html F-4 동기화**: `ROADMAP_V2.md` F-4가 1037 실측을 반영해 (통과표+F-4-1+F-4-2)로 크게 확장돼 있었는데 `.html` 미러는 이전 스텁 그대로였음 — 표·F-4-1(3개 발견)·F-4-2(2개 판정대기)를 전부 동기화, Playwright로 렌더 확인(누락 클래스 0·콘솔에러 0).

**② §0-B 1-0 여섯째 축 탐색(본조사보다 먼저 실행)**: "방법 목록 자체를 여는 것"을 먼저 시도 — Google Trends 개념어비교(**429 차단**)·Ahrefs 키워드볼륨(404)·Reddit 커뮤니티 원문(검색엔진 미색인)·앱스토어 리뷰(**실조회 성공, 그러나 UX불만뿐 정보유형 신호 없음**)·Amazon 베스트셀러(**503 차단**)·SEC 투자자FAQ(범주가 다름, 사기예방 축) 6후보 전부 시도. 🔴 **실행 가능한 새 축 0개** — 유망 후보(Google Trends·Reddit)는 개념 결함이 아니라 접근장벽이라 다음 STEP 후보로 이관.

**③ 1-0b 재검수**: 1037 수렴결과·F-4 순위표·F-4-1③(방법론 비공개) 전부 **유지**. F-4-1③은 유료 최상위($2,995/년 Zacks Ultimate)까지 포함해 반증되지 않고 오히려 강화.

**④ 방법D(채널 역추적)**: 증권사 4곳 실조회(Robinhood🟢완전공개·Fidelity🟡개괄만·Schwab🔴접근거부·Interactive Brokers🔴403) — "증권사 73%"는 브로커마다 공개도가 크게 다름을 확인. 동영상(저평가종목 장르 반복 확인)·앱인기종목(정보유형 아니라 티커인기, 범주밖으로 기록)·소셜/finfluencer(로그인벽, 미실행 명시).

**⑤ 🔑 방법E(유료 시장 역추적)**: Similarweb 트래픽 추정으로 7개 플랫폼 규모 순위(Seeking Alpha>Motley Fool>Morningstar>TipRanks>Simply Wall St>Zacks>GuruFocus, Validea 별도) 확보 + 요금제 페이지·서드파티 리뷰로 6곳 유료 티어 내용 확인. **6곳 전원 핵심 계산식("proprietary") 비공개.** 가중 목록: 종합등급(15점 1위)·심층스크리너(13점 2위)·적정가(9점 3위) — 적정가·종합등급 둘 다 F-4 표의 기존 두 행(관문②③ 탈락)과 정확히 겹침, 새 정보 없음. 🔴 **`BRAND_IDENTITY §33`의 5곳 대조 — Seeking Alpha(1위)·Morningstar(3위)만 상위, GuruFocus(6위)·Simply Wall St(5위)·Validea(**최하위, 7K/월 — 다른 후보 대비 1/75~1/800 규모**)는 규모 하위. §33에 없는 Motley Fool(2위)·TipRanks(4위)·Zacks(6위)가 §33 절반보다 큼 → §33이 규모 근거로 뽑힌 목록이 아님을 확인(판정 아님, 사실 보고).**

**⑥ 1-5/1-6**: 소비 정보 목록을 1037과 대조 — 겹치는 것 6개(배수·적정가·애널목표가·재무건전성·배당·방법론공개), **채널에만 있는 새 수요 0개**. 방법론 공개 전수(무료3+유료6=9곳) 전원 비공개 확인.

산출물 = `docs/probe_1038_channel_backtrace.md`. `docs/probe_1037_retail_demand.md`에 이어지는 조사 링크 추가(1037 결론은 무수정). `INDEX.md` 등재.

**못 한 것**: TipRanks·Zacks 종목페이지 재조회(1037에서 이미 403 확인, 재시도 안 함) · 소셜미디어그룹·finfluencer 콘텐츠 원문(로그인벽) · Motley Fool 무료/유료 화면 스크린샷 비교. **철회·정정**: 없음. **미측정**: Google Trends/Reddit 원시 데이터(다음 STEP 후보로 이관) · 유료가입자 수(Motley Fool 외 전부 불명) · 경제기사·자문가·지인·TV·팟캐스트·유료뉴스레터 6채널(조사 불가 명시).

🔴 **판정 금지. 다음은 ROADMAP_V2의 남은 두 층(완성의 정의·수익 모델)이다.**

---

## 2026-08-15 — 🔵 **STEP1037: 개인 수요 실측(관문① 근거 조사) + roadmap_v2.html F-1-1 캐치업 동기화**

> 코드·DB·크론 미접촉(🔵조사·판정 금지). 관문 ①("사람들이 원하는 정보인가")을 판정 가능하게 만드는 근거 조사.

**① roadmap_v2.html 캐치업**: `ROADMAP_V2.md`에 먼저 추가돼 있던 F-1-1("관문 ①을 좁게 읽지 않는다 — 차이는 계산이 아니라 문장이다") 절이 `.html` 미러에 누락돼 있던 것을 발견·동기화. `.fix` 블록으로 ❌"이 주식은 오를 것입니다" vs ✅"이 주가는 향후 5년 매출 성장률 12%를 가정하고 있습니다" 대비 표를 추가하고 Playwright로 렌더 확인 후 커밋(`bef5bdb`, CI 그린).

**② STEP1037 본조사**: ⓪-1b 재조사 금지 목록(`MODEL_UNIVERSE_63`·`MARKET_MODEL_USAGE_TOP20`·`MODEL_DEMAND_SURVEY`·`MODEL_ROSTER`·`USER_QUESTIONS`·`Q1_CARD_DESIGN §2`) 재확인 후, 세 방법(전수의 조건)을 교차:
- **방법 A(개인 직접)**: AAII 2022-04(4년 전) 의존 탈피를 시도 — FINRA NFCS Investor Survey 4th ed.(2,861명·2024-07~12), SEC THRIVE Panel(6,392~6,653명·확률표본, 보고서 본문 403 차단), WEF·BCG·Robinhood 2024 Global Retail Investor Outlook(미국 1,003명·2024-08 현장조사)을 신규 확보. 🔴 **핵심 발견**: 이 셋 전부 "정보를 어디서 얻는가"(채널)만 측정하고 "무슨 지표를 보는가"(내용)는 측정하지 않음 — AAII가 여전히 유일한 내용측 직접 증거로 남음.
- **방법 B(드러난 선호, link_hub 병행)**: `link_hub` US×(analysis·research) 29곳 중 4곳 실조회 — Stock Analysis·WallStreetZen·MarketBeat·Stock Rover 🟢 성공, Morningstar·TipRanks 🔴 403(즉시 중단·기록, 우회 없음). 공통 패턴(현재가→핵심지표그리드→회사개요) 확인, WallStreetZen·MarketBeat가 적정가·애널목표가를 유난히 이르게 배치.
- **방법 C(역방향)**: 무료로 이미 흔한 것(배수·배당·52주범위·애널목표가·적정가 추정)과 아무도 안 주는 것(계산방법론 완전공개·내용측 대형 설문)을 대조 — 새 축(방법론 공개 여부)을 A·B에 없던 발견으로 추가.

🔑 **교차 결과**: "무엇을 원하는가"는 A·B·C 셋 다 수렴(적정가·배수·배당·건전성·최근변화) → 목록 신뢰 가능. "어떻게 얻는가"는 방법마다 드러나는 정도가 다름(C가 새로 드러냄) — ⓪-4 매트릭스 "개인 직접 증거 3건 미만" 조건은 **채널 축은 충족(3건)·내용 축은 미충족(1건, AAII만)**으로 갈림.

산출물 = `docs/probe_1037_retail_demand.md`(1-1 재분류표·1-2/1-3/1-4 방법별 결과·1-5 원하는 정보 목록[「원한다」/「필요하다」열 분리]·1-6 Q0~Q5 매핑[방법론 공개는 어느 질문에도 안 붙음, 별도 목록화]·차단 목록 전수·3중 규칙). `INDEX.md` 등재, `ROADMAP_V2.md` F-4에 존재만 링크(판정 없음).

**못 한 것**: link_hub 후보 29곳 중 4곳만 실조회(나머지 25곳 미조회) · Statista 상세 데이터(페이월, 무료 스니펫만) · SEC THRIVE 본문 재확인(403). **철회·정정**: 없음. **미측정**: "무슨 지표를 보는가"를 직접 물은 2024~2026 대형 US 조사 존재 여부(못 찾음≠없음, LENS_DISPOSITION §2 전례) · Q0~Q5 그룹핑 재설계 여부(판정 대상, 범위 밖).

🔴 **판정 금지 — 다음은 F-4(순위 재정렬), 이 조사가 그 입력.**
