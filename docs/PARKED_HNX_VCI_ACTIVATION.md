...
> 🅿️ **동결 — 2026-08-08 「🇺🇸🔒 전면 US 단독」 규칙**(`CLAUDE.md`). 이 문서는 **VN 시장 주제**라 신규 착수 대상이 아니다. **내용은 유효하며 삭제·이동하지 않았다.** 🔴 **판정·감사·모델 선정에서 이 문서의 수치를 결론에 넣지 말 것.** ✅ US 완성 후 확장 1순위(한국)에서 재개.
> 🔴 **2026-08-16 STEP1050 정정(기록 신뢰 점검, `docs/probe_1050_sector_source_fix.md`)**: 아래 §"✅ HOSE는 정상(매일 야후로 업데이트)"는 **작성 시점(2026-07-09) 기준으로는 맞았으나 이후 두 사건으로 더는 사실이 아니다** — ① `PARKED_FIELD_SURFACES.md` §7(2026-07-28)로 VN 전체(HOSE 포함)가 검색·관심등록·종목상세·사이트맵에서 제품 표면 자체가 차단됨(HNX만이 아니라 HOSE도 이제 화면에 안 뜬다) ② 위 배너의 US 단독 규칙(2026-08-08)이 그 위에 한 겹 더 얹힘. 아래 본문은 원래 취지(HNX 활성화 절차)를 보존하려고 고치지 않았다 — "HOSE 정상"을 읽을 때 §7이 이미 그걸 덮어썼다는 걸 함께 봐야 한다.
