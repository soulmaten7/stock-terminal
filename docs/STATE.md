...
## HEAD / 배포
- HEAD = **이 커밋(STEP 920)** · 부모(STEP 919 `93b2b15`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 건 = 917 계측 배포 · 다음 크론 관측 대기**(변화 없음 — 아래 "▶ 다음" 00번) **② 결정 대기 3건**(안건 2 = 919 해소[`#17`채택·적용완료 · `#37`·`#43`현행유지] · ③`#67`=①단계로 구조적 해소 예정(값은 다음 실행 후) · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 920 = Cowork 육안 실측 반영 — `/revdcf` 표 한국어 어절 중간 줄바꿈 수정(CSS만·값·문구 불변).** 897·898·911에 이어 **네 번째 "이 세션 도구 한계 ≠ 시스템 한계"**. Cowork이 브라우저로 "원전"·"우리" 열 7곳에서 CJK 글자단위 줄바꿈 발견(898은 "항목" 열만 고쳤고 다른 열은 처리한 적 없었음). `git diff ab12d1e HEAD`로 문제 셀 텍스트가 919 전후 바이트 동일함을 확인 — "919가 만듦"은 배제, "919 무관"과 "919가 사유열을 늘려 폭배분에 영향(table-layout:auto)" 중 하나는 픽셀 재현 불가(이 세션 브라우저 없음)라 미확정. 처방 = "원전"·"우리"·"사유" `<td>` 세 곳에 `break-keep break-words`(항목 열의 기존 `whitespace-nowrap`은 안 건드림 — 좁게 시작). `messages/` diff 0(문구 불변 확인)·`lib/` diff 0. 로컬 dev curl로 클래스 렌더 확인 + "증분 재투자율" 정상 유지 확인 — **픽셀 재검증은 Cowork 몫으로 남김**(이 세션은 CSS 속성의 사양상 결정적 동작으로 대체 확인). `docs/LENS_DEV_PLAYBOOK.md` #92 신규.
- ✅ **STEP 919 = 🟢 안건2 승인 적용 + 905 ④단계 화면 3건.** `#17`채택→driver3 각주 DB배선(다모다란 실효세율 대 한계세율) · `#29`재실측 배선(46.6%는 stale, 현재 43.2%/465) · `#40`maxYears 배선(`app/api/cron/revdcf/constants.ts` 신규, lib/ 밖 상수공유) · `#41`유니버스공개 캐비어트 신설(867 §7-3 초안). `#37`·`#43` 화면 변경 0. `lib/` diff 0.
- ✅ **STEP 917~918 = A안 ①단계 계측 배포(917, `cron_heartbeats.note`) + 안건2 원전대조·권고 제출(918, T6·T8·T2 직접 개봉).**
- ✅ **STEP 912~916 = 라이브 이상징후 진단·설계 5단계**(912 최초진단→913 정정→914 US원인 확정→915 480코호트 규명→916 시간예산·플랫폼상한 실측). 상세 = `docs/CHANGELOG.md` (57)~(61).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~920(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+안건2 원전대조·권고 제출+안건2 승인적용·화면3건+**한국어 줄바꿈 수정**) 전부 유효 — 상세 CHANGELOG. `data/`·`.github/`·`vercel.json`·`messages/` diff 0(911~920) · `lib/`는 **917(계측)에서만 열렸고 918~920은 diff 0**.
