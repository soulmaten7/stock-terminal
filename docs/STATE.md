...
## HEAD / 배포
- HEAD = **이 커밋(STEP 891)** · 부모 `5a30f41`(STEP 890) · **push 완료 — origin/main + revdcf-preview 반영.**
- ✅ **STEP 891 = §1 시총 신선도 실측 → §3 DoD4 적용(✅) — 계산 전용(DB 쓰기 0).** 장은태 890 조건부 승인(2026-08-04) 충족: "모집단"을 실제 표본(604)·목표 조달범위(2,857·미구현)·7렌즈 표본으로 가름. 🔑 **부수 발견(불리한 사실로 병기)**: 604 중 **86사(14.2%)가 4일 이상 묵은 시총**으로 계산 중 — `us_market_cap`이 `symbol` 단일 PK 누적 캐시라 갱신 실패 심볼이 조용히 스테일해짐. 엔진 재현(15사·DB값과 100% 일치 확인)으로 실제 GAP 이동 확인(**CL 12→9년·DCI 12→13년**). 원인 = `revdcf` 크론이 신선도 필터 없이 전량 조회(`lensPrecompute.ts` 자신의 7일 TTL 폴백과 대비) — **결함으로 판정**, 코드 수정은 범위 밖. 스테일 표본 10개 전수 확인 = 전부 야후 정상 조회(상장폐지 아님). DoD2·DoD4 모두 이 발견으로 안 흔들림(판단 근거 = `LENS_COMPLETION_STANDARD.md` "4) 컷·분포" 판정서). 604⊂2,857 교집합은 **미측정**(2,857 종목 리스트 자체가 저장된 적 없음 — 재계산 안 함). 상세 = `docs/probe_891_mcap_staleness.json`·`docs/REVDCF_SPEC.md` A-11.
...
- 866~891(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측+**DoD4 적용·시총신선도 결함 발견**) 전부 유효 — 상세 CHANGELOG. `lib/`·`app/`·`components/`·`messages/`·`data/`·`.github/` 로직 **diff 0**(891은 `docs/`+`scripts/probe_891_mcap_staleness.ts` 신규뿐).
- **tsc 0 · vitest 155/155**(무변화) · `revdcf_results` = 2026-08-01/02/03 각 **604**(무변경 — 크론 미실행) · `us_market_cap` = **5,887**(총량 무변경 — 단 `as_of` 구성은 08-02→08-03으로 일부 이동, 891에서 확인. 쓰기 0).
...
- 878~891: `docs/CHANGELOG.md` (24)~(36) 그대로 — driver5/6/인플레 ③판정 확정, 세율순효과 실측, 문서정본화, 대조표 재분류(887), 표면 전수 감사(888), 표면 교정+DoD6 ✅(889), DoD4 전제 실측+판정서(890), 시총신선도 실측+DoD4 ✅ 적용(891).
