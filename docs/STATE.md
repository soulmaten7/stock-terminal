...
## HEAD / 배포
- HEAD = **이 커밋(STEP 923)** · 부모(STEP 922 `8fef1a7`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 건 = 917 계측 배포 · 다음 크론 관측 대기**(변화 없음) **② 대기 안건은 3번(`#67` 로그값) 하나만**(908 정본) **③ DoD7 = 여전히 🔶 미결**(`years` 권고는 승인됐으나 923이 종목명 불일치를 새로 발견 — `docs/DECISION_923_NAMING.md`, 수리는 별도 승인 필요·이 STEP은 진단만).
- ✅ **STEP 923 = 🟢🔴 922 `years` 권고 승인 · DoD7은 안 닫음(종목명 불일치 실측, 진단만).** §0: Cowork 브라우저 3중검증 — 카드(`/stock/NVDA`)는 922 코드추적과 정확히 일치(정상), 보드 배지는 위치를 못 찾아 미확인. US 탐색 목록에서 「Mo」·「Hst」(티커 title-case) vs 상세 「Altria Group, Inc.」·「Host Hotels & Resorts」(정상) 불일치 발견 — 외부검증(웹)으로 정식 회사명 확인, `/stock/MO` 상세는 정상이라 **데이터 결손 아니라 목록 경로만 못 가져옴**. §1: DoD7 "같은 이름" 원문 재확인 — 판정라벨/종목명 둘 다 가능한 **모호함**(단정 안 함). §2: 5표면 진단 — 목록(Explore lens-top)=`lens_scores.name`(런타임 Yahoo quote 경유) vs 상세=`data/us_symbols.json`(빌드타임 번들), **물리적으로 다른 소스**라 비대칭 구조적. DB 실측: `lens_scores`(US) 998행 중 **348행(34.9%)**이 `name=symbol`(티커 그대로) — 근본원인 `lensCompute.ts:139,147`. `UsMarketBoard`(별도 컴포넌트)는 정상 소스라 무관. 부가판정: Alphabet 중복=설계(티커 미표시)·NVIDIA 영문/엔비디아 한글=설계(`displayName.ts` STEP775/776 규칙)·모멘텀 중복=별도 조립버그(`ExploreClient.tsx:161`). §3: 보류경계 명시(검증=범위 안, 수리=별도승인). §4: DoD7 미결 유지(사유=922 비교범위 밖에서 위반 확인)·`years`승인과 별개사안 명시·수리선택지 A~D(대가 포함)·완성남은것 갱신(DoD7 종목명 추가). **진단만 — 코드 diff 0.**
- ✅ **STEP 921~922 = 안건4 재료완성(921)→승인적용+DoD7 boardBadge.years 재료·권고(922).** DoD9만 노출-순환, DoD7은 판단1건 규명(921). `#29`·`#40`·`#41` 문서미갱신 발견·정정. 권고=DoD9제외8항목=완성, `years`현행유지 권고(922).
...
- 866~923(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+안건2 원전대조·권고 제출+안건2 승인적용·화면3건+한국어 줄바꿈 수정+안건4 재료완성·§10 정정+안건4 승인적용·DoD7 마지막판단 권고+**years 권고 승인·종목명 불일치 진단**) 전부 유효 — 상세 CHANGELOG. `lib/`·`app/`·`components/`·`messages/`·`data/`·`.github/`·`vercel.json` diff 0(923).
...
- 🅿️ 보류: **DoD9(라이브 실측·노출 트랙, 921 분리)** — production 노출 승인 필요. 🔴 **DoD7은 여기서 뺀다**(921 정정 — 노출과 무관). `boardBadge.years` 판단은 922 권고(현행유지)가 923에서 승인됨 — 🔴 **그러나 DoD7은 여전히 🔶 미결**: 923이 종목명 불일치(목록="Mo"/"Hst" vs 상세="Altria Group, Inc." 등)를 실측 발견, `docs/DECISION_923_NAMING.md`. 베타 · 국가탭 확대 · 7렌즈 깊이 확장 — **모델 완성 전 재개 금지**
- 🔴 **923 경계 기록**: DoD7 종목명 진단은 **"7렌즈 깊이 확장"이 아니다** — 역DCF 완성의 필요조건(승인된 정의: DoD9 제외 8항목)이라 확인은 범위 안이다. 🔴 **그러나 목록 화면(7렌즈 보드) 코드를 고치는 것은 별개** — 그 목록은 실사용자에게 나가는 라이브 화면이라 수리는 반드시 별도 승인 후(899·901에서 보류를 넘어 잡힌 전례 있음, 경계를 여기 못박는다). 923은 진단만 했다(코드 diff 0).
