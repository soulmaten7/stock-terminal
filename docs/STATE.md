...
| **③** | **나스닥 스크리너 적재** — 테이블 신설 ＋ 갱신 경로. Storage 원본 업로드 포함 | ✅ **원안 완료(939 ③-1 + 940 ③-2) + 🔴 941 ③-3로 범위 확장(3번째 출처)**. 939: Storage 업로드·좌표 등재·SPDR 정답지 발견(일치율 99.6%). 940: `us_sector_nasdaq`·`us_sector_gics` 신설·적재(나스닥 7,127→7,127·SPDR 515−12→503). **941 ③-3**: 미분류 70건의 93%가 원래 Q0을 시작시킨 종목(ASML·SONY·BABA·TM·BHP 등)이라 3순위 표본도 3건뿐이던 문제 → **야후 `assetProfile`을 3번째 출처로 추가**(`us_sector_yahoo` 신설, 취득 성공 1,020/1,021=99.9%, 매핑표 밖 값 0건). 조합별 정확도(SPDR 503 대비, 대상=lens_scores US 1,021): 나스닥 단독 73.7%·SIC 단독 75.4%·**야후 단독 95.8%**·나스닥∩SIC(현행) 95.3%·2-of-3 다수결 94.8%(결정불가 36)·3-of-3 99.2%. **미분류 70건 중 야후 단독으로 70/70(100%) 회수 가능**(실측, `docs/probe_941_third_source.json`). 🔴 **`lib/sector.ts`는 941에서 미수정** — 합의 규칙을 이 실측에 맞춰 바꿀지는 **942 장은태 판정 대기** |
| **④** | **섹터 내 컷 계산** — p30/p70. 🔴 **표본 확보 섹터만**(하위 6개 26~48종목) | 🔵 **다음 — 942(합의 규칙 판정) 이후.** 940 §5 실측 재료: 섹터별 종목 수(2-of-3 다수결 기준, `docs/probe_941_third_source.json` `sectorCountsUnderMajory`)로 갱신 예정 — 942 판정에 따라 섹터당 종목 수가 다시 바뀐다(미분류 70건 최대 70건 추가 편입 가능). 착수 전 §7 ⓕⓖ 판정 필요는 아래 그대로 |
| **⑤** | **화면 표시** — 「업종 대비」 줄 ＋ **「미분류」는 그 줄만 비움** · 출처 표기(규칙 5-2 ④) · 🔴 §7 ⓕ **야후 어휘 충돌 해소** 선행 | DoD 6·7 |
| **⑥** | **테스트 ＋ 라이브 실측** | DoD 8·9 |

🔴 **착수 전 판정 필요**: §7 **ⓕ**(섹터 어휘 충돌) · **ⓖ**(모집단 1,021 vs 604) — ⑤ 이전에 정해져야 한다.

🔴🔴 **942 판정 대기(2026-08-08, 941 산출)**: `lib/sector.ts`의 「2-of-2 합의제」(0순위 SPDR·1순위 Damodaran·2순위 형제·3순위 나스닥∩SIC·4순위 미분류)를 **야후를 3번째 출처로 넣어 재설계할지** — 941 실측(SPDR 대비): 야후 단독 95.8% > 나스닥∩SIC 현행 95.3% ≈ 2-of-3 다수결 94.8% < 3-of-3 만장일치 99.2%. 미분류 70건은 야후 단독으로 70/70 회수 가능. **④(섹터 컷) 착수는 이 판정 이후로 미룬다** — 판정에 따라 섹터별 종목 수가 달라진다.
