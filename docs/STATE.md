...
66. ✅ **Q1 커버리지 20.9% — 벌크 도입 타당성 검증 완료(995), 도입 판정은 대기.** SEC `companyfacts.zip`(1.4GB, 매일 03시ET 갱신)을 ZIP Range 부분접근(전체 다운로드·해제 0 — 총 사용대역폭 17MB, 1.4GB의 1.2%)으로 검증. **값 일치 20/20 완전일치**(megacap·bank·IFRS해외필러·최근IPO 등 다양 표본, 벌크 vs API캐시 `computeDrivers()` 딥비교). **커버리지**: 현재 1,247행 → 벌크 로드 시 5,740행(**4.6배**), never-attempted(4,646건) 중 96.71% 커버, **JPM 포함 확인**. fiscal_year 확정 가능 비율은 200건 표본 기준 62%(전수 아님, 후보군 4,493건에 적용 시 추정 약 2,786건 신규). 적재 방식(스크립트 1회·크론 아님)·덮어쓰기 위험(969 백필 사고 전례로 스냅샷 선행 필수)·크론 후속 역할 3안·갱신주기는 **설계만, 미실행**. 🔴 993의 "유니버스 5,972" 정확한 재현은 이번에도 못함(최근접 5,893 사용, 비율 지표 위주라 결론에 영향 없음). → `docs/probe_995_sec_bulk.json`
