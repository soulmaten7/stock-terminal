...
## HEAD / 배포
- HEAD = **이 커밋(STEP 918)** · 부모(STEP 917 `a0fddbb`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 건 = 917 계측 배포 · 다음 크론 관측 대기**(변화 없음, 918은 라이브에 손 안 댐 — 아래 "▶ 다음" 00번) **② 결정 대기 4건**(②`#17`·`#37`·`#43` — **918 권고 제출 완료·승인 대기**[17=채택·37/43=현행유지 권고, `docs/DECISION_918_AGENDA2.md`] · ③`#67`=①단계로 구조적 해소 예정(값은 다음 실행 후) · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 918 = 안건 2(`#17`·`#37`·`#43`) 원전 대조·실측·권고 제출(코드 diff 0, 화면 무변경).** T6(driver3)·T8(PIE)·T2(driver1) 원문·셀 직접 개봉. `#17`: T6 Cash Tax Rate 시트는 단일 세율만 출력(병기 개념 원전에 없음)+`usTax`가 국가단일상수라 GAP 이동 구조적 0 → **채택 권고**(외부자료 인용, 계산영향 0). `#37`: T8 PIE 전문에 "계산불가/재검토" 라벨 0건(원전=단일종목 도구라 포트폴리오 라벨체계 개념 자체 없음), 프록시 bandCross 실측(revdcf_results 08-05: years 131건 중 54건·41.2%가 WACC±1%p로 판정뒤집힘) → **현행유지 권고**(새 판정축 발명 필요·창작금지 규칙). `#43`: T2 원문 확보("we combine our own analysis, analyst reports, and Value Line forecasts") — 원전도 도미노 3%/7%/11%를 알고리즘 아닌 서사적 판단으로 구성(T2엔 스프레드시트 자체 없음), 872 기존실측 재인용(야후프록시 부적합: 기준값 범위내 19.9%뿐·폭3.16%p vs 원전8%p) → **현행유지 권고**. 세 건 권고 근거는 서로 독립(17=인용 vs 37=축발명 vs 43=원전자체가 비알고리즘). `docs/DECISION_918_AGENDA2.md` 신설 + `DECISION_908_PENDING.md`·`LENS_COMPLETION_STANDARD.md`(driver1·3 각주에 상호참조만 추가, ③판정 불변)·`REVDCF_SPEC.md` §10·§11 갱신. **화면(#29·#40·#41) 무변경, 승인 전.**
- ✅ **STEP 917 = 🟢 장은태 승인 A안 ①단계: 계측만 배포(값 불변).** `cron_heartbeats.note`(기존 미사용 컬럼, 스키마 변경 0) 채택 — `CapDiag`에 `retryAllLen`(#67의 답)·`countHit`·`timeHit`·단계별ms 추가, US/KR 끝에 `recordHeartbeat()` 신규(try/catch). 게이트 산식·`RETRY_MAX`/`RETRY_MS`·`maxDuration` 전부 불변. **②단계(증액) 미판정.** 관측은 다음 크론(KR 10:00·10:30 UTC·US 21:30±지터) 이후.
...
- 866~918(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+**안건2 원전대조·권고 제출**) 전부 유효 — 상세 CHANGELOG. `app/**`·`data/`·`.github/`·`vercel.json` diff 0(911~918) · `lib/lensPrecompute.ts`는 **917에서만 계측 목적 한정 오픈**(918은 재봉인 상태 · lib/ diff 0).
