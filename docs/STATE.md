...
| # | 단계 | 완료 조건 |
|:--:|---|---|
| **①** | **섹터 해석 함수 신설** — 규칙 5-2: 출처 목록을 **파라미터로** 받고 `{ 섹터, 출처, 합의여부 }` 반환 | 🟡 **부분 착수(938)** — `lib/sector.ts`(`fetchSectorMap`) 신설, 단 **`industryGroup`(역DCF 입력조달) 모드만 구현.** 938이 **두 용도가 서로 다름**을 발견: 역DCF 계산은 `industry_group`(84~94개, `damodaran_beta`/`wacc`/`tax_rate`/`capex`/`working_capital` 5개 테이블이 이 이름을 키로 써서 **다른 출처로 대체 불가**) vs Q0 화면은 `primary_sector`(11개, 나스닥·SEC 보강 **가능**) — `{섹터,출처,합의여부}`(다중출처 합의) 모드는 **미구현**(939 이후) |
...
| **③** | **나스닥 스크리너 적재** — 테이블 신설 ＋ 갱신 경로. Storage 원본 업로드 포함 | 🟡 **③-1 완료(939) · ③-2(DB 테이블 신설·적재) 미착수(940)**. 939: 나스닥·SPDR 둘 다 Storage(`sources` 버킷) 업로드 완료(직접조회로 사이즈 확인) · `registry.ts`에 나스닥·SPDR 좌표 등재(값 아님) · 🔑 **③단계 자체가 확장됨** — 939가 `link_hub` 병행조회 중 **SPDR 섹터 ETF holdings**(S&P 500 진짜 GICS 정답지, 무료)를 새로 발견해 나스닥 하나였던 이 단계가 두 출처(나스닥=③단계 원본·SPDR=정답지 대조용)로 늘었다. 정답지 대조 실측(`docs/probe_939_gics_truth.json`): Damodaran `primary_sector` vs SPDR **일치율 99.6%**(불일치 2건 `APP`·`DD`) — Q0 「2-of-2 합의제」 1순위 출처가 정답지 대비 처음 검증됨. 남은 것(940) = `damodaran_industry`류처럼 나스닥 7,127행을 **Postgres 테이블로 적재** |
