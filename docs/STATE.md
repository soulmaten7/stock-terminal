...
## HEAD / 배포
- HEAD = **이 커밋(STEP 924 `bdd543c`)** · 부모(STEP 923 `570599b`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 건 = 917 계측 배포 · 다음 크론 관측 대기**(변화 없음) **② 대기 안건은 3번(`#67` 로그값) 하나만**(908 정본) **③ DoD7 = 여전히 🔶 미결**(923 발견 종목명 불일치의 **화면 증상은 924가 표시 계층에서 해소**했으나 — `lens_scores.name`/`lens_state_changes.name` 자체는 여전히 티커·DB는 안 건드림 — **"같은 이름"의 정의 자체가 미확정이라 DoD7 판정 칸은 924도 바꾸지 않았다**).
- ✅ **STEP 924 = 🟢 923의 B안(표시 계층 통일) 채택·적용 — DB 쓰기 0.** `lens_scores.name`/`lens_state_changes.name`이 야후 quote 실패로 티커 그대로 저장된 US 행(348/998·34.9%)을, 상세페이지와 같은 소스(`data/us_symbols.json`)로 표시 시점에만 대체. 삽입점 2곳: `app/api/explore/lens-top/route.ts`(→Explore "강점" 목록) · `lib/todayChanges.ts`(→ChangeRow·TodayClient·daily-brief·email-brief 4곳 공용, 한 곳 수정으로 전부 반영). `lib/stockName.ts`에 서버전용 헬퍼 `usSymbolRawName` 신설(기존 `_us` 모듈캐시 재사용). 348개 전수 대조 = **348개 전부 `us_symbols.json`에 존재 → 잔여 0**(라이브 재현: MO→"Altria Group, Inc."·HST→"Host Hotels & Resorts, Inc." 확인). `amountTop`(`/api/yahoo/us-list`)·`search`는 애초 다른(정상) 소스라 무손. KR 경로는 안 건드림(구조상 이 버그 없음). **모멘텀 중복**("모멘텀 모멘텀 상위권")도 조립 로직 수정(`lib/lensCopy.ts`에 `lensStateLine` 신설 — phrase가 이미 렌즈 이름 핵심단어를 담고 있으면 이름 생략, phrase 문구 자체는 무변경) — 전 렌즈·전 상태 71개 조합 재확인, 실제 중복은 momentum(ko/en) + valuation-ko(mid) 3그룹만이고 나머지는 회귀 없음(스크립트 대조 완료). tsc 0·vitest 182/182·`lib/lensPrecompute.ts`·`lib/revdcf/`·`data/`·`.github/`·`vercel.json` diff 0. **DoD7 판정 칸 불변·Alphabet 티커 미표시 미접촉·A안(근본수정) 미실행.** 🔴 사전 DB스냅샷을 구현 후에 찍음(순서 위반·§7 기록) — 대신 diff 자체에 `.update/.upsert/.insert/.delete` 0건임을 grep으로 확인해 안전성을 대체 입증.
- ✅ **STEP 921~923 = 안건4 재료완성→승인적용+DoD7 판단(921~922) → `years` 권고 승인·종목명 불일치 신규발견·진단만(923).** DoD9만 노출-순환, DoD7은 923에서 종목명 문제로 재개방(코드 diff 0 — 923은 순수 진단). `#29`·`#40`·`#41` 문서미갱신 발견·정정(921). 5표면 진단: 목록=`lens_scores.name`(런타임) vs 상세=`us_symbols.json`(빌드타임) 물리적으로 다른 소스, DB실측 348/998(34.9%). 수리선택지 A~D 카탈로그(`docs/DECISION_923_NAMING.md`) — 924가 B 채택.
- ✅ **STEP 917~920 = A안①계측(917)·안건2 원전대조(918)·안건2 승인적용+화면3건(919)·한국어 줄바꿈 수정(920).**
- ✅ **STEP 912~916 = 라이브 이상징후 진단·설계 5단계.** 상세 = `docs/CHANGELOG.md` (57)~(61).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~924(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+안건2 원전대조·권고 제출+안건2 승인적용·화면3건+한국어 줄바꿈 수정+안건4 재료완성·§10 정정+안건4 승인적용·DoD7 마지막판단 권고+years 권고 승인·종목명 불일치 진단+**B안 채택·표시계층 종목명 통일·모멘텀중복 조립수정**) 전부 유효 — 상세 CHANGELOG. `lib/lensPrecompute.ts`·`lib/revdcf/`·`data/`·`.github/`·`vercel.json` diff 0(924도 유지) — 924가 실제로 건드린 파일은 `app/api/explore/lens-top/route.ts`·`components/explore/ExploreClient.tsx`·`lib/lensCopy.ts`·`lib/stockName.ts`·`lib/todayChanges.ts` 5개뿐(DB 쓰기 0).
...
🔴🔴 **00. 라이브 이상징후(912→917 · 917 = 🟢 장은태 승인 A안 ①단계 계측 배포 완료·②단계 미판정) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**. **KR 컷은 문제 없음**(913 확정) — `kr-perf`·`kr-lens-scores` 크론 자체는 별개로 08-05분 확정 미실행 + 08-06분 대기(916). **원인**: "40초 예산이 464건 처리에 구조적으로 부족"은 산술로 확정 · "유일 원인"은 `retryAll.length` 실측 부재로 가설 유지(916 §1) — **917이 바로 이 값을 잴 계측을 배포했다.** 07-31 "유니버스 성장" 방아쇠 가설 = 기각(916 §2). **A안 형태**: Vercel Hobby maxDuration=300s가 기본값=절대상한(공식문서 확인·확장 불가)이나 라우트 실측 이력(224s/141s)엔 76~159s 여유가 있었음 → **①단계(계측, 리스크0) = 917에서 배포 완료**(`lib/lensPrecompute.ts`에 `CapDiag.retryAllLen`/`countHit`/`timeHit`/단계별ms + `computeLensScoresFor`의 `loopMs`/`pass2Ms`/`pruneMs`/`totalMs` + US·KR 양쪽 `cron_heartbeats.note`(기존 미사용 컬럼, 스키마 변경 0) 기록 — 값 계산 0건 변경, tsc 0·test 182/182). **②단계(보수적 증액)는 미판정 — 다음 정규크론(KR 10:00·10:30 UTC·US 21:30±지터) 실행 후 `cron_heartbeats.note` 실측값이 입력.** 회복속도(916 §3): 08-04→08-05 1사이클 −16만 유일 실측점, 23일은 신뢰 낮은 단일점 외삽. **사용자 영향 = 111/998(11.12%)**(914, 재측정 안 함). 결측 74~78(행 자체 없음)은 반복실패/신규미시도 구분 못 함(미측정). 선택지 **A(권고, ①단계 적용됨)·B(비권고)·C(비권고)·D(근거 없음)** 4안 병기. 판정서 = `docs/DECISION_912_LIVE.md`(§10=917). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(진단·계측, 장은태가 매 STEP 승인) — 다음 관측 후 ②단계 재판정 대기.
