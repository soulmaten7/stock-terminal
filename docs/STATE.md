...
## HEAD / 배포
- HEAD = **이 커밋(STEP 915)** · 부모(STEP 914 `5e08a39`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 이상징후(915 확정판, 아래 "▶ 다음" 00번 참조 — A안 실행가능 확정+KR크론 2일연속, 최우선) ② 결정 대기 3건**(②`#17`·`#37`·`#43` 결정형 · ③`#67`=채널확인·값미확보 · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 915 = 480 코호트 정체 규명 + A안 실행가능성 직접 검증.** KR 크론 재측정 — `kr-perf`·`kr-lens-scores` **2일 연속**(08-05·08-06분) 미실행 확인(US는 전부 정상, 별개 이상으로 등재). 07-30~07-31 git 커밋 전수 확인 — US 취득 코드 변경 **0건**(STEP 835는 KR 전용 함수만 변경, `git show`로 직접 확인). **`scripts/probe_915_cohort.ts` 신규 — 정지 코호트(464개) 중 20개 체계적표집 직접호출 = 20/20(100%) 성공**(no_data·rate_limited 0건) — 원천 취득불가 근거 없음. 산술 검증: A안 회복 목표치 `(5401+480)/5966=98.5753%`가 833 정상치(98.5743%)와 **0.001%p 일치** 확인. **A안(취득 고치기) 실행 가능성 = 가능으로 확정, 권고 근거 대폭 강화.** 선택지 (D)(취득불가 유니버스 제외) 신설했으나 근거 없음(404 0건). `docs/DECISION_912_LIVE.md` §8 추가(본문 불변). `docs/LENS_DEV_PLAYBOOK.md` #89 신규(로그 없으면 직접 재호출해 답을 얻는다). 코드 diff 0.
- ✅ **STEP 914 = US 원인 확정 + 영향 크기 실측** — US 111/998(11.12%) stale. 상세 = `docs/CHANGELOG.md` (59).
...
🔴🔴 **00. 라이브 이상징후(912→915 · 진단 완료·수리 안 함) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**(오늘 08-06 기준 7일째). **KR 컷은 문제 없음**(913 확정) — **단 `kr-perf`·`kr-lens-scores` 크론 자체가 별개로 08-05·08-06 예정분을 놓쳐 2일 연속 미실행**(915 §1, US 크론은 전부 정상). **US 원인 = 거의 확정**(취득게이트 커밋이 정지일과 같은 07-30 08:12 UTC, 914) · **임계(97%)는 833 실측 정상치(98.6%)에서 1.6%p 여유를 둔 문서화된 값**(임의 아님) — 지금 90.5%는 시스템 회귀. **🔴 A안(취득 고치기) 실행 가능성 = 가능(915 §3 확정적 근거)**: 정지된 464개 코호트 중 20개 직접 호출 **20/20(100%) 성공**(`scripts/probe_915_cohort.ts`) — 원천 취득불가 근거 0건. 회복 시 목표치 `(5401+480)/5966=98.5753%`가 833 정상치(98.5743%)와 **0.001%p 일치**(915 §4) — A안은 "임계를 넘기는 게 아니라 원상복구." **사용자 영향 = 111/998(11.12%)**(914, 재측정 안 함). 결측 74~78(행 자체 없음)은 반복실패/신규미시도 구분 못 함(미측정). 선택지 **A(권고, 근거 강화)·B(비권고)·C(비권고)·D(취득불가 종목 유니버스 제외, 근거 없음)** 4안 병기. 판정서 = `docs/DECISION_912_LIVE.md`(§8=915). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(진단, 장은태가 STEP 실행으로 범위 승인) — 장은태 확인·수리 여부 결정 대기.
