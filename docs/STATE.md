...
34. **`latestYear()` 604종목 전수 스캔 결과 — 정책 판정 대기**(976) — shares lag≥1 4건(HGTY류 폴백버그 패턴은 0건, 전부 1순위 자체 결측)·debt lag≥1 17건(96.84%는 lag=0)·nonOperatingAssets lag 0건. 🔑 **lag≥1인 21건 중 목표연도 대체값이 확인되는 건 TXRH 1건뿐** — 나머지 20건은 "낡은 값 vs 정확한 값"이 아니라 "낡은 값 vs 결측(skip)"의 문제라 latestYear() 상한 도입의 실익이 제한적일 수 있음. 처방 후보 4개(①현행+기록 ②lag상한 skip ③목표연도강제 ④태그커버리지확장) 등재만, 미선택. → `docs/probe_976_latest_year_freshness.json`
