...
## HEAD / 배포
- HEAD = **이 커밋(STEP 917)** · 부모(STEP 916 `1ea93e9`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 이상징후(917 = A안 ①단계 계측 배포 완료, 아래 "▶ 다음" 00번 참조 — 다음 정규크론 후 `cron_heartbeats.note` 값 확인이 다음 STEP 입력) ② 결정 대기 3건**(②`#17`·`#37`·`#43` 결정형 · ③`#67`=①단계로 구조적 해소 예정(값은 다음 실행 후) · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 917 = 🟢 장은태 승인 A안 ①단계: 계측만 배포(값 불변).** 🔴 **912~916 이후 처음으로 `lib/lensPrecompute.ts`를 열었다**(계측 목적 한정 승인). §0 채널조사 — `cron_heartbeats.note`(기존 미사용 text 컬럼, 스키마 변경 0) 채택. §1~2 구현 — `CapDiag`에 `retryAllLen`(#67의 답)·`countHit`·`timeHit`·단계별ms(`stage1/2/3Ms`·`acqMs`) 추가, `computeLensScoresFor` 반환에 `loopMs`·`pass2Ms`·`pruneMs`·`totalMs` 추가, US/KR 양쪽 끝에 `recordHeartbeat()`(try/catch·루프밖 1회) 신규 — 게이트 산식·`RETRY_MAX`/`RETRY_MS`·업서트 payload·`maxDuration`·`vercel.json` 전부 불변(diff 육안확인, 추가분 전부 계측). 부수발견: 894가 막은 건 "경고"(warning) 한정이었고 info 레벨 Sentry는 이미 같은 파일에 사용 중이었음. **②단계(증액)는 미판정, A·B·C·D 병기 유지.** 배포 전 스냅샷 = `docs/probe_917_baseline.json`. 관측은 다음 STEP(다음 크론: KR 10:00·10:30 UTC · US 21:30±지터).
- ✅ **STEP 912~916 = 라이브 이상징후 진단·설계 5단계**(912 최초진단→913 "8일 정체" 오독 정정[US=6일·KR=1일 정상]→914 US원인 확정+영향 111/998(11.12%) 실측→915 480코호트 규명+A안 실행가능 20/20 확정→916 시간예산 산술 확정+07-31가설 기각+플랫폼상한 실측). 상세 = `docs/CHANGELOG.md` (57)~(61).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~917(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 B 적용+관측 연결·상호주석 완성+스킵사유 3자대조·DoD5 판정+스킵사유 오표시 교정·MISSING_TAG 3분기+revdcf-preview 정체 규명·DoD5 ✅ 상향+Cowork 육안검증·방법론 표 가독성 수정+lossMaking 판정 불일치 재확인·공유 헬퍼 강화+DoD8 테스트 보강·✅ 판정+DoD7 표면 범위 판정·🔶 유지+보류 위반 기록·DoD3 결정서+DoD3 종결·🅿️ 도메인 상한+§10 미결 33건 전수 감사+"지금 가능" 11건 재확인·분류·권고순서+#42·#46 실측+#46 판정서·②단계 처리+#36 해소·결정대기 4건 수집+안건1 재료보완·안건4 사실정리+안건1 판정·화면공개+안건3 채널정정·크론신선도 실측+라이브 이상징후 진단+진단 재조준·정정+US원인 확정·영향 11.12% 실측+480코호트 규명·A안 실행가능 확정+시간예산 산술 확정·A안 형태 정밀화+**A안 ①단계 계측 배포**) 전부 유효 — 상세 CHANGELOG. `app/**`·`data/`·`.github/`·`vercel.json` diff 0(911~917) · `lib/lensPrecompute.ts`는 **917에서 계측 목적 한정으로 처음 열림**(승인 범위 §0).
