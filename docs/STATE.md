...
## HEAD / 배포
- 코드 HEAD **`746efe0`**(STEP 756b · 렌즈 도트 폴리시) · origin/main 동기화 · 트리 클린 · **배포 ✓ onetrillion.app** · CI 그린. ⚠️ **Vercel = Hobby: 크론 일 1회 한도(위반 시 배포 거부·07-18 실증) · 하루 100 배포.**
...
1. **KR 보드 렌즈 도트 → 6개국 미러** — 756(KR) 라이브 ✓·사용자 폰 확인 대기. 반응 OK면 US~GB 보드 미러(+`BoardTopLensCard` 정리·en 문구는 이미 패리티). 단, `lens_scores`는 KR·US만 선계산 — JP/CN/VN/GB는 도트 없음("—")이 정직한 상태(렌즈 선계산 확장은 별건).
...
4. **JP/CN/VN/GB 유니버스 프레시니스** — US 패턴(스크립트+GitHub Action·754/755) 재사용. JP는 브릿지(data_e)와 같은 소스라 ③과 묶으면 효율.
