...
🆕 **STEP 963 §5-3 신규 항목 — Citigroup Revenues 태그 이중값, 판정 대기(2026-08-09).** 같은 us-gaap `Revenues` 태그·같은 기간(2024-01-01~2024-12-31)에 SEC가 값을 두 개 갖고 있다: **81,139,000,000**(FY2024 10-K 원본, accn `0000831001-25-000067`, 2025-02-21 제출) vs **80,722,000,000**(FY2025 10-K의 전년 비교치, accn `0000831001-26-000011`, 2026-02-20 제출, `frame=CY2024`). 우리 저장값(us_fundamentals.revenue=80,722,000,000)은 후자(더 최근 제출본의 재작성 비교치)와 일치 — STEP958이 "설명 안 됨"으로 남겼던 0.51% 차이의 원인이 태그 선택이 아니라 **제출 버전(vintage) 문제**였음을 962/963에서 확인했다. 🔴 **"우리는 어느 제출 버전을 쓰는가"가 지금까지 명시적으로 정해진 적이 없다** — `annualMap`의 `filed` 최신값 우선 로직(drivers.ts:29-30, `String(e.filed) > String(prev.filed)`)이 사실상 "가장 최근에 제출된 값"을 자동 채택하고 있으나, 이게 의도된 정책인지 우연인지 판정된 적 없음. Revenues 외 다른 필드(net_income·equity 등)에서도 같은 재작성 이중값이 있을 수 있는지는 조사 안 함(Citigroup 사례만 확인). 판정 대기.
