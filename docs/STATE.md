...
14. 🔴 **`us_market_cap` 결측 — `LOCAL_OK_PROD_FAIL`, 13번과 단일근본(984 실측)** — 08-10 재확인 349건 정지(335건이 949의 380과 겹침=88% 지속). **B그룹(82건, 로컬에서도 marketCap 자체가 없는 것)과 겹침 0건** — 지금 결측 349건은 전부 D그룹(로컬은 되는데 프로드만 실패) 계열. 🔑 **공통점 실측**: NYSE 상장 83.4%(전체 대비 2배 과대표집)·3글자 심볼 84.8%·"Fund"명 7.4배·"Trust"명 2.7배 과대표집 — **그러나 XOM·HD·MCD·CRM·MRK 같은 초대형 우량주도 다수 포함돼 있어 이 패턴이 전부를 설명하진 못한다**(억지 결론 안 냄). heartbeat 실측: `noResponse=0`·`retryFailReasons={}`(예외 0건)·`recovered=0/400` — 네트워크실패가 아니라 **200 응답인데 특정 필드만 빔**. **crumb 가설**(모듈레벨 캐시·콜드스타트 3단계 핸드셰이크, 코드로 메커니즘 확인)은 구조적으로 그럴듯하나 GitHub 이슈 근거상 crumb 실패의 실제 증상은 "완전실패"라 우리 증상(부분결측)과 안 맞음 — **약화됨, 기각 아님**. 대안: sharesOutstanding×regularMarketPrice 재구성 가능성 확인(오차 0.001%, 미구현). 근본원인 여전히 미규명 — 처방 후보 4개(§4) 기록만. → `docs/probe_984_mcap_root_cause.json`·`docs/probe_984_search.md`
