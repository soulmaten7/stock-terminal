...
## HEAD / 배포
- HEAD = **이 커밋(STEP 919)** · 부모(STEP 918 `ab12d1e`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 건 = 917 계측 배포 · 다음 크론 관측 대기**(변화 없음 — 아래 "▶ 다음" 00번) **② 결정 대기 3건**(안건 2 = **919 해소**[`#17`채택·적용완료 · `#37`·`#43`현행유지·화면무변경] · ③`#67`=①단계로 구조적 해소 예정(값은 다음 실행 후) · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 919 = 🟢 장은태 승인(안건2 전부 권고대로) 적용 + 905 ④단계 화면 3건.** `#17` 채택 → `/revdcf` driver3 각주에 다모다란 실효세율(수익기업평균·집계) 대 한계세율을 **DB 배선**(하드코딩 0, 매 요청 `damodaran_tax_rate`/`damodaran_country_tax` 조회·`loadLedgerFigures()`). `#29`(운전자본 음수 경고): 46.6%는 stale — **재실측 43.2%(201/465, 08-05)**를 driver4 각주에 **배선**(revdcf_results 라이브 조회), 다모다란 원문 직접 인용("we should not assume that non-cash working capital will become more and more negative over time"). `#40`(maxYears 배선): 리터럴 25가 route.ts 3곳·화면 문구에 각자 있던 것을 `app/api/cron/revdcf/constants.ts`(신규, lib/ 밖) 단일 상수로 통일 — `overCapExplained`가 이제 `{years}` 파라미터로 받음. `#41`(유니버스 공개): 867 §7-3 초안(ko/en 3문단, OTC 486/133/8·다모다란 반대입장 병기)을 `universeCaveat`로 거의 그대로 배선(날짜 2026-08-02 명시). `#37`·`#43`은 화면 변경 0(현행유지 그대로). ko/en 패리티 통과(`messages.test.ts` 8/8) · 로컬 dev(`localhost:3333`) curl로 4건 전부 실 렌더 확인(43.2%/465·19.4%/20.2%/25.6%·486/133/8 문구 정확 출력, `overCapExplained`는 client-only라 hydration payload로 배선 확인). **`lib/` diff 0**(engine.ts 안 건드림 — 상수를 lib/ 밖에 둬서 산식 불변 요건 충족). `docs/DECISION_908_PENDING.md` 안건2 해소 표시 · `docs/DECISION_918_AGENDA2.md` 승인기록 헤더만 추가(본문 불변) · `REVDCF_SPEC.md` §10 갱신(불리한 사실 보존: bandCross 8.9%·872 실측).
- ✅ **STEP 918 = 안건 2(`#17`·`#37`·`#43`) 원전 대조·실측·권고 제출.** T6·T8·T2 원문·셀 직접 개봉 — `#17`채택 권고(외부자료 인용, 계산영향 0)·`#37`현행유지 권고(원전에 없음, 새 판정축 발명 필요)·`#43`현행유지 권고(원전자체가 비알고리즘). 상세 = `docs/DECISION_918_AGENDA2.md`.
- ✅ **STEP 917 = 🟢 장은태 승인 A안 ①단계: 계측만 배포(값 불변).** `cron_heartbeats.note` 채택 — `CapDiag`에 `retryAllLen`(#67의 답) 등 추가, US/KR 끝에 `recordHeartbeat()`. **②단계(증액) 미판정.** 관측은 다음 크론 이후.
- ✅ **STEP 912~916 = 라이브 이상징후 진단·설계 5단계**(912 최초진단→913 "8일 정체" 오독 정정→914 US원인 확정+영향 11.12% 실측→915 480코호트 규명+A안 실행가능 확정→916 시간예산 산술 확정+플랫폼상한 실측). 상세 = `docs/CHANGELOG.md` (57)~(61).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~919(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+안건2 원전대조·권고 제출+**안건2 승인적용·화면3건**) 전부 유효 — 상세 CHANGELOG. `data/`·`.github/`·`vercel.json` diff 0(911~919) · `lib/`는 **917(계측)에서만 열렸고 918·919는 diff 0**.
