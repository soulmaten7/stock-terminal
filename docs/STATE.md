...
## HEAD / 배포
- 코드 HEAD **`7bd48e5`** (문서 `19a56d6`) · origin/main 동기화 · 트리 클린 · **배포 ✓ onetrillion.app.**
...
- 최근(이번 세션 07-17): ②b-2 관심목록 렌즈 즉시화(`95d4d9f`) + ①-KR 종목명 영어화(`147bc39`·`aebda56`·`7bd48e5` · `name_en` 2766/2772 · 라이브 실측).

## ▶ 다음 (우선순위)
1. **name_en 정상운영화(KR)** — 새 종목 자동 영문명: 별도 크론 말고 기존 매일 `computeKrSnapshot`(`/api/cron/kr-perf`)에 "`name_en IS NULL`만 야후에서 채움" 증분 1스텝. (백필=완료 · 정상운영=이 스텝.)
2. **①-JP/CN/VN 종목명 영어화** — `/en`에서 자국어로 뜨는 것 영문명 소싱(JP 자체 리스트 · CN title-case · VN 야후).
3. **6개국 유니버스 프레시니스 잡** — 시드 5개국(US/JP/CN/VN/GB) 명단을 무료 심볼 디렉토리에서 주기 재생성(신규 상장 편입). 1·2와 같은 "프레시니스" 패턴 — 국가별 재구현 말고 재사용 잡 하나.
4. **(진행 중) 운영모델 경량화** — 문서 통합(이번 세션)·SYSTEM_MAP(완료) 다음 = 일일 헬스체크(3층 에스컬레이션)·읽기전용 서브에이전트 2~4개(로케일 감사·소스 프로브·검증).
5. **R4 정의** — R1(공시요약)·R2(브리핑)·R3(뉴스) 위 다음 기능층(후보: 렌즈 조합 스크리너·관심종목 알림·포트폴리오 렌즈). 아직 이름만·미정의.
