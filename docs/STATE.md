...
🆕 **새 항목 후보(판정 없음, STEP 952 2026-08-09 발견) — resolveSector damodaran tier 조사가 nasdaq 5순위 추가보다 먼저다.**
- **ⓐ damodaran tier 미매칭 원인 조사** — 미분류 90건 중 `damodaran_industry`에 실제로 행이 있는 게 **29건**(재확인 수치, 원 보고 53건은 `ticker_norm` 중복매핑으로 부풀려짐 — `docs/probe_952b_unclassified.json`). 그중 1건(`RAYA`)은 `is_us_listed=true` 행까지 있는데도 `resolveSector` tier-1이 못 잡았다 — **원인 미규명**. 🔴 **버그라면 Q0의 1,021종목 결과에도 같은 누락이 있었을 수 있다** — 그 경우 Q0 "커버리지 100%(0 미분류)" 주장을 다시 봐야 한다.
- **ⓑ 나스닥을 5순위로 추가할지** — 미분류 90건 중 나스닥 원시 존재 90건(100%), `NASDAQ_TO_GICS` 매핑 가능 79건(87.8%, 재확인 수치). 단독 정확도는 941 실측(정확한 수치는 그 STEP 참조) 기준으로 이미 3순위(야후 95.8%)보다 낮다고 판정됐다 — Q0에서 나스닥을 "교차검증 신호로만 쓴다"고 정한 것과 충돌한다.
- 🔴 **ⓐ가 ⓑ보다 먼저다.** 버그면 ⓑ가 필요 없어질 수 있다(더 정확한 damodaran tier가 이미 그 종목들을 잡아야 정상이므로, 나스닥으로 대체 커버할 필요 자체가 사라질 가능성).
