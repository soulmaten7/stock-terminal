...
- 🔴🔴 **빌드 실패로 프로덕션이 STEP964(`a532b31`) 코드에 멈춰 있었다 — STEP990(2026-08-11)에서 발견·복구.** 원인: `scripts/probe_965_invariance.ts`가 `/tmp/step965_old/drivers_old`를 **저장소 밖 절대경로**로 import — 로컬엔 그 파일이 남아 있어 `npm run build`가 통과했지만 Vercel의 깨끗한 빌드 환경엔 없어 타입체크에서 매번 실패했다(`Cannot find module '/tmp/step965_old/drivers_old'`). Vercel 빌드 로그가 가리킨 건 `probe_965_invariance.ts` 한 곳이었지만, **git 커밋 시각으로 직접 확인한 결과 같은 패턴이 967(`probe_967_invariance.ts`)·969(`probe_969_impact.ts`)에도 있었고, 세 파일 전부 STEP965 커밋(`e67a6e0`, 2026-08-09 22:17)부터 존재했다** — 즉 **"977 이후"가 아니라 STEP965 커밋 이후 모든 배포가 깨져 있었다**(배경 서술보다 범위가 크다는 걸 이번에 직접 확인·정정). **마지막으로 정상 배포됐던 커밋 = `a532b31`(STEP964).**
  - **영향 범위**: STEP965~989(973·974·975·976·977·978·979·980·981~989 전부)의 **코드 변경이 프로덕션에 한 번도 반영된 적 없다.** 오늘(2026-08-10 22:30 UTC) 정규 크론도 이 옛 코드로 돌았다 — DB로 직접 확인: `cron_heartbeats`(job=lens-scores)의 `freshCoverage=0.9303649146300637`가 STEP982에서 관측했던 08-09 값과 **완전히 동일**(같은 옛 코드가 매일 같은 결과를 반복 산출)하고, 986이 추가한 관측필드(`reconstructable` 등)가 note에 **전혀 없다**(986 코드가 반영 안 됐다는 직접 증거). `us_sector_relative as_of='2026-08-10'` 1,247행**전부** `sector`·`sector_as_of`가 null(DB 직접 조회 확인) — 이건 **973 자체의 결함이 아니라 973이 애초에 배포되지 않아서**다(973 이전 코드가 `us_sector_wide`를 못 찾는 구조였던 것 그대로 재현된 것).
  ...
  - **복구**: STEP990에서 tsconfig.json exclude에 위 3개 파일 추가(로컬 `/tmp` 경로를 일시적으로 옮긴 뒤 `npm run build`·`tsc`·전체 테스트가 통과함을 직접 확인 — `docs/probe_990_build_fix.json`). 스크립트 파일은 삭제하지 않고 그대로 보존(검증 기록).
  - 🔴 **절차 결함 기록**: Cowork이 STEP977부터 989까지 **매번 push 후 Vercel 배포 상태를 한 번도 확인하지 않았다.** "push 성공 = 배포 성공"으로 암묵 가정한 것이 이 사고의 진짜 원인이다 — git push는 원격 브랜치에 커밋을 반영할 뿐 빌드 성공을 보장하지 않는다는 걸 이번에 직접 겪었다.
  - **다음 확인**: 이 STEP990을 push한 뒤 Vercel 배포가 실제로 Ready가 되는지 확인(push 성공≠배포 성공, 이번 사고의 핵심 교훈). 그 뒤 다음 정규 크론(lens-scores 21:30 UTC·revdcf 22:45 UTC)이 새 코드로 도는 걸 기다린다 — 크론 수동 실행 안 함. → `docs/probe_990_build_fix.json`
...
54. 🔴🔴 **긴급 — 빌드 실패 복구(990). 프로덕션이 STEP964 코드에 멈춰 있었다(2026-08-09 22:17~2026-08-11).** `probe_965/967/969` 3개 스크립트의 저장소 밖 `/tmp` import가 원인 — 로컬엔 그 파일들이 남아 있어 계속 통과, Vercel 빈 컨테이너에선 매번 타입체크 실패. tsconfig exclude에 3개 파일 정확히 추가(글롭 아님, 다른 파일 영향 없음)로 수정, `/tmp` 3개 디렉토리를 실제로 옮긴 뒤 build·tsc·vitest 전부 통과 확인(로컬통과=증명아님 원칙에 정면 대응). git 커밋시각 대조로 배경("977 이후")보다 범위가 큼을 직접 확인·정정 — 965 커밋(e67a6e0)부터가 진짜 시작점, 965~989 전체 코드 미반영. DB 직접확인(오늘 heartbeat freshCoverage가 08-09와 완전동일값·986신규필드 부재·us_sector_relative 1247행 전부 sector=null)으로 재확인. 절차결함(push 후 배포상태 미확인) 기록. 규칙신설 제안만(적용 안 함). → `docs/probe_990_build_fix.json`
