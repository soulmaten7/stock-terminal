...
| **⑤** | **화면 표시** — 「업종 대비」 줄 ＋ **「미분류」는 그 줄만 비움** · 출처 표기(규칙 5-2 ④) · 🔴 §7 ⓕ **야후 어휘 충돌 해소** 선행 | 🟡 **부분 완료(945) — 리스트 쪽만.** ⓕ→ⓐ 확정(GICS명 통일, 장은태) 구현: `lib/sectorLabel.ts` 신설(야후·KR·GICS 3어휘 통일, 신규 messages 키 0개) → `EtfLensClient.tsx` 전환(동작 동일) · `/api/sector/us` 신규 · `ExploreClient.tsx` US 거래대금 리스트에 섹터 필터 칩 + 행별 라벨 + 출처 안내 한 줄(신규 키 1개) 추가(덧붙이기만, KR·기존 API·기존 정보 diff 0). 테스트 269/269. **🔴 라이브 화면 변경 — 배포됨·장은태 Preview 육안 확인 대기(완료 아님).** **🔴 남은 것 = 「각 카드에 업종 대비」(Q1~Q4 카드 자체가 아직 없어 이 STEP 범위 밖)** |
| **⑥** | **테스트 ＋ 라이브 실측** | DoD 8·9. 🔴 리스트 섹터 분류는 945에서 유닛테스트 269/269 완료, **라이브 실측(육안)은 별도 대기** |
