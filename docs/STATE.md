...
## HEAD / 배포
- HEAD = **이 커밋(STEP 912)** · 부모(STEP 911 `a793fef`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 이상징후(912, 아래 "▶ 다음" 00번 참조 — 7렌즈에 영향, 최우선) ② 결정 대기 3건**(②`#17`·`#37`·`#43` 결정형 · ③`#67`=채널확인·값미확보 · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 912 = 라이브 이상징후 진단(수리 금지·코드 diff 0).** §0에서 "진단≠깊이확장" 명시(보류 위반 아님, 장은태가 이 STEP 실행으로 범위 승인). **US**: `us_market_cap` 08-04 upsert 5,401/5,966(`STOCK_SYMS.length`) ≈ **freshCoverage 90.5%**(게이트임계 97% 미달) — `capGateDecision`이 `lens_cuts` upsert 자체를 차단하는 코드 확인(`lib/lensPrecompute.ts:400-401`). 값(`lens_scores`·`us_market_cap`)은 이 게이트와 무관한 별도 경로라 정상 갱신 — "값은 갱신, 컷만 정지" 관측과 코드 정합. **강한 정황증거(가설) — 로그 원값은 미확보**(다음 실행 21:30 UTC 이후 1h 내 확인 필요). **KR**: 현재 커버리지 100%(게이트 통과 수준)이나 과거 시점 값 불명 — 미상. **07-28 최초 정지 원인**: 취득게이트(STEP 833·07-30 커밋)가 정지 시각(07-28 04:33)보다 **이틀 뒤** 도입돼 최초 원인은 게이트와 무관할 수 있음 — 미상으로 기록. **892의 `retryBudgetHit` 미반영 지적은 여전히 사실, 894의 "게이트변경 범위밖" 판단의 전제(게이트가 실제로 막는지 몰랐음)가 이번에 바뀜 — 재검토 여부는 장은태 몫.** 판정서 = `docs/DECISION_912_LIVE.md`(권고: 코드 무변경, 다음 두 실행 로그 확인 후 별도 STEP).
- ✅ **STEP 911 = 안건3(`#67`) 채널 정정("장은태 전용"은 오기, 진짜 제약=Hobby 로그보존 1시간) + Hobby 크론한도(100개/1일1회) 확인 — "크론9개 모순" 해소 + KR크론2개·`lens_cuts` 이상 최초 발견.** 상세 = `docs/CHANGELOG.md` (56).
- ✅ **STEP 910 = 안건1(`#46`) 판정: 현행 유지 + 한계 공개** — `docs/LENS_COMPLETION_STANDARD.md` driver4 각주. 상세 = `docs/CHANGELOG.md` (55).
- ✅ **STEP 909 = 안건1 재료 보완(부호반전 246사 26건·10.6%) + 안건4 사실 정리** — `docs/probe_909_wc_sign.json`. 상세 = `docs/CHANGELOG.md` (54).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~912(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 B 적용+관측 연결·상호주석 완성+스킵사유 3자대조·DoD5 판정+스킵사유 오표시 교정·MISSING_TAG 3분기+revdcf-preview 정체 규명·DoD5 ✅ 상향+Cowork 육안검증·방법론 표 가독성 수정+lossMaking 판정 불일치 재확인·공유 헬퍼 강화+DoD8 테스트 보강·✅ 판정+DoD7 표면 범위 판정·🔶 유지+보류 위반 기록·DoD3 결정서+DoD3 종결·🅿️ 도메인 상한+§10 미결 33건 전수 감사+"지금 가능" 11건 재확인·분류·권고순서+#42·#46 실측+#46 판정서·②단계 처리+#36 해소·결정대기 4건 수집+안건1 재료보완·안건4 사실정리+안건1 판정·화면공개+안건3 채널정정·크론신선도 실측+**라이브 이상징후 진단**) 전부 유효 — 상세 CHANGELOG. `lib/**`·`app/**`·`app/api/**`·`data/`·`.github/`·`vercel.json` diff 0(911·912는 문서만).
...
🔴🔴 **00. 라이브 이상징후(912 진단·수리 안 함) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선**: `lens_cuts`(US·KR 분포유도컷 5개 렌즈가 씀) **8일째 정체**(07-28 04:33 이후 미갱신) · `kr-perf`·`kr-lens-scores` 크론 오늘(08-05) 미실행(지터창 초과). **원인 — US: 강한 정황증거**(`freshCoverage`≈90.5% DB역산, 게이트임계 97% 미달 → `capGateDecision`이 컷 재유도 자체를 차단하는 코드 확인. 로그 원값은 미확보, 다음 실행 08-05 21:30 UTC 이후 1시간 내 대시보드 확인 필요). **KR·최초원인(07-28)은 미상.** 🔴 **사용자 영향 크기 미측정.** 판정서 = `docs/DECISION_912_LIVE.md`(권고 = 코드 무변경, 다음 두 실행 로그 확인 후 별도 STEP). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(912 §0 — 진단이지 확장 아님, 장은태가 이 STEP 실행으로 범위 승인함) — 장은태 확인·수리 여부 결정 대기.
