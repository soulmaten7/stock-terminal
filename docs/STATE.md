...
**인프라·운영**
13. 🔴 **`lens_cuts`(US) 정지 = 14번의 증상(982 재구성)** — 08-09 실행분은 코드로 완전 특정: `freshCoverage 93.04%<임계 97%`(← 14번의 `us_market_cap` 결측) → `coverageOk=false` → `cutGateOk=false`(`lensPrecompute.ts:453`) → 컷 재유도 전체 스킵 + 크론 자체가 `ok:false`(HTTP 500) 보고. 즉 **13번은 독립 사고가 아니라 14번이 게이트를 통해 밖으로 드러난 결과다.** 🔴 **정정**: "lens_scores는 08-07까지 정상 갱신"은 실측과 다름 — 11일 중 5일(07-31·08-01·08-02·08-05·08-08)은 US 갱신 0건. 영향: 오늘 컷으로 재유도 시 최소 140종목(13.7%)이 렌즈 하나 이상에서 판정 변경(momentum 6.9%가 최대, 완전반전 0건). 07-31~08-08 개별 원인은 heartbeat 이력 없음(단일PK)·Vercel 로그 만료로 미특정 — **이 미특정 자체가 16번(관측 인프라 부재)의 직접 결과**. → `docs/LENS_CUTS_FREEZE_982.md`·`docs/probe_982_lens_cuts_freeze.json`
14. 🔴 **`us_market_cap` 결측 — `LOCAL_OK_PROD_FAIL`, 13번의 근본원인(982 재구성)** — 09-08 측정 380건(전부 07-30 고정) → 08-10 재확인 338건(42건 자연회복). 13번(lens_cuts 정지)과 **같은 acquisition 함수**(`topByMarketCap()`)·**같은 날짜**(07-30) — 그러나 **왜 그 380여 종목만 프로덕션에서 계속 실패하는지(진짜 근본원인)는 여전히 미규명**. 983이 규명에 필요한 최소 관측항목(HTTP상태·에러사유·전체실패목록 등)과 미검증 가설 5개(IP차단·시간예산·버전차이·야후API변경·서버측라우팅)를 정리했으나 **관측 인프라(16번)가 없어 아직 착수 불가**. 처방은 순서상 16번 다음. → `docs/probe_949_mcap_gap.json`·`docs/CRON_OBSERVABILITY.md` §2
