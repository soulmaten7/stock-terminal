...
## HEAD / 배포
- HEAD = **이 커밋(STEP 907)** · 부모(STEP 906 `d09101f`) · **push 완료 — origin/main + revdcf-preview 반영.**
- ✅ **STEP 907 = `#46` 판정서 + 905 권고 ②단계(`#67`·`#36`·`#32`) — 코드 diff 0.** **`docs/DECISION_907_WC_DEF.md` 신설**: 질문을 "수준형이냐 한계형이냐"(875 기결정)가 아니라 "수준형 안에서 유동부채 전액이냐 이자부제외냐"로 정확히 좁힘. 847 선례(원전세부4항목 26% vs 844집계 91%, 65%p 격차로 91% 채택)와 대조 — 906(이자부제외 53% vs 현행 100%, 47%p 격차)은 격차가 좁으나 여전히 크고, 847엔 없던 새 근거(도미노 사례 **부호 반전**)가 추가됨. **권고 = 하이브리드(플래그) 방식**(태그 확보되는 53%는 이자부제외, 나머지는 현행 유지·종목별 플래그로 표시, 809 peBasis 선례) — **채택 여부는 판정 안 함.** **`#32` ✅ 해소**(§5 B-2 표·§11 원장 stale "4.17/4.45"→4.46% 정정+무위험3.95% 신규기록·B분류 배선 대상 아님으로 판단·CLAUDE.md 동일 stale값은 범위 밖으로 명시 배제). **`#36` 여전히 미해소** — 내용은 저비용(문자열 2개)이나 `lib/revdcf/registry.ts`가 907 자신의 `lib/` diff-empty 게이트에 걸려 이 STEP에서 실행 불가("저비용"≠"이 STEP에서 가능") — `lib/` 허용 STEP으로 이관. **`#67` 여전히 도구 제약** — `vercel logs --help` 재확인("5분만 지원", 근본 제약 불변) — 대시보드 확인을 장은태에게 권고만.
- ✅ **STEP 906 = #42·#46 실측** — 906(3.36%p)이 REVDCF_SPEC:754(844) 모순 발견해 정본화. `docs/probe_906_*.json`. 상세 = `docs/CHANGELOG.md` (51).
- ✅ **STEP 905 = "지금 가능" 11건 재확인·분류·권고순서** — `docs/DECISION_905_NEXT.md`. 상세 = `docs/CHANGELOG.md` (50).
- ✅ **STEP 904 = §10 미결 76개 전수 감사** — ✅해소11·⛔무효4·↗이관2·🔴미해소18. `docs/AUDIT_904_OPEN_ITEMS.md`. 상세 = `docs/CHANGELOG.md` (49).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~907(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 B 적용+관측 연결·상호주석 완성+스킵사유 3자대조·DoD5 판정+스킵사유 오표시 교정·MISSING_TAG 3분기+revdcf-preview 정체 규명·DoD5 ✅ 상향+Cowork 육안검증·방법론 표 가독성 수정+lossMaking 판정 불일치 재확인·공유 헬퍼 강화+DoD8 테스트 보강·✅ 판정+DoD7 표면 범위 판정·🔶 유지+보류 위반 기록·DoD3 결정서+DoD3 종결·🅿️ 도메인 상한+§10 미결 33건 전수 감사+"지금 가능" 11건 재확인·분류·권고순서+#42·#46 실측+**#46 판정서·②단계 처리**) 전부 유효 — 상세 CHANGELOG. `lib/**`·`app/**`·`components/**`·`messages/**`·`data/`·`.github/` diff 0(904~907 전부 문서+스크립트만).
- **tsc 0 · vitest 182/182**(무변화) · `revdcf_results` = 2026-08-01~04 각 **604** · `us_market_cap` = **5,888**(둘 다 무변경·쓰기 0) · `lens_scores`·`lens_cuts` 쓰기 0.
- ⚠️ **Vercel = Hobby: 크론 일 1회 한도 · 하루 100 배포.**(904~907 범위 밖)
...
- 🔴 **다음 세션 필독**: DoD 9항목 중 **7·9만 남음(둘 다 보류)**. "지금 가능" 11건 중 5건 처리됨(#42·#46 측정+판정서, #32 해소) — **`docs/DECISION_905_NEXT.md` §7·`docs/DECISION_907_WC_DEF.md`가 재료**. 🔴 **#46 채택 여부·전체 순서는 아직 안 정해짐 — 장은태 결정 대기.** 남은 것: #36(`lib/` 허용 STEP 몫) · #67(대시보드, 장은태만) · ③#17·#37·#43 결정 · ④#29·#40·#41 화면. "모델 완성" 정의(7개 vs 9개 닫힘) 자체도 장은태 결정 대기 — 다음 세션은 STATE 이 문단부터 읽고 시작할 것.
