...
🔴🔴 **00. 라이브 이상징후(912→934) — 역DCF 밖, 7렌즈 실사용자 노출 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**(KR은 08-06 갱신·문제 없음, 913 확정). 917 계측 배포(장은태 승인 A안①단계) → **932**(KR 첫 실측: 게이트통과·158초·여유142초) → **933**(US 첫 실측: `retryAllLen`=511·`countHit=true`·`timeHit=false` — 🔴 916의 "시간절단이 먼저"를 **반증**, 병목은 개수·`#67` 소진) → **934**(🔴 A안 실현가능성 산술 = **불가** — 미시도 111건 전부 살려도 93.30%로 97% 미달, 부수발견: 재시도성공 0건 가능성[간접도출·미확인·별건]). **②단계(예산증액)는 4개 STEP 연속 미판정 — 장은태 승인 사항.** `RETRY_MAX`·`RETRY_MS`·게이트·임계값·`maxDuration` 전부 불변. A·B·C·D 선택지 병기 유지(A안의 성패는 934로 산술상 불가로 기울었으나 판정은 장은태 몫). 🅿️ 930의 `send_later` 예약(2026-08-06 22:45 UTC·`trig_016oNSwKrTa9qSSGQXQDXGqo`)은 관측 시각이 지나 소모됨. 상세 = `docs/DECISION_912_LIVE.md` §10~§13 · `docs/REVDCF_SPEC.md` §11.
