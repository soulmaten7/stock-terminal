...
## HEAD / 배포
- 코드 HEAD **`e8cbd48`**(STEP 752 · CN 파이프라인 복구 완결) · origin/main 동기화 · 트리 클린 · **배포 ✓ onetrillion.app**(라이브 실측·자동배포 웹훅 정상). ⚠️ **Vercel = Hobby 플랜: 크론 일 1회 한도 — 위반 스케줄은 배포 자체가 거부됨**(07-18 실증).
...
5. **운영모델 경량화** — ✅ **일일 헬스체크 가동**(STEP 749 · `/api/cron/health` 매일 12:00 UTC · 10개 파이프라인 25h 임계 · stale→Sentry 알림). 남은 = 읽기전용 서브에이전트 2~4개.
