...
## HEAD / 배포
- HEAD = **이 커밋(STEP 933 `95d635f`)** · 부모(STEP 932 `edd4b2a`) · **push 완료 — origin/main + revdcf-preview 반영(rebase 불필요).** STEP 933 = `retryAllLen`=511 획득·`#67` 해소·916 예측 반증(병목=개수, 시간 아님) — ②단계 여전히 미판정, 코드 diff 0.
...
🔴🔴 **00. 라이브 이상징후(912→917 · 917 = 🟢 장은태 승인 A안 ①단계 계측 배포 완료·②단계 미판정) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**. **KR 컷은 문제 없음**(913 확정) — `kr-perf`·`kr-lens-scores` 크론 자체는 별개로 08-05분 확정 미실행(원인 미규명·별건) + ~~08-06분 대기(916)~~ 🟢 **932에서 해소 — 08-06분 정상 실행 확인**(`cron_heartbeats.kr-lens-scores` 10:35:49 UTC ok=true). ~~원인: "40초 예산이 464건 처리에 구조적으로 부족"은 산술로 확정 · "유일 원인"은 `retryAll.length` 실측 부재로 가설 유지(916 §1)~~ 🔴 **933 반증 — 정반대였다**: `retryAllLen`=511 실측·`countHit=true`·`timeHit=false`·1건당 34.5ms(동시성6 실측, 916의 순차벤치마크 120ms보다 3~4배 빠름) — **병목은 시간이 아니라 개수**(`RETRY_MAX`=400<511). `stage2Ms` 13.8초/40초만 사용(916 예측과 반대). `#67` 소진. 07-31 "유니버스 성장" 방아쇠 가설 = 기각(916 §2). **A안 형태**: Vercel Hobby maxDuration=300s가 기본값=절대상한(공식문서 확인·확장 불가)이나 라우트 실측 이력(224s/141s)엔 76~159s 여유가 있었음 → **①단계(계측, 리스크0) = 917에서 배포 완료**(`lib/lensPrecompute.ts`에 `CapDiag.retryAllLen`/`countHit`/`timeHit`/단계별ms + `computeLensScoresFor`의 `loopMs`/`pass2Ms`/`pruneMs`/`totalMs` + US·KR 양쪽 `cron_heartbeats.note`(기존 미사용 컬럼, 스키마 변경 0) 기록 — 값 계산 0건 변경, tsc 0·test 182/182). **②단계(보수적 증액)는 미판정 — 다음 정규크론(KR 10:00·10:30 UTC·US 21:30±지터) 실행 후 `cron_heartbeats.note` 실측값이 입력.** 회복속도(916 §3): 08-04→08-05 1사이클 −16만 유일 실측점, 23일은 신뢰 낮은 단일점 외삽. **사용자 영향 = 111/998(11.12%)**(914, 재측정 안 함). 결측 74~78(행 자체 없음)은 반복실패/신규미시도 구분 못 함(미측정). 선택지 **A(권고, ①단계 적용됨)·B(비권고)·C(비권고)·D(근거 없음)** 4안 병기. 판정서 = `docs/DECISION_912_LIVE.md`(§10=917). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(진단·계측, 장은태가 매 STEP 승인) — 다음 관측 후 ②단계 재판정 대기. 🔴 **930 — Cowork이 `send_later` 예약 등록**: **2026-08-06 22:45 UTC**(한국 08-07 07:45) · `trig_016oNSwKrTa9qSSGQXQDXGqo` · 목적 = `cron_heartbeats.note`의 `retryAllLen`·`countHit`/`timeHit`·단계별 elapsed 관측(안건 3·`#67`). **이 예약은 그 Cowork 세션으로만 배달된다 — 새 세션은 이 결과를 받지 못한다.** 🟢 **932 — 917 계측 첫 실측값(KR) 확인**: `cron_heartbeats.kr-lens-scores.note`에 `acqMs 4,119·loopMs 151,439·routeMs 157,948(≈158초, 300초 상한 대비 여유≈142초)·coverageOk/cutGateOk true` 등 KR 단계별 elapsed 전부 확보(917 설계대로 `retryAllLen` 등 US전용 필드는 KR엔 원래 없음 — 결손 아님). 🔴 **KR 158초를 US에 적용 금지**(유니버스 1,000 vs 5,966·US만 재시도구조 존재) — 상세 = `docs/DECISION_912_LIVE.md` §11. 🔴 **933 — US `lens-scores`도 실측 도착(22:06 UTC·ok=false)**: `cutGateOk=false`(coverageOk·compositionOk 둘 다 false, `freshCoverage`=91.44%, 916 DB역산 90.5%와 근접해 확인) — KR(`cutGateOk=true`→08-06 갱신) 대 US(false→07-30 정지) **대조군 완성, 게이트가 원인이라는 가장 강한 증거.** `RETRY_MAX`를 올리면 97% 넘는지는 이 실측만으론 모름(511건 전부 성공 보장 없음). **②단계(예산증액)는 이 STEP도 미판정 — 장은태 승인 사항, `RETRY_MAX`·`RETRY_MS`·게이트·임계값·`maxDuration` 전부 불변.** 상세 = `docs/DECISION_912_LIVE.md` §12.
