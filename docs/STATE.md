...
## HEAD / 배포
- 코드 HEAD **`7bd48e5`**(마지막 코드 변경) · 문서는 이후 정본화·전략 확정 커밋 · origin/main 동기화 · 트리 클린 · **배포 ✓ onetrillion.app.**
...
- 최근(07-17~18): ②b-2 관심목록 렌즈 즉시화(`95d4d9f`) + ①-KR 종목명 영어화(`147bc39`·`aebda56`·`7bd48e5`·`name_en` 2766/2772·라이브) → **문서 통합**(핸드오프 6개→`_archive`·정본 STATE/SYSTEM_MAP·`b842b59`) → **전략 확정**(Phase 1 한국 베타 / Phase 2 글로벌화 · `ROADMAP §2-1`).
...
2. **①-JP/CN/VN 종목명 브릿지** — 자국어(JP=`極洋`)라 한국인·영어권 다 못 읽음 → **영어 링구아프랑카 + 인기종목 한글** 소싱(JP 자체 리스트 · CN title-case · VN 야후). `/en`뿐 아니라 **한국 사용자용** 브릿지(Phase 1 한국 베타의 JP/CN 사용성 전제).
3. **6개국 유니버스 프레시니스 잡** — 시드 5개국(US/JP/CN/VN/GB) 명단을 무료 심볼 디렉토리에서 주기 재생성(신규 상장 편입). 1·2와 같은 "프레시니스" 패턴 — 국가별 재구현 말고 재사용 잡 하나.
4. **운영모델 경량화** — 문서 통합·SYSTEM_MAP ✅완료. 남은 = 일일 헬스체크(3층 에스컬레이션)·읽기전용 서브에이전트 2~4개(로케일 감사·소스 프로브·검증).
