...
🔴 **STEP 951 적용 직후 확인(최우선 — 2026-08-09 07:45 KST 크론 이후 최초 세션에서)** — 이 push(`e39595d`, 2026-08-08)로 새 창 코드가 Production에 배포됐다(Vercel 배포 성공 확인 완료). `REVDCF_ENABLED`는 OFF라 화면엔 안 보이지만 크론은 매일 돈다. 다음 세션은 아래 6개를 반드시 확인한다(이 목록이 없으면 다음 세션은 오늘 맥락을 모른다 — 규칙 6):
...
🆕 **새 항목 후보(판정 없음, 951 보강② 2026-08-08 발견) — revdcf 유니버스 자기참조 구조.** `app/api/cron/revdcf/route.ts`의 역DCF 계산 대상(`revdcf_results`)은 매일 "직전 as_of의 CIK 집합"을 그대로 이어받는다(외부 마스터 리스트 없음). 표본 30종목 중 5건(`AMST`·`ANF`·`AVAH`·`ACRS`·`ACT`)이 이 구조 때문에 `revdcf_results`에 없다 — 2026-08-01~08-08 전체 8개 as_of 어디에도 존재한 적이 없음을 Supabase 직접 조회로 확인(`count=0`). **한 번 빠진 종목(또는 애초에 초기 유니버스 밖이었던 종목)이 다시 들어올 경로가 현재 없다.** 판정 없음 — 별도 STEP 후보로만 등재.
