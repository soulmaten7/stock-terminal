...
🔴 **00-e. ORDER 없는 페이지네이션 31곳(재확인: 30곳) — 조사 완료, 처방 미정(2026-08-09, STEP 953).** `lib/sector.ts`에서 952b가 실측한 비결정성이 같은 패턴의 다른 지점에도 있는지 전수 조사. 등급: **A(유니버스 선정) 6곳·B(집계·통계) 6곳(gb/vn Perf는 행수<1000이라 D)·C(화면 목록) 6곳·D(단일페이지·무해) 8곳·order 있어 제외 5곳**(등급 정의·전수 = `docs/probe_953_pagination.json`). 🔴 **10회 반복 실측 — 실제로 흔들린 지점은 damodaran_industry를 읽는 2곳뿐**(`lib/sector.ts:21`·`:64`, 미분류 89/89/**95**/89/89·6937/6937/**6819**/6937/6937). 같은 등급 A인 `revdcf/route.ts:48·106·112`·`search/route.ts:50`(us_cik_map 10,432행 포함)은 10회 전부 완벽히 안정적이었다 — **"모든 order-less 읽기가 위험하다"는 가정은 실측으로 기각된다.** `EXPLAIN`으로 damodaran_industry 쿼리가 Index Scan(파라렐 스캔 아님)임을 확인 — **정확한 불안정 메커니즘은 미확정, 인과 단정 안 함.** 기존 미해결 3건과의 연결: **STEP 949(`us_market_cap` 결측 380건, 위 00-c) = 설명 안 됨**(`STOCK_SYMS`는 `data/us_symbols.json` 파일 기반, DB 페이지네이션과 무관 — 직접 확인) · **STEP 952b(revdcf 5종목 미편입, 위 00-3) = 설명 안 됨**(그 읽기 자체가 604<1000 단일페이지·8일치 CIK 집합 전수대조 완전 무변동, 자기참조 설계 그 자체가 원인) · **00번(lens_cuts US 정지) = 알 수 없음**(lensPrecompute.ts 직접 경로에서 관련 order-less 읽기를 못 찾음 — US는 파일기반·KR은 order 있음, 단 933~937의 전체 조사체인은 재검증 안 함). 🔴 **처방 후보 4개는 고르지 않음** — `docs/probe_953_pagination.json` §4. **아키텍처 위험 등재 = `docs/SYSTEM_MAP.md`.**
