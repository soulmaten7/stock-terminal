...
- 866~925(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 적용+스킵사유 3자대조·DoD5 판정+오표시 교정+DoD5 ✅ 상향+육안검증·가독성 수정+lossMaking 재확인+DoD8 보강·✅ 판정+DoD7 표면판정·🔶 유지+DoD3 결정서·종결·🅿️ 도메인상한+§10 미결 33건 전수감사+"지금가능" 11건 분류·권고순서+#42·#46 실측·판정서+#36 해소·결정대기 4건 수집+안건1 재료보완·판정·화면공개+안건3 채널정정+라이브 이상징후 진단·재조준·정정+US원인 확정·영향 실측+480코호트 규명+시간예산 확정·A안 형태 정밀화+A안 ①단계 계측 배포+안건2 원전대조·권고 제출+안건2 승인적용·화면3건+한국어 줄바꿈 수정+안건4 재료완성·§10 정정+안건4 승인적용·DoD7 마지막판단 권고+years 권고 승인·종목명 불일치 진단+B안 채택·표시계층 종목명 통일·모멘텀중복 조립수정+**brief 라벨조립 진단(수리 미승인)**) 전부 유효 — 상세 CHANGELOG. `lib/lensPrecompute.ts`·`lib/revdcf/`·`data/`·`.github/`·`vercel.json` diff 0(925도 유지) — 925는 `scripts/probe_925_brief_labels.ts`(프로브)와 문서만 추가, `lib/`·`app/`·`components/` 코드 diff 0.
