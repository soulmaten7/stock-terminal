...
| **⑤** | **화면 표시** — 「업종 대비」 줄 ＋ **「미분류」는 그 줄만 비움** · 출처 표기(규칙 5-2 ④) · 🔴 §7 ⓕ **야후 어휘 충돌 해소** 선행 | 🔵 **다음 — 미착수.** 재료 전부 완성: `us_sector_resolved`(종목별 섹터·출처)·`sector_cuts`(적용/제외 플래그)·`docs/probe_944_persist.json`(표시불가 320건 목록) |
| **⑥** | **테스트 ＋ 라이브 실측** | DoD 8·9 |
