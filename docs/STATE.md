...
🔴🔴 **00-2. 방향 재검토 (2026-08-07 · 대화 산출 · 🔴 판정 대기).** 장은태 지적에서 출발: *"TR-AI 렌즈가 정확한 정보를 못 준다 → 제대로 된 1개를 만들자"*의 그 리스트가 저장소에 없다. 3차에 걸쳐 재조사했다(`MODEL_DEMAND_SURVEY`·`MARKET_MODEL_USAGE_TOP20`·`MODEL_UNIVERSE_63`·`MODEL_BUILD_ORDER`). **실측 결론 5**: ① 실사용 1위는 **배수(PER·PBR·EV/EBITDA)** — 은행 리포트 2,263건 중 배수 97%·P/B 56%·P/E 51%, CFA 1,980명 설문 "P/E 가장 널리·EV/EBITDA는 EV배수 1위" ② **7렌즈 중 5가 학술 팩터라 수요 20위 밖**(모멘텀·저변동·자산성장·퀄리티) ③ **`financials` 테이블 0행 · 쓰는 코드 0건** — 렌즈 재무는 야후 `fundamentalsTimeSeries` 매 요청 호출(= 밸류 결함 ②의 구조적 원인) · `dividends` 60행 · `macro_indicators` 0행 ④ **EV/EBITDA는 SEC 태그 추가 0으로 즉시 가능**(EV 재료·D&A를 역DCF가 이미 뽑음) — P/E는 태그 1(`NetIncomeLoss`), P/B는 태그 1(`StockholdersEquity`) ⑤ **KR은 DART 배치 적재 0건 · 현행 호출이 `fs_div=OFS`(개별) 고정** → "배수는 KR·US 100%"는 **야후 값 기준이었고 철회함**. 🔑 **역DCF 재평가: 산출물은 604종목(연수 산출 25.3%)뿐이나 SEC 파이프라인·EV 재료·D&A를 깔아 배수 4종의 재현 비용을 0~1로 낮춘 인프라였다.** 🔴 **장은태 판정 대기 4건**: ⓐ 7렌즈 존치/폐기 ⓑ 다음 모델 순서(EV/EBITDA 우선 권고) ⓒ 재무 원자료 DB 적재 착수 여부(1~8위 전부의 공통 선행) ⓓ `CLAUDE.md:43` "사람들이 가장 많이 원하는 모델" 수식어 처리. **AI 판정**: 새 예측모델 자체 개발은 🔴 근거 부족(Gu·Kelly·Xiu 신경망 out-of-sample R² **0.40%** · 헤드라인 선지식에도 적중 51.5% · AlphaBench "LLM은 팩터 생성 0.72이나 평가 0.40 미만"), **정확한 계산 위 AI 해석층(B안)은 창작 금지와 충돌 없음.** Hou·Xue·Zhang: 이상현상 452개 중 65%(다중검정 82%) 재현 실패 — "원전 그대로"의 근거는 *원전이 옳아서*가 아니라 **대조 가능해서**임이 확인됨.

🔴🔴 **00-3. 해자 주장 정정 + 유니버스 자기참조 (2026-08-07 실측 · 🔴 판정 대기).** ① **`revdcf_results`가 08-01~08-06 6일 연속 정확히 604행/604 CIK — 무증감.** 원인은 `app/api/cron/revdcf/route.ts:28` *"유니버스 = 직전 `as_of`의 CIK"* — **신규 편입 경로가 코드에 없다**(890·891이 "자기참조 고정"으로 인지했으나 열린 채 남음). 유니버스 소스는 이미 있음(`us_market_cap` 5,900행). ② **경쟁자 실측**: New Constructs가 **2,748사**를 지수·거래량 규칙으로 체계 커버(월 5~10사 추가) — 🔴 *"전 종목 분포를 주는 곳이 없다"*는 **틀렸다**. TIKR·`dcfkr.com`(KR)은 사용자 입력형 계산기이고 dcfkr은 **정방향**이라 우리 정의 반대편. ③ **따라서 해자 문장 정정**(`CLAUDE.md:291`·`REVDCF_SPEC.md:30`에 각주 반영): 유효한 것은 *"개인이 못 한다"*이지 *"아무도 안 한다"*가 아니며, **남는 차별화 = 무료 · 원전 대조 가능 · 한국 시장**. 🔑 **604→모집단 확대가 지금 가장 레버리지 큰 단일 작업일 수 있다**(새 모델이 아니라 작동 중인 것의 모집단을 여는 일). 🔴 **다만 모집단 변경은 전 종목 판정·컷 이동을 유발한다(832 전례) — 장은태 판정 필요.**
