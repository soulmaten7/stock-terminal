...
14. 🔴 **`us_market_cap` 결측 — `LOCAL_OK_PROD_FAIL`, 13번과 단일근본(984) + 재구성 처방 구현·미배선(985)** — 08-10 재확인 349건 정지(335건이 949의 380과 겹침=88% 지속). **B그룹(82건, 원천 없음)과 겹침 0건** — D그룹(로컬은 되는데 프로드만 실패) 계열. 공통점 실측(NYSE 2배·Fund 7.4배 과대표집이나 초대형주 예외로 불완전) · heartbeat 실측(네트워크실패 아니라 200응답인데 필드만 빔) · crumb 가설(약화, 기각 아님)은 984 그대로. 🔴 **근본원인은 여전히 미규명 — 985는 원인규명이 아니라 증상완화책이다.** ✅ **985: `lib/marketCapReconstruct.ts`(순수함수, 미배선) 구현·8케이스 단위테스트 통과.** 전수검증(US 5,972종목, 이 세션 환경): marketCap 있는 5,891종목 대조 중앙값 0%·p90 20%·상위20 전부 **복수클래스주식**(TAP-A·MKC-V 등, sharesOutstanding=클래스별·marketCap=회사전체라 구조적 불일치) — XOM 0.001%는 예외 아닌 대표값. 🔴 **핵심 한계**: 이 환경에서는 freshCoverage가 재구성 전부터 이미 98.64%(97%게이트 통과)이고 XOM·HD·MCD 등 984 지목 초대형주 7/7 전부 정상 수신돼 **984의 프로덕션 실패(349건) 자체를 재현 못 함** — 재구성이 실제로 프로덕션 문제를 회복시키는지는 이 STEP에서 검증 불가(원리상 프로덕션에서만 관측 가능, 크론 미배선 규칙). 배선설계(위치·예상효과·140/1023종목 판정변경 재인용)만 문서화, 미구현. → `docs/probe_985_mcap_reconstruction.json`·`docs/probe_985_search.md`·`docs/VALUATION_SPEC.md` "시총 취득 경로" 절
