...
**인프라·운영**
13. 🔴 **`lens_cuts`(US) 07-30부터 정지 — 08-09 실행분은 원인 코드로 완전 특정(982)**. cron_heartbeats 실측: `freshCoverage 93.04%<임계 97%` → `coverageOk=false` → `cutGateOk=false`(`lensPrecompute.ts:453`) → 컷 재유도 전체 스킵 + 크론 자체가 `ok:false`(HTTP 500) 보고. 🔴 **정정**: "lens_scores는 08-07까지 정상 갱신"은 실측과 다름 — 11일 중 5일(07-31·08-01·08-02·08-05·08-08)은 US 갱신 0건. 영향: 오늘 컷으로 재유도 시 최소 140종목(13.7%)이 렌즈 하나 이상에서 판정 변경(momentum 6.9%가 최대, 완전반전 0건). 07-31~08-08 개별 원인은 heartbeat 이력 없음(단일PK)·Vercel 로그 만료로 미특정. → `docs/LENS_CUTS_FREEZE_982.md`·`docs/probe_982_lens_cuts_freeze.json`
14. **`us_market_cap` 결측 — `LOCAL_OK_PROD_FAIL`**(949·982) — 09-08 측정 380건(전부 07-30 고정) → 08-10 재확인 338건(42건 자연회복). 🔴 **982 발견**: 13번(lens_cuts 정지)과 **같은 acquisition 함수**(`topByMarketCap()`)·**같은 날짜**(07-30)에서 나옴 — 강한 정황이나 인과 미확정(둘 다 근본원인 여전히 미규명). 처방 후보 A~D 기록만, 미채택. → `docs/probe_949_mcap_gap.json`·`docs/LENS_CUTS_FREEZE_982.md` §2-2
