...
## HEAD / 배포
- HEAD = **이 커밋(STEP 913)** · 부모(STEP 912 `ae47b76`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 이상징후(913 정정판, 아래 "▶ 다음" 00번 참조 — US만 영향, 최우선) ② 결정 대기 3건**(②`#17`·`#37`·`#43` 결정형 · ③`#67`=채널확인·값미확보 · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 913 = `lens_cuts` 진단 재조준 — 912의 "8일 정체" 프레임을 대폭 정정.** 원인: `lens_cuts.updated_at`이 트리거 0건 + upsert payload 미포함이라 최초 INSERT 이후 영원히 안 바뀌는 열이었음(스키마·코드 직접 확인). 진짜 신선도 열(`as_of`) 재확인 — **US=6일 전(07-30, 진짜 밀림)·KR=1일 전(08-04, 정상)** — KR "게이트 모순"은 전제가 틀려 소멸. `lens_cuts` 쓰기 주체 전수 확인(8개 파일 직접 열람) — `computeLensScoresFor` 하나뿐, 별도 시딩 스크립트 없음. Vercel MCP 로그도 403(Cowork 실측, 다른 계정 인증) — 로그 확인 수단은 **인증 브라우저 1개뿐**. `docs/DECISION_912_LIVE.md` §6에 정정 추가(본문 §1~5는 취소선만, 안 지움). `docs/LENS_DEV_PLAYBOOK.md` #86 신규("updated_at"이라는 이름만 보고 신선도 열이라 가정 금지). 코드 diff 0.
- ✅ **STEP 912 = 라이브 이상징후 최초 진단** — US freshCoverage≈90.5% DB역산(게이트 97% 미달). 상세 = `docs/CHANGELOG.md` (57).
...
🔴🔴 **00. 라이브 이상징후(912 진단 → 913 재조준·대폭 정정 · 수리 안 함) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선**: ~~`lens_cuts` US·KR 둘 다 8일째 정체~~ **913 정정 — "8일 정체"는 `lens_cuts.updated_at`(트리거 0건·upsert payload에 없음 → 최초 INSERT 이후 영원히 안 바뀌는 열)을 신선도 신호로 오독한 것.** 진짜 신선도 열(`as_of`)로 재확인: **US = 6일 전(07-30, 실제로 밀림) · KR = 1일 전(08-04, 정상)** — KR은 문제 없었음. **US 원인 = 강한 정황증거**(`freshCoverage`≈90.5% DB역산, 게이트임계 97% 미달 → `capGateDecision`이 컷 재유도를 차단하는 코드 확인, 로그 원값 미확보). `kr-perf`·`kr-lens-scores` 크론은 오늘(08-05) 미실행 그대로 유효(별개 사안, `lens_cuts`와 무관). Vercel MCP 로그 채널도 403(Cowork 실측) — **로그 확인 수단은 인증 브라우저 1개뿐.** 🔴 **US 영향 크기 미측정.** 판정서 = `docs/DECISION_912_LIVE.md`(§6=913 정정 · 권고: KR 확인 철회·US는 유지, 코드 무변경 오늘 21:30 UTC 이후 로그 확인). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(진단이지 확장 아님, 장은태가 STEP 실행으로 범위 승인) — 장은태 확인·수리 여부 결정 대기.
