...
## HEAD / 배포
- HEAD = **이 커밋(STEP 916)** · 부모(STEP 915 `0beabca`) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔴🔴 **다음 세션 최우선 필독**: **① 라이브 이상징후(916 확정판, 아래 "▶ 다음" 00번 참조 — 시간예산 산술 확정+A안 형태 정밀화+KR크론 상태정정, 최우선) ② 결정 대기 3건**(②`#17`·`#37`·`#43` 결정형 · ③`#67`=채널확인·값미확보(우선순위 하향) · ④"모델 완성" 정의) — `docs/DECISION_908_PENDING.md` 정본.
- ✅ **STEP 916 = A안의 실제 형태를 정한다(진단·설계만, 코드 diff 0).** §1 시간예산: 코드 자체 벤치마크(330건/40s) < RETRY_MAX(400) < 코호트(464) — **"40초가 464건에 구조적으로 부족"은 산술로 확정, "이것이 유일 원인"은 `retryAll.length` 미실측이라 가설 유지.** §2 07-31 방아쇠 가설(유니버스 성장) = **기각**(GHA 자동커밋 2건 직접 파싱, stock 5,964→5,964 불변) — `retryAll` 순서는 결정론적(코드 확인). §3 회복속도: 어제·오늘 코호트 정확비교 불가(전체목록 미저장), 08-04→08-05 1사이클 −16만 유일한 실측점, 23일은 신뢰 낮은 단일점 외삽. §4 Vercel Hobby maxDuration 공식문서 확인 = **300s가 기본값=절대상한(확장불가)** · 현재 설정 이미 그 상한과 일치 — 그러나 라우트 실측 이력(224s/141s)엔 76~159s 여유가 있었음(단계별 계측은 코드에 없음). **A안 형태 = "예산 증액" 단순이 아니라 "계측(1단계·리스크0)→보수적증액(2단계)→필요시 구조변경(3단계)"으로 정밀화**(권고 자체는 유지). KR 크론 상태 정정: 08-05분 확정 미실행 + 08-06분 아직 도래 전(대기) — 915의 "2일 연속" 표현 정정. `docs/DECISION_912_LIVE.md` §9 추가(본문 불변). `docs/LENS_DEV_PLAYBOOK.md` #90 신규. 새 프로브 없음(기존 `probe_915_cohort.json` 재분석+공식문서+git+코드 확인만).
- ✅ **STEP 912~915 = 라이브 이상징후 진단 4단계**(912 최초진단→913 "8일 정체" 오독 정정[US=6일·KR=1일 정상]→914 US원인 확정+영향 111/998(11.12%) 실측→915 480코호트 규명+A안 실행가능 20/20 확정). 상세 = `docs/CHANGELOG.md` (57)~(60).
- 🔴 **프로덕션 도메인 = `https://onetrillion.app`**(869부터 저장소에 커밋 · `docs/PROD_ACCESS_ANSWER2_2026-08-02.md`).
- 866~916(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+DoD4 전제 실측·적용+신선도 원인·인과분해+처방 B 적용+관측 연결·상호주석 완성+스킵사유 3자대조·DoD5 판정+스킵사유 오표시 교정·MISSING_TAG 3분기+revdcf-preview 정체 규명·DoD5 ✅ 상향+Cowork 육안검증·방법론 표 가독성 수정+lossMaking 판정 불일치 재확인·공유 헬퍼 강화+DoD8 테스트 보강·✅ 판정+DoD7 표면 범위 판정·🔶 유지+보류 위반 기록·DoD3 결정서+DoD3 종결·🅿️ 도메인 상한+§10 미결 33건 전수 감사+"지금 가능" 11건 재확인·분류·권고순서+#42·#46 실측+#46 판정서·②단계 처리+#36 해소·결정대기 4건 수집+안건1 재료보완·안건4 사실정리+안건1 판정·화면공개+안건3 채널정정·크론신선도 실측+라이브 이상징후 진단+진단 재조준·정정+US원인 확정·영향 11.12% 실측+480코호트 규명·A안 실행가능 확정+**시간예산 산술 확정·A안 형태 정밀화**) 전부 유효 — 상세 CHANGELOG. `lib/**`·`app/**`·`app/api/**`·`data/`·`.github/`·`vercel.json` diff 0(911~916은 문서+스크립트만).
...
🔴🔴 **00. 라이브 이상징후(912→916 · 진단·설계 완료·수리 안 함) — 역DCF 밖, 7렌즈는 실사용자에게 나가는 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**. **KR 컷은 문제 없음**(913 확정) — `kr-perf`·`kr-lens-scores` 크론 자체는 별개로 **08-05분 확정 미실행 + 08-06분 아직 도래 전(대기)**(916, 915의 "2일 연속" 표현 정정). **원인 이원화 확정(916 §1)**: "40초 예산이 464건 처리에 구조적으로 부족"은 코드 벤치마크(330건/40s<400<464) 기반 **산술로 확정** · "이것이 유일 원인"은 `retryAll.length` 실측 부재(로그보존 1시간)로 **여전히 가설**(정합적이나 확정 아님). 07-31 "유니버스 성장" 방아쇠 가설은 **기각**(916 §2, GHA 커밋 직접 파싱 5,964→5,964 불변). **🔴 A안(취득 고치기) 형태가 정밀화됨(916 §4)**: Vercel Hobby maxDuration=300s가 **기본값=절대상한(공식문서 확인·확장 불가)**, 현재 설정 이미 일치 — 그러나 라우트 실측 이력(224s/141s)엔 76~159s 여유가 있었으나 단계별 계측이 코드에 없어 안전 증액폭을 모름. **형태 = "계측(1단계·리스크0)→보수적증액(2단계)→필요시 구조변경(3단계)"** — 단순 설정 한 줄도, 곧바로 구조변경도 아님. 회복속도(916 §3): 08-04→08-05 1사이클 −16만 유일 실측점, 23일은 신뢰 낮은 단일점 외삽 — "영구동결도 안정회복도 아님". **사용자 영향 = 111/998(11.12%)**(914, 재측정 안 함). 결측 74~78(행 자체 없음)은 반복실패/신규미시도 구분 못 함(미측정). 선택지 **A(권고, 형태 정밀화)·B(비권고)·C(비권고)·D(근거 없음)** 4안 병기. 판정서 = `docs/DECISION_912_LIVE.md`(§9=916). **7렌즈는 보류 목록의 "깊이 확장 금지"와 다른 사안**(진단, 장은태가 STEP 실행으로 범위 승인) — 장은태 확인·수리 여부 결정 대기.
