...
## HEAD / 배포
- HEAD = **이 커밋(STEP 934 `065df95`)** · 부모(STEP 933 `95d635f`) · **push 완료 — origin/main + revdcf-preview 반영(rebase 불필요).** STEP 934 = A안(RETRY_MAX 증액) 실현가능성 산술 = **불가**(미시도 111건 전부 살려도 93.30%, 97% 미달) — ②단계 여전히 미판정, 코드 diff 0.
...
🔴🔴 **00. 라이브 이상징후(912→934) — 역DCF 밖, 7렌즈 실사용자 노출 중이라 최우선.** `lens_cuts`(US)가 `as_of` 기준 **07-30부터 정지**(KR은 08-06 갱신·문제 없음, 913 확정). 917 계측 배포(장은태 승인 A안①단계) → **932**(KR 첫 실측: 게이트통과·158초·여유142초) → **933**(US 첫 실측: `retryAllLen`=511·`countHit=true`·`timeHit=false` — 🔴 916의 "시간절단이 먼저"를 **반증**, 병목은 개수·`#67` 소진) → **934**(🔴 A안 실현가능성 산술 = **불가** — 미시도 111건 전부 살려도 93.30%로 97% 미달) → **935**(🔴 934의 부수발견 재검증: "재시도성공 0건"은 **대수적 우연이 아니라 코드 항등식으로 도출됨**[관측은 아님] · 34.5ms/건은 DB upsert 포함된 상한치 · 915 표본[동일함수·다른동시성·다른맥락]은 크론에 적용 불가 · Yahoo 공식 레이트리밋 문서 없음[확인]·비공식 크럼만료 이슈 다수이나 시간규모 불일치 · **원인 재분류 가능성**(예산문제→취득실패문제, 확정 아님) — 934 "불가" 판정은 유지·오히려 강화). **②단계는 5개 STEP 연속 미판정 — 장은태 승인 사항.** `RETRY_MAX`·`RETRY_MS`·게이트·임계값·`maxDuration` 전부 불변, 계측 추가 0. A·B·C·D 선택지 병기 유지. 🅿️ 930의 `send_later` 예약(2026-08-06 22:45 UTC·`trig_016oNSwKrTa9qSSGQXQDXGqo`)은 관측 시각이 지나 소모됨. 상세 = `docs/DECISION_912_LIVE.md` §10~§14 · `docs/REVDCF_SPEC.md` §11.
