...
| **③** | **나스닥 스크리너 적재** — 테이블 신설 ＋ 갱신 경로. Storage 원본 업로드 포함 | ✅ **완료(939 ③-1 + 940 ③-2)**. 939: Storage 업로드·`registry.ts` 좌표 등재·SPDR 정답지 발견 및 대조(일치율 99.6%). **940 ③-2**: 마이그레이션 `20260808_us_sector_sources.sql`로 `us_sector_nasdaq`·`us_sector_gics` 신설(RLS = `damodaran_industry`와 동일 패턴, 직접조회 확인) → `scripts/ingest_us_sector.ts`가 939 로컬 원본만 읽어 적재(재취득 0) — 나스닥 **7,127행→7,127행**(정확히 일치) · SPDR **515행−ⓙ제외12건→503행**. 남은 것 = 없음(이 단계 완결) |
| **④** | **섹터 내 컷 계산** — p30/p70. 🔴 **표본 확보 섹터만**(하위 6개 26~48종목) | 🔵 **다음 — 미착수.** 940 §5 실측이 재료 제공: 섹터별 종목 수(하위 6개, `resolveSector` 확정분 기준) = Utilities 40·Communication Services 44·Consumer Staples 49·Real Estate 49·Energy 50·Materials 50(`docs/probe_940_sector_resolve.json` `sectorCounts`). 착수 전 §7 ⓕⓖ 판정 필요는 아래 그대로 |
