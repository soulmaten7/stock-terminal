...
| **④** | **섹터 내 컷 계산** — p30/p70. 🔴 **표본 확보 섹터만**(하위 6개 26~48종목) | ✅ **완료(943)**. `sector_cuts` 신설(RLS=`lens_cuts`와 동일, `lens_cuts` 자체는 절대 안 씀·읽기도 안 함 — diff 0). 78개(섹터×지표) 컷 산출·skip 10건(전부 `gap_years`, revdcf 표본이 작아 섹터로 쪼개면 n<20). **부트스트랩(시드943·1,000회)**: Utilities(n=41~43) IQR 대비 구간폭 0.15~1.58, n↔흔들림 단조관계 관찰 안 됨(판정 아님, 사실만). **🔑 시장 전체 컷 vs 섹터 컷 판정 변경 종목 수**(결함⑤ 실측 크기): momentum20.1%·lowvol33.6%·valuation33.0%·quality33.8%·assetgrowth15.8%. 상세 = `docs/probe_943_sector_cuts.json`. **🔴🔴 판정 ⓚ 대기(장은태)**: 위 흔들림 폭·판정변경 비율을 보고 섹터 컷 **전면 적용 여부**를 정한다 — 이 STEP은 재료만 만들고 판정하지 않았다 |
| **⑤** | **화면 표시** — 「업종 대비」 줄 ＋ **「미분류」는 그 줄만 비움** · 출처 표기(규칙 5-2 ④) · 🔴 §7 ⓕ **야후 어휘 충돌 해소** 선행 | DoD 6·7 |
| **⑥** | **테스트 ＋ 라이브 실측** | DoD 8·9 |

🔴 **⑤ 착수 전 판정 필요**: §7 **ⓕ**(섹터 어휘 충돌) · **ⓚ**(섹터 컷 전면 적용 여부, 943 재료 완성) · **ⓛ**(disagree 표시 규칙). ~~ⓖ(모집단 1,021 vs 604)~~ **해소**(2026-08-08 장은태 — "지표별로 그 지표가 존재하는 종목 전체에서 컷 계산" 원칙으로 정정, 943이 이미 이 원칙으로 계산함).
