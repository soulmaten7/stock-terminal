...
## HEAD / 배포
- HEAD = **이 커밋(STEP 890)** · 부모 `d5765b9`(STEP 889) · **push 완료 — origin/main + revdcf-preview 반영.**
- 🔍 **STEP 890 = DoD4 전제 실측 + 판정서(`docs/DECISION_890_DOD4.md`) — 코드·데이터·워크플로 변경 0.** 🔑 **핵심 발견**: `docs/LENS_COMPLETION_STANDARD.md`의 "모집단=N=2,857 확정"은 **실제 운영 표본을 가리키지 않는다.** 역DCF가 실제로 매일 판정하는 표본은 **604**(2026-07-31 `us_market_cap` 스냅샷 유래·이후 `app/api/cron/revdcf/route.ts`가 자기참조로 고정 — "로컬 파일 의존 없음"). 2,857은 867이 승인한 **목표치**일 뿐 604 자리에 구현된 적이 없다. `data/us_symbols.json`(일일 자동갱신 6,771~6,783)은 `us_market_cap`(7렌즈 표본)까지만 연결되고 866/867 숫자와는 무관(스키마에 거래소 필드 자체가 없어 866 필터 적용이 원리적으로 불가능). 부수 발견: `us_market_cap`은 `as_of` **단 2일치**(07-30 520·08-02 5,367)뿐, 08-03 갱신 없음(upsert-only라 삭제 없음). **권고안 = 조건부**(모집단 서술을 "실제 604"/"목표 2,857 미구현"으로 가르면 DoD4 ✅ 가능) — DoD 현황표는 미변경. 상세 = `docs/DECISION_890_DOD4.md`·`docs/REVDCF_SPEC.md` A-10·§10 #63~64.
- ✅ **STEP 889 = 888 감사 결과 교정 + DoD 6(주장 정합) = ✅.** 상세 = `docs/CHANGELOG.md` (34).
- 🔍 **STEP 888 = 표면 전수 감사**(889가 교정) — 상세 = `docs/CHANGELOG.md` (33).
...
- 866~890(유니버스 조달 범위 확정·API 게이팅·문구정정·driver1~6+인플레 판정 완료+세율순효과 실측+대조표 재분류+표면 감사·교정+**DoD4 전제 실측**) 전부 유효 — 상세 CHANGELOG. `lib/`·`app/`·`components/`·`messages/`·`data/`·`.github/` 로직 **diff 0**(890은 `docs/`+`scripts/probe_890_universe_drift.ts` 신규뿐).
- **tsc 0 · vitest 155/155**(무변화) · `revdcf_results` = 2026-08-01/02/03 각 **604**(무변경 — 크론 미실행) · `us_market_cap` = **5,887**(무변경 — 실제로도 08-02 이후 갱신 안 됨, 890에서 확인).
...
| 4 | 컷·분포 | 🔶 **유지**(판정서 제출·890) — N=2,857은 승인된 목표치이지 실제 표본(604·자기참조 고정)이 아님이 890에서 드러남. 권고=조건부(`DECISION_890_DOD4.md`) |
...
- 878~890: `docs/CHANGELOG.md` (24)~(35) 그대로 — driver5/6/인플레 ③판정 확정, 세율순효과 실측, 문서정본화, 대조표 재분류(887), 표면 전수 감사(888), 표면 교정+DoD6 ✅(889), DoD4 전제 실측+판정서(890).
...
1-2. 🔍 **DoD4 판정서 제출(890)** — `docs/DECISION_890_DOD4.md`. "모집단=2,857"은 목표치, 실제 표본=604(자기참조 고정). 권고=조건부. DoD 현황표 미변경.
...
7. 🔴 **DoD 4·5·6·8은 차이 9행(887 재분류 후 현재 4행)이 정리된 뒤에 판정한다.** 특히 DoD 4(컷·분포)는 원전에 없는 우리 추가물 층이라 원전 구현보다 먼저 올 수 없다. 🔴 887은 대조표 재분류만 했다 — 이 항목의 "정리됐는가" 판정 자체는 다시 열지 않았다. **890**: DoD4는 판정서 제출(권고=조건부)까지만 — 여전히 미확정.
