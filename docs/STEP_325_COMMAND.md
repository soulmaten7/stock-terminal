<!-- 2026-06-20 -->
# STEP 325 — [수정] 종목·상품: 증권사 순위를 우측(기존 미리보기 자리)로 + 표 8컬럼 유지
...
그다음:
```
@docs/STEP_325_COMMAND.md 파일 내용대로 실행해줘
```
- **전제**: STEP 324(`69d3958`).
...
## 🎯 목표 (피드백 반영)
- 증권사 순위를 **표 아래가 아니라 우측(기존 미리보기 자리)**으로 → 좌: 종목 표 / 우: 증권사 순위.
- 우측 패널이 폭을 먹어도 **표 8컬럼(1일~1년)이 다 보이게** → 셀 여백 `px-3`→`px-2`로 압축 + `overflow-x-auto` 안전장치.
- 우측 증권사 순위는 **전 증권사 랭킹**(`BrokerRanking`), 스크롤 시 따라오게 `sticky`.
...
개발 서버:
1. 좌측 종목 표 — **1일~1년 8컬럼이 우측 패널 옆에서도 다 보임**(여백 압축).
2. 우측(기존 미리보기 자리)에 **증권사 순위**(전 증권사·거래대금순·바로가기), 스크롤하면 따라옴.
3. 표 아래에 있던 증권사 블록은 사라짐.

> 만약 좁은 화면(1280px 근처)에서 1년이 살짝 잘리면 표가 가로 스크롤되니 끝까지 확인 가능. 더 여유 두려면 우측 패널 폭을 줄이는 후속 조정 가능.

---

## 📦 커밋·푸시
```bash
cd ~/stock-terminal && git add components/toolbox/MarketBoard.tsx && git commit -m "feat(market): 증권사 순위를 우측(미리보기 자리)로 이동 + 표 8컬럼 여백 압축 유지 (STEP 325)" && git push
```

---

> **한 줄 요약**: 증권사 순위를 표 아래→우측(미리보기 자리)로, 종목 표 셀 여백 압축해 1일~1년 8컬럼 유지.
