...
| 경로 | 결과 | 근거 |
|---|---|---|
| 야후 `quote`(현재 정본) | 93.9% | 16일째 게이트 미달 |
| 야후 `quoteSummary` | 0/284 | STEP1010 — 프로파일 필드가 프로덕션 환경 자체에서 전부 결측(직렬화 문제 아님) |
| 나스닥 스크리너 | 접근 불가(`robots.txt` disallow · 60초로 늘려도 timeout) | STEP1017·1018·1019 |
| SEC 자체 조립(주식수×종가) | 96.95%(엄격 95.96%), 97% 게이트에 근소 미달 | STEP1019·1020 |
| SEC 조회창 확장(`frames`→`companyfacts`) | +18건(9.9%), 그중 17건은 중앙값 5.3년(최대 16.6년) 낡아 실질 +1건 | STEP1022 |
...
🔴 **커버리지 게이트(97%)의 분모(`data/us_symbols.json` "stock" 5,976건)에 무엇이 들어 있는지 한 번도 검증된 적이 없었다** — STEP1021이 최초로 이름 패턴(1차) + `lens_scores`/`revdcf_results` 편입 여부(교차확인)로 전수 분류했고, **STEP1024가 SEC SIC(정본, `submissions/CIK##########.json`) 전수 조회로 재판별해 1021을 검증했다.**
...
**가상 커버리지(🔴 채택 안 함)**: SIC 6726(0건)+6770(272건) 제외 시 결합 커버리지 **96.95%→96.91%**(하락). REIT는 어느 시나리오에서도 빼지 않았다. **분모를 정확히 정리해도 97% 게이트에 도달하지 못한다 — 분자(STEP1019~1023)에 이어 분모 경로도 소진됐다.** 판정은 장은태 몫, 유니버스는 이 STEP에서 바꾸지 않았다. 상세 = `docs/probe_1021_universe_composition.md`(이름패턴, 1차) · `docs/probe_1024_universe_sic.md`(SIC 정본, 검증).
