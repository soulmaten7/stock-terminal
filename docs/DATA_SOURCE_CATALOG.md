<!-- 2026-08-16 STEP1049 US 전용 명시 · 4층 추가 (원 신설 2026-08-12 STEP 997 · STEP998 ②단계 추가) -->
# 🗂️ 데이터 소스 카탈로그 — ① 이미 쓰는 5개 기관 + ② 3라운드 전수 조사

> 🔴 **이 카탈로그는 US 종목 분석 전용이다(2026-08-16 STEP1049 명시 — 이전엔 암묵이었다).** 기관 5개 전부 US 소스(SEC·Yahoo·Nasdaq·Damodaran·SPDR)이고 DART/KRX 언급이 0건인 것이 그 증거였으나, 문서 어디에도 "US 전용"이라는 선언 자체는 없었다 — 그래서 `financials`·`dividends`(KR 파일럿 테이블)가 이 카탈로그 밖에 있다는 사실이 "US 재료가 아직 없다"로 잘못 읽혔다(`docs/probe_1047_kr_pilot_removal.md`·`docs/probe_1048_kr_pilot_parked.md`). 다른 국가 데이터는 이 카탈로그의 관할이 아니다 — 필요해지면 그 나라 전용 카탈로그를 새로 만든다.
...
## §0-A. ~~시가총액 커버리지 — 분자 경로 소진~~ (신규, STEP1023, 2026-08-14)

🔴 **정정(STEP1034, 2026-08-15) — 이 절 전체가 「고정 97% 게이트」 기준의 판정이었다.** STEP1031(2026-08-14)이 게이트를 **절대 하한 85% + 전일 대비 낙폭 상한 3%p("급락 탐지")로 실전환**하면서, 아래 다섯 경로 중 넷(야후 93.9%·SEC 자체 조립 96.95%·나스닥 결합 예상 97.25%·유니버스 정리 후 96.91%)이 **전부 85% 기준으로는 통과**한다 — "분자 경로 소진"이라는 이 절의 제목·결론 자체가 더 이상 성립하지 않는다. 🔴 **다만 이게 "SEC 조립을 채택한다"는 뜻은 아니다** — 아래 표는 원문 그대로 보존하고, 새 기준에서 무엇이 달라지는지만 옆에 병기한다. 어느 경로를 정본으로 쓸지는 **판정하지 않는다**(재검토 목록, §0-A′ 참조).

~~🔴 **97% 게이트를 못 넘긴 시가총액(분자) 취득 경로를 STEP1017~1022가 순서대로 전부 실측했고, 다섯 경로 모두 막히거나 소진됐다.** 다음에 이 길을 다시 파지 않도록 여기 한곳에 정리한다.~~

| 경로 | 결과 | 근거 | 🔴 85% 기준(STEP1031, 정정) |
|---|---|---|---|
| 야후 `quote`(현재 정본) | 93.9% | 16일째 게이트 미달(97% 기준) | ✅ **통과**(94.16%, 08-14 실측 — 아래 §0-A′ 참조) |
| 야후 `quoteSummary` | 0/284 | STEP1010 — 프로파일 필드가 프로덕션 환경 자체에서 전부 결측(직렬화 문제 아님) | 해당없음(경로 자체가 죽어 있어 게이트 기준과 무관) |
| 나스닥 스크리너 | 접근 불가(`robots.txt` disallow · 60초로 늘려도 timeout) | STEP1017·1018·1019 | 해당없음(**게이트와 무관한 별개 장벽** — 커버리지 자체는 97.25%로 충분했음, 접근 불가가 문제) |
| SEC 자체 조립(주식수×종가) | ~~96.95%(엄격 95.96%), 97% 게이트에 근소 미달~~ | STEP1019·1020 | ✅ **통과**(96.95%, 현행 야후 94.16%보다 높음 — §0-A′ 참조) |
| SEC 조회창 확장(`frames`→`companyfacts`) | +18건(9.9%), 그중 17건은 중앙값 5.3년(최대 16.6년) 낡아 실질 +1건 | STEP1022 | 실질 개선폭이 작아 게이트 통과 여부에 사실상 무영향 |

🔑 **남은 커버리지 경로는 분모(유니버스 재정의)뿐이고, 그건 분류 정확도 검증이 선결이다**(§🟢 유니버스 종류 구성 참조 — CEF/신탁 320건 중 7건이 전부 REIT 오분류였다). 조회창 확장이 찾아낸 "322건 낡음"은 커버리지가 아니라 **이미 채워진 값의 정확도** 문제였으나, STEP1023이 이 322건 자체가 조사 스크립트 내부의 비교였을 뿐 프로덕션(`us_fundamentals.shares`)과는 무관함을 확인했다(위 슬롯 #2 STEP1023 정정 참조).

~~🔴 **STEP1024 추가 — 분모 경로도 소진됐다.** SEC SIC 정본으로 유니버스를 재판별한 결과, 실사용 가능한 제외 코드(SPAC 6770, 272건)만 정확히 빼도 결합 커버리지는 96.95%→**96.91%로 오히려 하락**했다(REIT 178건은 어느 시나리오에서도 빼지 않음). 1021이 보고한 "96.95%→98.39%"는 이름 패턴이 REIT 54건·은행 3건 등 정상 운영회사를 함께 잘라내며 생긴 산술 착시였다 — 정확하게 골라내면 사라진다. **분자(위 다섯 경로)·분모(유니버스 정리) 둘 다 소진** — 남은 것은 게이트 임계 자체의 재검토뿐이며 이는 장은태 판정이다.~~ 🔴 **정정(STEP1034) — "게이트 임계 자체의 재검토"는 STEP1031(2026-08-14)에서 이미 실행됐다.** SIC 정리 후 96.91%도 85% 기준으로는 통과. "오히려 하락"이라는 표현은 97% 기준에서만 의미 있었다(둘 다 미달이라 하락이 손해였음) — 85% 기준에서는 둘 다 통과이므로 이 하락폭 자체가 더 이상 판정을 가르지 않는다. 상세 = `docs/probe_1024_universe_sic.md`.

### 🔴 §0-A′. 재검토 목록(STEP1034 신규 — 판정 아님, 목록만)

새 게이트(85%)에서 SEC 자체 조립(96.95%)이 현행 야후(94.16%, 2026-08-14 실측)보다 커버리지가 **높다.** 97% 기준에선 둘 다 탈락이라 비교 의미가 없었지만, 85% 기준에선 둘 다 통과이고 SEC 쪽이 더 낫다. **이 STEP은 판정하지 않는다** — 아래 재검토 목록(문서 말미 "🔴 재검토 목록" 절)에 정식으로 올린다.
...
## 🟢 유니버스 종류 구성 (STEP1021 이름패턴 → STEP1024 SIC 정본으로 재판별, 2026-08-14)

🔴 **커버리지 게이트(97%, 🔴 정정(STEP1034) — 이 조사 당시[STEP1021~1024] 기준이었고 현재는 85%로 바뀜, §0-A 참조)의 분모(`data/us_symbols.json` "stock" 5,976건)에 무엇이 들어 있는지 한 번도 검증된 적이 없었다** — STEP1021이 최초로 이름 패턴(1차) + `lens_scores`/`revdcf_results` 편입 여부(교차확인)로 전수 분류했고, **STEP1024가 SEC SIC(정본, `submissions/CIK##########.json`) 전수 조회로 재판별해 1021을 검증했다.**

### 1021 이름패턴 분류(취소선 보존 — 아래 SIC 정본과 다름을 그대로 남긴다)
...
~~| 우선주·워런트·라이트 | 0 | 0% | scripts/refresh_us_symbols.ts의 EXCLUDE_NAME 정규식이 이미 걸러냄 |~~

~~🔴🔴 분류 방법의 결함을 발견·정정: "CEF_TRUST" 320건 중 7건이 lens_scores에 있어 "CEF가 모델 대상에 섞였다"를 기대했으나, 직접 확인 결과 7건 전부 REIT(FRT·EQR·CPT·AMH·NTRS·ESS·DLR)였다. revdcf_results(604건)엔 CEF·신탁·SPAC이 단 0건.~~

~~가상 커버리지(채택 안 함): CEF/신탁·로열티트러스트 제외 시 결합 커버리지 96.95%→98.39%(97% 게이트를 넘김).~~

### 🔴 STEP1024 정정 — SIC 정본 재판별 결과 위 서술은 두 군데가 틀렸다

1. **"7건 전부 REIT"는 과소·부정확했다.** 전수(SIC 6798) 재확인 결과 **54건**이 REIT 오분류였고(1021의 7건은 `lens_scores` 교차확인에서 우연히 걸린 일부일 뿐), 그중 **`NTRS`는 REIT가 아니라 은행**(SIC 6022)이었다 — "Trust"는 신탁은행 브랜드명이지 부동산투자신탁이 아니다.
2. **"96.95%→98.39%"는 산술 착시였다.** SIC로 정확히 골라(실사용 0건인 6726 대신 6770 SPAC 272건만) 제외하면 결합 커버리지는 **96.95%→96.91%로 오히려 하락**한다. 1021의 상승분은 이름 패턴이 REIT 54건·은행 3건 등 정상 운영회사를 대거 함께 잘라내며 생긴 것이었다.

### SIC 정본 분류(STEP1024, 전수 5,976)

| SIC 종류 | 건수 | 비율 | 비고 |
|---|--:|--:|---|
| COMMON_SIC(사업 SIC 보유) | 5,134 | 85.91% | 최소 11건이 SIC "0000"(미분류 플레이스홀더) 오염 — 전체 규모 미측정 |
| **REIT_6798** | **178** | 2.98% | 🔴 제외 대상 아님. 정상 운영회사 |
| CEF_FUND_6726 | **0** | 0% | 🔑 등록 CEF는 1940년법 투자회사라 애초에 SIC 자체가 없다(entityType='other') — 이 카테고리는 정본에서 실사용되지 않는다 |
| SPAC_6770(Blank Checks) | 272 | 4.55% | 1021의 이름패턴(217건)보다 55건 많음. 단 15건은 6770이 아닌 사업코드로 채워져 있어 SIC도 SPAC을 완전히 못 잡는다 |
| 판별불가_투자회사(entityType≠operating, SIC 없음) | 321 | 5.37% | 진짜 CEF 신호가 여기 있다(CEF_TRUST 명칭 236건과 일치) — 단 외국 상장사(20-F 제출자)도 같은 신호를 받아 단독 판별력은 약함 |
| 판별불가_SIC없음 | 54 | 0.90% | — |
| 판별불가_CIK없음 | 17 | 0.28% | — |

**교차검증(전수)**: `lens_scores`(상위1000)·`revdcf_results`(604) 내 SIC 제외후보(6726/6770) = **0건**(1021과 동일 결론 재확인). entityType≠operating로 다시 봐도 190건은 전부 대형 외국 상장사(ASML·SONY·TSM·HSBC 등)였고 CEF는 0건 — **두 각도 모두 오염 없음**.

**가상 커버리지(🔴 채택 안 함)**: SIC 6726(0건)+6770(272건) 제외 시 결합 커버리지 **96.95%→96.91%**(하락). REIT는 어느 시나리오에서도 빼지 않았다. ~~**분모를 정확히 정리해도 97% 게이트에 도달하지 못한다 — 분자(STEP1019~1023)에 이어 분모 경로도 소진됐다.** 판정은 장은태 몫,~~ 🔴 **정정(STEP1034) — "97% 게이트"는 STEP1031이 85%로 바꿨고, 96.91%는 새 기준으로 통과다. "판정은 장은태 몫"이라던 게이트 임계 재검토 자체는 STEP1031에서 이미 실행됐다** — 다만 유니버스는 이 STEP에서 바꾸지 않았다(원문 그대로). 상세 = `docs/probe_1021_universe_composition.md`(이름패턴, 1차) · `docs/probe_1024_universe_sic.md`(SIC 정본, 검증).
...
🔴 **STEP1034 재대조(2026-08-15)** — 1011 이후 STEP1025~1033이 슬롯 인용 파일에 낸 diff를 `git log`로 전수 확인한 결과 **`lib/lensPrecompute.ts` 1건만 변경**(1025·1031·1032, 슬롯 #1 관련) — 나머지 19슬롯이 인용하는 파일(`lib/revdcf/drivers.ts`·`lib/valuation.ts`·`app/api/cron/revdcf/route.ts`·`lib/sector.ts`·`lib/revdcf/compute.ts`·`scripts/ingest_damodaran.ts` 등)은 이 기간 커밋 0건이라 **파일이 안 변했으므로 줄번호도 안 변했음을 git으로 증명**(수동 재확인 아님). Damodaran 계열 DB(`damodaran_country_tax`·`damodaran_beta`·`damodaran_global_inputs`·`damodaran_credit_spread`)도 행수·`as_of` 전부 1011 스냅샷과 동일(재적재 없음). 불일치 1건(#1, 위 취소선 정정) / 일치 19건 / 확인불가 0. 상세 = `docs/probe_1034_catalog_sync.md`.

| # | 슬롯 | 지금 쓰는 소스(우선순위) | 자격있는 후보(카테고리#) | 요구정밀도 | 검증방법 |
|---|---|---|---|---|---|
| 1 | 시가총액 | ~~Yahoo 배치조회(정본, us_market_cap)~~ → **Yahoo 3단 취득(배치→개별재시도 400건/40s→7일폴백)**(정본, us_market_cap. ~~`lib/lensPrecompute.ts:107-236`~~ 🔴 **정정(STEP1034) — `lib/lensPrecompute.ts:146-300`**(STEP1025·1031·1032가 이 파일 앞부분에 `capGateDecision`·`pruneDecision` 등을 추가하며 39줄 밀림. 알고리즘 자체는 무변경 — 배치 `:161-180`·개별재시도 `:181-221`·7일폴백 `:222-299`, 소비처 `app/api/cron/revdcf/route.ts`는 이 기간 diff 0이라 기존 인용 그대로 유효)) (1011 정정 — 코드 대조 결과 배치 1단계만 기재돼 있었음, 실제론 개별재시도·7일폴백 2단계가 더 있다) | 카테고리3 전부(Nasdaq스크리너 등) — 전부 계산기준 미명시라 **검증 없이 못 씀** | 값 자체가 답(Q1 분모) | 카테고리1(기말발행×종가)과 대조해야 신뢰 가능. 🔴 **단, #5(자기자본)·#6(매출)과 주식수 기준이 같아야 함(의존관계②·ⓑ) — 975 실측 PBR 8.29%·PSR 8.34% 잔차가 이 불일치 때문이었고, 프로덕션엔 아직 처방이 안 배선됨** |
| 2 | 발행주식수 | SEC XBRL 태그(역DCF), ~~Yahoo 부수값(밸류)~~ → **(밸류에이션은 shares 미사용 — marketCap 직접사용, `lib/valuation.ts:38-49` ValuationInputs에 shares 필드 자체가 없음)** (1011 정정 — 코드 근거 없는 서술이었음. 부수 발견: `us_fundamentals.source_tags`에 shares 키 없음, 채택 태그명은 `revdcf_results.flags.sharesTag`에만 실림·604건 한정) 🔴 **STEP1022 추가 — SEC `frames` 조회창 결함**: 이 세션의 조사 스크립트(`scripts/probe_1019~1021`)가 취득 가능성 판정에 쓴 "6분기 조회창"(`scripts/probe_1019_sec_marketcap_assembly.ts:74`)은 **근거 없는 임의값**이었다(프로덕션 코드 아님, 조사 스크립트 한정). `companyfacts`는 창 개념 없이 전체 이력을 준다 — 1022가 182건 전수 재조회한 결과 **회복 18건(9.9%)뿐, 그중 17건은 시점 중앙값 5.3년 낡아 사실상 무용지물**. 🔴 **더 중요한 발견 — 조회창 결함이 이미 채워진 값에도 있다**: 기존 충족 4,557건 전수 재조회 결과 **322건(7.07%)이 프레임보다 더 최신 값을 갖고 있었다**(중앙값 49일·p90 86일, 역년 분기 경계와 실제 회계분기 마감일 불일치가 원인으로 추정). 상세 = `docs/probe_1022_sec_window_audit.md` 🔴 **STEP1023 정정 — 위 "322건"은 프로덕션 데이터가 아니다.** `scripts/probe_1022_sec_window_audit.ts`에 `us_fundamentals` 참조 0건(직접 확인) — 322건은 이 조사 스크립트가 자체적으로 만든 두 임시값(구 기준선=`frames` 6분기창 / 신 기준선=`companyfacts` 전체이력) 사이의 비교였고 어느 쪽도 DB에 저장된 적이 없다. **프로덕션(`app/api/cron/revdcf/route.ts:278-281`)은 애초에 `frames`를 쓴 적이 없다** — `companyfacts`(창 없는 전체이력)를 직접 호출하고, 태그 우선순위도 다르며(`WeightedAverageNumberOfDilutedSharesOutstanding` 최우선, `lib/revdcf/drivers.ts:365-373`), 복수클래스는 이미 `MULTI_CLASS_SHARES`로 스킵 처리된다(`drivers.ts:375-384`). **7.07%는 조사 도구 내부 현상이었지 `us_fundamentals.shares`의 실제 낡음률이 아니다.** 상세 = `docs/probe_1023_stale_shares_impact.md` | 카테고리4(SEC 정본)·5(이력, 미사용) | 값 자체가 답 | 카테고리4와 직접 대조 |
...
| # | 조건 | 현재 상태(1002 기준) |
|---|---|---|
| 1 | 20개(실질 19개) 슬롯의 "지금 쓰는 소스" 칸이 **주장이 아니라 코드 대조로 검증**됨 | ✅ **완료(STEP1011·STEP1034)** — 🔴 정정(STEP1034): 이 행이 1002 기준으로 멈춰 있었으나, **STEP1011(2026-08-13)이 이미 20개 전부 일괄 코드 대조를 완료**했다(일치15/불일치5). **STEP1034(2026-08-15)가 그 이후 변경분(git log로 전수 확인, 1건만 해당)까지 재대조**해 지금도 유효함을 확인 — 위 슬롯 매핑 표 상단 두 주석 참조 |
