...
> - **리딩방·검증**: 금감원 신고 유사투자자문 1,738건(`fss_advisors`) → 플랫폼탭(텔레그램115/카카오톡0/네이버432/기타1191, **host 정밀파싱**) · 100/페이지 · 가나다↑↓/추천순 · **리딩방명=info_name**(업체명과 다름) · 분할 미리보기(금감원 정보, **sticky**, 툴박스 overflow-hidden 제거) · **좋아요**(`room_likes`,1인1회,로그인) · **신고**(`room_reports`, `/api/reports`) · **만료 자동숨김**. 집계뷰 **`advisor_directory`**.
...
> - **자가등록 + 관리자 + UI (301~311)**: 운영자 **"+ 리딩방 등록"**(로그인, `room_submissions`, 사업자번호 **FSS 자동대조** `/api/rooms/submit`) → 디렉토리 합류(뷰 **UNION**, source='user'='이용자 등록' 중립표시, ✅금감원등록은 본인확인 후). **관리자 페이지 `/admin`**(role=admin 전용, 신고·자가등록 표). 리딩방 컨트롤 레이아웃 정렬(302~307: 정렬탭·등록버튼·미리보기 칼럼). UI: **커스텀 드롭다운**(네이티브 select 대체), `color-scheme:light`(OS 다크 깨짐). **✅ 신고 경로 작동 검증**(테스트 신고 1건 적재).
> - (272~280은 V7 전환 전 옛 홈 버그픽스 — isKrxCode·ETF/ETN 차트·스티키·100상품·분봉.)
>
> **▶ 다음 후보**(보류·사용자 결정):
> - **관리자 편의**: 헤더 '관리자' 링크 / **신고 알림(메일·텔레그램 푸시)** — 외부 키 필요. (신고·자가등록은 완성, `/admin`에서 봄.)
> - **본인확인**(휴대폰 실명인증, **사업자등록 후 유료** ~40원/건 KG이니시스·포트원 등) → 자가등록 **✅금감원등록확인(대표명==인증자명)**의 전제.
