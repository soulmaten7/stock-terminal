<!-- 2026-06-27 -->
# STEP 439 — [클레임 빌드] 내 업체 카드: 금감원 검증사실 미리보기 + 무료 링크 1개
...
```
```
@docs/STEP_439_COMMAND.md 파일 내용대로 실행해줘
```

## 🎯 목표
마이페이지 '내 업체' 카드를 **리딩방 디렉토리 상세와 동일한 정보**로 채운다.
1. **금감원 검증 사실(읽기 전용)** — 등록업체·대표·주소·신고기간·신고수·바로가기 자동 표시(수정 불가). 사용자에게 보이는 그대로 owner도 봄.
2. **업체 제공 링크** — 인증 업체가 **본인 링크 1개를 무료로 실제 추가/삭제**. 2번째부터는 "광고·유료" 버튼(결제 준비 중 스텁).
3. **[검증 사실] vs [업체 제공]** 두 블록 시각 분리 + "무료/광고" 라벨.

> 정책: 금감원 `homepage`(바로가기)는 업체가 못 건드리는 검증 데이터로 자동 유지. 업체가 추가하는 건 본인 채널 1개(무료) + 추가 유료. ROADMAP §3.

## 전제
- 최신 main + 클레임 빌드(미커밋, STEP 430~438). 커밋 보류.
- 파일 3개: `app/api/business/mine/route.ts`(전체 교체), `app/api/business/manage/route.ts`(addLink 1곳), `components/business/MyBusinessClient.tsx`(전체 교체).
- **API 라우트 로직 변경 → 클린 재시작 필요.**

---

## (1) `app/api/business/mine/route.ts` — 전체 교체
`advisor_directory` 뷰에서 주소·신고기간·홈페이지·신고수까지 함께 반환.
...
## (2) `app/api/business/manage/route.ts` — addLink 블록만 교체
첫 링크는 무료(`is_paid:false`), 2번째부터는 결제 필요(402, 준비 중).
...
## 확인 (localhost)
- 마이페이지 → **내 업체** 탭.
- 카드 위쪽 **금감원 검증 사실** 블록: 등록업체·대표·주소·신고기간·신고수·바로가기가 읽기전용으로 뜸(디렉토리 상세와 동일).
- **업체 제공 링크** 블록: "링크 추가 (무료·1개)" → 폼(종류·주소·이름) → **무료 링크 등록** → 목록에 "무료" 라벨로 추가됨.
- 무료 1개 등록 후엔 추가 버튼이 **"링크 추가 (광고·유료·링크당)"** 로 바뀌고, 누르면 **"결제 준비 중"** 안내만.
- 링크 삭제(🗑) 동작.
- 소개 저장·관리자 공유 기존대로.

## 빌드·커밋
- 보류. 확인 후 **클레임 빌드 전체(STEP 430~439) 한 번에 커밋** 예정. push·배포는 사용자 지시 시.
