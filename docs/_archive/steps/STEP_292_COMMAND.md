<!-- 2026-06-20 -->
# STEP 292 — [V7 ④-5] 리딩방 재설계: 플랫폼 탭 + 한줄 카드 + 분할 미리보기
...
그다음 터미널에:
```
@docs/STEP_292_COMMAND.md 파일 내용대로 실행해줘
```

- **전제 상태(HEAD)**: STEP 291(`7228585`). 빌드 ✓.
- **사전 작업(완료, DB 직접)**: `advisor_directory` 뷰에 **platform 컬럼** 추가(host 정밀 파싱: telegram/kakao/naver/etc). 실측 분류 = 텔레그램 115·네이버 432·기타 1,191·카카오톡 0.
...
리딩방·검증 탭 레이아웃 재설계:
1. **플랫폼 필터 탭(왼쪽)**: 텔레그램 · 카카오톡 · 네이버 · 기타 + **정렬 탭(오른쪽)**: 가나다↑/↓/추천순
2. **한 줄 압축 카드**: `번호 · [플랫폼아이콘] 업체명 ✅ ··· ♡수 · 신고 · 바로가기`
3. **오른쪽 미리보기 패널**(데스크탑): 행 선택 시 그 방의 **금감원 상세**(대표·주소·신고기간·☎·바로가기). 모바일은 하단 시트.
4. **검색은 플랫폼 무관 전체** (등록여부 확인용). 플랫폼 탭 클릭 시 검색 해제.
5. 플랫폼 분류는 **host 정밀 파싱**이라 유튜브 오탐 등은 자동으로 '기타'로.

> 파일 2개 전체 교체: `/api/advisors`(platform 파라미터) + `AdvisorDirectory`. ToolboxClient는 그대로(이미 isLoggedIn 전달).
...
type PlatformKey = 'telegram' | 'kakao' | 'naver' | 'etc';
      ...
      <dl className="space-y-1.5 text-xs">
        {[['대표', a.representative], ['주소', a.address], ['신고기간', `${a.valid_from ?? '—'} ~ ${a.valid_to ?? '—'}`], ['연락처', a.phone]].map(([k, v]) => (
          <div key={k as string} className="flex gap-2">
            ...
            <dd className="min-w-0 flex-1 text-unjong-primary">{(v as string) || '—'}</dd>
          </div>
        ))}
      </dl>
  ...
  async function submitReport() {
    ...
    try {
      const r = await fetch('/api/reports', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ target_type: 'fss_advisor', target_id: reporting.biz_no, target_name: reporting.company_name, reason: reportReason, content: reportContent }),
      });
      const j = await r.json();
    ...
    <section className="min-w-0">
      ...
      <div className="relative mb-2">
        ...
        <input
          ...
          placeholder="업체명·대표자 전체 검색 (등록 여부 확인)"
      ...
      <div className="flex gap-4">
                ...
                return (
                  ...
                  >
                    <button type="button" onClick={() => setSelected(a)} className="flex min-w-0 flex-1 items-center gap-2 text-left">
                      ...
                      <span className="truncate text-sm font-semibold text-unjong-primary">{a.company_name}</span>
        ...
        <aside className="hidden w-72 shrink-0 lg:block">
          <div className="sticky top-20 rounded-xl border border-unjong-border bg-unjong-surface p-4">
            {selected ? (
...
npm run build
```
- 빌드 무에러 (lucide `Globe` import 포함).

개발 서버(`npm run dev`, 포트 3333):
1. 리딩방·검증 탭 → 위에 **플랫폼 탭(텔레그램/카카오톡/네이버/기타)** 왼쪽 + **정렬 탭** 오른쪽.
2. **텔레그램**(115) 기본 → 한 줄 카드: 번호·아이콘·업체명·✅·♡·신고·바로가기.
3. **네이버** 탭(432), **기타** 탭(1,191), **카카오톡**(0 → "없습니다").
4. 행 클릭 → **오른쪽 미리보기**에 대표·주소·신고기간·☎·바로가기 (모바일은 하단 시트).
5. 검색 `잇츠스탁` → 플랫폼 무관 전체에서 검색됨.
6. 정렬/페이지/좋아요/신고 그대로 작동.
...
```bash
cd ~/stock-terminal && git add -A && git commit -m "feat(v7): 리딩방 재설계 - 플랫폼탭(텔레그램/카카오톡/네이버/기타 host정밀파싱)+한줄카드+분할 미리보기 패널 (V7 ④-5, STEP 292)" && git push
```

---

> **한 줄 요약**: 리딩방 탭을 플랫폼 필터 탭 + 정렬 탭 + 한줄 압축 카드 + 오른쪽 금감원 미리보기 패널로 재설계. 플랫폼은 host 정밀 파싱(유튜브 오탐 제거), 검색은 전체.
