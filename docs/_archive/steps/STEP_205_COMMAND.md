<!-- 2026-06-07 -->
# STEP 205 — 투자상품 랭킹 (rename + ETF hover 미리보기 + 정렬 라벨 명확화)

## 실행 명령어 (Sonnet — 기본)
```bash
cd ~/stock-terminal && claude --dangerously-skip-permissions --model sonnet
```
> 그 다음: `@docs/STEP_205_COMMAND.md 파일 내용대로 실행해줘`

## 목표 (사용자 지시)
1. 탭 이름 **"~랭킹"**으로 통일: 투자상품→**투자상품 랭킹**, 리딩방→**리딩방 랭킹**, 주식 관련 채널→**주식 관련 채널 랭킹**
2. **ETF hover 미리보기** — 종목에서 쓰는 미리보기(`HomeStockDetail`: 캔들+커뮤니티)를 그대로 재사용(ETF도 6자리 종목코드)
3. **정렬 라벨 명확화**: "인기순"→**"거래대금순"**(=실시간), 기간 칩은 **"1개월 수익률 / 3개월 수익률 / 6개월 수익률 / 1년 수익률"** (이미 수익률 데이터 — 라벨만 명확히)

## 전제 상태
- HEAD: STEP 204 상태
- 변경: `components/home-v6/HomeEtfRanking.tsx`(전면 교체) + `components/home-v6/HomeRankingTabs.tsx`(탭 라벨 3개)

---

## 작업 1/4~3/4 — `components/home-v6/HomeRankingTabs.tsx` 탭 라벨 rename

**찾기:** `  { key: "etf", label: "투자상품" },`
**바꾸기:** `  { key: "etf", label: "투자상품 랭킹" },`

**찾기:** `  { key: "room", label: "리딩방" },`
**바꾸기:** `  { key: "room", label: "리딩방 랭킹" },`

**찾기:** `  { key: "channel", label: "주식 관련 채널" },`
**바꾸기:** `  { key: "channel", label: "주식 관련 채널 랭킹" },`

---

## 작업 4/4 — `components/home-v6/HomeEtfRanking.tsx` (파일 전체 교체)
...
> 변경: 정렬 라벨(거래대금순/기간 수익률) · 행 hover→오른쪽 `HomeStockDetail` 미리보기(캔들+커뮤니티, 종목과 동일) · 행 클릭→`/stock/{code}` · 1위 자동 미리보기.
...
빌드 ✓ 후:
```bash
cd ~/stock-terminal && git add components/home-v6/HomeEtfRanking.tsx components/home-v6/HomeRankingTabs.tsx && git commit -m "feat(v7): 투자상품 랭킹 — 탭 rename(랭킹) + ETF hover 미리보기 + 정렬 라벨(거래대금순/기간 수익률) (STEP 205)" && git push
```

## 완료 보고 (Cowork 에게 전달할 것)
- [ ] `npm run build` exit 0 / 커밋·push
- [ ] 탭 3개 이름 "투자상품 랭킹 / 리딩방 랭킹 / 주식 관련 채널 랭킹"으로
- [ ] 투자상품 정렬 칩이 **거래대금순 / 1개월 수익률 / 3개월 수익률 / 6개월 수익률 / 1년 수익률** (인기순 사라짐), 지표 컬럼 헤더도 맞춰 바뀜
- [ ] ETF 행 hover → **오른쪽 미리보기**(캔들차트+커뮤니티, 종목과 동일), 1위 자동 표시
- [ ] 행 클릭 → `/stock/{code}`
- ⚠️ 화면 그대로면 `.next` stale → 진짜 터미널 재시작

## 주의·예상 이슈
- ETF 미리보기 캔들은 KIS 차트(6자리)라 ETF도 정상. 커뮤니티는 해당 ETF 토론(없으면 "첫 의견" 안내).
- 정렬은 라벨만 명확화 — 데이터는 STEP 198 그대로(거래대금 실시간 / 기간 수익률). 토스식 "기간 거래대금순"은 과거 거래대금 데이터 없어 미적용(정직).
- 다음: STEP 206 리딩방 랭킹(경고 문구 + 좋아요/싫어요/조회 클릭 정렬), STEP 207 채널 랭킹.

---
> STEP 205 = 투자상품 랭킹. 전제 STEP 204. 다음: 리딩방 랭킹 정렬·문구. 문서 묶어 갱신.
