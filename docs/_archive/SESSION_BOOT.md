...
> - **②b-2 관심목록 즉시화**(`95d4d9f`): `/api/watchlist/quotes`에 `lens_scores` 선계산 톤(state→tone 결정론·라이브 로직 1:1) 배치 포함 → `WatchlistClient`가 로드 즉시 렌더("읽는 중…" 스켈레톤 없이)·선계산 밖(소형주·비KR/US)만 실시간 `/api/lens` 폴백(동시성4). **②b 완결.**
> - **①-KR 종목명 영어화**(`147bc39`): `kr_stock_snapshot.name_en` 컬럼(Cowork MCP `add column`) ← 야후 `longName`(신규 `scripts/enrich_kr_names.ts`·배치 quote 100/콜·동시성8) + `krx/ranking`(nameEn 반환)·`MarketBoard`(useLocale·3곳 `isEn?nameEn:name`)·`lib/stockName.ts`(KR en)·`watchlist/quotes`+`WatchlistClient`가 `/en` 영문명 선택(한글 폴백). 종목상세는 기존 `resolveStockName` 배선으로 자동.
> - **🐞 페이지네이션 버그(Claude Code가 잡음)**: 초기 스크립트 `select("symbol,market")`가 PostgREST 기본 1000행 캡에 걸려 2772 중 ~1772종목(SK하이닉스 포함) **조용히 누락** → `.range(from,from+999)` 루프로 전 유니버스 처리(`aebda56`). 최종 `name_en` **2766/2772**(나머지 6=야후 미제공 소형주·정직한 결측).
> - **렌즈 미리보기 공유카드**(`7bd48e5`): ④에서 주인공化한 `LensPreview`(데스크톱 우측 레일)·`BoardTopLensCard`(모바일 인라인)가 `stock.name`을 로케일 무관 렌더하던 것 → `displayName=locale==='en'?(nameEn??name):name`(한글 폴백). 두 컴포넌트 모두 `useLocale` 기존 보유·KR 보드는 이미 `nameEn` 실린 row 전달.
> - **✅ 라이브 실측**: `/en` KR 보드 리스트+우측 미리보기 카드+종목상세 전부 영문명(SK hynix Inc.·Samsung Electronics Co., Ltd.·Hyundai Motor Company·Kia Corporation·NAVER Corporation)·`/ko` 한글 유지. 광고 슬롯 증권사명(대신증권)은 광고주 콘텐츠라 별개.
> - **🅿️ name_en 자동화(미착수·방식 재논의)**: 별도 크론 불필요 — 매일 `computeKrSnapshot`/`kr-perf` upsert가 payload에 `name_en` 없어 기존값 보존·새 종목만 null → "**name_en IS NULL만 야후에서 채움**" 증분 1스텝을 그 일일 흐름에 추가하면 새 상장 자동 커버(백필=이번 수동 실행·정상운영=증분). 사용자: 크론까지 필요한지 재검토 요청 → 문서 먼저 마무리 후 대화.
> - **🧭 운영모델 논의(POTAL식 멀티에이전트 조직)**: 사용자가 POTAL_AI_Agent_Org(16 Division·59 에이전트)를 트릴리언에도 적용할지 문의. 결론(3중 검색·검증) = **지금 단계엔 과설계**. 근거: 멀티에이전트는 독립 병렬 리서치에만 이득(Anthropic·토큰~15배)·코딩은 단일 스레드가 맞음(Cognition "Don't Build Multi-Agents"·컨텍스트 파편화)·CC 서브에이전트는 2~4개 좁게가 정석. 트릴리언 처방 = 단일 스레드 개발 유지 + **시스템맵 1장** + **읽기전용 서브에이전트 2~4개**(로케일 감사·데이터소스 프로브·검증) + **일일 헬스체크(3층 에스컬레이션)**. POTAL "59 에이전트"도 실제론 크론+스케줄드태스크2+복붙 프롬프트+단일 개발 루프. **계획 미확정·대화 계속.**
...
> - **④A 종목 페이지 렌즈 헤더**(`dcb1bf6`): `StockLensClient` 상단(이름·현재가 아래)에 압축 렌즈 요약(점7+강점/주의/보통·`LensSummary` 재사용) + "종합 매수·매도 점수 없음·판단은 당신" 노트. 상세 카드 위·ETF 제외. 라이브 삼성전자 강점2·주의2·보통3.
> - **④B 보드 상단 자동 미리보기**(클릭 전에도 맨 위 거래상위 종목 렌즈): 744(`be7407d`·KR) `LensPreview` `example` 라벨 + 신규 `BoardTopLensCard`(모바일 인라인·팝업 아님) + aside `selectedStock ?? sorted[0]`(**표시만 폴백**·상태 불변·URL 복원 비충돌) · 745(`9998c7b`) US·JP·CN·VN·GB 미러(CN `.cur` 보존). 라이브 SK하이닉스·Micron + "거래 상위 예시".
> - **②b-1 KR 렌즈 선계산**(`fe02a41`+RPC 마이그 라이브): `computeLensScoresFor(universe,market)` 파라미터화(US 보존)+KR 유니버스(`kr_stock_snapshot` 거래대금 상위·admin 클라)+`/api/cron/kr-lens-scores`(Vercel cron `30 10 * * *`)+로컬 러너+백분위 RPC 시장필터(`041_...sql` 아카이브). **KR 489행**. MCP 검증: 삼성전자 states→강점2·주의2·보통3(live 일치)·백분위 시장격리(KR vs KR·US 무오염)·KR도 백분위 획득.
> - **🔑 발견**: `lens_scores.name` KR 값이 **야후 영어**("SamsungElec") → **①(비미국 종목명)에 야후 `longName` 활용 가능** 확인.
> - **▶ 다음 = ②b-2**(관심목록/보드가 `lens_scores` 상태 배치로 읽어 "읽는 중…" 없이 즉시화·`/api/watchlist/quotes`에 톤·없으면 실시간 폴백) **→ ①**(비미국 종목명 영어화).
>
> 🔎 **2026-07-17 (2) · 베타 준비도 평가 + ⭐ 관심목록 렌즈 개편(②a) + /en 종목명 갭 (HEAD `44fa289`).**
> - **🔎 베타 준비도**(경쟁 3중 조사): "클로즈드 베타엔 충분·네이버/토스/초이스스탁 정면 공개경쟁엔 아직." 차별점=비추천 에토스·방법 투명성·6개국·유사투자자문. 갭=스크리너 UI·재방문훅·비미국 종목명.
> - **⭐ ②a 관심목록 렌즈 개편(739~742)**: 739(`9464f0e`) 배치 시세 `/api/watchlist/quotes`(관심 심볼을 국가별 스냅샷 `.in`) · 740(`e4571e7`) WatchlistClient 재설계(시세 즉시+**행별 지연 렌즈 요약** 강점/주의/보통·점7·`/api/lens` `verdict.tone`·fscore≥7/≤3·동시성4·반응형 2줄) · 741(`0fe6971`) 관심종목 hero+리딩방 `/en` 숨김 · 742(`44fa289`) 데스크톱 현재가 세로정렬(이름 `flex-1`·렌즈 `sm:w-64`)+설명 종목화. **라이브 실측**(가격 우측끝 x=1269 정렬·삼성전자 강점2·주의2·보통3·en 리딩방 숨김·테스트종목 add→검증→remove).
> - **🔑 제약**: `lens_scores`(선계산)=**US 상위 1000종목만** — KR/JP/CN/VN·소형주 없음. 그래서 관심목록 KR 렌즈는 **live `/api/lens` 지연**으로 해결(005930→005930.KS 실시간 계산 확인). 즉시화는 **②b**(KR 렌즈 선계산 크론) 후속.
> - **🔍 발견/다음**: **①** 비미국 종목명 `/en` 한글 — KR API에 `name`(한글) 하나뿐·`name_en` 없음, 이전 i18n이 **US SEC 실명만** 커버 → 영문명 소싱(야후 `longName`)+저장+보드·관심목록 배선(ko 폴백). **④** 종목페이지 렌즈헤더(풀 7막대)+보드 상단 자동 미리보기(데스크톱 패널·모바일 인라인 카드·"거래상위 예시" 중립·스플래시 반대)·목업 확정·미구현. **②b** KR 렌즈 선계산.
> - **▶ 다음 순서 = ④ → ①(비미국 종목명 영어화) → ②b(KR 렌즈 선계산).**
>
> 🎨 **2026-07-17 (1) · 베타 UX·가독성 폴리시 + 지수 티커 이중방어 + 광고문의 언어권 차등 + /en 푸터 정리 (HEAD `21c6649`).**
> - **코인 탭 숨김**(`4135463`·데이터 준비 전 노출 제거·라우트 보존).
> - **🌑 다크 가독성**: muted 토큰 `#8A8D93→#9CA3AF`(`6b2ce97`·소형 보조텍스트 AA턱걸이 5.95→AAA 7.8:1·토큰 전역) + 헤더/티커 `#0E1116` 바 위 하드코딩 흰색 `white/40~55→/65~70`(`ed425f4`·태그라인 3.8→8.2:1·크기 무변·위계 유지).
> - **📱 모바일 홈보드 풀블리드**(`659b0e2`): `ToolboxClient` 박스 `border-y sm:rounded-2xl sm:border` + 페이지 `px-0 sm:px-6` = 모바일 화면끝까지(좌우 3중여백 제거·폭 ~14% 회복)·데스크톱 카드 유지. + 티커↔보드 빈공간 제거(`155da9e`·모바일 `pt-0 pb-4`). **근거=반응형은 일관성이지 동일함 아님**(웹서치 3회: 모바일 풀블리드+데스크톱 max-width 카드 표준·CNN·Paystack·Robinhood).
> - **🐞 지수 티커 사라짐 이중방어**: 근본원인=`/api/yahoo/indices`가 21심볼 `Promise.all` 조회 중 **하나만 야후 실패해도 전체 reject→빈 응답(200)** + 클라 `HomeIndexStrip`이 빈 응답으로 기존 티커 덮어씀(간헐 ~20%·API 5회 중 1회 0개 실측). **클라**(`fa8a201`·데이터 있을 때만 갱신+5초 재시도) + **서버 STEP 737**(`d391c0c`·심볼별 try/catch 격리=하나 실패해도 나머지 반환·`_lastGood` fallback). 라이브 16회 연속 빈 0(min 20=격리 작동).
> - **🔤 헤더 로고 반응형 확대**(`0431d06`): 22px→`h-6 w-6 lg:h-8 lg:w-8`(모바일24/데스크톱32px·2줄 텍스트 높이 정합) + Trillion `leading-none`·태그라인 `mt-0.5`(간격 ≈9→2px).
> - **📢 광고문의 언어권 차등 STEP 738**(`8c7c7a8`): 리딩방(유사투자자문)=한국 특유→**en(US·국제)에서 room 슬롯·rule2·폼옵션·note·phCompany 제거**·ko 100% 유지·server `getLocale`+client `useLocale`. 라이브 en 2슬롯3규칙/ko 3슬롯4규칙.
> - **🐞 교훈**: Vercel 배포 큐 간헐 지연(2회)→빈 커밋(`--allow-empty`) 재트리거 · `Promise.all` 하나 실패=전체 실패(부분허용=심볼별 try/catch) · 클라 API 빈응답 무비판 반영 금지(데이터 있을 때만 갱신) · `resize_window`가 최대화 창엔 뷰포트 무반영(모바일 실측은 DOM 클래스/computed+사용자 폰) · 반응형=일관성≠동일함.
> - **🧹 /en 푸터 정리(후속)**: 유사투자자문 disclaimer2 en 숨김(`bba2ec0`·KR 전용·disclaimer1 범용은 양쪽 유지·ko 불변) + 사업자 주소 제거(`21c6649`·`businessInfo` ko·en·**무거래 정보서비스라 전자상거래법 §10 주소표시 비대상**·상호/대표자/사업자번호 유지·법인 전환 시 법인주소 재추가) → `/en` KR 전용 잔재 0(라이브 en/ko 실측).
> - **▶ 다음(선택) = 클로즈드 베타 초대(BETA_INVITE.md 준비됨) · (선택) 모바일 폰트 크기 미세조정.**
>
> 🌐 **2026-07-15 (5) · US 뎁스 완결(배당·ETN) + /about 개선 + /en 완전 영어화(link_hub·뉴스) (HEAD `3ecadaa`).**
...
> - **🌑 다크 테마 개선 (라이트→다크·3단계·안 깨지게)**: **1/3 토큰화**(`f029d91`·겉모습 변화 0) — 배경전용 토큰 `unjong-strong` 신설 + `bg-unjong-primary`→`strong`(글자/배경 역할 분리·33곳) + `bg-white`→`bg-unjong-surface`(28곳). **2/3 플립**(`07fc4bf`) — `app/globals.css` 토큰 값만 다크로(background #0A0A0A·surface #17181C·border #2A2C31·primary글자 #E9EAEC·muted #8A8D93·strong #2C303A) + body·html `color-scheme:dark`·스크롤바. 앱 전체 다크·**라이브 검증 완료**. **3/3 폴리시**(`3c2fc8b`·🔴Opus) — 앰버 배지→`bg-amber-400/10 text-amber-300`·게이지 fill→`/45`·상태색(emerald-600→400 등) 다크 대비 + 구글 로그인버튼·StockLogo 실로고원 라이트 유지(정석)·레터아바타 이니셜 `text-unjong-strong`. 근거 = 브랜드가 미드나잇+민트·헤더 이미 다크·데이터밀도 금융툴은 다크가 정석. **개선이지 전환 아님.**
...
>
> 🟢 **2026-07-13 · 🎯 종목보드 UX 마감 + 🔵 브랜드 정체성 인앱 정합 + 🧭 nav 2탭 + 🧪 베타 피드백 + 📚 지침 정체성 정리 (HEAD `c0d3b80`).**
...
> - **🧪 개발 안전망 가동**(STEP 695): vitest 유닛 + GitHub Actions CI(매 푸시 tsc→test→lint). 이후 로직 변경 회귀를 기계가 자동 검증.
...
> - **680·685 탭 14→3 재구조**(`ToolboxClient.tsx`): 상단 = 종목·정보·검증. 나머지 12개(뉴스·공시·리포트·기업재무·거시·ETF·공모주·증권사·차트·거래소·토론커뮤니티·유튜브) = **"정보" 하위탭**. 증권사=정보 하위(참조 디렉토리)·검증=상단탭(KR 게이팅)·유튜브=KR 게이팅. 근거=catch-all 금지(네이버·다음·야후 관행)+빅테크식 최소·직관.
> - **677·681 미리보기 광고 완전 제거**→광고는 리스트 10개마다로 일원화. UI 태그라인 3곳(푸터·about·로그인) 새 문구. **678·679** OG/SEO 문구(`app/layout.tsx` 메타·홈 `app/page.tsx` title override 제거→layout `title.default` 단일 소스). **상하이종합→SSE Composite**(지수 티커 영문 통일).
> - **683·684 증권사 소개글**: 20곳 계열/유형 중립 사실 note(Supabase `brokers.note` 갱신·옛 "20년 연속 1위" 대체·SK증권=독립계)·이름 옆(`ListRow` meta)·PC 너비. **685** 종목 리스트 10개마다 증권사 데모 광고(KR 대신증권·`BrokerAdRow`·`lib/ads.boardBrokerAd`)="거래처 안내"지 투자권유 아님.
> - **686~690 ETF/상품 구성**(`docs/ETF_LENS_PLAN.md`): `/stock/{ETF}`가 기업재무 렌즈 대신 **"상품 구성"** 뷰(상위보유·섹터·보수율·운용사·추종지수). US=Yahoo topHoldings·KR=네이버 m.stock `etfAnalysis`(키 없음·Vercel 도달 확인). `lib/instrumentType.ts`·`app/api/etf-holdings/route.ts`·`EtfLensClient.tsx`·미리보기 요약(`LensPreview`). **영숫자 KRX 코드(0193T0) 인식 버그 수정**(`krCode /^\d[0-9A-Z]{5}$/`). 라벨="상품 구성"(AI 분석 아님·정체성). REIT=단일주식·ETN=구성없음·KRX getJsonData=LOGOUT→네이버 채택. 증권사 거래연결(수익화)=별도·미룸.
> - **3중 검수 통과**: 코드리뷰 8파일 클린 + Supabase 브로커 20/20·홍보 0 + 라이브 US/KR ETF·브로커 API·홈 3탭/OG + 링크 카테고리 전부 채워짐.
> - **신규 파일**: `docs/BRAND_IDENTITY.md`(재작성)·`docs/AD_MONETIZATION_PLAYBOOK.md`·`docs/ETF_LENS_PLAN.md`·`lib/ads.ts`·`lib/instrumentType.ts`·`components/toolbox/BrokerAdRow.tsx`·`app/api/etf-holdings/route.ts`·`app/stock/[symbol]/EtfLensClient.tsx`·STEP 파일 673~690.
> - **▶ 다음 = ETF 증권사 거래연결(수익화) · ETN 정제 · JP/CN/VN/GB ETF 구성 · AI 구독(Phase 5) · 오늘 개편 모바일 실사용 QA.**
>
> 🟢 **2026-07-09 (5th) · STEP 668~672D — 보드 성능 스냅샷화 + 데이터 검수 Round 1 + VN HNX 보류(배선 완비).**
