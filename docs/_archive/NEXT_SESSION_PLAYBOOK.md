...
>
> 🌐 **2026-07-15 (최신) 스냅샷 — HEAD `3ecadaa` · US 뎁스 완결(배당·ETN) + /about 개선 + /en 완전 영어화.** **US 뎁스**=731(`d2ff68d`) US 배당→UsOfferingsFeed(IPO+배당 토글·KR 동급)·732(`33f24d2`) US ETN 서브탭(주식/ETF/REITs/ETN·Yahoo 실명+live·18 live)→US=KR급. **/about**(`b0fee55`)=표준 골격(문제·3기둥·렌즈 방법 투명화[7렌즈+계보]·비추천 헤드라인·커버리지)·경쟁사 8곳 근거·ko/en 패리티·라이브 검증. **🌐 /en 완전 영어화**=734(`971e237`) link_hub description_en(490)·735(`868c8a5`) site_name_en(139·기관 공식명)·736(`3ecadaa`) 뉴스 lang 파라미터(en=KR/JP/CN/VN→영어·무료 구글번역+translation_cache 재사용·비용≈0·ko 보존)→`/en`=로고 외 한국어 0. 마이그(MCP)=description_en·site_name_en. 🐞 교훈=i18n 레이어(큐레이션·제3자 피드도 스코프)·뉴스는 lang화만으로(무료·OpenAI 가정 오판). ▶ 다음(선택)=클로즈드 베타·GB 뉴스 ko·site_name 품질. (아래 스냅샷들은 히스토리.)
>
> 🇺🇸 **2026-07-15 스냅샷 — HEAD `9d977f0` · 라이브 QA + 727 메타타이틀 + 다크 D + US 구조화 IPO 피드.** i18n 100% 후 라이브 QA 스윕(8페이지) → 잔재 마지막(정적 페이지 6종 `/en` 메타 타이틀 한글) 수정(**727** `d15dbed`·`about`·`advertise`·`feedback`·`favorites`·`business`·`coin` `generateMetadata` 로케일화·en 영어·ko byte 동일·711 패턴·robots 보존·terms/privacy/admin 제외)→**i18n 잔재 0**. **다크 D**(`1f661e3`) 미사용 `.shadow-soft` 죽은 CSS 제거·폴리시 백로그 소진. **🇺🇸 729 US IPO 피드**(`9d977f0`): US IPO 탭 뉴스검색→구조화 캘린더(Nasdaq 공개 API·`/api/ipo/us-feed`[이번+지난달·예정+최근상장]+`UsIpoFeed`[2섹션·priced→내부 종목상세]+Toolbox 배선[JP/CN/VN/GB 뉴스·KR OfferingsFeed 불변]+i18n[ko 상장예정/최근상장·en Upcoming/Recently priced 패리티]). ✅ 라이브 실측=Vercel 200·실데이터 30건(MetaOptics/MOT·Csquare/CSQR)·403 없음·/en·/ko 렌더. **US=KR급 IPO 뎁스.** 🐞 교훈=`export const metadata` 정적도 generateMetadata로·Nasdaq IPO 공개 API(무키·헤더·다음달 0건→이번+지난달 병합·Vercel 403 없음). ▶ 다음(선택)=US 배당 캘린더(KR 완전 동급)·ETN 서브탭·클로즈드 베타. (아래 스냅샷들은 히스토리.)
>
> 🏁 **2026-07-15 스냅샷 — HEAD `6bccc45` · i18n 100% 완결: OAuth 로케일 쿠키(710E) + US 폴리시(725·726).** `/en` 로그인이 한국어 `/`로 떨어지던 마지막 gap 제거 → 로케일을 **쿠키(`post_login_locale`·SameSite=Lax)로 왕복**(redirectTo/Supabase 허용목록 **byte 불손상** — 710D 로그인 사망 회피 절대원칙). 신규 `lib/authRedirect.ts`(`safeNextPath`=오픈리다이렉트 가드·`localizePath`=as-needed 프리픽스)+유닛테스트(vitest 49/49)·콜백이 쿠키 읽어 프리픽스+소비 삭제·로그인 2곳 쿠키 세팅. ✅ 라이브 실측 성공(실제 구글 로그인 `soulmaten7`·JWT 3분 내·`/en`→`/en` 복귀·세션 활성·격리 테스트로 `NEXT_LOCALE=ko`인데도 쿠키가 `/en` 구동 증명). **725**(`3cef637`) 종목상세 통화기호·**726**(`713084c`) US 종목명 title-case(약어·camelcase 보존·mixed-case 무영향). **🏁 i18n 100%**=정적 UI(710B)+결정론(715~719)+LLM(720~724)+로그인 왕복(710E). 🐞 교훈=next-intl `NEXT_LOCALE`도 로케일 독립 구동(둘 다 Lax·실사용 일치)—`post_login_locale`은 콜백 자체 리다이렉트를 옳게 만들어 더 견고. 상세=`docs/PARKED_OAUTH_LOCALE_ACTIVATION.md`(활성화 완료). ▶ 다음(선택)=다크 폴리시 D·클로즈드 베타 초대·빈 뉴스 명시 UX. (아래 스냅샷들은 히스토리.)
>
> 🎉 **2026-07-15 (2) 스냅샷 — HEAD `5c0c348` · Tier 3: LLM 생성물 영어화 완결 → /en 100% 영어.** `/en`의 마지막 한국어(LLM 생성물=브리핑 R2·news-brief R3·공시요약 R1)를 영어화. 설계 `docs/TIER3_LLM_I18N_DESIGN.md`(스키마 A `*_en` 컬럼·on-demand). **720**(`2645cf9`) `*_en` 컬럼 마이그(MCP) · **721**(`e34fee3`) `/api/brief` R2[🐞 `brief_ko` NOT NULL→en-first INSERT 조용한 LLM 누수 될 뻔·DROP NOT NULL+에러로깅] · **722**(`60d5d8b`) `/api/news-brief` R3[+`tags_en`·한국어 강제 후처리 `ko` 게이팅] · **723**(`9329993`) `/api/events/summary` R1 US · **724**(`5c0c348`) `kr/jp/cn/gb/vn-events/summary` R1[723 복제]. 캐시 컬럼 분리(`*_en`)로 언어 교차 오염 차단·on-demand 과금·KR byte 동일(라이브 삼성 공시 ko/en 독립·vitest 43/43). 결과=`/en` 로고 빼고 한국어 0(US 영어 시장 제품 완성). 교훈 `LENS_DEV_PLAYBOOK` #31. ▶ 다음(선택)=US 통화기호·빈뉴스 UX·OAuth 로케일 쿠키·다크 폴리시 D·클로즈드 베타. (아래 스냅샷들은 히스토리.)
>
