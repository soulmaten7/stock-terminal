...
## §1-4b 배포 후 육안 확인(게이트8)에서 발견·수정한 잔여 갭

라이브 배포 확인 중 `curl https://onetrillion.app/business` → **404**(정상, 페이지 삭제됨). 다만 `app/sitemap.ts`가 여전히 `/business`를 색인 대상으로 광고하고 있어(search-engine에 죽은 링크를 계속 알리는 문제) 발견 즉시 제거 — **커밋③ = `fca39a1`**. 배포·CI 재확인 후 `sitemap.xml`에 `/business` 0건, `/`·`/advertise`·`/ko` 전부 200 확인.
