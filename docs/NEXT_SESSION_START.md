...
> **Last updated**: 2026-06-07 (STEP 199 — 인기토론 홈 + 투자상품 랭킹 ETF·증권사 링크)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: STEP 199 까지 완료 — **V7(토스 오마주 + 운종 차별화 진입)**. 빌드 ✓ (HEAD `ef5f6ec`). 홈 = **[헤더밑 얇은 지수 티커 고정] → [🔥 인기토론 2열 라이브](좋아요순·반론노출·20초폴링·빈상태 CTA) → [실시간차트 탭] / 관심레일**. 실시간차트 탭 = 토스식 필터칩+기간행+투자위험 토글 ｜ 카테고리 2열(국내·미국섹터ETF) ｜ 투자자동향 3열 ｜ **투자상품**(ETF 인기순/수익순 1·3·6·12M + 펀드 준비중). 종목 상세에 **"어디서 거래할까" 증권사 바로가기**(거래 X 동선). 로고 logo.dev · 등락색 한국식(상승빨강/하락파랑). ⚠️ 수급은 홈에서 빠짐(투자자동향 탭엔 유지). **⏳ STEP 162 KRX 공식 OpenAPI = 키 대기**.
> ▶ 다음: **리딩방 랭킹**(FSS 검증 — 인증/등록 위 + 추천 아래 + 신고/주의 신호) · 펀드 데이터 소스 · STEP 162 키 승인 시 실행. **광고는 사용자 지시 시에만(철칙도 그때 사용자가 정함)**.
