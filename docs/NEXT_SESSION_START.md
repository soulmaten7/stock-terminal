...
> **Last updated**: 2026-06-07 (STEP 214 — 홈 실시간채팅 + 우측 레일 정리)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: STEP 214 까지 완료 — **V7(토스 오마주 + 운종 차별화)**. 빌드 ✓ (HEAD `6ef495f`). 홈 = [지수 티커 고정] → [🔥 인기토론 2열] → [랭킹 탭 6개]: 실시간차트 ｜ 카테고리 2열 ｜ 투자자동향 3열 ｜ 투자상품 랭킹(ETF 거래대금/수익률 + hover 미리보기) ｜ 리딩방 랭킹(텔레/카톡) ｜ 주식 관련 채널 랭킹(유튜브·디스코드·인스타·페북, 팔로워순). **상세 페이지 보강 완료**: 방/채널(`/room/{id}` 로고+투표 👍/👎+조회수+입장) · 종목(`/stock` 색버그 수정·헤더 로고·기본탭 차트·전일대비). **DB**: 023(투표)·024(조회수 RPC `increment_room_view`)·025(팔로워 `follower_count`) 운종 DB 적용. 로고 logo.dev·등락색 한국식. ⚠️ 수급은 홈에서 빠짐(투자자동향 탭). **홈 우측 레일** = ⚡실시간채팅(`HomeLiveChat`, chat_messages HOME방 realtime·반화면 46vh) + 관심종목(폭 400px·고정 아님, 페이지 스크롤 따라 늘어남). **⏳ STEP 162 KRX = 키 대기**.
> ▶ 다음: **유튜브 팔로워 자동수집(B)**(YouTube Data API 키 + 채널ID, 주1회 크론 — 인스타/페북=등록 기반) · 펀드 소스 · 상세 페이지 추가 다듬기 · STEP 162 키. **광고는 사용자 지시 시에만(철칙도 그때 사용자가 정함)**.
