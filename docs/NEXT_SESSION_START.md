...
> **Last updated**: 2026-06-04 (STEP 143 — 홈 빈 섹션·버그 수정)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: STEP 143 까지 완료. 빌드 ✓ (HEAD `0d7c23b` — STEP 143 docs). 홈 = `components/home-v6/HomeClientV6` 포털형. 브리핑(야후 라이브러리)·거래량랭킹(실값)·업종테마(market=KR/US) 복구 + 랭킹 레터아바타. 종목 탭 5종: 차트·시세/토론/뉴스/공시/인사이트.
> ▶ 다음: 사용자가 섹션별 "운종 방향 수정" 지시 → 빠른 반복 (문구·강조·제거·데이터 교체).
...
## 0. 진행 상태 한눈 (2026-06-04 기준 · HEAD `0d7c23b` STEP 143)
...
| 순위 | 후보 | 의미 |
|------|------|------|
| 1 | **브리핑 overnight 값 안정화** (간밤 지수 표기 신뢰화) | 홈 데이터 품질 |
| 2 | **지수 카드 스파크라인** (HomeIndexBar 미니 차트) | 홈 시각 |
| 3 | **홈 레이아웃 비율 미세조정** (메인/우측레일 균형) | 포털 완성도 |
| 4 | **인기글 시드** (HotDiscussions·평가글 0건 → 초기 콘텐츠) | 빈 섹션 채우기 |
| 5 | **외국인보유율·상장주식수 메타** (종목 InfoPanel 보강) | 정보 깊이 |
| 6 | **Sponsored 분리 UI** (광고 ↔ 평가 시각 분리) | MVP 2.0 신뢰 |
| 7 | **카카오 OAuth 활성화** (사용자 직접) | 추천/비추천 투표 실동작 전제 |
| 8 | **Vercel 배포 + unjong.com** (사용자 결정) | FSS cron 활성 + 출시 |
