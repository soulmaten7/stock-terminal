...
> 🆕 **2026-06-15 현재 (STEP 241)** — 아래 'Last updated' 일부 단락은 STEP 227 시점 옛 상태(6탭·인기토론 등)가 섞여 있음. **최신은 이 블록 기준.**
> - **이번 세션** = 전략 대화로 정체성 재확정 → UI 전면 재설계. 빌드 ✓ (HEAD `bbf4e88`).
> - **홈** = 지수 티커 → **랭킹 탭(상품 타입)**: 주식 · ETF · ETN · 펀드 · 리츠 ｜ 리딩방 리스트 (속보 카드 제거, 성적표 최상단). 우측 레일 = 실시간채팅 + 관심종목.
> - **성적표** = 단일 '[기간] 대비' 칼럼 + 기간칩(1일·1주일·1개월·3개월·6개월·1년, 기본 1일전, 칩=기간 선택·정렬). **주식 1주~1년·시총 = "—"(미연동)**. ETF는 1·3·6·12개월 yahoo 실데이터(1주일 —). ETN·리츠·펀드 = '준비 중'.
> - **헤더** = 홈 · 상품 리스트(`/market`) · 주식 관련 링크모음 (뉴스·시황 제거, 마켓→상품 리스트).
> - ⚠️ **STEP 240(ETF 미리보기 폭 주식과 동일) 미적용** — `docs/STEP_240_COMMAND.md`만 있음(239→241 건너뜀). 적용 대기.
> - ▶ **다음**: ① STEP 240 적용 ② **주식 1주~1년·시총 실데이터 연동**(yahoo kr-performance 등 — 데이터 레이어, 큰 작업) ③ ETN·리츠·펀드 데이터 ④ `/market` 상품 통합 디렉토리화. **광고는 사용자 지시 시에만.**
