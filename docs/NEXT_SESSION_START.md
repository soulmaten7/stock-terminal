...
> - **🔑 발견**: `lens_scores.name` KR 값이 **야후 영어**("SamsungElec") → **①에 야후 `longName` 활용 가능** 확인.
...
> - **⭐ ②a 관심목록 렌즈 개편(739~742)**: 739(`9464f0e`) 배치 시세 `/api/watchlist/quotes`(국가별 스냅샷 .in) · 740(`e4571e7`) WatchlistClient 재설계(시세 즉시+행별 지연 렌즈 요약 강점/주의/보통·점7·`/api/lens` tone·동시성4·반응형2줄) · 741(`0fe6971`) 종목 hero+리딩방 /en 숨김 · 742(`44fa289`) 현재가 세로정렬(이름 flex-1·렌즈 w-64)+설명 종목화. 라이브(가격 x=1269 정렬·삼성전자 강점2·주의2·보통3·en 리딩방 숨김).
> - **🔑 제약**: `lens_scores`=US 1000종목만 → KR 관심목록 렌즈는 live `/api/lens` 지연(005930→.KS 실시간). **②b**=KR 렌즈 선계산 크론(즉시화 후속).
> - **🔍 발견/다음**: **①** 비미국 종목명 /en 한글(name_en 없음·US SEC만 커버)→영문명 소싱(야후 longName)+저장+배선 · **④** 종목페이지 렌즈헤더+보드 상단 자동미리보기(목업 확정·미구현·"거래상위 예시" 중립·스플래시 반대→인라인 카드) · **②b** KR 렌즈 선계산.
> - **▶ 다음 순서 = ④ → ①(비미국 종목명) → ②b.**
>
> 🎨 **2026-07-17 (1) — 베타 UX·가독성 폴리시 + 지수 티커 이중방어 + 광고문의 언어권 차등 + /en 푸터 정리. HEAD `21c6649`.**
> - **코인 탭 숨김**(`4135463`·베타 준비) · **다크 가독성**: muted `#8A8D93→#9CA3AF`(`6b2ce97`·AA→AAA) + 헤더/티커 `#0E1116` 흰색 `/40~55→/65~70`(`ed425f4`·태그라인 3.8→8.2:1).
> - **📱 모바일 풀블리드**(`659b0e2`): 홈보드 박스 `border-y sm:rounded-2xl sm:border`+페이지 `px-0 sm:px-6`(모바일 화면끝·데스크톱 카드·반응형=일관성≠동일함·웹서치 근거) + 티커↔보드 빈공간 제거(`155da9e`·모바일 `pt-0`).
> - **🐞 지수 티커 사라짐 이중방어**: 원인=`/api/yahoo/indices` `Promise.all`서 심볼 하나 야후 실패→전체 reject→빈 응답(200) + 클라가 빈 응답으로 기존 티커 덮어씀(간헐 ~20%). **클라**(`fa8a201`·데이터 있을 때만 갱신+5초 재시도) + **서버 STEP 737**(`d391c0c`·심볼별 try/catch 격리+`_lastGood` fallback). 라이브 16회 0빈(min 20=격리 작동).
> - **🔤 헤더 로고 반응형**(`0431d06`): 22px→`h-6 lg:h-8`(24/32px·2줄 정합)+Trillion `leading-none`·태그라인 `mt-0.5`(간격 9→2px).
> - **📢 광고문의 언어권 STEP 738**(`8c7c7a8`): 리딩방=한국 특유→en 제거(room 슬롯·rule2·폼옵션·note·phCompany)·ko 유지·server `getLocale`+client `useLocale`. 라이브 en 2슬롯3규칙/ko 3슬롯4규칙.
> - **🐞 교훈**: Vercel 배포 큐 지연→빈 커밋 재트리거 · `Promise.all` 하나 실패=전체(부분허용=심볼별 try/catch) · 클라 빈응답 무비판 반영 금지 · `resize_window` 최대화 창 무반영 · 반응형=일관성≠동일함.
...
>
> 🌐 **2026-07-15 (5) — US 뎁스 완결(배당·ETN) + /about 개선 + /en 완전 영어화. HEAD `3ecadaa`.**
...
> - **▶ 다음(선택) = US 배당 캘린더(Nasdaq calendar/dividends→KR 완전 동급) · ETN 서브탭 · 클로즈드 베타.**
>
...
> - **🌑 다크 테마(라이트→다크·3단계·안 깨지게)**: **1/3**(`f029d91`) 토큰화(배경전용 `unjong-strong` 신설·`bg-unjong-primary`→strong·`bg-white`→surface·겉모습 변화 0). **2/3**(`07fc4bf`) 플립(`app/globals.css` 토큰 값만 다크·background #0A0A0A·surface #17181C·border #2A2C31·`color-scheme:dark`·**라이브 검증**). **3/3**(`3c2fc8b`·🔴Opus) 폴리시(앰버 배지·게이지·상태색 다크 대비+구글버튼·StockLogo 실로고원 라이트 유지·레터아바타 이니셜 strong). 근거=미드나잇+민트·헤더 이미 다크·데이터밀도 금융툴 다크 정석. **개선이지 전환 아님.**
...
> - **▶ 다음 = 데이터 검수 Round 2(Chrome 라이브)·Round 3(교차) + CN #2(A주 소형주 ~1,600) → 한국어 광고.** ⚠️ cninfo·HKEXnews·지수 Vercel 도달성 최종 실측 잔여.

> 🆕 **2026-07-09 (2nd) — STEP 665~667: UI 리파인 묶음 완성 + 지수 티커 6개국.**
> - **665** 표 AI렌즈 컬럼 제거(5열·TLensLogo 정리)·수익률 '1일전…'·브리핑 13px. **665B** 클릭 힌트 컨트롤 줄(하위탭 뒤). **666** 지수 티커 TOPIX·상하이종합 추가(22개)·KR→JP→CN→VN→US→GB→ETC·블록 구분선·라이브 확인. **667** 검증 배지 민트→다크틸 AA(`text-unjong-success`)·6개 보드 빨강/파랑 범례(데스크탑).
> - **662~664**(같은 세션): 증권사 독립 탭 + 종목보드 우측 레일=AI 렌즈 미리보기(렌즈+브리핑·`LensPreview` 공유·6개 보드+모바일) + 663D `marketDate`(as_of 시장 로컬날짜) + 광고 CTA 하단 1개.
> - **🌍 `docs/LOCALE_SOURCE_PLAYBOOK.md` 신설** — 언어권 데이터소스 발견·검증·기록 런북(의미우선 스키마·검증게이트·실패원장·서학개미 §5). **새 locale·데이터소스 착수 전 필독**(CLAUDE.md 참조 등록).
> - **▶ 다음 = 광고 대화**(진짜 광고 데이터 모델·게재·결제 — 사용자와 전략 논의 먼저) 또는 **서학개미 relevance 파이프라인**(플레이북 §5·data.go.kr 키 필요·`localeRules`+`marketDate` 패턴 확장). UI 리파인 묶음(662~667) 종료. ⚠️ 배포 후 cninfo·HKEXnews·지수 Vercel 도달성 최종 실측.
...
## 3. 운종 정체성 (V6 — 2026-06-03 · [이력·폐기])

> ⚠️ **폐기 — 현행 아님.** 현행 정체성 = `docs/BRAND_IDENTITY.md`(3기둥 무기·직시·자립 · "종목을 보는 눈을, 누구에게나."). 아래 §3~6은 V6/V7 스냅샷(보존용).
...
## 6. 절대 잊지 말 것 (운종 결정사항 — [이력·V6 스냅샷])

> ⚠️ 아래는 V6 결정 스냅샷(보존용). 현행 정체성·경계는 `docs/BRAND_IDENTITY.md`·`docs/SESSION_BOOT.md` 기준(예: "영어판 X"·"허브"는 현행 아님).
