...
> 🆕 **2026-06-15 현재 (STEP 246)** — 아래 'Last updated' 일부 단락은 STEP 227 시점 옛 상태(6탭·인기토론 등)가 섞여 있음. **최신은 이 블록 기준.**
> - **이번 세션** = 전략 대화로 정체성 재확정 → UI 전면 재설계 → **기간 수익률 실데이터 연동 → /market 통합 디렉토리**. 빌드 ✓ (HEAD `bfa7d97`).
> - **홈** = 지수 티커 → **랭킹 탭(상품 타입)**: 주식 · ETF · ETN · 펀드 · 리츠 ｜ 리딩방 리스트. 우측 레일 = 실시간채팅 + 관심종목.
> - **성적표** = 단일 '[기간]전 대비' 칼럼 + 기간칩(1일·1주일·1개월·3개월·6개월·1년, 기본 1일, 칩=기간 선택·정렬). **주식·ETF·리츠·미국 = yahoo 실데이터 ✅**(기간 수익률 채워짐). ETN·펀드 = '준비 중'.
> - **`/market`('상품 리스트')** = **전 타입 통합 디렉토리** — 주식·ETF·리츠를 한 테이블에 같은 기간 수익률 자로 가로질러(타입 배지·타입 필터·기간칩, `MarketDirectoryClient`). **핵심 차별점 '가로질러 비교' 실현.**
> - **헤더** = 홈 · 상품 리스트(`/market`) · 주식 관련 링크모음.
> - **데이터 소스**: 기간 수익률 = yahoo 과거 시세(`/api/yahoo/{kr,etf,reit,us}-performance`, 영업일 오프셋, 30분 캐시). 주식 1일·시총 = KRX. **ETN 보류**(yahoo 없음 → KRX ETN 엔드포인트) · **펀드 보류**(KOFIA 소스).
> - ▶ **다음**: ① US를 /market 통합에 합류(us-performance에 name 추가) ② **종목→증권사 바로가기**(허브 연결, `lib/brokers` 재사용) ③ ETN(KRX)·펀드(KOFIA) 소스 ④ AI 해설. **광고는 사용자 지시 시에만.**
...
> **Last updated**: 2026-06-15 (STEP 228~241 — 홈 = 상품 성적표 재편 + 헤더 정리)
