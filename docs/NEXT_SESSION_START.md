...
> 🆕 **2026-06-15 현재 (STEP 260)** — 아래 'Last updated' 일부 단락은 옛 상태가 섞여 있음. **최신은 이 블록 기준.** (STEP 254~260 = ETN 실데이터 연결 + 펀드 시도→제거 + 탭 새로고침 유지. 펀드는 무료 수익률 소스가 없어 제거 = 유료 데이터 영역.)
...
> - ▶ **다음**: 스코프 재정렬(거래 상품 수익률 + 리딩방·채널 검증 + 신뢰). 빌드 가능 잔여: **US·ETN을 /market 통합 디렉토리에 합류** · **종목→증권사 바로가기**(허브, `lib/brokers`) · **리딩방·채널 검증 강화**(차별점). 사용자 결정: 유튜브 팔로워 API 키 · 카카오 OAuth(투표) · AI 해설 빌드 여부(`docs/AI_LENS_SPEC.md`) · 평가·검증 MVP 2.0 · (펀드 수익률은 유료 데이터 도입 시). **광고는 사용자 지시 시에만.**
