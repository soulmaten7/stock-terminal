...
> **Last updated**: 2026-06-03 (STEP 142 — 포털형 홈 전면 재구성)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: ~141 + 포털형 홈 재구성(142) 완료. 빌드 ✓ (HEAD `29c3404` — STEP 142). 홈 = `components/home-v6/HomeClientV6` (지수바·브리핑·뉴스·검증평가·랭킹·업종테마·인기글·가상자산(placeholder)·퀵링크·ETF·이용자종목(placeholder)·우측레일). 데이터없는 섹션은 placeholder shell. 종목 탭 5종: 차트·시세/토론/뉴스/공시/인사이트.
> ▶ 다음: 사용자가 섹션별 "운종 방향 수정" 지시 → 빠른 반복 (문구·강조·제거·데이터 교체).
> ⚠️ **미적용 마이그레이션 (Cowork Supabase MCP 대기)**: `022_discussion_dislike.sql`(종목 토론 비추천). 적용 전엔 종목 토론 투표가 에러 — UI·빌드는 정상. (020·021 은 적용 완료)
