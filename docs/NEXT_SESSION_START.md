...
> **Last updated**: 2026-06-07 (STEP 206 — 리딩방·채널 랭킹 + 좋아요/싫어요 DB + 투자상품 미리보기)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: STEP 206 까지 완료 — **V7(토스 오마주 + 운종 차별화)**. 빌드 ✓ (HEAD `2bf1bc7`). 홈 = [헤더밑 지수 티커 고정] → [🔥 인기토론 2열] → [랭킹 탭] / 관심레일. **랭킹 탭 6개**: 실시간차트 ｜ 지금 뜨는 카테고리 2열 ｜ 국내 투자자 동향 3열 ｜ **투자상품 랭킹**(ETF 거래대금순/1·3·6·12M 수익률 + hover 미리보기 + 펀드 준비중) ｜ **리딩방 랭킹**(텔레/카톡, 금감원 인증·👍/👎/👁·좋아요·싫어요·조회 정렬칩·hover 프리뷰[연결카드+투표+방평가]) ｜ **주식 관련 채널 랭킹**(유튜브·디스코드·인스타·페북). 종목/ETF hover→미리보기, 종목 상세에 증권사 바로가기. **DB 023**(`leading_room_votes`) 적용. 로고 logo.dev·등락색 한국식. ⚠️ 수급은 홈에서 빠짐(투자자동향 탭). **⏳ STEP 162 KRX 공식 OpenAPI = 키 대기**.
> ▶ 다음: **상세 페이지 디테일**(방/채널/종목 클릭 후 토스처럼) · 채널 **팔로워순**(`follower_count` DB + 유튜브 Data API 주1회 크론, 인스타/페북=등록 기반) · 펀드 소스 · STEP 162 키. **광고는 사용자 지시 시에만(철칙도 그때 사용자가 정함)**.
