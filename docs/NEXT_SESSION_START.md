...
> **Last updated**: 2026-06-04 (STEP 145 — 홈 데이터 품질 P0: 지수 스파크라인·브리핑 overnight 안정화)
> ⭐ **더 디테일한 마스터 인수인계**: `docs/NEXT_SESSION_PLAYBOOK.md`. **마스터 비전**: `docs/PRODUCT_SPEC_V6.md` (정체성 축 = "안 속는 곳").
>
> **현재 상태**: STEP 145 까지 완료. 빌드 ✓ (HEAD `90cb8a3` — STEP 145 코드). 홈 = `components/home-v6/HomeClientV6` 포털형. 지수바 30일 스파크라인(inline SVG) + 브리핑 overnight "—" 안정화(가짜 초록 0.00 제거). 종목 탭 5종: 차트·시세/토론/뉴스/공시/인사이트.
...
## 0. 진행 상태 한눈 (2026-06-04 기준 · HEAD `90cb8a3` STEP 145)
...
| 1 | **홈 레이아웃 비율 미세조정** (메인/우측레일 균형) | 포털 완성도 |
| 2 | **인기글 예시 시드** (HotDiscussions·평가글 0건 → 초기 콘텐츠, '예시' 명확 표기) | 빈 섹션 채우기 |
| 3 | **외국인보유율·상장주식수 메타** (종목 InfoPanel 보강) | 정보 깊이 |
