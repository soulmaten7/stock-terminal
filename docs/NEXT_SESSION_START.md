...
> 🆕 **2026-06-20 현재 (STEP 300)** — **최신은 이 블록 기준** (아래 STEP 271 단락부터는 V6 히스토리).
> **V7 완성**: 운종 = **검증된 중립 관문(게이트웨이) + 리딩방 검증.** 네이버 클론(랭킹·차트·종목상세·`/market`) 폐기. 마스터 비전 `docs/PRODUCT_SPEC_V7.md`. HEAD `2a0e1b2`(300), 빌드 ✓.
...
> - **유튜브 Top100**(구독자순, 주간 크론 `youtube_channels`) · **리딩방·검증**(금감원 신고 1,738건 `fss_advisors` → 플랫폼탭/검색/좋아요/신고/분할 미리보기, 뷰 `advisor_directory`) · **로그인**(카카오+구글, `middleware.ts` 추가로 작동, 헤더 이니셜 아바타).
> - **▶ 다음 후보(보류·사용자 결정)**: 자가등록(운영자 직접 등록+사업자번호 FSS대조 → 즉시공개·배지없음, ✅등록확인은 본인확인 후) · 본인확인(사업자등록 후 유료) · 푸터 V7 정리(개인정보처리방침 필수) · **업체명(운종) 변경 검토 중**(확정 전 사업자등록·도메인·이메일 보류, 기능은 이름 무관 진행 가능).
> - ⚠️ 보안: 유튜브 API키·구글 Client Secret 스크린샷 노출 → rotate 권장(미실행). ⚠️ DB 직접변경(git 아님): youtube_channels·room_reports·room_likes·advisor_directory·link_hub+8.
> - 상세는 **`docs/SESSION_BOOT.md`(최우선 1번)** · `docs/CHANGELOG.md`(STEP 272~300).
>
