...
> **Last updated**: 2026-06-01 (STEP 127 — 가독성 리뉴얼: Pretendard + 16px 루트 + 텍스트/패딩 상향)
> **현재 상태**: V5 PC 핵심 + 핫픽스(126) + 가독성 리뉴얼(127). 빌드 ✓. 다음은 사용자 시각 확인 후 미세 조정.

### STEP 127 검증 (사용자 — 하드 리프레시 Cmd+Shift+R 필수)
- 전 페이지 텍스트 크게 + Pretendard 폰트 + 카드 여유. 네이버 페이 증권 수준 목표.
- ⚠️ 루트 16px 로 올려 rem 텍스트·spacing 이 전부 확대됨 → 과하면 어디를 줄일지 피드백
- ⚠️ 고정 높이 바(Header h-[60px]·TickerBar)에서 커진 텍스트 짤림 없는지 확인
- 잔여: `app/layout.tsx` Inter import 미사용(무해) 추후 정리 / text-sm→text-base·gap 상향은 미적용
