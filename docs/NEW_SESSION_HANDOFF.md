...
> **갱신 시점: 2026-07-06 · HEAD `b2079b7`(STEP 611) · 앱 배포 ✓ onetrillion.app.**
> **🆕 2026-07-06 (최신): STEP 600~611 — 'AI 렌즈' 브랜딩·발견성 + KR AI 확장(R2·R3) + JP R3 뉴스 (HEAD `b2079b7`).** 옛 'TRAI'→**'AI 렌즈' 박스 배지**("기법별 전망" 제거) · 종목 뒤로=`router.back()`+**시트 URL 복원**(`useSheetSync`) · **AI 렌즈 발견성 표식**(현재가↔1일전·PC 컬럼/모바일 헤더라벨+행 아이콘) · **KR=R1·R2·R3 완성**(R2 DART 공시·R3 한글명 ko·짜깁기 금지·`cf22aba`) · **JP=R3 뉴스**(야후 일본명 ja·요약 한국어 번역 폴백·옛연도 문장 삭제·60일 최근성·`b2079b7`). **국가별 AI: US R1·R2·R3 / KR R1·R2·R3 / JP R3 / CN 미착수.** R4(Q&A)=영구 보류(어드바이저=포지셔닝 밖+무료 상충). 🔒 규칙=Claude Code 3회 반복+Cowork MCP 실물 재검수. **교훈: JP 뉴스는 야후 영어명·구글 옛기사 재순환 탓에 프롬프트 2회 실패→결정론 코드 후처리로 확정.** ▶ 다음=CN R3 / JP EDINET(R1·R2·무료 키) / SEO — 국가 확장은 승인 후. (아래는 히스토리.)
>
