...
> **갱신 시점: 2026-07-02 · HEAD `5bdf56f`(STEP 537) · 앱 배포 ✓ onetrillion.app.**
> **🆕 2026-07-02 (최신): 🏁 신뢰도 업그레이드 사이클 종료(STEP 525~537).** 5렌즈 전부에 논문급 *방법론*(월별 롱숏 t값·샤프·Fama-French 팩터알파·거래비용) 적용 → "다 검증"을 정직한 등급으로 재정렬: **모멘텀=검증·유의**(t≈2.5·샤프0.71·비용/FF3 후 유의) · **저변동=위험대비 강**(위험18%·알파유의) · **밸류=정설이나 표본 약함**(βHML0.71·월별 t<2) · **F-Score=수익 신호 아님**(12코호트 t0.70·건전성만) · **기술=참고용·비독립**(RSI 유의 손실·200일선 모멘텀 흡수). 메타교훈=유의≠수익수준(생존편향 과대·방향만 신뢰)·소표본 유의는 노이즈(F t2.24→t0.70)·무료 방법론 논문급이나 CRSP급 데이터는 벽. 자산 `lib/backtest_stats.ts`·`scripts/backtest_*_rigor.ts`·`_alpha.ts`·Ken French(`data/ff`)·플레이북 #18~22. **원칙: 예측 아닌 방향성. 수익화·유료 AI보기(STEP 511)·UX 계속 뒤로. ▶ 다음 = KR/글로벌 렌즈 확장 or 새 기법(퀄리티·마법공식) or 생존편향 없는 데이터 승격.**
