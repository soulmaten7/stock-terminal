...
3. **새 파일 안 만들었는가** — 이 파일(`probe_1017_cron_failure_causes.md`)에 §6로만 추가, 새 probe 파일 없음. `model_list_30_9.html` 기존 문단에 한 줄만 추가(삭제 없음).

> 🔴 **STEP1079 정정(2026-08-19)**: 위 §6~§8은 원래 STEP1078이 `§5`~`§7`로 붙였으나, 이 문서에 **이미 §5(:252)·§6(:258)이 있어**(STEP1058이 먼저 씀, "먼저 있던 번호가 우선") 셋 다 하나씩 밀어 §6~§8로 재번호했다. 내용은 무변경, 번호만 정정.
