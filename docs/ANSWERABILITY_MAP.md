...
> 🟢 **2026-08-14 STEP 1017 — 어젯밤(08-13 21:30~22:45 UTC) 크론 실행 결과 확인.** `revdcf`는 `revdcf_results`(604행)·`us_valuation`(5,820행)까지 완료했으나 **heartbeat가 기록되지 않았다** — 코드상 `finally`가 보장돼야 하는데도 안 돌았다는 정황(`app/api/cron/revdcf/route.ts:374-399`), Vercel 로그는 403으로 미확인. `us-perf`의 나스닥 폴백은 **20초 timeout으로 실패**(로컬 1회 재현도 동일 결과 — egress 전용설 근거 약화). `us_market_cap`은 1008의 "296건 고정" 서술과 달리 **287로 9건 회복**을 확인(고정 아님). 상세 = `docs/probe_1017_cron_failure_causes.md`.
> 🟢 **2026-08-14 STEP 1023 — F(재무 원문 수치) 정정.** STEP1022가 "322건(7.07%) 낡은 주식수"라 보고한 것을 F의 한계 고지에 반영했었으나, **그 322건은 `us_fundamentals.shares`가 아니라 STEP1019~1022 조사 스크립트 내부의 임시 비교값이었다**(`scripts/probe_1022_sec_window_audit.ts`에 `us_fundamentals` 참조 0건, 직접 확인). 프로덕션(`app/api/cron/revdcf/route.ts:278-281`)은 이 조사가 문제 삼은 SEC `frames`(6분기 조회창)를 애초에 쓴 적이 없고 `companyfacts`(창 없는 전체 이력)를 직접 호출한다 — **프로덕션 영향 0.** 상세 = `docs/probe_1023_stale_shares_impact.md`.
...
| 🔴 현재 상태(1017 추가) | `us_sector_relative`가 **08-10에서 4일째 정지**(1016 관측 대비 하루 더 경과, 상태 불변). 어젯밤(08-13 22:45 UTC) `revdcf` 크론이 `revdcf_results`(604행)·`us_valuation`(5,820행)은 완료했으나 **heartbeat 자체가 기록되지 않았다** — `computeAndSaveSectorRelative`(`app/api/cron/revdcf/route.ts:384`) 단계에서 함수가 완주하지 못했다는 정황증거(로그 미확보라 확정은 아님). 즉 D의 정지는 **일회성이 아니라 매일 밤 같은 지점에서 반복되는 중일 가능성**이 있다. 상세 = `docs/probe_1017_cron_failure_causes.md` §1-1 |
...
| 한계 고지 | 🟡 **SEC 제출 원문 기반**(벤더 정규화 아님, i18n 상세 미대조). 🔴 **`shares`는 `source_tags`에 태그 기록이 없다**(STEP1011 실측·1015 재확인) — 단 F 전체가 서빙되지 않는다는 더 근본적 사실 위에 얹힌 지엽 사항. ~~🔴 **STEP1022 추가 — 주식수 시점(freshness) 문제**: 이 세션의 조사에서 쓴 SEC `frames` 6분기 조회창은 **이미 채워진 값 중 7.07%(322/4,557)가 실제로는 더 최신 주식수를 놔두고 며칠~몇 달 더 낡은 값을 쓰고 있음**을 드러냈다(중앙값 49일). 결측 보완 목적으로 조회창을 넓힐 경우엔 **17/18건이 5년 이상 낡은 값**이라 커버리지와 정확도가 상충한다 — 상세 = `docs/probe_1022_sec_window_audit.md`~~ → 🔴 **정정(STEP1023) — 위 322건은 `us_fundamentals.shares`를 가리키지 않는다.** `scripts/probe_1022_sec_window_audit.ts`는 `us_fundamentals`를 읽거나 쓰지 않는다(grep 확인, 0건) — 322건은 이 조사 스크립트가 자체 실행 중 만든 두 휘발성 값(구 기준선=SEC `frames` 6분기창, 신 기준선=SEC `companyfacts` 전체이력)을 서로 비교한 결과였다. **프로덕션(`app/api/cron/revdcf/route.ts:278-281`)은 애초에 `frames`를 쓴 적이 없다** — `companyfacts`를 직접 호출하고, 태그 우선순위도 다르며(`WeightedAverageNumberOfDilutedSharesOutstanding` 최우선, `lib/revdcf/drivers.ts:365-373`), 복수클래스는 이미 `MULTI_CLASS_SHARES`로 스킵 처리된다(`drivers.ts:375-384`). **F가 서빙되지 않는다는 근본 한계는 그대로 유효하나, "322건 낡음"은 그 한계에 얹을 근거가 아니다.** 상세 = `docs/probe_1023_stale_shares_impact.md` |
| 근거 | 🟡 **확인불가(기능 자체 없음)** — `grep -rln "us_fundamentals" app/ lib/ components/` 결과 3개 내부 파일만, 서빙 라우트 0건. 상세 = `docs/probe_1015_answerability_audit.md` §1-F · 🔴 주식수 시점 문제는 프로덕션과 무관함이 확인됨(`docs/probe_1023_stale_shares_impact.md`, 원 조사 = `docs/probe_1022_sec_window_audit.md`) |
...
| 🔴 현재 상태 | **373건이 2026-07-30 이후 갱신 없음**(~~296건 고정~~ → 🔴 **1017/1018에서 정정: 고정이 아니라 완만히 회복 중** — 296→287, 아래 행 참조) — 게이트가 없어 이 373건도 차단 없이 그대로 응답에 나간다(1015 확인). `asOf`만 07-30으로 정직하게 찍힘 |
...
| 🔴 현재 상태(1017 추가) | 어젯밤 크론 정상 갱신 확인 — `us_market_cap` 08-13에 5,611건 신규(전체 5,913건). 🔑 **1008의 "296건 고정" 서술이 어긋남을 확인** — 07-30 고정 코호트가 296→**287로 9건 회복**, 반면 08-02~08-11 사이 14건은 전혀 안 움직임(고정 서브그룹과 움직이는 서브그룹이 섞여 있음). `us-perf`의 나스닥 시총 폴백은 어젯밤도 **20초 timeout으로 실패**(`nasdaqRows:0`) — 로컬 1회 재현도 동일하게 20,008ms에서 timeout, "Vercel egress 전용 문제" 단정 근거는 약해짐. 상세 = `docs/probe_1017_cron_failure_causes.md` §1-2·§1-3·§1-4 |
