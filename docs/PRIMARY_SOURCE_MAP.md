...
> **왜 만들었나**: 차이 9행(887 재분류 후 현재 4행)을 한 행씩 판정하는데, 지금까지의 판정 근거가 **원본이 아니라 이전 STEP의 요약 문장**에 기대어 있었다. 2026-08-02 하루에 그런 인용이 **여섯 번** 뒤집혔다(플레이북 #72).
...
### 🔴 877 재정정 — `T7 Inputs!C10` 직접 개봉 결과 (2026-08-03 · 오늘 두 번째 정정)

**위 §1의 "결론"(원전은 세율을 두 개 쓴다)이 다시 틀렸다.** `T7.xlsx` `Inputs`·`WACC` 시트를 직접 열어 셀 값을 확인한 결과:

```
T7 Inputs!C10 (Corporate tax rate) = 0.165
T7 WACC!C6 (After-tax cost of debt) = C5×(1−Inputs!C10) = 0.037959
```

**`0.165`는 한계세율이 아니라 T6가 계산한 도미노 현금세율과 같은 값이다**(T6 최근 2년 0.164/0.167 · T8 `Inputs!C15 Cash tax rate`도 0.165로 동일). → **원전은 NOPAT·WACC 양쪽에 같은 현금세율(16.5%)을 쓴다.** ~~"WACC 세후 부채비용=한계세율(T7 B26)"~~은 **틀렸다** — `Tutorial 8` B26 본문이 *"We enter the **marginal tax rate** in cell C10"*이라 서술하지만, 그 서술과 실제 셀 값이 다르다(**서술≠셀**, 플레이북 #76).

**정정된 표**

| 쓰이는 곳 | 원전 | 우리 |
|---|---|---|
| NOPAT | 현금세율 **16.5%** | 한계세율 25.63% |
| WACC 세후 부채비용 | 현금세율 **16.5%**(~~한계세율~~) | 한계세율 25.63% |

🔑 **driver 3의 진짜 차이는 여전히 "세율이 하나냐 둘이냐"가 아니라 "어느 세율(현금 vs 한계)이냐"다** — 위 46행의 "하나냐 둘이냐" 결론은 **철회.** 구조(하나를 양쪽에)는 원전·우리가 같고, **값만** 다르다.

### 🔴 기존 기각 사유가 성립하지 않는다 (①③은 유지, ②는 877로 재수정)

`REVDCF_SPEC` §622·§966·§1311이 든 이유:
> ~~*"현금세율은 이자 세금방패를 이미 반영 → WACC 세후 부채비용에서 또 세는 **이중 계산**. **다모다란 지적**을 따름"*~~

1. **T6 현금세율은 unlevered** — 방패를 **빼낸** 값이다. "이미 반영"이 아니다. (유지)
2. ~~**원전은 WACC에 현금세율을 쓰지 않는다**(T7 = 한계세율).~~ 🔴 **877 재수정**: 원전은 WACC에도 **현금세율(같은 값)**을 쓴다. 그럼에도 이중계산이 발생할 구조는 아니다 — **하나의 값을 양쪽에 한 번씩** 쓸 뿐, 어느 쪽도 "이미 반영된 것을 또 반영"하지 않기 때문이다(이중계산은 방패를 두 번 빼거나 두 번 더할 때 생긴다).
3. **다모다란 출처 미확인** — 그의 세율 Q&A(`pages.stern.nyu.edu/~adamodar/New_Home_Page/valquestions/taxrate.htm`)는 이자 방패 이중계산을 **다루지 않는다.** (유지)
