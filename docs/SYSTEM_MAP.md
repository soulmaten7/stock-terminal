...
> 🆕 **2026-09-06 콘텐츠 번역(신설, 65개)**: **`channel_report_translations`**(report_id·target_lang 복합키 — title/reasons/earnings_summary 번역 캐시. stock_name/broker/verdict는 여기 없음, 아래 참조). `channel_reports` AFTER INSERT 트리거(`trigger_translate_channel_report`, pg_net 비동기)가 `/api/internal/translate-channel-report`를 즉시 호출 — 채널이 리포트를 적재하는 순간 번역까지 끝난다(독자 지연 0). 인증은 Supabase Vault(`channel_report_translate_secret`) + Vercel env(`CHANNEL_REPORT_TRANSLATE_SECRET`) 대조. 읽기 경로(`lib/channelReportI18n.ts`)는 로케일별로 이 테이블을 조회해 있으면 교체, 없거나 status='failed'면 원문 그대로(번역 실패가 화면을 깨지 않음). stock_name/broker는 자유번역이 아니라 `kr_stock_snapshot.name_en` 조회(+비상장 증권사 소규모 용어집, `BROKER_EN_GLOSSARY`)로 대체하고, verdict는 `components/reports/ReportRow.tsx`의 고정 사전(`VERDICT_LABEL`)으로만 매핑 — 어느 경로도 LLM 자유번역을 안 거친다(가이던스 상향≠목표주가 상향류 사실 왜곡 방지). 기존 68건(KR 15·US 53) 소급 번역 = `scripts/backfill-channel-report-translations.ts`(일회성, 크론 아님). 🔴 **2026-09-07 퍼센트 하드 게이트 보정**: "1%p"(퍼센트포인트)가 번역에서 "1 percentage point"처럼 "%" 없이 풀어써지면 개수가 안 맞아 `percent_mismatch`로 오탐 저장 실패했다(에이피알 리포트 실사례) — `extractPct()`가 "%p"·"percentage point(s)"·"pp"도 같은 개념으로 인식하도록 양쪽 트리거(`translate-channel-report`·`translate-growth-story`)에 동일 수정. 상세 = `.claude/rules/mistakes.md`.
> 🆕 **2026-09-07 성장스토리 신설(68개로 증가)**: **`channel_growth_stories`**(채널 제작 "회사 소개" 콘텐츠 — symbol/stock_name/country/source_doc[표시용 문자열]/**source_date**[판정용 date]/intro/challenge/response/summary/video_url[현재 항상 NULL — 업로드가 수동이라 채널이 알려주기 전까지]/episode_folder. `channel_reports`와 근본적으로 다른 지점: **종목당 1행이 원칙**(`(symbol, country)` UNIQUE) — 재제작은 그 행을 UPDATE, 새 행을 안 쌓는다). **`channel_growth_story_scripts`**(원본 대본 CUT별 나레이션 이력 — append-only, story_id FK, 화면 비노출. 갱신 때마다 옛 대본이 사라지면 안 돼서(이미 영상으로 나간 실물) 별도 테이블로 분리). **`channel_growth_story_translations`**(story_id·target_lang 복합키 — intro/challenge/response/summary 번역 캐시, `channel_report_translations`와 동일 원칙). 번역 트리거(`trigger_translate_growth_story`)는 `channel_reports`와 달리 **AFTER INSERT OR UPDATE** 둘 다 잡는다(행이 갱신되므로) — `/api/internal/translate-growth-story` 호출, 인증은 Vault(`channel_growth_story_translate_secret`) + Vercel env(`CHANNEL_GROWTH_STORY_TRANSLATE_SECRET`, 2026-09-07 등록·재배포 완료·실측 검증 — 8건 재발화 → 번역 8건 `status='ok'` 확인). 화면: `app/[locale]/stock/[symbol]/StockLensClient.tsx`의 `CompanyIntroLayer`가 `/api/channel-growth-stories`(`lib/growthStoryI18n.ts`)를 읽어 **리포트 레이어 바로 위**(B안 — "회사 소개"를 리포트보다 고정 상단)에 렌더, 없으면 섹션 자체가 안 보인다(빈 상태 문구 없음). 4파트는 라벨 없이 문단으로만 이어붙임(사용자 확정 — 제작 용어가 노출되면 콘텐츠가 얇아 보인다는 판단).
> 🔴 **2026-09-07 pg_net 타임아웃 로그 정정**: `trigger_translate_channel_report`·`trigger_translate_growth_story` 둘 다 `net.http_post()`의 `timeout_milliseconds` 기본값(5000)이 실제 처리시간(6~8초)보다 짧아, **성공한 호출도 `net._http_response`에 timeout으로만 로깅**되고 있었다(동작 자체엔 영향 없음 — fire-and-forget이라 Vercel 함수는 서버에서 계속 실행돼 정상 완료). 둘 다 `timeout_milliseconds := 15000`으로 수정 후 재발화해 `status_code: 200` 확인 완료. 상세 = `.claude/rules/mistakes.md` 2026-09-07.
