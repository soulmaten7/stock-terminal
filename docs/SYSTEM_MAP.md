...
> 🔑 **전수 인벤토리(행수·시장귀속·근거·화면도달·판정) = `docs/probe_1049_table_inventory.md`.** 아래 그룹핑은 요약이며, 근거·판정 대조는 그 문서가 정본이다.
...
> 🔴 **아래 §"유사투자자문/리딩방" 블록은 STEP1035(2026-08-15)로 전부 DROP돼 더 이상 존재하지 않는다**(`fss_advisors`·`leading_rooms`·`leading_room_votes`·`room_*`·`link_previews` — `spinoff/advisor-directory/`로 이관됨, `docs/probe_1035_advisor_spinoff.md` 참고). `ai_view_cache`는 STEP1049 실측 결과 **0행·코드 참조 0건인 죽은 테이블**이다(마이그레이션 파일에도 없어 MCP로 직접 생성된 것으로 추정) — "AI 캐시" 그룹에 있다고 살아있는 것은 아니다.
...
- **피처 플래그**: `REVDCF_ENABLED`(854 · 역DCF 노출 게이트 · `=== "true"`만 ON · 기본 OFF · 🔴 **NEXT_PUBLIC_ 접두어 금지**=빌드캐시 인라인 함정이라 서버 env로 · `lib/revdcf/flag.ts`). 프로덕션 미설정=OFF · 프리뷰에서만 `=true`로 장은태 육안 검증. OFF면 종목페이지 역DCF 섹션·`/revdcf`(404)·보드 배지 컬럼 전부 미렌더(단 `/api/revdcf`·`/api/cron/revdcf` 데이터 배관은 계속 가동).
