...
**근거**:
- **ⓑ(제거+Q1대체)**는 US 단독 원칙(§3-2가 밝힌 "US만 못 끔" 구조)과 라이브 화면 수정 승인 둘 다에 걸린다 — 지금 착수할 조건이 안 갖춰져 있다.
- **ⓒ(소스 교체)**는 라이브 화면 값이 바뀌는 작업이라 별도 승인이 필요하다 — 조사 STEP에서 정할 사안이 아니다.
- **ⓓ(병기 표시)**는 표시 설계 문제라 지금은 그 시점이 아니다(980의 미결 43번과 같은 자리).

🔴 **Q1_ENABLED가 Production OFF라 두 PER(야후 밸류 렌즈 vs Q1/SEC)이 같은 화면에 나타난 적이 없다.** 지금은 순수하게 백엔드 데이터 정합성 문제이지 사용자에게 노출된 모순이 아니다 — **이 문제의 실제 처방은 Q1 카드를 켜는 시점에 결정된다**는 것이 이번 판정의 전제다.

🔴 **선결 질문 등재**: *"KR도 지나가는 코드 경로를 읽고 확인하는 것이 US 단독 규칙 위반인가."* — §3-2에서 `LENSES`·`LENS_KEYS`·`CUT_LENSES`가 KR·US 공유임을 코드로 확인했는데, 이 사실 확인 자체는 "US 새 작업"에 해당하는가, 아니면 "기존 공유 코드를 읽었을 뿐"이라 해당 없는가. **ⓑ를 실제로 설계·실행하려면 이 질문부터 정해야 한다** — ⓑ가 KR 코드에 새 분기를 넣어야 하는 작업이라면(§3-2의 결론), "US만 만든다"는 원칙이 KR 코드를 읽는 것까지 막는지 아니면 KR 코드를 새로 쓰는 것만 막는지가 갈린다.
...
**판정이 필요한 것**
- ✅ **982에서 판정 완료** — 현행 유지(ⓐ), Q1 카드 켜는 시점에 재론.
- 선결 질문(982 §판정): "KR 공유 코드를 읽고 확인하는 것이 US 단독 규칙 위반인가" — ⓑ 착수 전 정해야 함.
- lens_cuts(US) 정지(07-30부터, 미해결 13번) — 982에서 별도 조사·원인 부분특정(`docs/LENS_CUTS_FREEZE_982.md`), 처방은 미정.
