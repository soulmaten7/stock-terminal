<!-- 2026-06-20 -->
# STEP 294 — [V7 ④-7] 리딩방명(info_name) + 전체 탭 + 미리보기 스티키
...
그다음 터미널에:
```
@docs/STEP_294_COMMAND.md 파일 내용대로 실행해줘
```

- **전제 상태(HEAD)**: STEP 292(`174be41`). 빌드 ✓. **(STEP 293은 이 STEP에 포함 — 293 건너뛰어도 됨)**
- **사전 작업(완료, DB 직접)**: `advisor_directory` 뷰에 **info_name(리딩방명)** 컬럼 추가 끝.

---

## 🎯 목표

1. **리딩방명 표시**: 업체명(법인명)과 다른 실제 **리딩방/채널 이름(info_name)**을 메인으로(예: `귀인마케팅`→**신혼테크**, `프롬어스 철이네`→**철이네 경공방**). 업체명은 미리보기에 "등록업체"로. 검색도 리딩방명 포함.
2. **전체 탭**: 플랫폼 탭 맨 앞에 **[전체]** 추가 + 기본 선택.
3. **미리보기 스티키**: 스크롤 시 미리보기 패널이 **티커(36px) 밑에 고정**돼 따라오게. (원인=툴박스 카드 `overflow-hidden`이 sticky 가둠 → 제거 + `top-11`)
4. (293 포함) 미리보기에서 대표를 이름 바로 밑에.

> 파일 3개: ToolboxClient(overflow 제거 1줄) + `/api/advisors`(전체교체) + `AdvisorDirectory`(전체교체).

---

## 📄 파일 1 — `components/toolbox/ToolboxClient.tsx` (overflow-hidden 제거 → sticky 허용)
...
1. 리딩방·검증 탭 → 플랫폼 탭 맨 앞 **[전체]**(기본, 1,738곳) · 텔레그램 · 카카오톡 · 네이버 · 기타.
2. 카드 이름이 **리딩방명**으로 (예: `신혼테크`, `철이네 경공방`). 미리보기엔 **등록업체**(법인명)도 표시.
3. 검색 `신혼테크` → 리딩방명으로도 검색됨.
4. **스크롤 내리면 오른쪽 미리보기가 티커 밑에 고정**되어 따라옴.
5. 미리보기: 이름 → 바로 밑 대표 → 금감원 뱃지 → 등록업체·주소·신고기간·연락처.
...
```bash
cd ~/stock-terminal && git add -A && git commit -m "feat(v7): 리딩방명(info_name) 메인표시+검색, 전체 탭, 미리보기 티커밑 스티키(overflow 제거), 대표 이름밑 (V7 ④-7, STEP 294)" && git push
```

---

> **한 줄 요약**: 리딩방명(info_name)을 메인으로(업체명은 등록업체), 전체 탭 추가(기본), 미리보기를 티커 밑 스티키로(툴박스 overflow-hidden 제거 + top-11), 대표를 이름 밑으로. STEP 293 포함.
