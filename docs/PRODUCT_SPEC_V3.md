...
|---|---|---|---|---|---|
| 가격 | $2,000/월 | $39~119/월 | 무료 | 무료 (계좌 필수) | **완전 무료** (비로그인 포함) |
...
| 수익 모델 | 구독 | 구독 | 광고 | 계좌(거래수수료) | **Partner-Agnostic Lead Gen** ⭐ |
...
│ Side │                                                      │ Slot       │
│ bar  │  ┌─────────────────┬─────────────────┐              │ (home_     │
│      │  │ 관심종목 라이브   │ 외국인/기관 수급 │              │  featured) │
│ 320px│  │ (WatchlistLive) │ (InstFlow)      │              │            │
│      │  └─────────────────┴─────────────────┘              │ 240px      │
│ ───  │  ┌─────────────────┬─────────────────┐              │            │
│ 채팅 │  │ 거래량 급증      │ 시장 지수(미니차트)│             │ (W4 PartnerSlot│
│      │  └─────────────────┴─────────────────┘              │  컴포넌트로     │
│      │  ┌─────────────────┬─────────────────┐              │  교체.          │
│      │  │ 경제 캘린더      │ IPO / 실적 일정  │              │  W4 이전까지    │
│      │  └─────────────────┴─────────────────┘              │  빈 공간)       │
│      │  ┌─────────────────────────────────────┐            │            │
│      │  │ 브레이킹 피드 (뉴스 + 공시)           │            │            │
│      │  └─────────────────────────────────────┘            │            │
...
※ 왼쪽 레거시 AdColumn (인증/일반 배너) — V3 에서 **완전 제거**. 광고 배너 수익
  모델 폐기, Partner Slot + Lead Gen 으로 대체 (섹션 5 참고).
```
...
**모든 기능은 무료.** 결제 시스템 없음. 회원 가입은 "대화/저장" 기능을 쓰기 위한 장치.
비로그인도 실시간 시세/차트/호가/재무/공시/뉴스까지 **모두 OK** → **회원은 왜 가입하는가?** 에 대한 답:
...
> **중요**: 유료 티어/결제 시스템/AI 리포트/CSV 다운로드 — V3 Phase 1 범위에서 **전부 빠짐**.
> 이유: 수익은 **Lead Gen 단일 모델** 로만 가고, 플랫폼은 100% 무료 놀이터로 유지.
> 사용자를 최대한 많이 모아야 리드 단가가 유지되기 때문에 결제 장벽을 세우지 않는다.
...
> **수익 모델은 Phase 1 ~ 3 전체를 통틀어 "Partner-Agnostic Lead Gen" 단일.**
> 구독/결제/AI 리포트/À la carte 프리미엄 — **모두 범위 제외**.
> 이유: 결제 장벽을 세우면 이용자 풀이 줄어든다. 이용자가 많을수록 리드 품질/양이 올라간다. 무료 놀이터 + 리드 단일 수익이 가장 단순하고 가장 빠르다.
...
### Phase 2 (5~12주) — **트래픽 확보 + 리드 퀄리티 고도화**

- 종목 상세 7~8번 탭 (수급/비교) 완성 — 머무는 시간 증가 = 리드 기반 확장
- KRX 공매도/프로그램매매 크롤링 (데이터 깊이 추가)
- 알림 시스템 (가격/공시 푸시) — **무료**. 결제 게이트 없음. 이용자 재방문 유도용
- 파트너 슬롯 5~10개 동시 운영 (Contextual/Category/Home/Detail 분산)
- 유저 활동 로깅 기반 **리드 퀄리티 스코어** 계산 → 광고주 리드 단가 협상력 ↑
- 개인정보 처리방침 변호사 검토 + PIPA 정식 대응
- 광고(구글/유튜브) 집행해서 플랫폼 트래픽 확보 → 리드 공급 확대

### Phase 3 (12개월+) — **광고주 확장 + 글로벌**

- SEC EDGAR API (미국 주식) — 머무는 시간/이용자 풀 확대
- 일본 TSE / 홍콩 HKEX
- 광고 인벤토리 세분화 (Category Slot 20+ 개) — 파트너 수평 확장
- Make/자동화 시나리오 확장 (리드 전송/정산 자동화)
- **여전히 구독/결제 없음.** 글로벌 확장 후 새로운 수익 구조 검토는 별건.

---

## 11. "하지 말 것" 리스트 (Anti-patterns)

- ❌ **구독/결제/유료 티어/Pro 플랜 만들기** — Phase 1~3 전체 범위에서 완전 제외
- ❌ **AI 종목 분석 리포트/CSV 다운로드/프리미엄 기능 개발** — Lead Gen 에 집중
- ❌ **토스페이먼츠/Paddle 등 결제 연동** — 추후 별도 의사결정 없이는 시작 금지
- ❌ **기존 AdColumn (인증/일반 배너 20일 5만/3만원)** — 레거시. `HomeClient.tsx` 에서 제거
- ❌ 종목별 채팅방 분리
- ❌ 위젯 크기 3종 이상 사용
