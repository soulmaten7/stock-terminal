...
| `george_hwang_2004_52week_high_momentum.pdf` | **George & Hwang, "The 52-Week High and Momentum Investing"**, Journal of Finance 59(5), 2004 (저자 자신의 취업논문 사본 `bauer.uh.edu/tgeorge/papers/gh4-paper.pdf`, 2026-08-18 취득 — 출판본은 유료라 무료 공개본으로) | **STEP1061(기술 렌즈 감사 ⑤) — `pos52w`(52주 위치)의 "현상의 원전" 후보로 조사·확보.** 정의(본문 그대로 확인, Table II·§B) — 그들의 52주고 전략은 **`P_{i,t-1} / high_{i,t-1}`**(현재가 ÷ 52주 최고가의 **비율**)로 승자/패자를 가른다. 🔴 **우리 코드(`lenses.ts:204`, `(last-lo)/(hi-lo)*100`)와 산식이 다르다** — 우리는 52주 **저가**까지 쓰는 구간 내 위치(%)이고, 원전은 저가를 아예 쓰지 않는 고가 대비 비율. **같은 이름("52주 위치")을 걸었지만 계산은 원전과 일치하지 않으므로, 이 논문을 "우리 지표의 원전"이라 인용할 수 없다** — `lensCopy.ts`에 인용이 없는 것이 오히려 정확한 상태(있었다면 오귀속이었을 것). 우리 산식은 오히려 스토캐스틱 %K(조지 레인, 1950년대·학술 검증 없는 실무 관행)와 형태가 같다(구간 252일로 확대한 변형) |
| `jegadeesh_titman_1993_momentum.pdf` | **Jegadeesh & Titman, "Returns to Buying Winners and Selling Losers: Implications for Stock Market Efficiency"**, The Journal of Finance 48(1), 1993, pp.65-91 (JSTOR 스캔본, `bauer.uh.edu/rsusmel/phd/jegadeesh-titman93.pdf` 경유 무료 취득, 2026-08-19 — 유료화면 없음) | **STEP1084(최근변화 칸 ①원전탐색) — 모멘텀(12-1) "현상의 원전" 확인.** §I 본문 그대로 확인 — **10분위(decile) 포트폴리오**(승자/패자 각 10%씩, 동일가중), 형성기간 J=3·6·9·12개월 × 보유기간 K=3·6·9·12개월 조합(총 16전략) + **형성-보유 사이 1주 skip**(bid-ask·가격압박 회피 목적, 두 번째 16전략 세트) · 표본 NYSE·AMEX 1965~1989년(CRSP 일별). 🔴 **우리와 다른 점 3가지**: ①원전=10분위 동일가중 포트폴리오, 우리=개별 종목을 유니버스(시총 상위 1,000) 안에서 백분위로 줄세움 ②원전=1주 skip(선택), 우리=1개월 skip(고정) — **1개월 skip은 J-T 원문에 없고 Carhart(1997)/FF 관행**(이미 `lensCopy.ts`·`LENS_AUDIT_02` 결함④에서 대조됨) ③원전=J·K 16개 조합 전수 비교, 우리=12-1(J=12,K=1) 단일 사양. 🔑 **"현상의 발견"(승자가 계속 오른다)은 이 논문이 맞고, "계산 사양"(형성기간·skip·백분위)은 Carhart/FF를 따른다는 STEP1083 #10의 이중 귀속이 원문 대조로 확정됨** |
| `piotroski_2000_value_investing_fscore.pdf` | **Piotroski, "Value Investing: The Use of Historical Financial Statement Information to Separate Winners from Losers"**, Journal of Accounting Research 38(Supplement), 2000 (Ivey Business School 사본 `ivey.uwo.ca/media/3775523/...pdf`, 2026-08-18 취득) | **F-스코어 렌즈(`lib/fscore.ts`)의 원전** — `LENS_DEV_PLAYBOOK.md` #48(STEP818)이 이미 Table1+§2.3 대조를 완료해뒀고, STEP1062가 **§2.3.1~2.3.3 본문을 직접 재확인**(중복 조사 아니라 독립 재검증). 확인된 정의(본문 그대로) — *"I define ROA and CFO as net income before extraordinary items and cash flow from operations, respectively, **scaled by beginning-of-year total assets**"*(818의 "분모=기초" 확인과 일치) · *"ΔLEVER … historical change in the ratio of total long-term debt **to average total assets**"*(818이 지목한 "우리는 기말, 원전은 평균" 편차 재확인) · *"current ratio as the ratio of current assets to current liabilities **at fiscal year-end**"*(일치) · *"ΔTURN … total sales scaled by **beginning-of-the-year total assets**"*(본문 자체는 기초로 명시 — 818이 지목한 "Table1과의 자기모순"은 본문 vs 표 사이에 있으며 우리는 본문 쪽을 따름) · *"EQ_OFFER equal to one if the firm did not issue common equity"*(원문은 현금흐름표 신주발행 항목 직접 사용, 우리는 발행주식수 변화로 근사 — 818이 이미 지목한 편차) |
