...
| `link_previews` | 테이블 | 1,005 | OG 링크 프리뷰 캐시(advisor 채널 링크 전용, `/api/link-preview`가 lazy 크롤 후 upsert). 최초 DROP 라운드에서 놓쳤다가 같은 날 후속 리뷰로 DROP됨 — 아래 참고. |
...
✅ **`link_previews` 처리 완료(같은 날 후속 리뷰)**: `lib/og.ts`(OG 스크레이퍼, 이 폴더로 이관됨)를 조사하다 발견 — `link_previews` 테이블(1,005행, advisor 채널 링크 OG 캐시 전용)은 이름에 `advisor`/`room`/`leading`/`business_*` 패턴이 없어서 최초 DB 정리 때 조회 범위에 안 걸렸다. 본체 참조 0곳·개인정보 없음(URL+OG 메타데이터뿐, `user_id`류 컬럼 자체가 없음) 재확인 후 `schema.sql` §10에 DDL 추가하고 DROP했다(데이터는 재크롤 가능한 캐시라 미덤프 — `schema.sql` 헤더 참고). 🔴 이때 함께 확인한 `link_hub`는 **본체에서 여전히 쓰는 완전히 다른 테이블**이라 손대지 않았다.
...
DROP된 14개 테이블 + 1개 뷰 전체(`fss_advisors`·`leading_rooms`·`leading_room_votes`·`room_favorites`·`room_likes`·`room_reports`·`room_submissions`·`room_reviews`·`room_review_reports`·`business_claims`·`business_members`·`business_listing`·`business_links`·`link_previews`·`advisor_directory`)의 완전한 DDL은 `schema.sql`에 있다(`link_previews`는 최초 라운드가 아니라 같은 날 후속 리뷰로 추가 DROP — `schema.sql` 헤더 참고). 그중 `leading_rooms`·`leading_room_votes`·`room_reviews`·`room_review_reports`는 **이번 STEP 이전에 이미 죽어 있던(코드 미참조·0행) V6 시절 레거시**였지만, 이 스키마 클러스터 안에 있고(FK로 `fss_advisors`와 연결) 다른 살아있는 기능이 참조하지 않아 함께 DROP했다.
