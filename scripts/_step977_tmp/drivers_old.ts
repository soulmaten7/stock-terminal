/**
 * 🔴 STEP 980 복구 노트: 이 파일은 `git show 62f55a7:lib/revdcf/drivers.ts`로 추출한 STEP977 이전
 * drivers.ts 스냅샷이다(`scripts/probe_977_invariance.ts`가 import). STEP977 종료 시 정리 대상으로
 * 삭제했다가, `tsc --noEmit`/`npm run build`가 이 정적 import를 못 찾아 빌드가 깨지는 것을 STEP980에서
 * 발견 — 다시 추출해 이번엔 삭제하지 않고 커밋한다. 수정하지 말 것(비교 기준 스냅샷).
 *
